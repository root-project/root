...
if(http AND NOT builtin_civetweb)
  ...
  if(fail-on-missing)
    ...
    if(civetweb_FOUND)
      ...
      message(WARNING "civetweb not found or system version too old: disabling http component. To reenable, use `-Dhttp=ON` and install system civetweb package or add `-Dbuiltin_civetweb=ON`.")
      set(http OFF CACHE BOOL "Disabled because system-wide civetweb not found or too old" FORCE)
    endif()
  endif()
  if(civetweb_FOUND)
    ...
    if(NOT "${CIVETWEB_HAS_WEBSOCKET}" STREQUAL "1" OR NOT "${CIVETWEB_HAS_ZLIB}" STREQUAL "1" OR NOT "${CIVETWEB_HAS_X_DOM_SOCKET}" STREQUAL "1")
      ...
      endforeach()
      if(fail-on-missing)
        message(SEND_ERROR "System-wide civetweb found but does not include websocket or zlib or xdomsocket components (-DCIVETWEB_ENABLE_WEBSOCKETS=ON -DCIVETWEB_ENABLE_ZLIB=ON -DCIVETWEB_ENABLE_X_DOM_SOCKET=ON). Use `-Dbuiltin_civetweb=ON` as workaround.")
      else()
        message(WARNING "System-wide civetweb found but does not include websocket or zlib or xdomsocket components (-DCIVETWEB_ENABLE_WEBSOCKETS=ON -DCIVETWEB_ENABLE_ZLIB=ON -DCIVETWEB_ENABLE_X_DOM_SOCKET=ON): disabling http component. To reenable, use `-Dhttp=ON` and `-Dbuiltin_civetweb=ON`.")
        set(http OFF CACHE BOOL "Disabled since found system-wide version does not include websocket or zlib or xdomsocket components (-DCIVETWEB_ENABLE_WEBSOCKETS=ON -DCIVETWEB_ENABLE_ZLIB=ON -DCIVETWEB_ENABLE_X_DOM_SOCKET=ON)" FORCE)
      endif()
    endif()
  endif()
endif()
