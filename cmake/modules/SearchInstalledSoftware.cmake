  ...
  if(NOT DEFINED NO_CONNECTION)
    if(NOT check_connection)
      ...
      if(${STATUS_CODE} EQUAL 0)
        ...
        # Error
        message(WARNING "No internet connection. Please check your connection, set '-D${option}' or disable 'fail-on-missing' to automatically disable options requiring internet access. You can also bypass the connection check with -Dcheck_connection=OFF.")
  ...
  if (APPLE) # builtin OpenSSL is only supported on macOS
    ...
    if (NOT builtin_openssl)
      ...
      if(NOT OPENSSL_FOUND)
        ...
        list(APPEND HOTFIX_BUILD_FLAGS -Dbuiltin_openssl=ON)
      endif()
    else()
...
if(opengl OR cocoa)
  ...
  if(NOT OPENGL_FOUND OR NOT OPENGL_GLU_FOUND)
    if(cocoa AND NOT opengl)
      message(SEND_ERROR "OpenGL package (with GLU) not found and opengl option required for \"cocoa=ON\". Set -Dcocoa=OFF")
      list(APPEND HOTFIX_BUILD_FLAGS -Dcocoa=OFF)
    else()
      message(SEND_ERROR "OpenGL package (with GLU) not found and opengl option required. Set -Dopengl=OFF")
...
if(opengl AND NOT asimage)
  ...
  list(APPEND HOTFIX_BUILD_FLAGS -Dasimage=ON)
endif()
...
if(gviz)
  ...
  if(NOT GRAPHVIZ_FOUND)
    message(SEND_ERROR "Graphviz libraries not found while -Dgviz=ON. Install them on the system or set -Dgviz=OFF")
    list(APPEND MISSING_PACKAGES 'Graphviz')
    list(APPEND HOTFIX_BUILD_FLAGS -Dgviz=OFF)
  endif()
endif()
...
if(xml)
  ...
  if(NOT LIBXML2_FOUND)
    message(SEND_ERROR "LibXml2 libraries not while -Dxml=ON. Install them on the system or set -Dxml=OFF")
    list(APPEND MISSING_PACKAGES 'LibXml2')
    list(APPEND HOTFIX_BUILD_FLAGS -Dxml=OFF)
  endif()
endif()
...
if(fcgi)
  ...
  if(NOT FASTCGI_FOUND)
    message(SEND_ERROR "FastCGI library not found while -Dfcgi=ON. Install it on the system or set -Dfcgi=OFF")
    list(APPEND MISSING_PACKAGES 'FastCGI')
    list(APPEND HOTFIX_BUILD_FLAGS -Dfcgi=OFF)
  endif()
endif()
  ...
  if(civetweb_FOUND)
    ...
    if (CIVETWEB_FEATURE_API)
      ...
      if(COMPILE_RESULT)
        ...
        message(SEND_ERROR "Could not run civetweb features: ${BUILD_LOG}. Try fixing the install or use builtin_civetweb=ON or switch `-Dhttp=OFF`")
        list(APPEND HOTFIX_BUILD_FLAGS -Dbuiltin_civetweb=ON)
      endif()
      ...
      message(SEND_ERROR "Could not check for civetweb features: ${CIVETWEB_FEATURE_API_LOG}. Try fixing the install or use builtin_civetweb=ON")
      list(APPEND HOTFIX_BUILD_FLAGS -Dbuiltin_civetweb=ON)
    endif()
...
if(sqlite)
  ...
  find_package(Sqlite)
  if(NOT SQLITE_FOUND)
    message(SEND_ERROR "SQLite libraries not found while -Dsqlite=ON. Install them on the system or set -Dsqlite=OFF")
    list(APPEND MISSING_PACKAGES 'SQLite')
    list(APPEND HOTFIX_BUILD_FLAGS -Dsqlite=OFF)
  endif()
endif()
...
if(pythia8)
  ...
  if(NOT PYTHIA8_FOUND)
    message(SEND_ERROR "Pythia8 libraries not found while -Dpythia8=ON. Install them on the system or set -Dpythia8=OFF")
    list(APPEND MISSING_PACKAGES 'Pythia8')
    list(APPEND HOTFIX_BUILD_FLAGS -Dpythia8=OFF)
  endif()
endif()

#---Check for FFTW3-------------------------------------------------------------------
if(builtin_fftw3)
  ROOT_CHECK_CONNECTION("builtin_fftw3")
  if(NO_CONNECTION)
    message(SEND_ERROR "No internet connection, check it or disable the 'builtin_fftw3' option")
    list(APPEND MISSING_PACKAGES 'fftw3')
    list(APPEND HOTFIX_BUILD_FLAGS -Dbuiltin_fftw3=OFF)
  endif()
endif()
if(builtin_fftw3)
  add_subdirectory(builtins/fftw3)
  if (NOT fftw3)
    message(SEND_ERROR "builtin_fftw3=ON is incompatible with fftw3=OFF. Set -Dfftw3=ON")
...
if(fitsio OR builtin_cfitsio)
  if(builtin_cfitsio)
    ROOT_CHECK_CONNECTION("builtin_cfitsio")
    if(NO_CONNECTION)
      message(SEND_ERROR "No internet connection, check it or disable the 'builtin_cfitsio' option")
      list(APPEND MISSING_PACKAGES 'CFITSIO')
      list(APPEND HOTFIX_BUILD_FLAGS -Dbuiltin_cfitsio=OFF)
    endif()
  endif()
  if(builtin_cfitsio)
    ...
    if(NOT fitsio)
      message(SEND_ERROR "builtin_cfitsio=ON is incompatible with fitsio=OFF. Set -Dfitsio=ON")
...
if(shadowpw)
  if(NOT EXISTS /etc/shadow)  #---TODO--The test always succeeds because the actual file is protected
    if(NOT CMAKE_SYSTEM_NAME MATCHES Linux)
      message(SEND_ERROR "Support Shadow password not found. Switch off shadowpw option -Dshadowpw=OFF")
      list(APPEND HOTFIX_BUILD_FLAGS -Dshadowpw=OFF)
    endif()
  endif()
endif()
...
if(builtin_xrootd)
  ...
  if(NO_CONNECTION)
    message(SEND_ERROR "No internet connection. Please check your connection, or disable the 'builtin_xrootd'"
      " option and 'xrootd' options.")
    list(APPEND HOTFIX_BUILD_FLAGS -Dxrootd=OFF)
    list(APPEND HOTFIX_BUILD_FLAGS -Dbuiltin_xrootd=OFF)
  endif()
  add_subdirectory(builtins/xrootd)
  if (NOT xrootd)
    message(SEND_ERROR "builtin_xrootd=ON is incompatible with xrootd=OFF (${xrootd_description}). Set xrootd=ON")
...
if(arrow)
  ...
  if(NOT ARROW_FOUND)
    ...
    list(APPEND HOTFIX_BUILD_FLAGS -Darrow=OFF)
  endif()
endif()
...
if(dcache)
  ...
  if(NOT DCAP_FOUND)
    ...
    list(APPEND HOTFIX_BUILD_FLAGS -Ddcache=OFF)
  endif()
endif()
...
if(davix)
  ...
  if (DAVIX_FOUND)
    ...
    message(SEND_ERROR "Davix libraries (${davix_description}) not found while -Ddavix=ON. Install them on the system or set -Ddavix=OFF")
    list(APPEND MISSING_PACKAGES 'Davix')
    list(APPEND HOTFIX_BUILD_FLAGS -Ddavix=OFF)
  endif()
endif()
...
if(curl)
  ...
  if(NOT CURL_FOUND)
    message(SEND_ERROR "libcurl not found (${curl_description}) and -Dcurl=ON option required. Install on the system or set -Dcurl=OFF.")
    list(APPEND MISSING_PACKAGES 'libcurl')
    list(APPEND HOTFIX_BUILD_FLAGS -Dcurl=OFF)
  endif()
endif()
...
if (uring)
  if(NOT CMAKE_SYSTEM_NAME MATCHES Linux)
    message(SEND_ERROR "liburing only available on Linux but -During=ON option required. Set -During=OFF.")
    list(APPEND HOTFIX_BUILD_FLAGS -During=OFF)
  else()
    message(STATUS "Looking for liburing")
    find_package(liburing)
    message(SEND_ERROR "liburing not found and -During=ON option required. Install on the system or set -During=OFF.")
    list(APPEND MISSING_PACKAGES 'liburing')
    list(APPEND HOTFIX_BUILD_FLAGS -During=OFF)
  endif()
endif()
...
if (daos AND daos_mock)
  message(SEND_ERROR "Options `daos` and `daos_mock` are mutually exclusive; only one of them should be specified.")
  list(APPEND HOTFIX_BUILD_FLAGS -Ddaos=OFF)
endif()
if (testing AND NOT daos AND NOT WIN32 AND NOT daos_mock)
  message(SEND_ERROR "`-Dtesting=ON` requires either `daos` or `daos_mock`.")
  list(APPEND HOTFIX_BUILD_FLAGS -Ddaos_mock=ON)
endif()
...
if (daos OR daos_mock)
  ...
  if(NOT libuuid_FOUND)
    ...
    list(APPEND MISSING_PACKAGES 'libuuid')
    list(APPEND HOTFIX_BUILD_FLAGS -Ddaos=OFF)
    list(APPEND HOTFIX_BUILD_FLAGS -Ddaos_mock=OFF)
  endif()
endif()
if (daos)
  find_package(DAOS)
  if(NOT DAOS_FOUND)
    ...
    list(APPEND MISSING_PACKAGES 'DAOS')
    list(APPEND HOTFIX_BUILD_FLAGS -Ddaos=OFF)
  endif()
endif()
...
if(builtin_tbb)
  ...
  if(NO_CONNECTION)
    message(SEND_ERROR "No internet connection, check your connection or disable 'builtin_tbb' and 'imt' options")
    list(APPEND HOTFIX_BUILD_FLAGS -Dbuiltin_tbb=OFF)
    list(APPEND HOTFIX_BUILD_FLAGS -Dimt=OFF)
  endif()
endif()
...
if(builtin_vdt)
  ...
  if(NO_CONNECTION)
    message(SEND_ERROR "No internet connection, check your connection or disable 'builtin_vdt' and 'vdt' options")
    list(APPEND HOTFIX_BUILD_FLAGS -Dbuiltin_vdt=OFF)
    list(APPEND HOTFIX_BUILD_FLAGS -Dvdt=OFF)
  endif()
endif()
...
if (vecgeom)
  ...
  if(NOT VecGeom_FOUND)
    ...
    list(APPEND HOTFIX_BUILD_FLAGS -Dvecgeom=OFF)
  else()
...
if(experimental_adaptivecpp)
  ...
  if(NOT DEFINED ADAPTIVECPP_SOURCE_DIR)
    ROOT_CHECK_CONNECTION("experimental_adaptivecpp")
    if(NO_CONNECTION)
      message(SEND_ERROR "No internet connection, check your connection or disable 'experimental_adaptivecpp' option")
      list(APPEND HOTFIX_BUILD_FLAGS -Dexperimental_adaptivecpp=OFF)
    endif()
  endif()
  ...
  if (AdaptiveCpp_FOUND)
    ...
    list(APPEND MISSING_PACKAGES 'AdaptiveCpp')
    list(APPEND HOTFIX_BUILD_FLAGS -Dexperimental_adaptivecpp=OFF)
  endif()
endif()
...
if(tmva AND testing AND test_tmva_sofie)
  ...
  if(NOT BLAS_FOUND)
    ...
    list(APPEND MISSING_PACKAGES 'BLAS')
    list(APPEND HOTFIX_BUILD_FLAGS -Dtest_tmva_sofie=OFF)
  endif()
endif()
...
if(tmva-cpu)
  if (NOT tmva)
    message(SEND_ERROR "-Dtmva-cpu=ON is incompatible with -Dtmva=OFF.")
    list(APPEND HOTFIX_BUILD_FLAGS -Dtmva-cpu=OFF)
  elseif(NOT imt)
    message(SEND_ERROR "-Dtmva-cpu=ON is incompatible with -Dimt=OFF.")
    list(APPEND HOTFIX_BUILD_FLAGS -Dtmva-cpu=OFF)
  endif()
endif()
if(tmva-cpu)
  if (NOT use_gsl_cblas)
    find_package(BLAS)
    if(NOT BLAS_FOUND)
      # If no optimized BLAS library was found, we suggest attempting to
      # use the GSL CBLAS.
      message(SEND_ERROR "Option tmva-cpu requires a BLAS library, but none could be found on the system. Either install a BLAS library like OpenBLAS (preferred), or set use_gsl_cblas=ON (possibly also builtin_gsl=ON if GSL not installed on the system). Or disable tmva-cpu.")
      list(APPEND MISSING_PACKAGES 'BLAS')
      list(APPEND HOTFIX_BUILD_FLAGS -Dbuiltin_gsl=ON)
    endif()
  endif()
endif()
...
if(mathmore OR builtin_gsl OR (tmva-cpu AND use_gsl_cblas))
  if(builtin_gsl)
    ROOT_CHECK_CONNECTION("builtin_gsl")
    if(NO_CONNECTION)
      message(SEND_ERROR "No internet connection, check your connection or disable 'builtin_gsl', 'tmva-cpu' and 'use_gsl_cblas' options")
      list(APPEND HOTFIX_BUILD_FLAGS -Dbuiltin_gsl=OFF)
      list(APPEND HOTFIX_BUILD_FLAGS -Dtmva-cpu=OFF)
      list(APPEND HOTFIX_BUILD_FLAGS -Duse_gsl_cblas=OFF)
    endif()
  endif()
  if(builtin_gsl)
...
if(tmva-cpu)
  ...
  elseif(use_gsl_cblas AND GSL_FOUND)
    ...
    message(SEND_ERROR "tmva-cpu can't be built because BLAS was not found (${tmva-cpu_description})! Install it or disable tmva-cpu")
    list(APPEND MISSING_PACKAGES 'BLAS')
    list(APPEND HOTFIX_BUILD_FLAGS -Dtmva-cpu=OFF)
  endif()
endif()
if(tmva)
  if(tmva-gpu AND NOT CMAKE_CUDA_COMPILER)
    message(SEND_ERROR "tmva-gpu can't be built because CUDA was not found! Install it or disable tmva-gpu")
    list(APPEND MISSING_PACKAGES 'CUDA')
    list(APPEND HOTFIX_BUILD_FLAGS -Dtmva-gpu=OFF)
  elseif(tmva-gpu)
    ...
    if (tmva-cudnn)
      ...
      if (CUDNN_FOUND)
        ...
        list(APPEND MISSING_PACKAGES 'CUDNN')
        list(APPEND HOTFIX_BUILD_FLAGS -Dtmva-cudnn=OFF)
      endif()
    endif()
  endif()
  if(tmva-pymva)
    if(NOT Python3_NumPy_FOUND OR NOT Python3_Development_FOUND)
      message(SEND_ERROR "TMVA: numpy python package or Python development package not found and tmva-pymva component required"
                          " (python executable: ${Python3_EXECUTABLE}). Install them or disable tmva-pymva.")
      list(APPEND MISSING_PACKAGES 'numpy')
      list(APPEND HOTFIX_BUILD_FLAGS -Dtmva-pymva=OFF)
    endif()
  endif()
else()
  if (tmva-gpu)
    message(SEND_ERROR "'-Dtmva-gpu=ON' is incompatible with '-Dtmva=OFF' (${tmva-gpu_description})")
    list(APPEND HOTFIX_BUILD_FLAGS -Dtmva-gpu=OFF)
  endif()
  if (tmva-cudnn)
    message(SEND_ERROR "'-Dtmva-cudnn=ON' is incompatible with '-Dtmva=OFF' (${tmva-cudnn_description})")
    list(APPEND HOTFIX_BUILD_FLAGS -Dtmva-cudnn=OFF)
  endif()
  if (tmva-pymva)
    message(SEND_ERROR "'-Dtmva-pymva=ON' is incompatible with '-Dtmva=OFF' (${tmva-pymva_description})")
    list(APPEND HOTFIX_BUILD_FLAGS -Dtmva-pymva=OFF)
  endif()
endif(tmva)
...
if(pyroot)
  ...
  if(Python3_Development.Module_FOUND)
    ...
    message(SEND_ERROR "PyROOT: Python development package not found and pyroot component required"
                       " (python executable: ${Python3_EXECUTABLE}). Install it or disable pyroot option.")
    list(APPEND MISSING_PACKAGES 'Python3_Development.Module')
...
if(tpython)
  ...
  if(NOT Python3_Development_FOUND)
    message(SEND_ERROR "TPython: Python development package not found and tpython component required"
                       " (python executable: ${Python3_EXECUTABLE}). Install it or disable tpython option.")
    list(APPEND MISSING_PACKAGES 'Python3_Development')
    list(APPEND HOTFIX_BUILD_FLAGS -Dtpython=OFF)
  endif()
...
if (mpi)
  ...
  if(NOT MPI_FOUND)
    ...
    list(APPEND HOTFIX_BUILD_FLAGS -Dmpi=OFF)
  endif()
endif()
...
if (roofit_multiprocess)
    ...
    # We always look for ZeroMQ and cppzmq with REQUIRED to fail configuration if not available.
    find_package(ZeroMQ 4.3.5)
    if (NOT ZeroMQ_FOUND)
      message(SEND_ERROR "ZeroMQ not found. Install it or disable option 'roofit_multiprocess'")
      list(APPEND MISSING_PACKAGES 'ZeroMQ')
      list(APPEND HOTFIX_BUILD_FLAGS -Droofit_multiprocess=OFF)
    endif()
    ...
    find_package(cppzmq)
    if (NOT cppzmq_FOUND)
      message(SEND_ERROR "cppzmq not found. Install it or disable option 'roofit_multiprocess'")
      list(APPEND MISSING_PACKAGES 'cppzmq')
      list(APPEND HOTFIX_BUILD_FLAGS -Droofit_multiprocess=OFF)
    endif()
...
if (testing OR testsupport)
  if (builtin_gtest)
    ...
    if(NO_CONNECTION)
      message(STATUS "No internet connection, check connection or disable the 'testing', 'testsupport' and 'builtin_gtest' options")
      list(APPEND HOTFIX_BUILD_FLAGS -Dtesting=OFF)
      list(APPEND HOTFIX_BUILD_FLAGS -Dtestsupport=OFF)
      list(APPEND HOTFIX_BUILD_FLAGS -Dbuiltin_gtest=OFF)
    else()
...
if (testing OR testsupport)
  ...
  foreach(LIBNAME gtest_main gmock_main gtest gmock)
    if(NOT TARGET GTest::${LIBNAME} AND NOT TARGET ${LIBNAME})
      message(SEND_ERROR "Missing installation of GTest subcomponent ${LIBNAME}. Install it or disable testing and testsupport.")
      list(APPEND MISSING_PACKAGES '${LIBNAME}')
      list(APPEND HOTFIX_BUILD_FLAGS -Dtesting=OFF)
      list(APPEND HOTFIX_BUILD_FLAGS -Dtestsupport=OFF)
      list(APPEND HOTFIX_BUILD_FLAGS -Dbuiltin_gtest=OFF)
    endif()
...
if(webgui AND NOT builtin_openui5)
  ...
  if(NO_CONNECTION)
    message(SEND_ERROR "No internet connection, check it or switch ON the 'builtin_openui5' option")
    list(APPEND HOTFIX_BUILD_FLAGS -Dbuiltin_openui5=ON)
  endif()
endif()
...
if(test_distrdf_pyspark)
  ...
  if (NOT PySpark_FOUND)
    message(SEND_ERROR "PySpark not found. Install it or disable option 'test_distrdf_pyspark'")
    list(APPEND MISSING_PACKAGES 'PySpark')
    list(APPEND HOTFIX_BUILD_FLAGS -Dtest_distrdf_pyspark=OFF)
  endif()
endif()
...
if(test_distrdf_dask)
  ...
  if (NOT Dask_FOUND)
    message(SEND_ERROR "Dask not found. Install it or disable option 'test_distrdf_pyspark'")
    list(APPEND MISSING_PACKAGES 'Dask')
    list(APPEND HOTFIX_BUILD_FLAGS -Dtest_distrdf_dask=OFF)
  endif()
endif()

if(webgui AND qt6web)
  find_package(Qt6 COMPONENTS Core WebEngineCore WebEngineWidgets CONFIG)
  if(NOT Qt6_FOUND)
    message(SEND_ERROR "Could NOT find Qt6 (WebEngineCore, WebEngineWidgets), install missing packages on the system or disable option 'qt6web'")
    list(APPEND MISSING_PACKAGES 'Qt6 WebEngineCore WebEngineWidgets')
