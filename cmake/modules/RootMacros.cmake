...
function(ROOT_GENERATE_DICTIONARY dictionary)
  ...
  if((PROJECT_NAME STREQUAL ROOT) AND (TARGET ${ARG_MODULE}))
    ...
    if(target_incdirs)
       foreach(dir ${target_incdirs})
          ...
          if(NOT ${dir} MATCHES "^[$]")
            ...
            string(FIND ${dir} "${ROOT_SOURCE_DIR}" src_dir_in_dir)
    ...
    list(FILTER incdirs EXCLUDE REGEX "^${ROOT_SOURCE_DIR}")
    list(FILTER incdirs EXCLUDE REGEX "^${ROOT_BINARY_DIR}/ginclude")
    list(FILTER incdirs EXCLUDE REGEX "^${ROOT_BINARY_DIR}/externals")
    list(FILTER incdirs EXCLUDE REGEX "^${ROOT_BINARY_DIR}/builtins")
    list(INSERT incdirs 0 ${ROOT_BINARY_DIR}/include)
    # endif()

    # this instruct rootcling do not store such paths in dictionary
    set(excludepaths ${ROOT_SOURCE_DIR} ${ROOT_BINARY_DIR}/ginclude ${ROOT_BINARY_DIR}/externals ${ROOT_BINARY_DIR}/builtins)
      ...
      if(${fp} MATCHES "[*?]") # Is this header a globbing expression?
        ...
        if(IS_ABSOLUTE ${fp})
          ...
          foreach(incdir ${incdirs})
            string(FIND ${incdir} "${ROOT_SOURCE_DIR}" src_dir_in_dir)
            string(FIND ${incdir} "${ROOT_BINARY_DIR}" bin_dir_in_dir)
    ...
    if(PROJECT_NAME STREQUAL ROOT)
      list(APPEND incdirs ${ROOT_BINARY_DIR}/include)
      list(APPEND incdirs ${ROOT_BINARY_DIR}/etc/cling) # This is for the RuntimeUniverse
      # list(APPEND incdirs ${ROOT_SOURCE_DIR})
      set(excludepaths ${ROOT_SOURCE_DIR} ${ROOT_BINARY_DIR})
...
#        "${library_target_name} mismatch. Deduction stem: ${dictionary}.")
  ...
  if(ARG_MODULE)
    ...
    if(runtime_cxxmodules)
      ...
      if(cpp_module)
        ...
        if (cpp_module_file AND PROJECT_NAME STREQUAL ROOT)
		      set(runtime_cxxmodule_dependencies copymodulemap "${ROOT_BINARY_DIR}/include/ROOT.modulemap")
        endif()
      endif(cpp_module)
    endif()
  endif()
  ...
  if(ARG_STAGE1)
    if(MSVC AND CMAKE_ROOTTEST_DICT)
      set(command ${CMAKE_COMMAND} -E ${ROOT_BINARY_DIR}/bin/rootcling_stage1.exe)
    else()
      set(command ${CMAKE_COMMAND} -E env "LD_LIBRARY_PATH=${ROOT_BINARY_DIR}/lib:$ENV{LD_LIBRARY_PATH}" $<TARGET_FILE:rootcling_stage1>)
    endif()
    ...
    if(PROJECT_NAME STREQUAL ROOT)
      if(MSVC AND CMAKE_ROOTTEST_DICT)
        set(command ${CMAKE_COMMAND} -E env "ROOTIGNOREPREFIX=1" ${ROOT_BINARY_DIR}/bin/rootcling.exe)
      else()
        set(command ${CMAKE_COMMAND} -E env "LD_LIBRARY_PATH=${ROOT_BINARY_DIR}/lib:$ENV{LD_LIBRARY_PATH}"
...
function (ROOT_CXXMODULES_APPEND_TO_MODULEMAP library library_headers)
  ...
  if (NOT "${PROJECT_NAME}" STREQUAL ROOT OR ROOT_CXXMODULES_WRITE_TO_CURRENT_DIR)
...
function(ROOT_EXECUTABLE executable)
  ...
  if(NOT (PROJECT_NAME STREQUAL "ROOT"))
    ...
    include_directories(BEFORE ${ROOT_BINARY_DIR}/include)
  elseif(MSVC)
...
function(ROOT_ADD_TEST test)
  ...
  if(ARG_BUILD)
    ...
    set_property(TEST ${test} PROPERTY ENVIRONMENT ROOT_DIR=${ROOT_BINARY_DIR})
  else()
...
function(ROOT_PATH_TO_STRING resultvar path)
  ...
  string(REPLACE "${ROOT_SOURCE_DIR}" "" relativepath ${realfp})
...
function(ROOT_ADD_TEST_SUBDIRECTORY subdir)
  file(RELATIVE_PATH subdir ${ROOT_SOURCE_DIR} ${CMAKE_CURRENT_SOURCE_DIR}/${subdir})
...
function(generateHeader target input output)
  add_custom_command(OUTPUT ${output}
    MAIN_DEPENDENCY
      ${input}
    DEPENDS
      ${ROOT_SOURCE_DIR}/build/misc/argparse2help.py
    COMMAND
      ${PYTHON_EXECUTABLE} -B ${ROOT_SOURCE_DIR}/build/misc/argparse2help.py ${input} ${output}
  )
