...
can be retrieved using an index access operator of a vector.) All these accesses should be replaced by the getters `get_curWeight()`
...
```
  With the modernised interface, one would write this:
```
  processEvent(dataHist.get(i), dataHist.weight(i));
```
  To modernise old code, try replacing all `h.get(i); h.func()` by `h.func(i);`. One may `#define R__SUGGEST_NEW_INTERFACE` to switch on deprecation
  warnings if the old functions are used.
...
```
- More const correctness. `calcTreeIndex()`, in particular, doesn't rely on side effects, any more. Instead of overwriting the internal
...
```
  one should now pass these values on:
```
  // In a subclass:
