...
* The `rmkdepend` executable is deprecated and will be removed from the public executables shipped with the installation after ROOT 6.44. Use instead `makedepend`, `CMake` or `-MM` compiler flags for dependency tracking. A builtin option (OFF by default) is created: `builtin_makedepend`, installing makedepend on the system is preferred.
