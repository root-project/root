...
To fix it well in advance on your side in downstream code, since no warnings are emitted in the meantime, we recommend getting a `preview` of those to-be errors by adding in your code `#ifndef R__LESS_INCLUDES #define R__LESS_INCLUDES #endif`, adding this at the very top before including any ROOT header. An alternative is to define this on CMake side via `target_compile_definitions`.
