...
void generateFile(string nameOfChain, size_t numberOfFiles, size_t numberOfEntriesInFile) {
   ...
   for (size_t i=0;i<numberOfFiles;i++) {
      TFile* newfile = new TFile(Form("%s_%i.root", nameOfChain.c_str(), i), "RECREATE"); 
      TTree*  outputTree = new TTree(nameOfChain.c_str(), nameOfChain.c_str());
      TString branchname; branchname.Form("value%d_%d",numberOfFiles,numberOfEntriesInFile);
      TString leafname; leafname.Form("leafvalue%d_%d/I",numberOfFiles,numberOfEntriesInFile);
