...
void MakeHisto(TTree *tree, TDirectory* To) {
   ...
   TH2F *refPxInd = RefClone(where,"hPxInd");
   ...
   for (Int_t ev=0;ev<nev;ev++) {
      ...
      refSqrtNtrack->Fill(sqrt((float)event->GetNtrack()));
      ...
      if (bits.TestBitNumber(43)) refFiltTriggerBits->Fill(nbits);
