// @(#)root/test:$Id: Event.cxx,v 1.1.1.1 2002/08/02 22:24:36 pcanal Exp $
...
{
   fEventType = kRaw;
   if (gRandom->Rndm() > 0.2) fEventType = kESD;
   if (gRandom->Rndm() > 0.6) fEventType = kAOD;
   if (gRandom->Rndm() > 0.9) fEventType = kMicro;
...
//______________________________________________________________________________
void UShortVector::FillVector()
{
   // Create a big track object.
   fNshorts = (Int_t)(10000*gRandom->Rndm());
   for (Int_t i=0;i<fNshorts;i++) {
      push_back(i);
   }
}
