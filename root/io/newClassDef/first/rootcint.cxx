// @(#)root/utils:$Id: rootcint.cxx,v 1.58 2002/01/25 11:34:50 brun Exp $
   ...
   if (cl.IsTmplt()) {
      ...
      if (templ) {
         if (templ->line == cl.LineNumber()
             && strcmp(cl.FileName(),G__srcfilename(templ->filenum))==0) {
...
# endif
   ...
   fprintf(fp, "#endif\n\n");
   fprintf(fp, "// Since CINT ignore the std namespace, we need to do so in this file.\n");
   ...
   // first to allow template specialisation to occur before template instantiation (STK)
   cl.Init();
   while (cl.Next()) {
      if ((cl.Property() & G__BIT_ISCLASS) && cl.Linkage() == G__CPPLINK) {
