...
public:   
   ...
   std::vector<double > *fScalarPtrArr[4];
   UInt_t                fScalarPtrArrVarSize;
   std::vector<int >    *fScalarPtrArrVar; //[fScalarPtrArrVarSize]
   ...
   std::vector<THelper >  fTObject;      //||
   std::vector<THelper > *fTObjectPtr;   //||

   std::vector<THelper* > fPtrTObject;   //||
   std::vector<THelper* >*fPtrTObjectPtr;//||
#else
   std::vector<THelper >  fTObject;
   std::vector<THelper > *fTObjectPtr;
   ...
   std::vector<std::string>         fString;
   std::vector<std::string*>        fPtrString;
#if defined(R__NO_NESTED_CONST_STRING)
   std::vector<const std::string*>  fPtrConstString; //!  this version of ROOT does not support nested const string
#else
   std::vector<const std::string*>  fPtrConstString;
#endif

   std::vector<TString>  fTString;
   std::vector<TString*> fPtrTString;

#if defined(R__CANNOT_SPLIT_VECTOR_OBJECT)
   std::vector<TNamed>   fTNamed;    //||
   std::vector<TNamed*>  fPtrTNamed; //||
   std::vector<const TNamed*>  fPtrConstTNamed; //||
#endif
   ...
   std::vector<std::vector<Helper > > fNestedVV; //!
   std::vector<std::deque<Helper > > fNestedVD; //!
#else
   nesting_t fNested;  
   std::vector<std::vector<Helper > > fNestedVV; 
   std::vector<std::deque<Helper > > fNestedVD; 
