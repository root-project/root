...
public:
   ...
   Double_t    GetVertex(Int_t i=0) {return (i<3)?fVertex[i]:0;}
   ...
   Double_t    GetPointValue(Int_t i=0) const { return (i<fNsp)?fPointValue[i]:0; }
...
private:
   ...
   Int_t          fMeasures[10];
   Double_t     fMatrix[4][4];
   Double_t    *fClosestDistance;   //[fNvertex]
   EventHeader    fEvtHdr;
...
public:
   ...
   void          SetMatrix(UChar_t x, UChar_t y, Double_t what) { if (x<3&&y<3) fMatrix[x][y]=what;}
   ...
   Double_t    GetMatrix(UChar_t x, UChar_t y) { return (x<4&&y<4)?fMatrix[x][y]:0; }
   TBits&        GetTriggerBits() { return fTriggerBits; }
