...
TGLScenePad::TGLScenePad(TVirtualPad *pad)
   : TVirtualViewer3D(), TGLScene(),

     fPad(pad), fInternalPIDs(kFALSE), fNextInternalPID(1), // 0 reserved
     fLastPID(0),                                           // 0 reserved
     fAcceptedPhysicals(0), fComposite(nullptr), fCSLevel(0), fSmartRefresh(kFALSE)
{
   // Constructor.
}
...
RootCsg::TBaseMesh* TGLScenePad::BuildComposite()
{
   ...
   if (opCode != TBuffer3D::kCSNoOp) {
      ...
      default: Error("BuildComposite", "Wrong operation code %d\n", opCode); return nullptr;
      }
