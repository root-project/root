...
{
   // Set maximum phi Step. This method name should be SetMaxAng(Float_t).
...
void TEveTrackPropagator::StepRungeKutta(Double_t* vect, Double_t* vout)
{
   ...
   Double_t yt;
   Double_t zt;
   const Double_t maxit  = 10;
   const Double_t maxcut = 11;

   const Double_t hmin   = 1e-3; // !!! MT ADD
   const Double_t kdlt   = 1e-4;   // !!! MT CHANGE from 1e-4, should be
   ...
   Double_t tl = 0.;
   Double_t h = fH.fStepSize;
   Double_t rest;

   do {
      rest  = fH.fStepSize - tl;
      ...
      vout[5] = cba*c;
      rest = fH.fStepSize - tl;
      if (fH.fStepSize < 0.) rest = -rest;
      if (rest < 1.e-5*TMath::Abs(fH.fStepSize))
      {
...
{
   // Move one step in track propagation.
   // Method called from LoopToBounds(), LoopToVertex(), and HelixIntersectPlane().

   fH.fB = fMagFieldObj->GetField(v);

   if (fStepper == kHelix)
   {
      if (fMagFieldObj->IsConst())
      {
         if (fH.fValid) fH.fPt = p - fH.fPl;
      }
      else {
         SetupStepper(v, p);
      }
      ...
      if (fH.fValid)
      {
         ...
         fH.fE2    = fH.fPt;
         fH.fE2.Normalize();
         TMath::Cross(fH.fE1.Arr(), fH.fE2.Arr(), fH.fE3.Arr());
         if ( fH.fCharge < 0) fH.fE3.NegateXYZ();

         TEveVector d = fH.fE2*(fH.fR*fH.fSin) + fH.fE3*(fH.fR*(1-fH.fCos)) + fH.fE1*fH.fLStep;
         vOut    += d;
         vOut.fT += fH.fLStep;

         pOut = fH.fPl + fH.fE2*(fH.fPtMag*fH.fCos) + fH.fE3*(fH.fPtMag*fH.fSin);
         fH.fPhi += fH.fPhiStep;
      }
      else
      {
   ...
   else if (fStepper == kRungeKutta)
   {
      ...
      Double_t vecRKOut[7];
      StepRungeKutta(vecRKIn, vecRKOut);

      vOut.fX = vecRKOut[0];
      vOut.fY = vecRKOut[1];
      vOut.fZ = vecRKOut[2];
      vOut.fT = v.fT + fH.fStepSize;
...
//______________________________________________________________________________
void TEveTrackPropagator::SetupStepper(const TEveVector& v, const TEveVector& p)
{
   // Configure stepper parameres.
   //
   // In case stepper is Helix, calculate : phi step,
   // radius and set cached values (e.g.  sin, cos, pt-mag ...)
   //
   // In stepper Runge-Kutta evaluate initial max step size from
   // fDelta, momentum and max filed value.
   ...
   if (fStepper == kHelix)
   {
      ...
      fH.fPlMag = p.Dot(fH.fE1);
      fH.fPl    = fH.fE1 * fH.fPlMag;
      fH.fPt    = p - fH.fPl;
      fH.fPtMag = fH.fPt.Mag();

      Float_t a = fgkB2C * fH.fB.Mag() * Abs(fH.fCharge);
      fH.fValid = (a > kAMin && fH.fPtMag*fH.fPtMag > kPtMinSqr);
      if (fH.fValid)
      {
         fH.fR   = Abs(fH.fPtMag / a);
         fH.fLam = fH.fPl.Mag() / fH.fPtMag;
         if (fH.fPlMag < 0) fH.fLam = - fH.fLam;

         // get phi step, compare fMaxStepAng with fDelta
         fH.fPhiStep = fMaxStepAng * TMath::DegToRad();
         if (fH.fR > fDelta )
         {
            Float_t ang  = 2*ACos(1 - fDelta/fH.fR);
            if (ang < fH.fPhiStep) fH.fPhiStep = ang;
         }
         fH.fStepSize = fH.fR * fH.fPhiStep * Sqrt(1 + fH.fLam*fH.fLam);

         // add additional info for Helix stepper
         fH.fLStep = fH.fR * fH.fPhiStep * fH.fLam;
         fH.fSin   = Sin(fH.fPhiStep);
         fH.fCos   = Cos(fH.fPhiStep);
      }
      else
      {
         fH.fStepSize = fMaxStep;
      }
   }
   else if (fStepper == kRungeKutta)
   {
      // set initial step size RungeKutta
      ...
      // the step-size is used for time value
      fH.fStepSize = fH.fR * fH.fPhiStep * Sqrt(1 + fH.fLam*fH.fLam);
      if (fH.fStepSize > fMaxStep)
         fH.fStepSize = fMaxStep;
   }
}
   ...
   if (np > first_point)
   {
      ...
      if (d1.Mag() > kStepEps)
      {
         ...
         if (step_frac > 0)
         {
            ...
            Float_t origStep = fH.fStepSize;
            fH.fStepSize *= step_frac;
            Step(currV, p, forwV, forwP);
            p     = forwP;
            currV = forwV;
            fPoints.push_back(currV);
            ++np;
            fH.fStepSize = origStep;
         }
...
Bool_t TEveTrackPropagator::GoToVertex(TEveVector& v, TEveVector& p)
{
   ...
   if (fMagFieldObj && fH.fCharge)
      return LoopToVertex(v, p);
   else
      return LineToVertex(v);
}
