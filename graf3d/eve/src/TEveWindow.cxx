...
TEveCompositeFrame::TEveCompositeFrame(TGCompositeFrame *parent, TEveWindow *eve_parent)
   : TGCompositeFrame(parent, 0, 0, kVerticalFrame),

     fTopFrame(nullptr), fToggleBar(nullptr), fTitleBar(nullptr), fIconBar(nullptr), fEveWindowLH(nullptr),

     fMiniBar(nullptr),

     fEveParent(eve_parent), fEveWindow(nullptr),
     ...
     fShowInSync(kTRUE)
{
   ...
   if (fEveParent == nullptr) fEveParent = gEve->GetWindowManager();
...
void TEveCompositeFrame::AcquireEveWindow(TEveWindow* ew)
{
   ...
   if (ew == nullptr) throw eh + "Called with 0 argument.";
...
TEveCompositeFrameInMainFrame::TEveCompositeFrameInMainFrame(TGCompositeFrame *parent, TEveWindow *eve_parent,
                                                             TGMainFrame *mf)
   : TEveCompositeFrame(parent, eve_parent), fMainFrame(mf), fOriginalSlot(nullptr), fOriginalContainer(nullptr)
{
...
{
   if (w == fOriginalSlot) fOriginalSlot = nullptr;

   if (w == fOriginalContainer) fOriginalContainer = nullptr;
}
...
void TEveCompositeFrameInMainFrame::MainFrameClosed()
{
   ...
   if (fEveWindow != nullptr) fEveWindow->DestroyWindowAndSlot();
...
TEveWindow::TEveWindow(const char *n, const char *t)
   : TEveElementList(n, t),
     ...
     fEveFrame(nullptr), fShowTitleBar(kTRUE)
{
   // Constructor.
...
void TEveWindow::SwapWindow(TEveWindow* w)
{
   ...
   if (w == nullptr) throw eh + "Called with null argument.";
...
void TEveWindow::SwapWindowWithCurrent()
{
   ...
   if (current == nullptr) throw eh + "Current eve-window is not set.";
...
void TEveWindow::UndockWindow()
{
   ...
   if (return_cont && !return_cont->CanMakeNewSlots()) return_cont = nullptr;
...
void TEveWindow::UndockWindowDestroySlot()
{
   ...
   ((TEveCompositeFrameInMainFrame *)fEveFrame)->SetOriginalSlotAndContainer(nullptr, return_cont);
...
void TEveWindow::SwapWindows(TEveWindow* w1, TEveWindow* w2)
{
   ...
   if (w1 == nullptr || w2 == nullptr) throw eh + "Called with null window.";
...
TEveWindowSlot::TEveWindowSlot(const char *n, const char *t)
   : TEveWindow(n, t), fEmptyButt(nullptr), fEmbedBuffer(nullptr)
{
...
{
   TEveWindowPack *eve_pack =
      new TEveWindowPack(nullptr, "Pack", "Window container for horizontal and vertical stacking.");
...
{
   TEveWindowTab *eve_tab = new TEveWindowTab(nullptr, "Tab", "Window container for horizontal and vertical stacking.");
...
TGCompositeFrame* TEveWindowSlot::StartEmbedding()
{
   ...
   if (fEmbedBuffer != nullptr) throw eh + "Already embedding.";
...
TGCompositeFrame* TEveWindowFrame::GetGUICompositeFrame()
{
   ...
   if (cf == nullptr) throw kEH + "The registered frame is not a composite-frame.";
