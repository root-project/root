...
TEveCaloViz::TEveCaloViz(TEveCaloData *data, const char *n, const char *t)
   : TEveElement(), TNamed(n, t), TEveProjectable(),

     fData(nullptr), fCellIdCacheOK(kFALSE),

     fEtaMin(-10), fEtaMax(10),

     fPhi(0.), fPhiOffset(TMath::Pi()),

     fAutoRange(kTRUE),

     fBarrelRadius(-1.f), fEndCapPosF(-1.f), fEndCapPosB(-1.f),

     fPlotEt(kTRUE),

     fMaxTowerH(100), fScaleAbs(kFALSE), fMaxValAbs(100),

     fValueIsColor(kFALSE), fPalette(nullptr)
{
   // Constructor.
...
TEveCalo2D::TEveCalo2D(const char *n, const char *t)
   : TEveCaloViz(nullptr, n, t), TEveProjected(), fOldProjectionType(TEveProjection::kPT_Unknown), fMaxESumBin(0),
     fMaxEtSumBin(0)
{
}
...
void TEveCalo2D::CellSelectionChangedInternal(TEveCaloData::vCellId_t& inputCells, std::vector<TEveCaloData::vCellId_t*>& outputCellLists)
{
   ...
   for (UInt_t b = 0; b <= nBins; ++b) outputCellLists[b] = nullptr;
