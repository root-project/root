...
TEveGeoShape::TEveGeoShape(const char *name, const char *title)
   : TEveShape(name, title), fNSegments(0), fShape(nullptr), fCompositeShape(nullptr)
{
   InitMainTrans();
}
                                                  ...
                                                  TEveElement*         parent)
{
   ...
   if (gse->HasElements())
   {
      ...
      while ((chld = (TEveGeoShapeExtract *)next()) != nullptr) SubImportShapeExtract(chld, gsre);
   }
...
TEveGeoShapeProjected::TEveGeoShapeProjected() : TEveShape("TEveGeoShapeProjected"), fBuff(nullptr)
{
}
