...
TEveGListTreeEditorFrame::TEveGListTreeEditorFrame(const TGWindow *p, Int_t width, Int_t height)
   : TGMainFrame(p ? p : gClient->GetRoot(), width, height), fFrame(nullptr), fLTFrame(nullptr), fListTree(nullptr),
     fSplitter(nullptr), fEditor(nullptr), fCtxMenu(nullptr), fSignalsConnected(kFALSE)
{
...
TEveBrowser::TEveBrowser(UInt_t w, UInt_t h)
   : TRootBrowser(nullptr, "Eve Main Window", w, h, "", kFALSE), fFileBrowser(nullptr), fEvePopup(nullptr),
     fSelPopup(nullptr), fHilPopup(nullptr)
{
   // Construct Eve menu.
