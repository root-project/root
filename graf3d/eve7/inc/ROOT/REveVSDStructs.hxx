...
#ifndef ROOT_REveVSDStructs
#define ROOT_REveVSDStructs
...
public:
   ...
   REveMCTrack() : fLabel(-1), fIndex(-1), fEvaLabel(-1),
...
public:
   ...
   REveRecTrackT() : fLabel(-1), fIndex(-1), fStatus(0), fSign(0), fV(), fP(), fBeta(0), fDcaXY(0), fDcaZ(0), fPVX(0), fPVY(0), fPVZ(0) {}
   virtual ~REveRecTrackT() {}

   Float_t Pt() { return fP.Perp(); }

   ClassDef(REveRecTrackT, 2); // Template for reconstructed track (also used in VSD).
};
...
public:
   ...
   REveRecV0() : fStatus(), fVNeg(), fPNeg(), fVPos(), fPPos(),
...
public:
   ...
   REveRecCascade() : fStatus(),  fVBac(), fPBac(),
...
class REveMCRecCrossRef : public TObject
{
public:
   ...
   REveMCRecCrossRef() : fIsRec(false), fHasV0(false), fHasKink(false),
