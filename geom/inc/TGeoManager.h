 ...
 *************************************************************************/
// Author : Andrei Gheata - date Thu 25 Oct 2001 03:32:14 PM CEST

/*************************************************************************
 * TGeoManager - package description
 *
 *
 *
 *************************************************************************/
...
{
public:
static Int_t kGeoDefaultNsegments;  // default number of segments for drawing circles
static Int_t kGeoVisLevel;          // default depth for drawing
static Double_t *kGeoSinTable;      // table of SIN
static Double_t *kGeoCosTable;      // table of COS
static const char *kGeoOutsidePath; // default path when outside geometry
enum EGeoVisOption {
...
// data members
   ...
   Int_t                 fNsegments;        // number of segments for drawing circles
   ...
   Int_t                *fIgnored;          // list of nodes already checked
   Int_t                 fILevel;           // level of the ignored nodes
   Int_t                 fNignored;         // number of nodes to be ignored
...
//--- private methods
   ...
   TGeoNode              *FindNodeLast(Bool_t downwards); // obsolete
   TGeoNode              *FindNodeOverlap(); // obsolete
   Int_t                  GetTouchedCluster(Int_t start, Double_t *point, Int_t *check_list,
                                            Int_t ncheck, Int_t *result);
   void                   Down(Int_t id);
   void                   Top();
   void                   Up();
   Bool_t                 IsLoopingVolumes()     {return fLoopVolumes;}
   const char            *Path();
   void                   SetLoopVolumes(Bool_t flag=kTRUE) {fLoopVolumes=flag;}
   void                   Voxelize(Option_t *option = 0);

protected:
   Bool_t                IsIgnored(Int_t inode) 
                            {return (fNignored==0)?kFALSE:TGeoVoxelFinder::SearchFast(inode, fNignored, fIgnored);}
   void                  ResetIgnored() {fNignored=0;}
public:
   // constructors
   TGeoManager();
   ...
   Int_t                  GetNsegments()     {return fNsegments;}
   TVirtualGeoPainter    *GetMakeDefPainter();
   Int_t                  GetBombMode()      {return fExplodedView;}
   const char            *GetDrawPath()      {return fVisBranch;}
   Int_t                  GetVisLevel()      {return fVisLevel;}
   Int_t                  GetVisOption()     {return fVisOption;}
   ...
   void                   CloseGeometry();
   Bool_t                 IsClosed() {return ((fCache==0)?kFALSE:kTRUE);}
   ...
   //--- list getters
   TList                 *GetListOfMatrices()      {return fMatrices;}
   TList                 *GetListOfMaterials()     {return fMaterials;}
   TList                 *GetListOfVolumes()       {return fVolumes;}
   TList                 *GetListOfGVolumes()      {return fGVolumes;} 
   TList                 *GetListOfShapes()        {return fShapes;}

   //--- modeler state getters/setters
   TGeoNode              *GetNode(Int_t level)  {return (TGeoNode*)fNodes->At(level);}
   TGeoNode              *GetMother(Int_t up=1) {return gGeoNodeCache->GetMother(up);}
   TGeoNode              *GetCurrentNode()      {return fCurrentNode;}
   Double_t              *GetCurrentPoint()     {return fPoint;}
   TGeoVolume            *GetCurrentVolume() {return fCurrentNode->GetVolume();}
   TGeoHMatrix           *GetCurrentMatrix() {return (TGeoHMatrix*)fGlobalMatrices->At(fLevel);}
   Double_t              *GetCldirChecked()  {return fCldirChecked;}
   Double_t              *GetCldir()         {return fCldir;}
   Double_t              *GetNormalChecked() {return fNormalChecked;}
   Double_t              *GetNormal()        {return fNormal;}
   Int_t                  GetLevel()         {return fLevel;}
   const char            *GetPath();
   Int_t                  GetStackLevel()    {return gGeoNodeCache->GetStackLevel();}
   TGeoVolume            *GetTopVolume()     {return fTopVolume;}
   TGeoNode              *GetTopNode()       {return fTopNode;}
...
//   TGeoShape             *GetShape(const char *name);
   ...
   TGeoNodeCache         *GetCache()         {return fCache;}
