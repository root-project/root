...
void TGeoConeEditor::DoApply()
{
   ...
   if (rmin1 < 0 || rmin1 > rmax1)
      return;
   if (rmin2 < 0 || rmin2 > rmax2)
      return;
   if (dz <= 0)
      return;
...
TGeoConeSegEditor::TGeoConeSegEditor(const TGWindow *p, Int_t width, Int_t height, UInt_t options, Pixel_t back)
   : TGeoConeEditor(p, width, height, options | kVerticalFrame, back)
{
   fLock = kFALSE;
   MakeTitle("Phi range");
   TGTextEntry *nef;
   TGCompositeFrame *compxyz =
      new TGCompositeFrame(this, 155, 110, kHorizontalFrame | kFixedWidth | kFixedHeight | kRaisedFrame);
   // Vertical slider
...
void TGeoConeSegEditor::DoApply()
{
   ...
   Double_t rmin1 = fERmin1->GetNumber();
   Double_t rmax1 = fERmax1->GetNumber();
   if (rmin1 < 0 || rmax1 < rmin1)
      return;
   Double_t rmin2 = fERmin2->GetNumber();
   Double_t rmax2 = fERmax2->GetNumber();
   if (rmin2 < 0 || rmax2 < rmin2)
      return;
   Double_t dz = fEDz->GetNumber();
   Double_t phi1 = fEPhi1->GetNumber();
   Double_t phi2 = fEPhi2->GetNumber();
