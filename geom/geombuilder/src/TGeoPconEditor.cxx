...
Bool_t TGeoPconEditor::CheckSections(Bool_t change)
{
   ...
   for (Int_t isect = 0; isect < fNsections; isect++) {
      ...
      zmin = sect->GetZ();
      if (sect->GetRmin() < 0 || (sect->GetRmax() < 0) || ((sect->GetRmin() == 0) && (sect->GetRmax() == 0))) {
         if (!change)
            return kFALSE;
         sect->SetRmin(rmin);
         sect->SetRmax(rmax);
...
void TGeoPconEditor::DoSectionChange(Int_t isect)
{
   ...
   if (isect < fNsections - 1)
      secthi = (TGeoPconSection *)fSections->At(isect + 1);
...
void TGeoPconEditor::DoPhi()
{
   Double_t phi1 = fEPhi1->GetNumber();
   Double_t dphi = fEDPhi->GetNumber();
   if (TMath::Abs(phi1) > 360)
      fEPhi1->SetNumber(0);
   if (dphi > 360)
      fEDPhi->SetNumber(360);
   DoModified();
   if (!IsDelayed())
      DoApply();
}
