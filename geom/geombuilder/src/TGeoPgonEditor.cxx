...
TGeoPgonEditor::TGeoPgonEditor(const TGWindow *p, Int_t width, Int_t height, UInt_t options, Pixel_t back)
...
void TGeoPgonEditor::DoApply()
{
   ...
   if (TMath::Abs(phi1 - fShape->GetPhi1()) > 1.e-6)
      fShape->Phi1() = phi1;
   if (TMath::Abs(dphi - fShape->GetDphi()) > 1.e-6)
      fShape->Dphi() = dphi;
   if (nedges != shape->GetNedges())
      shape->SetNedges(nedges);
