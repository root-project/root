                                          ...
                                          Double_t * /*safe*/) const
{
   ...
   mybvh->intersect<false, use_robust_traversal>(ray, mybvh->get_root().index, stack, [&](size_t begin, size_t end) {
      for (size_t prim_id = begin; prim_id < end; ++prim_id) {
         ...
         auto thisdist =
            rayFacet(Vertex_t(point[0], point[1], point[2]), Vertex_t(dir[0], dir[1], dir[2]), facet, fVertices, 0.);
...
bool TGeoTessellated::Contains(Double_t const *point) const
{
   ...
   mybvh->intersect<false, use_robust_traversal>(ray, mybvh->get_root().index, stack, [&](size_t begin, size_t end) {
      for (size_t prim_id = begin; prim_id < end; ++prim_id) {
         ...
         if (rayFacetHit(Vertex_t(point[0], point[1], point[2]),
                         Vertex_t(test_dir[0], test_dir[1], test_dir[2]), facet, fVertices, 0.)) {
            ++crossings;
         }
      }
      return false;
   ...
   do {
      if (currnode.is_leaf()) {
         ...
         for (auto p_id = begin_prim_id; p_id < end_prim_id; p_id++) {
            ...
            auto thissafetySQ = pointFacetDistSq(Vec3f{point[0], point[1], point[2]}, facet, fVertices);
...
Double_t TGeoTessellated::DistFromInside_Loop(const Double_t *point, const Double_t *dir) const
{
   ...
   for (size_t i = 0; i < fFacets.size(); ++i) {
      ...
      const double t = rayFacet(p, d, facet, fVertices, 0.);
...
bool TGeoTessellated::Contains_Loop(const Double_t *point) const
{
   ...
   for (size_t i = 0; i < fFacets.size(); ++i) {
      ...
      if (rayFacetHit(p, test_dir, facet, fVertices, 0.)) {
         ++crossings;
      }
   }
   return (crossings & 1);
