...
struct Index {
   ...
   Index() = default;
   explicit Index(Type val) : value(val) {}

   // bool operator == (const Index&) const = default;
   // bool operator != (const Index&) const = default;
   bool operator==(const Index &other) const { return other.value == value; }
   bool operator!=(const Index &other) const { return other.value != value; }
   ...
   BVH_ALWAYS_INLINE void set_first_id(size_t first_id) { *this = Index{first_id, prim_count()}; }

   BVH_ALWAYS_INLINE void set_prim_count(size_t prim_count) { *this = Index{first_id(), prim_count}; }
   ...
   static BVH_ALWAYS_INLINE Index make_inner(size_t first_child) { return Index{first_child, 0}; }

private:
   explicit Index(size_t first_id, size_t prim_count)
      : Index((static_cast<Type>(first_id) << prim_count_bits) | (static_cast<Type>(prim_count) & max_prim_count))
   {
