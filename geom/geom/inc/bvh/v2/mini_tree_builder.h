...
public:
   ...
   build(ThreadPool &thread_pool, std::span<const BBox> bboxes, std::span<const Vec> centers, const Config &config = {})
   {
...
private:
   ...
   struct LocalBins {
      ...
      {
         bins.resize(std::remove_if(bins.begin(), bins.end(), [](const Bin &bin) { return bin.ids.empty(); }) -
                     bins.begin());
      }
   ...
   struct BuildTask {
      ...
      BuildTask(MiniTreeBuilder *builder_, Bvh<Node> &bvh_, std::vector<size_t> &&prim_ids_)
         : builder(builder_), bvh(bvh_), prim_ids(std::move(prim_ids_))
      {
      }
   ...
   BVH_ALWAYS_INLINE MiniTreeBuilder(ThreadPool &thread_pool, std::span<const BBox> bboxes,
                                     std::span<const Vec> centers, const Config &config)
   ...
   std::vector<Bvh<Node>> prune_mini_trees(std::vector<Bvh<Node>> &&mini_trees)
   {
      ...
      executor_.for_each(0, pruned_roots.size(), [&](size_t begin, size_t end) {
         for (size_t i = begin; i < end; ++i) {
            if (pruned_roots[i].second == 0)
               pruned_trees[i] = std::move(mini_trees[pruned_roots[i].first]);
            else
               pruned_trees[i] = mini_trees[pruned_roots[i].first].extract_bvh(pruned_roots[i].second);
         }
      });
      return pruned_trees;
   }
   ...
   Bvh<Node> build_top_bvh(std::vector<Bvh<Node>> &mini_trees)
   {
      ...
      executor_.for_each(0, mini_trees.size(), [&](size_t begin, size_t end) {
         for (size_t i = begin; i < end; ++i) {
            auto &mini_tree = mini_trees[i];
            ...
            std::copy(mini_tree.prim_ids.begin(), mini_tree.prim_ids.end(), bvh.prim_ids.begin() + prim_offsets[i]);
         }
      });
