...
public:
   ...
   BVH_ALWAYS_INLINE static Bvh<Node>
   build(std::span<const BBox> bboxes, std::span<const Vec> centers, const Config &config = {})
   {
...
protected:
   ...
   struct Bin {
      ...
      BVH_ALWAYS_INLINE void add(const BBox &bbox_, size_t prim_count_ = 1)
      {
         this->bbox.extend(bbox_);
         this->prim_count += prim_count_;
      }
   ...
   BVH_ALWAYS_INLINE void fill_bins(PerAxisBins &per_axis_bins, const BBox &bbox, size_t begin, size_t end)
   {
      ...
      for (size_t i = begin; i < end; ++i) {
         ...
         static_for<0, Node::dimension>([&](size_t axis) {
            size_t index = std::min(BinCount - 1, static_cast<size_t>(robust_max(pos[axis], static_cast<Scalar>(0.))));
   ...
   size_t fallback_split(size_t axis, size_t begin, size_t end)
   {
      ...
      std::partial_sort(prim_ids_.begin() + begin, prim_ids_.begin() + mid, prim_ids_.begin() + end,
   ...
   std::optional<size_t> try_split(const BBox &bbox, size_t begin, size_t end) override
   {
      ...
      auto split_pos = fast_mul_add(bbox.get_diagonal()[best_split.axis] / static_cast<Scalar>(BinCount),
                                    static_cast<Scalar>(best_split.bin_id), bbox.min[best_split.axis]);
