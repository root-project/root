...
Int_t TGeoPainter::DistanceToPrimitiveVol(TGeoVolume *volume, Int_t px, Int_t py)
{
   ...
   while ((daughter = next())) {
      ...
      if (volume->IsVisContainers()) {
         ...
         if (last || !daughter->IsVisDaughters())
            next.Skip();
      }
   }
   return dist;
...
const char *TGeoPainter::GetVolumeInfo(const TGeoVolume *volume, Int_t /*px*/, Int_t /*py*/) const
{
   ...
   if (fPaintingOverlaps) {
      ...
      if (fOverlap->IsExtrusion())
         ...
         ovtype = "OVERLAP";
      if (volume == fOverlap->GetFirstVolume())
         name = volume->GetName();
      else
         name = fOverlap->GetSecondVolume()->GetName();
      info = TString::Format("%s: %s of %g", name.Data(), ovtype.Data(), fOverlap->GetOverlap());
      return info;
   } else
      info = TString::Format("%s, shape=%s", fVolInfo.Data(), volume->GetShape()->ClassName());
   return info;
}
...
{
   if (!fGeoManager || !fTopVolume)
      return;
   Bool_t is_padviewer = kTRUE;
   if (gPad)
      is_padviewer = (!strcmp(gPad->GetViewer3D()->ClassName(), "TViewer3DPad")) ? kTRUE : kFALSE;

   fIsRaytracing = fTopVolume->IsRaytracing();
   if (fTopVolume->IsVisContainers())
      fVisOption = kGeoVisDefault;
   else if (fTopVolume->IsVisLeaves())
      fVisOption = kGeoVisLeaves;
   else if (fTopVolume->IsVisOnly())
      fVisOption = kGeoVisOnly;
   else if (fTopVolume->IsVisBranch())
      fVisOption = kGeoVisBranch;
...
void TGeoPainter::PaintVolume(TGeoVolume *top, Option_t *option, TGeoMatrix *global)
{
   ...
   while ((daughter = next())) {
      ...
      if (top->IsVisContainers()) {
         ...
         // Check if we have to skip the branch
         if (!drawDaughters || last || !daughter->IsVisDaughters())
            next.Skip();
      }
   }
   if (fPlugin)
...
void TGeoPainter::Raytrace(Option_t *)
{
   ...
   Double_t cov[3];
   for (Int_t i = 0; i < 3; i++)
      cov[i] = 0.5 * (min[i] + max[i]);
   Double_t cop[3];
   for (Int_t i = 0; i < 3; i++)
      cop[i] = cov[i] - dir[i] * dview;
   ...
   for (px = pxmin; px < pxmax; px++) {
      for (py = pymin; py < pymax; py++) {
         ...
         if (rtMode > 0) {
            ...
            if (!norm)
               norm = fGeoManager->FindNormalFast();
            if (!norm)
               continue;
         }
...
{
   if (level == fVisLevel && fLastVolume == fTopVolume)
      return;
   fVisLevel = level;
   if (!fTopVolume)
      return;
...
void TGeoPainter::SetVisOption(Int_t option)
{
   ...
   if (fTopVolume) {
      ...
      switch (option) {
      case kGeoVisDefault: att->SetAttBit(TGeoAtt::kVisContainers, kTRUE); break;
      case kGeoVisLeaves: break;
      case kGeoVisOnly: att->SetAttBit(TGeoAtt::kVisOnly, kTRUE); break;
      }
   }

   if (fVisOption == option)
      return;
   fVisOption = option;
   if (fVisLock) {
      ClearVisibleVolumes();
...
Int_t TGeoPainter::ShapeDistancetoPrimitive(const TGeoShape *shape, Int_t numpoints, Int_t px, Int_t py) const
{
   ...
   for (Int_t i = 0; i < numpoints; i++) {
      ...
      points[j] = dmaster[0];
      points[j + 1] = dmaster[1];
      points[j + 2] = dmaster[2];
   ...
   if (dist < maxdist && fIsPaintingShape)
      gPad->SetSelected((TObject *)shape);
   return dist;
}
