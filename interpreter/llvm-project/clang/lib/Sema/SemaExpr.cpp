          ...
          if (const AnnotateAttr *A = VD->getAttr<AnnotateAttr>()) {
            ...
            if (!aTy->isDeduced() && A->getAnnotation() == "__Auto") {
              ...
              TemplateDeductionResult Result = DeduceAutoType(
                  TrivialTSI->getTypeLoc(), RHSExpr, ResTy, Info);
              ...
              // A reference to the decl is semantically closest to what we
              // want.
