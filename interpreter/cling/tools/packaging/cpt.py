...
def _perror(e):
    print("""subprocess.CalledProcessError: Command '%s'
    ...
    if "github.com" in LLVM_GIT_URL and args[
        ...
        print('Extracting: ' +
              os.path.join(workdir, 'cling-patches-r%s.tar.gz' %
                           llvm_revision))
        tar = tarfile.open(
            os.path.join(workdir, 'cling-patches-r%s.tar.gz' % llvm_revision))
...
def download_llvm_binary():
    ...
    if DIST == "Ubuntu":
        ...
        if llvm_config_path != '' and tar_required is False:
            ...
            llvm_flags = """-DLLVM_BINARY_DIR={0} -DLLVM_CONFIG={1} -DLLVM_LIBRARY_DIR={2}
            -DLLVM_MAIN_INCLUDE_DIR={3} -DLLVM_TABLEGEN_EXE={4} \
        ...
        raise Exception('''Building clang using LLVM binary not possible.
            Please invoke cpt without --with-binary-llvm
            and --with-llvm-tar flags''')

    if tar_required:
        llvm_flags = """-DLLVM_BINARY_DIR={0} -DLLVM_CONFIG={1} -DLLVM_LIBRARY_DIR={2}
        -DLLVM_MAIN_INCLUDE_DIR={3} -DLLVM_TABLEGEN_EXE={4} \
        -DLLVM_TOOLS_BINARY_DIR={5} -DLLVM_TOOL_CLING_BUILD=ON""".format(
            srcdir, os.path.join(srcdir, 'bin', 'llvm-config'),
            os.path.join(srcdir, 'lib'), os.path.join(srcdir, 'include'),
            os.path.join(srcdir, 'bin', 'llvm-tblgen'),
            os.path.join(srcdir, 'bin'))
        if DIST == "Ubuntu" and REV == '22.04' and is_os_64bit():
            download_link = "http://releases.llvm.org/5.0.2/"
            ...
            exec_subprocess_call(
                ...
                workdir)
            exec_subprocess_call(
                'mv clang+llvm-5.0.2-x86_64-linux-gnu-ubuntu-16.04 %s' %
                srcdir, workdir)
            ...
            exec_subprocess_call(
                ...
                workdir)
            exec_subprocess_call(
                'mv clang+llvm-5.0.2-x86_64-linux-gnu-ubuntu-14.04 %s' %
                srcdir, workdir)
            ...
            exec_subprocess_call('wget %s' % download_link, workdir)
            exec_subprocess_call(
                'tar xvf clang+llvm-5.0.2-x86_64-apple-darwin.tar.xz', workdir)
            exec_subprocess_call(
                'sudo mv clang+llvm-5.0.2-x86_64-apple-darwin %s' % srcdir,
                workdir)
        else:
            raise Exception('''Building clang using LLVM binary not possible.
                Please invoke cpt without --with-binary-llvm
...
def fetch_cling(arg):
    ...
    def get_fresh_cling():
        ...
        # if arg == 'last-stable':
        #    checkout_branch = exec_subprocess_check_output
        # ('git describe --match v* --abbrev=0 --tags | head -n 1',
        # CLING_SRC_DIR)
...
def compile(arg):
    ...
    # Either because ccache(when enabled)
    # or maybe the virtualization environment
    ...
    build = Build()
    cmake_config_flags = (
        srcdir + ' -DLLVM_BUILD_TOOLS=Off -DCMAKE_BUILD_TYPE={0}'.format(
            build.buildType) +
        ' -DCMAKE_INSTALL_PREFIX={0} '.format(TMP_PREFIX) +
...
def compile_for_binary(arg):
    ...
    cmake_config_flags = (
        ...
        'NVPTX -DCLING_CXX_HEADERS=ON -DCLING_INCLUDE_TESTS=ON' +
        EXTRA_CMAKE_FLAGS)
            ...
            for regex in regex_array:
                ...
                if re.search(regex, f):
                    ...
                    if not os.path.isdir(
                            os.path.join(prefix, os.path.dirname(f))):
                        os.makedirs(os.path.join(prefix, os.path.dirname(f)))
                    shutil.copy(os.path.join(TMP_PREFIX, f),
...
def install_prefix_for_binary():
    travis_fold_start("install")
    global prefix
    CPT_SRC_DIR = os.path.join(clangdir, 'tools', 'cling', 'tools',
                               'packaging')
    set_vars_for_lit()
...
def setup_tests():
    ...
    llvm_revision = urlopen(
        "https://raw.githubusercontent.com/root-project/cling/"
        "master/LastKnownGoodLLVMSVNRevision.txt").readline().strip().decode(
            'utf-8')
    ...
    subprocess.Popen(
        [
            "sudo wget https://github.com/llvm/llvm-project/archive/{0}.zip" +
            ' && sudo unzip {0}.zip "llvm-project-{0}/llvm/utils/*"'.format(
                commit)
        ],
    ...
    subprocess.Popen(
        [
            "sudo cp -r llvm-project-{0}/llvm/utils/FileCheck FileCheck".
            format(commit)
        ],
        cwd=os.path.join(CLING_SRC_DIR, "tools"),
        shell=True,
    ...
    with open(os.path.join(CLING_SRC_DIR, 'tools', 'CMakeLists.txt'),
              'a') as file:
        file.writelines('add_subdirectory(\"FileCheck\")')
    exec_subprocess_call("cmake {0}".format(LLVM_OBJ_ROOT), CLING_SRC_DIR)
    exec_subprocess_call(
        "cmake --build . --target FileCheck -- -j{0}".format(
            multiprocessing.cpu_count()), LLVM_OBJ_ROOT)
    if not os.path.exists(os.path.join(CLING_SRC_DIR, "..", "clang", "test")):
        llvm_dir = exec_subprocess_check_output("llvm-config --src-root",
                                                ".").strip()
        if llvm_dir == "":
            if tar_required:
...
def cleanup():
    ...
    if args['current_dev'] == 'deb' or args['last_stable'] == 'deb' or args[
            'deb_tag']:
        print('Create output directory: ' +
              os.path.join(workdir, 'cling-%s-1' % (VERSION)))
        os.makedirs(os.path.join(workdir, 'cling-%s-1' % (VERSION)))

        for file in glob.glob(os.path.join(workdir, 'cling_%s*' % (VERSION))):
            print(file + '->' +
                  os.path.join(workdir, 'cling-%s-1' %
    ...
    if args['current_dev'] == 'dmg' or args['last_stable'] == 'dmg' or args[
            'dmg_tag']:
        if os.path.isfile(
                os.path.join(workdir, 'cling-%s-temp.dmg' % (VERSION))):
            print("Remove file: " +
                  os.path.join(workdir, 'cling-%s-temp.dmg' % (VERSION)))
...
def check_ubuntu(pkg):
    if pkg == "gnupg":
        SIGNING_USER = exec_subprocess_check_output(
            'gpg --fingerprint | grep uid | sed s/"uid *"//g', '/').strip()
        if SIGNING_USER == '':
        ...
        output = exec_subprocess_check_output(
            '{cmake} --version'.format(cmake=CMAKE),
            '/').strip().split('\n')[0].split()
        if (output
                == []) or (not check_version_string_ge(output[-1], '3.4.3')):
        ...
        if float(
                exec_subprocess_check_output('gcc -dumpversion',
                                             '/')[:3].strip()) <= 4.7:
        ...
        if float(
                exec_subprocess_check_output('g++ -dumpversion',
                                             '/')[:3].strip()) <= 4.7:
            print(pkg.ljust(20) + '[UNSUPPORTED VERSION (<4.7)]'.ljust(30))
            return False
        else:
        ...
        if exec_subprocess_check_output(
                'which llvm-config-{0}'.format(llvm_vers), workdir) != '':
            print(pkg.ljust(20) + '[OK]'.ljust(30))
            return True
        else:
            print(pkg.ljust(20) + '[NOT INSTALLED]'.ljust(30))
            return False
    elif exec_subprocess_check_output(
            "dpkg-query -W -f='${Status}' %s" +
            " 2>/dev/null | grep -c 'ok installed'" % (pkg),
            '/').strip() == '0':
        print(pkg.ljust(20) + '[NOT INSTALLED]'.ljust(30))
        return False
    else:
...
def tarball_deb():
    ...
    tar = tarfile.open(
        os.path.join(workdir, 'cling_' + VERSION + '.orig.tar.bz2'), 'w:bz2')
    tar.add(prefix, arcname=os.path.basename(prefix))
    tar.close()
...
def debianize():
    SIGNING_USER = exec_subprocess_check_output(
        'gpg --fingerprint | grep uid | sed s/"uid *"//g',
        CLING_SRC_DIR).strip()
    ...
    print("Create file: " +
          os.path.join(prefix, 'debian', 'source', 'lintian-overrides'))
    f = open(os.path.join(prefix, 'debian', 'source', 'lintian-overrides'),
             'w')
    f.write('cling source: source-is-missing')
    f.close()

    # This section is no longer valid.
    # I have kept it as a reference if we plan to
...
Comment: Cling can also be licensed under University of Illinois/NCSA
 ...
 http://root.cern.ch/gitweb?p=cling.git;a=blob_plain;f=LICENSE.TXT;hb=HEAD
'''
    f.write(template.strip())
    f.close()
...
''' % (VERSION, VERSION)
    ...
    if '~dev' in VERSION:
        ...
        template = exec_subprocess_check_output(
            'git log v' + TAG + '...HEAD --format="  * %s" | fmt -s',
            CLING_SRC_DIR)
    ...
    template = exec_subprocess_check_output(
        'git log v0.1 --format="  * %s%n -- %an <%ae>  %cD%n"', CLING_SRC_DIR)
...
def check_redhat(pkg):
    if pkg == "cmake":
        ...
        if not check_version_string_ge(
            ...
            print(pkg.ljust(20) + '[OK]'.ljust(30))
    elif exec_subprocess_check_output("rpm -qa | grep -w %s" % (pkg),
                                      '/').strip() == '':
        print(pkg.ljust(20) + '[NOT INSTALLED]'.ljust(30))
        return False
    else:
...
def rpm_build():
    ...
    shutil.move(os.path.join(workdir,
                             os.path.basename(prefix) + '.tar.bz2'),
                os.path.join(workdir, 'rpmbuild', 'SOURCES'))

    box_draw("Generate RPM SPEC file")
    print('Create file: ' +
          os.path.join(workdir, 'rpmbuild', 'SPECS', 'cling-%s.spec' %
                       (VERSION)))
    f = open(
        os.path.join(workdir, 'rpmbuild', 'SPECS',
                     'cling-%s.spec' % (VERSION)), 'w')
...
mv $RPM_BUILD_DIR/%{name}-%{version}/docs/*
$RPM_BUILD_DIR/%{name}-%{version}/usr/share/doc/
mv $RPM_BUILD_DIR/%{name}-%{version}/lib/ $RPM_BUILD_DIR/%{name}-%{version}/usr
mv $RPM_BUILD_DIR/%{name}-%{version}/include/
$RPM_BUILD_DIR/%{name}-%{version}/usr
mv $RPM_BUILD_DIR/%{name}-%{version}/share/*
$RPM_BUILD_DIR/%{name}-%{version}/usr/share

rm -Rf $RPM_BUILD_DIR/%{name}-%{version}/docs
rm -Rf $RPM_BUILD_DIR/%{name}-%{version}/share

if [ ${RPM_ARCH} = 'x86_64' ]; then
    mv $RPM_BUILD_DIR/%{name}-%{version}/usr/lib
    $RPM_BUILD_DIR/%{name}-%{version}/usr/lib64
fi
...
'''
    ...
    exec_subprocess_call(
        ...
        (os.path.join(workdir, 'rpmbuild', 'SPECS', 'cling-%s.spec' %
                      (VERSION))), os.path.join(workdir, 'rpmbuild'))
...
def check_win(pkg):
    ...
    if pkg == "msvc":
        if exec_subprocess_check_output(
                'REG QUERY HKEY_CLASSES_ROOT\\VisualStudio.DTE.14.0',
                'C:\\').find('ERROR') == -1:
            print(pkg.ljust(20) + '[OK]'.ljust(30))
        else:
            print(pkg.ljust(20) + '[NOT INSTALLED]'.ljust(30))
...
def get_win_dep():
    ...
    if args['current_dev'] == 'nsis' or (args['current_dev'] == 'pkg'
                                         and OS == 'Windows'):
        box_draw("Download NSIS compiler")
        html = urlopen(
            'https://sourceforge.net/p/nsis/code/6780/log/?path=/NSIS/tags'
        ).read().decode('utf-8')
        pin = '<p>Tagging for release'
        NSIS_VERSION = html[html.find(pin):html.find('</div>', html.find(pin)
                                                     )].strip(pin + ' ')
        print('Latest version of NSIS is: ' + NSIS_VERSION)
        wget(url="https://sourceforge.net/projects/nsis/" +
             "files/NSIS%%203/%s/nsis-%s.zip" % (NSIS_VERSION, NSIS_VERSION),
             out_dir=TMP_PREFIX)
        print('Extracting: ' + os.path.join(TMP_PREFIX, 'nsis-%s.zip' %
                                            (NSIS_VERSION)))
        zip = zipfile.ZipFile(
            os.path.join(TMP_PREFIX, 'nsis-%s.zip' % (NSIS_VERSION)))
        zip.extractall(os.path.join(TMP_PREFIX, 'bin'))
        print('Remove file: ' + os.path.join(TMP_PREFIX, 'nsis-%s.zip' %
                                             (NSIS_VERSION)))
        os.rename(os.path.join(TMP_PREFIX, 'bin', 'nsis-%s' % (NSIS_VERSION)),
                  os.path.join(TMP_PREFIX, 'bin', 'nsis'))

    def tryCmake(cmake):
        try:
            rslt = exec_subprocess_check_output(cmake + ' --version',
                                                TMP_PREFIX)
            vers = [int(v) for v in rslt.split()[2].split('.')]
            if vers[0] >= 3 and (vers[1] > 6 or
                                 (vers[1] == 6 and vers[2] >= 2)):
                return cmake
        except Exception:
            pass
        return False
...
def make_nsi():
    ...
    VIProductVersion = exec_subprocess_check_output("git describe --match v*"
                                                    " --abbrev=0 --tags",
                                                    CLING_SRC_DIR)
    VIProductVersion = VIProductVersion.strip().splitlines()[0]
...
!define VERSION "%s"
!define COPYRIGHT "Copyright © 2007-2014 by the Authors; Developed by
...
!define MUI_LICENSEPAGE_TEXT_BOTTOM "The source code for Cling is freely
redistributable under the terms of the GNU Lesser General Public License
...
; file section
Section "MainFiles"
''' % (prefix, VERSION, os.path.basename(prefix) + '-setup.exe',
       VIProductVersion.replace('v', ''), CPT_SRC_DIR, NSIS,
...
Section make_uninstaller
 ...
 \\Uninstall\\Cling" "DisplayName" "Cling"
 WriteRegStr HKLM "Software\\Microsoft\\Windows\\CurrentVersion
 \\Uninstall\\Cling" "UninstallString" "$INSTDIR\\uninstall.exe"
 WriteRegDWORD HKLM "Software\\Microsoft\\Windows\\CurrentVersion
 \\Uninstall\\Cling" "NoModify" 1
 WriteRegDWORD HKLM "Software\\Microsoft\\Windows\\CurrentVersion
 \\Uninstall\\Cling" "NoRepair" 1
...
Section "Shortcuts"
 ...
 CreateShortCut "$SMPROGRAMS\\Cling\\Uninstall.lnk"
 "$INSTDIR\\uninstall.exe" "" "$INSTDIR\\uninstall.exe" 0
 CreateShortCut "$SMPROGRAMS\\Cling\\Cling.lnk"
 "$INSTDIR\\bin\\cling.exe" "" "${MUI_ICON}" 0
 CreateDirectory "$SMPROGRAMS\\Cling\\Documentation"
 CreateShortCut "$SMPROGRAMS\\Cling\\Documentation\\
 Cling (PS).lnk" "$INSTDIR\\docs\\llvm\\ps\\cling.ps" "" "" 0
 CreateShortCut "$SMPROGRAMS\\Cling\\Documentation\\
 Cling (HTML).lnk" "$INSTDIR\\docs\\llvm\\html\\cling\\cling.html" "" "" 0
...
; Function to detect Windows version and abort if
Cling is unsupported in the current platform
...
is_winnt:
  StrCpy $1 $0 1
  StrCmp $1 4 is_error ; Aborting installation
  for Windows versions older than Windows 2000
...
is_error:
  ...
  IfErrors 0 +2
  StrCpy $0 "Unknown"
  MessageBox MB_ICONSTOP|MB_OK "This version of Cling
  cannot be installed on this system.
  Cling is supported only on Windows NT systems.
...
Function CheckPrevVersion
  ...
  MessageBox MB_OK|MB_ICONSTOP "Another Cling installation
  (with the same version) has been detected. Please uninstall it first."
  Abort
otherver:
  StrCpy $0 0
  StrCpy $2 ""
...
finalcheck:
  ...
  "Another Cling installation (version $2) has been detected.
  It is recommended to uninstall it if you
  intend to use the same installation directory.
  Do you want to proceed with the installation anyway?" IDYES done IDNO 0
  Abort
done:
  ClearErrors
  Pop $2
...
def build_nsis():
    ...
    exec_subprocess_call(
        '%s -V3 %s' % (os.path.join(
            NSIS, 'makensis.exe'), os.path.join(workdir, 'cling.nsi')),
        workdir)
...
def check_mac(pkg):
    if pkg == "cmake":
        ...
        if pkg == "clang++":
            if float(
                    exec_subprocess_check_output('clang++ -dumpversion',
                                                 '/')[:3].strip()) <= 4.1:
...
def make_dmg():
    ...
    if os.path.isdir(os.path.join(workdir, '%s.app' % (APP_NAME))):
        print("Remove directory: " + os.path.join(workdir, '%s.app' %
                                                  (APP_NAME)))
    ...
    if os.path.isdir(os.path.join(workdir, '%s.app' % (APP_NAME))):
        print("Remove directory:", os.path.join(workdir,
                                                '%s.app' % (APP_NAME)))
        shutil.rmtree(os.path.join(workdir, '%s.app' % (APP_NAME)))
    ...
    os.makedirs(
        os.path.join(workdir, '%s.app' % (APP_NAME), 'Contents', 'Resources'))
    shutil.copyfile(
    ...
    plist_path = os.path.join(workdir, '%s.app' % (APP_NAME), 'Contents',
                              'Info.plist')
    f = open(plist_path, 'w')
    plist_xml = '''
...
<dict>
  ...
  <string>Copyright © 2007-2014 by the Authors;
  Developed by The ROOT Team, CERN and Fermilab</string>
...
'''.format(version=VERSION,
    ...
    exec_subprocess_call('strip -u -r %s' % os.path.join(workdir, APP_EXE),
                         workdir)
    ...
    exec_subprocess_call(
        '{dmgbuild} -s {settings} -D app'.format(
            dmgbuild=os.path.join(workdir, 'pip', 'bin', 'dmgbuild'),
            settings=os.path.join(CPT_SRC_DIR, 'settings.py')) +
        '={app} -D size={size}M "{volname}" {dmg}'.format(app=os.path.join(
            workdir, '%s.app' % (APP_NAME)),
                                                          dmg=DMG_FINAL,
                                                          volname=VOL_NAME,
                                                          size=SIZE), workdir)
...
parser.add_argument(
    '-c',
    '--check-requirements',
    help='Check if packages required by the script are installed',
    action='store_true')
parser.add_argument('--current-dev',
                    help=('''--current-dev:<tar | deb | nsis | rpm | dmg | pkg>
    will package the latest development snapshot in the given format''' +
                          '''\n--current-dev:branch:<branch> will
    build <branch> on llvm, clang, and cling''' +
                          '''\n--current-dev:branches:<a,b,c> will build
    branch <a> on llvm, <b> on clang, and <c> on cling'''))
parser.add_argument('--last-stable',
                    help='''Package the last stable snapshot in one
    of these formats: tar | deb | nsis | rpm | dmg | pkg''')
parser.add_argument(
    '--tarball-tag',
    help='Package the snapshot of a given tag in a tarball (.tar.bz2)')
parser.add_argument(
    '--deb-tag',
    help='Package the snapshot of a given tag in a Debian package (.deb)')
parser.add_argument(
    '--rpm-tag',
    help='Package the snapshot of a given tag in an RPM package (.rpm)')
parser.add_argument(
    '--nsis-tag',
    help='Package the snapshot of a given tag in an NSIS installer (.exe)')
parser.add_argument(
    '--dmg-tag',
    help='Package the snapshot of a given tag in a DMG package (.dmg)')

# Variable overrides
parser.add_argument('--with-llvm-url',
                    action='store',
                    help='Specify an alternate URL of LLVM repo')
parser.add_argument('--with-clang-url',
                    action='store',
                    help='Specify an alternate URL of Clang repo',
                    default='http://root.cern.ch/git/clang.git')
parser.add_argument('--with-cling-url',
                    action='store',
                    help='Specify an alternate URL of Cling repo',
                    default='https://github.com/root-project/cling.git')
parser.add_argument('--cling-branch', help='Specify a particular Cling branch')

parser.add_argument(
    '--with-binary-llvm',
    help='Download LLVM binary and use it to build Cling in dev mode',
    action='store_true')
parser.add_argument('--with-llvm-tar',
                    help='''Download and use LLVM binary release tar to
    build Cling for debugging''',
                    action='store_true')
parser.add_argument('--no-test',
                    help='Do not run test suite of Cling',
                    action='store_true')
parser.add_argument('--skip-cleanup',
                    help='Do not clean up after a build',
                    action='store_true')
parser.add_argument('--use-wget',
                    help='Do not use Git to fetch sources',
                    action='store_true')
parser.add_argument('--create-dev-env',
                    help='Set up a release/debug environment')

if platform.system() != 'Windows':
    parser.add_argument('--with-workdir',
                        action='store',
                        help='Specify an alternate working directory for CPT',
                        default=os.path.expanduser(
                            os.path.join('~', 'ci', 'build')))
else:
    parser.add_argument('--with-workdir',
                        action='store',
                        help='Specify an alternate working directory for CPT',
...
parser.add_argument('--make-proper',
                    help='Internal option to support calls from build system')
parser.add_argument('--verbose',
                    help='Tell CMake to build with verbosity',
                    action='store_true')
parser.add_argument('--with-cmake-flags',
                    help='Additional CMake configuration flags',
                    default='')
parser.add_argument(
    '--stdlib',
    help=('C++ Library to use, stdlibc++ or libc++.'
          '  To build a spcific llvm <tag> of libc++ with cling '
          'specify libc++,<tag>'),
    default='')
parser.add_argument('-y',
                    help='Non-interactive mode (yes to all)',
                    action='store_true')
...
if OS == 'Windows':
    ...
    try:
        ...
        if choice in yes:
            subprocess.call("sudo {0} -m pip install distro".format(
                sys.executable),
                            shell=True)
            import distro
        else:
...
if args.get(
        'stdlib') and EXTRA_CMAKE_FLAGS.find('-DLLVM_ENABLE_LIBCXX=') != -1:
...
if not os.path.isdir(workdir):
    os.makedirs(workdir)
if not (TRAVIS_BUILD_DIR
        or APPVEYOR_BUILD_FOLDER) and os.path.isdir(TMP_PREFIX):
    shutil.rmtree(TMP_PREFIX)
...
if args['with_binary_llvm'] and args['with_llvm_url']:
    raise Exception(
        "Cannot specify flags --with-binary-llvm and --with-llvm-url together")
...
if args['check_requirements']:
    ...
    if DIST == 'Ubuntu':
        install_line = ""
        prerequisite = [
            'git', 'cmake', 'gcc', 'g++', 'debhelper', 'devscripts', 'gnupg',
            'zlib1g-dev'
        ]
        ...
        if install_line != '':
            choice = custom_input(
                '''
    CPT will now attempt to update/install
    the requisite packages automatically.
    Do you want to continue? [yes/no]: ''', args['y']).lower()
            while True:
                if choice in yes:
                    # Need to communicate values to the shell.
                    # Do not use exec_subprocess_call()
    ...
    '''.format(install_line, llvm_binary_name))
                    ...
                    choice = custom_input(
                        "Please respond with 'yes' or 'no': ", args['y'])
                    continue
        if no_install is False:
            if llvm_binary_name != "" and tar_required is False:
                try:
                    subprocess.Popen([
                        'sudo apt-get install llvm-{0}-dev'.format(llvm_vers)
                    ],
                                     shell=True,
                                     stdin=subprocess.PIPE,
                                     stdout=None,
...
''')
    ...
    '''.format(install_line, llvm_binary_name))
        ...
        if no_install is False and llvm_binary_name != "":
            subprocess.Popen(
                ['sudo port install {0}'.format(llvm_binary_name)],
                shell=True,
                stdin=subprocess.PIPE,
                stdout=None,
...
if args['current_dev']:
    ...
    if TRAVIS_BUILD_DIR or APPVEYOR_BUILD_FOLDER:
        var = TRAVIS_BUILD_DIR if TRAVIS_BUILD_DIR else APPVEYOR_BUILD_FOLDER
        ciCloned = var
        if TRAVIS_BUILD_DIR:
        ...
        exec_subprocess_call(
            'git log -5 --pretty="format:%h <%ae> %<(60,trunc)%s"', dir)
        print('\n')
    else:
    ...
    if args['current_dev'] == 'tar':
        if OS == 'Windows':
            ...
            compile(
                os.path.join(
                    workdir,
                    'cling-win-' + platform.machine().lower() + '-' + VERSION))
        else:
            if DIST == 'Scientific Linux CERN SLC':
                compile(
                    os.path.join(
                        workdir, 'cling-SLC-' + REV + '-' +
                        platform.machine().lower() + '-' + VERSION))
            else:
                compile(
                    os.path.join(
                        workdir, 'cling-' + DIST + '-' + REV + '-' +
                        platform.machine().lower() + '-' + VERSION))
        install_prefix()
        if not args['no_test']:
            if args['with_binary_llvm']:
    ...
    elif args['current_dev'] == 'deb' or (args['current_dev'] == 'pkg'
                                          and DIST == 'Ubuntu'):
    ...
    elif args['current_dev'] == 'rpm' or (args['current_dev'] == 'pkg'
                                          and platform.dist()[0] == 'redhat'):
        compile(
            os.path.join(workdir,
                         'cling-' + VERSION.replace('-' + REVISION[:7], '')))
        install_prefix()
        if not args['no_test']:
            test_cling()
        tarball()
    ...
    elif args['current_dev'] == 'nsis' or (args['current_dev'] == 'pkg'
                                           and OS == 'Windows'):
        get_win_dep()
        compile(
            os.path.join(
                workdir, 'cling-' + RELEASE + '-' +
                platform.machine().lower() + '-' + VERSION))
        install_prefix()
        if not args['no_test']:
            test_cling()
    ...
    elif args['current_dev'] == 'dmg' or (args['current_dev'] == 'pkg'
                                          and OS == 'Darwin'):
        compile(
            os.path.join(
                workdir, 'cling-' + DIST + '-' + REV + '-' +
...
if args['last_stable']:
    tag = json.loads(
        urlopen("https://api.github.com/repos/vgvassilev/cling/tags").read().
        decode('utf-8'))[0]['name'].encode('ascii', 'ignore').decode("utf-8")
    ...
    if CLING_BRANCH is None:
        llvm_revision = urlopen(
            "https://raw.githubusercontent.com/root-project/cling/%s/"
            "LastKnownGoodLLVMSVNRevision.txt" %
            tag).readline().strip().decode('utf-8')
    ...
    elif args['last_stable'] == 'deb' or (args['last_stable'] == 'pkg'
                                          and DIST == 'Ubuntu'):
        set_version()
        compile(os.path.join(workdir, 'cling-' + VERSION))
        install_prefix()
    ...
    elif args['last_stable'] == 'rpm' or (args['last_stable'] == 'pkg'
                                          and platform.dist()[0] == 'redhat'):
        set_version()
        compile(os.path.join(workdir, 'cling-' + VERSION))
        install_prefix()
    ...
    elif args['last_stable'] == 'nsis' or (args['last_stable'] == 'pkg'
                                           and OS == 'Windows'):
        set_version()
        get_win_dep()
        compile(
    ...
    elif args['last_stable'] == 'dmg' or (args['last_stable'] == 'pkg'
                                          and OS == 'Darwin'):
        set_version()
        compile(
            os.path.join(
...
if args['tarball_tag']:
    llvm_revision = urlopen(
        "https://raw.githubusercontent.com/root-project/cling/%s/"
        "LastKnownGoodLLVMSVNRevision.txt" %
        args['tarball_tag']).readline().strip().decode('utf-8')
    if args["with_binary_llvm"]:
        compile = compile_for_binary
        install_prefix = install_prefix_for_binary
        fetch_clang(llvm_revision)
...
if args['deb_tag']:
    llvm_revision = urlopen(
        "https://raw.githubusercontent.com/root-project/cling/%s/"
        "LastKnownGoodLLVMSVNRevision.txt" %
        args['deb_tag']).readline().strip().decode('utf-8')
    fetch_llvm(llvm_revision)
    fetch_clang(llvm_revision)
...
if args['rpm_tag']:
    llvm_revision = urlopen(
        "https://raw.githubusercontent.com/root-project/cling/%s/"
        "LastKnownGoodLLVMSVNRevision.txt" %
        args['rpm_tag']).readline().strip().decode('utf-8')
    fetch_llvm(llvm_revision)
    fetch_clang(llvm_revision)
...
if args['nsis_tag']:
    llvm_revision = urlopen(
        "https://raw.githubusercontent.com/root-project/cling/%s/"
        "LastKnownGoodLLVMSVNRevision.txt" %
        args['nsis_tag']).readline().strip().decode('utf-8')
    fetch_llvm(llvm_revision)
    fetch_clang(llvm_revision)
    ...
    compile(
        os.path.join(
            workdir, 'cling-' + DIST + '-' + REV + '-' + platform.machine() +
            '-' + VERSION))
    install_prefix()
    if not args['no_test']:
        test_cling()
...
if args['dmg_tag']:
    llvm_revision = urlopen(
        "https://raw.githubusercontent.com/root-project/cling/%s/"
        "LastKnownGoodLLVMSVNRevision.txt" %
        args['dmg_tag']).readline().strip().decode('utf-8')
    fetch_llvm(llvm_revision)
    fetch_clang(llvm_revision)
...
if args['create_dev_env']:
    llvm_revision = urlopen(
        "https://raw.githubusercontent.com/root-project/cling/master/"
        "LastKnownGoodLLVMSVNRevision.txt").readline().strip().decode('utf-8')
    fetch_llvm(llvm_revision)
    fetch_clang(llvm_revision)
    fetch_cling('master')
    ...
    with open(os.path.join(LLVM_OBJ_ROOT, 'config.log'), 'r') as log:
        for line in log:
            if re.match('^LLVM_PREFIX=', line):
                prefix = re.sub('^LLVM_PREFIX=', '', line).replace("'",
                                                                   '').strip()
