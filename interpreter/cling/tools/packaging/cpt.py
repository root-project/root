...
###############################################################################
class Cpt:
    def __init__(self, tar_required, CLING_SRC_DIR, VERSION, REVISION, gInCleanup, EXEEXT, SHLIBEXT, prefix, llvm_revision, CMAKE, EXTRA_CMAKE_FLAGS, dir, CPT_SRC_DIR):
        self.tar_required = tar_required
        self.CLING_SRC_DIR = CLING_SRC_DIR
        self.VERSION = VERSION
        self.REVISION = REVISION
        self.gInCleanup = gInCleanup
        self.EXEEXT = EXEEXT
        self.SHLIBEXT = SHLIBEXT
        self.prefix = prefix
        self.llvm_revision = llvm_revision
        self.CMAKE = CMAKE
        self.EXTRA_CMAKE_FLAGS = EXTRA_CMAKE_FLAGS
        self.dir = dir
        self.CPT_SRC_DIR = CPT_SRC_DIR
        
    def download_llvm_binary(self):
        global llvm_flags
        ...
        if DIST=="Ubuntu":
            ...
            if llvm_config_path != '' and self.tar_required is False:
                ...
                self.tar_required = True
        elif DIST == 'MacOSX':
            subprocess.call(
                "sudo -H {0} -m pip install lit".format(sys.executable), shell=True
                )
            if self.tar_required is False:
    ...
    def set_vars_for_lit(self):
        with open(os.path.join(self.CLING_SRC_DIR, "test", "lit.site.cfg.in"), "r") as file:
            lines = file.readlines()
        for i in range(len(lines)):
            if lines[i].startswith("config.llvm_tools_dir ="):
                lines[i] = 'config.llvm_tools_dir = "{0}"\n'.format(os.path.join(LLVM_OBJ_ROOT, "bin"))
                break
        with open(os.path.join(self.CLING_SRC_DIR, "test", "lit.site.cfg.in"), "w") as file:
        ...
        if self.tar_required:
            ...
            with open(os.path.join(self.CLING_SRC_DIR, "test", "lit.site.cfg.in"), "w") as file:
                file.writelines(lines)
        elif DIST == 'MacOSX' and self.tar_required is False:
            llvm_dir = os.path.join("/opt", "local", "libexec", "llvm-" + llvm_vers)
            with open(os.path.join(self.CLING_SRC_DIR, "test", "lit.site.cfg.in"), "r") as file:
                lines = file.readlines()
            for i in range(len(lines)):
    ...
    def setup_tests(self):
        ...
        subprocess.Popen(
            ...
            cwd=os.path.join(self.CLING_SRC_DIR, "tools"),
            shell=True,
        ...
        with open(os.path.join(self.CLING_SRC_DIR, 'tools', 'CMakeLists.txt'), 'a') as file:
            file.writelines('add_subdirectory(\"FileCheck\")')
        exec_subprocess_call("cmake {0}".format(LLVM_OBJ_ROOT), self.CLING_SRC_DIR)
        exec_subprocess_call("cmake --build . --target FileCheck -- -j{0}".format(multiprocessing.cpu_count()), LLVM_OBJ_ROOT)
        if not os.path.exists(os.path.join(self.CLING_SRC_DIR, "..", "clang", "test")):
    ...
    def fetch_cling(self, arg):
        if args["with_binary_llvm"]:
            self.CLING_SRC_DIR = os.path.join(clangdir, 'tools', 'cling')
            self.dir = clangdir
        else:
            self.dir = srcdir
        ...
        def get_fresh_cling():
            if CLING_BRANCH:
                exec_subprocess_call('git clone --depth=10 --branch %s %s cling'
                                    % (CLING_BRANCH, CLING_GIT_URL), os.path.join(self.dir, 'tools'))
            else:
                exec_subprocess_call('git clone %s cling' % CLING_GIT_URL, os.path.join(self.dir, 'tools'))
            ...
            exec_subprocess_call('git checkout %s' % checkout_branch, self.CLING_SRC_DIR)
        ...
        def update_old_cling():
            ...
            exec_subprocess_call('git fetch --tags', self.CLING_SRC_DIR)
            ...
            exec_subprocess_call('git pull origin %s' % checkout_branch, self.CLING_SRC_DIR)

        if os.path.isdir(self.CLING_SRC_DIR):
            update_old_cling()
        else:
            get_fresh_cling()
    ...
    def set_version(self):
        ...
        self.VERSION = open(os.path.join(self.CLING_SRC_DIR, 'VERSION'), 'r').readline().strip()

        # If development release, then add revision to the version
        self.REVISION = exec_subprocess_check_output('git log -n 1 --pretty=format:%H', self.CLING_SRC_DIR).strip()

        if '~dev' in self.VERSION:
            self.VERSION = self.VERSION + '-' + self.REVISION[:7]
    ...
    def set_vars(self):
        ...
        with open(os.path.join(LLVM_OBJ_ROOT, 'test', 'lit.site.cfg.py'), 'r') as lit_site_cfg:
            for line in lit_site_cfg:
                if re.match('^config.llvm_shlib_ext = ', line):
                    self.SHLIBEXT = re.sub('^config.llvm_shlib_ext = ', '', line).replace('"', '').strip()
                elif re.match('^config.llvm_exe_ext = ', line):
                    self.EXEEXT = re.sub('^config.llvm_exe_ext = ', '', line).replace('"', '').strip()
        ...
        print('EXEEXT: ' + self.EXEEXT)
        print('SHLIBEXT: ' + self.SHLIBEXT)
    ...
    def cleanup(self):
        ...
        if os.path.isdir(self.prefix):
            print("Remove directory: " + self.prefix)
        ...
        if args['current_dev'] == 'deb' or args['last_stable'] == 'deb' or args['deb_tag']:
            print('Create output directory: ' + os.path.join(workdir, 'cling-%s-1' % (self.VERSION)))
            os.makedirs(os.path.join(workdir, 'cling-%s-1' % (self.VERSION)))

            for file in glob.glob(os.path.join(workdir, 'cling_%s*' % (self.VERSION))):
                print(file + '->' + os.path.join(workdir, 'cling-%s-1' % (self.VERSION), os.path.basename(file)))
                shutil.move(file, os.path.join(workdir, 'cling-%s-1' % (self.VERSION)))

            if not os.listdir(os.path.join(workdir, 'cling-%s-1' % (self.VERSION))):
                os.rmdir(os.path.join(workdir, 'cling-%s-1' % (self.VERSION)))

        if args['current_dev'] == 'dmg' or args['last_stable'] == 'dmg' or args['dmg_tag']:
            if os.path.isfile(os.path.join(workdir, 'cling-%s-temp.dmg' % (self.VERSION))):
                print("Remove file: " + os.path.join(workdir, 'cling-%s-temp.dmg' % (self.VERSION)))
                os.remove(os.path.join(workdir, 'cling-%s-temp.dmg' % (self.VERSION)))
            ...
            if os.path.isdir(os.path.join(workdir, 'cling-%s-temp.dmg' % (self.VERSION))):
                print('Remove directory: ' + os.path.join(workdir, 'cling-%s-temp.dmg' % (self.VERSION)))
                shutil.rmtree(os.path.join(workdir, 'cling-%s-temp.dmg' % (self.VERSION)))
    ...
    def rpm_build(self):
        ...
        shutil.move(os.path.join(workdir, os.path.basename(self.prefix) + '.tar.bz2'),
                    os.path.join(workdir, 'rpmbuild', 'SOURCES'))

        box_draw("Generate RPM SPEC file")
        print('Create file: ' + os.path.join(workdir, 'rpmbuild', 'SPECS', 'cling-%s.spec' % (self.VERSION)))
        f = open(os.path.join(workdir, 'rpmbuild', 'SPECS', 'cling-%s.spec' % (self.VERSION)), 'w')
    ...
    * Sun Apr 13 2014  Anirudha Bose <ani07nov@gmail.com>
    ...
    '''
        ...
        exec_subprocess_call('rpmbuild --define "_topdir ${PWD}" -bb %s' % (
            os.path.join(workdir, 'rpmbuild', 'SPECS', 'cling-%s.spec' % (self.VERSION))), os.path.join(workdir, 'rpmbuild'))
    ...
    def tarball(self):
        ...
        tar = tarfile.open(self.prefix + '.tar.bz2', 'w:bz2')
        print('Creating archive: ' + os.path.basename(self.prefix) + '.tar.bz2')
        tar.add(self.prefix, arcname=os.path.basename(self.prefix))
        tar.close()
    ...
    def make_nsi(self):
        ...
        VIProductVersion = \
            exec_subprocess_check_output('git describe --match v* --abbrev=0 --tags', self.CLING_SRC_DIR).strip().splitlines()[0]
    ...
    ''' % (self.prefix,
        self.VERSION,
        os.path.basename(self.prefix) + '-setup.exe',
        VIProductVersion.replace('v', ''),
        self.CPT_SRC_DIR,
        NSIS,
        os.path.join(self.CLING_SRC_DIR, 'LICENSE.TXT'))
    ...
    '''
        ...
        def iterate():
            for (basepath, children) in walktree():
                f.write(' Delete "%s\\*.*"\n' % (basepath.replace(self.prefix, '$INSTDIR')))
                f.write(' RmDir "%s"\n' % (basepath.replace(self.prefix, '$INSTDIR')))
    ...
    def fetch_llvm(self, llvm_revision):
        box_draw("Fetch source files")
        print('Last known good LLVM revision is: ' + self.llvm_revision)
        print('Current working directory is: ' + workdir + '\n')
        ...
        if "github.com" in LLVM_GIT_URL and args['create_dev_env'] is None and args['use_wget']:
            ...
            wget(url='https://github.com/%s/%s' % (user, repo.replace('.git', '')) +
                    '/archive/cling-patches-r%s.tar.gz' % self.llvm_revision,
                out_dir=workdir)

            print('Extracting: ' + os.path.join(workdir, 'cling-patches-r%s.tar.gz' % self.llvm_revision))
            tar = tarfile.open(os.path.join(workdir, 'cling-patches-r%s.tar.gz' % self.llvm_revision))
            tar.extractall(path=workdir)
            tar.close()

            os.rename(os.path.join(workdir, 'llvm-cling-patches-r%s' % self.llvm_revision), srcdir)

            if os.path.isfile(os.path.join(workdir, 'cling-patches-r%s.tar.gz' % self.llvm_revision)):
                print("Remove file: " + os.path.join(workdir, 'cling-patches-r%s.tar.gz' % self.llvm_revision))
                os.remove(os.path.join(workdir, 'cling-patches-r%s.tar.gz' % self.llvm_revision))

            print()
            return

        def checkout():
            exec_subprocess_call('git checkout cling-patches-r%s' % self.llvm_revision, srcdir)
    ...
    def get_win_dep(self):
        ...
        cmakeEXE = tryCmake('cmake.exe') or tryCmake(self.CMAKE)
        if cmakeEXE:
            self.CMAKE = cmakeEXE
    ...
    def check_redhat(self, pkg):
        if pkg == "cmake":
            self.CMAKE = os.environ.get('CMAKE', 'cmake')
            if not check_version_string_ge(exec_subprocess_check_output('{cmake} --version'.format(cmake=self.CMAKE), '/').strip().split('\n')[0].split()[-1], '3.4.3'):
    ...
    def make_dmg(self):
        ...
        VOL_NAME = "%s-%s" % (APP_NAME.lower(), self.VERSION)
        ...
        shutil.copyfile(
            os.path.join(self.CPT_SRC_DIR, 'LLVM.icns'),
    ...
    '''.format(
        ...
        exec_subprocess_call(
            '{dmgbuild} -s {settings} -D app={app} -D size={size}M "{volname}" {dmg}'.format(
                ...
                settings=os.path.join(self.CPT_SRC_DIR, 'settings.py'),
    ...
    def debianize(self):
        SIGNING_USER = exec_subprocess_check_output('gpg --fingerprint | grep uid | sed s/"uid *"//g',
                                                    self.CLING_SRC_DIR).strip()

        box_draw("Set up the debian directory")
        print("Create directory: debian")
        os.makedirs(os.path.join(self.prefix, 'debian'))

        print("Create directory: " + os.path.join(self.prefix, 'debian', 'source'))
        os.makedirs(os.path.join(self.prefix, 'debian', 'source'))

        print("Create file: " + os.path.join(self.prefix, 'debian', 'source', 'format'))
        f = open(os.path.join(self.prefix, 'debian', 'source', 'format'), 'w')
        f.write('3.0 (quilt)')
        f.close()

        print("Create file: " + os.path.join(self.prefix, 'debian', 'source', 'lintian-overrides'))
        f = open(os.path.join(self.prefix, 'debian', 'source', 'lintian-overrides'), 'w')
        ...
        if False:
            print('Create file: ' + os.path.join(self.prefix, 'debian', 'postinst'))
            template = '''
    #! /bin/sh -e
    ...
    exit 0
    '''
            f = open(os.path.join(self.prefix, 'debian', 'postinst'), 'w')
            f.write(template)
            f.close()

        print('Create file: ' + os.path.join(self.prefix, 'debian', 'cling.install'))
        f = open(os.path.join(self.prefix, 'debian', 'cling.install'), 'w')
        template = '''
    bin/* /usr/bin
    docs/* /usr/share/doc
    include/* /usr/include
    ...
    '''
        ...
        print('Create file: ' + os.path.join(self.prefix, 'debian', 'compact'))
        # Optimize binary compression
        f = open(os.path.join(self.prefix, 'debian', 'compact'), 'w')
        f.write("7")
        f.close()

        print('Create file: ' + os.path.join(self.prefix, 'debian', 'compat'))
        f = open(os.path.join(self.prefix, 'debian', 'compat'), 'w')
        f.write("9")
        f.close()

        print('Create file: ' + os.path.join(self.prefix, 'debian', 'control'))
        f = open(os.path.join(self.prefix, 'debian', 'control'), 'w')
        template = '''
    Source: cling
    Section: devel
    Priority: optional
    ...
    ''' % (SIGNING_USER)
        ...
        print('Create file: ' + os.path.join(self.prefix, 'debian', 'copyright'))
        f = open(os.path.join(self.prefix, 'debian', 'copyright'), 'w')
        template = '''
    ...
    '''
        ...
        print('Create file: ' + os.path.join(self.prefix, 'debian', 'rules'))
        f = open(os.path.join(self.prefix, 'debian', 'rules'), 'w')
        template = '''
    #!/usr/bin/make -f
    # -*- makefile -*-
    ...
    '''
        ...
        print('Create file: ' + os.path.join(self.prefix, 'debian', 'changelog'))
        f = open(os.path.join(self.prefix, 'debian', 'changelog'), 'w')
    ...
    * [Debian] Upload to unstable for version: %s
    ''' % (self.VERSION, self.VERSION)
        ...
        if '~dev' in self.VERSION:
            TAG = str(float(self.VERSION[:self.VERSION.find('~')]) - 0.1)
            template = exec_subprocess_check_output('git log v' + TAG + '...HEAD --format="  * %s" | fmt -s', self.CLING_SRC_DIR)

            f = open(os.path.join(self.prefix, 'debian', 'changelog'), 'a+')
            f.write(template)
            f.close()
        ...
        template = exec_subprocess_check_output('git log v0.1 --format="  * %s%n -- %an <%ae>  %cD%n"', self.CLING_SRC_DIR)
        ...
        exec_subprocess_call('debuild', self.prefix)
        
    def fetch_clang(self, llvm_revision):
        if "github.com" in CLANG_GIT_URL and args['create_dev_env'] is None and args['use_wget']:
            ...
            os.rename(os.path.join(srcdir, 'tools', 'clang-cling-patches-r%s' % self.llvm_revision),
        ...
        if args["with_binary_llvm"]:
            ...
            self.dir = os.path.join(srcdir, 'tools')
        global clangdir
        clangdir = os.path.join(self.dir, 'clang')
        def checkout():
            exec_subprocess_call('git checkout cling-patches-r%s' % self.llvm_revision, clangdir)

        def get_fresh_clang():
            exec_subprocess_call('git clone %s' % CLANG_GIT_URL, self.dir)
            checkout()
        ...
        def update_old_clang():
            ...
            checkout()
            exec_subprocess_call('git fetch --tags', clangdir)
            exec_subprocess_call('git pull origin refs/tags/cling-patches-r%s' % self.llvm_revision,
                                    clangdir)
        ...
        if os.path.isdir(clangdir):
            ...
            get_fresh_clang()
    def tarball_deb(self):
        box_draw("Compress compiled binaries into a bzip2 tarball")
        tar = tarfile.open(os.path.join(workdir, 'cling_' + self.VERSION + '.orig.tar.bz2'), 'w:bz2')
        tar.add(self.prefix, arcname=os.path.basename(self.prefix))
        tar.close()
    
    def check_ubuntu(self, pkg):
        if pkg == "gnupg":
            ...
            self.CMAKE = os.environ.get('CMAKE', 'cmake')
            output = exec_subprocess_check_output('{cmake} --version'.format(cmake=self.CMAKE), '/').strip().split('\n')[0].split()
    ...
    def runSingleTest(self, test, Idx = 2, Recurse = True):
        try:
            test = os.path.join(self.CLING_SRC_DIR, 'test', test)

            if os.path.isdir(test):
                if Recurse:
                    for t in os.listdir(test):
                        if t.endswith('.C'):
                            self.runSingleTest(os.path.join(test, t), Idx, False)
                return
    ...
    class Build(object):
        ...
        def config(self, configFlags=''):
            ...
            exec_subprocess_call('%s %s' % (Cpt.CMAKE, configFlags), LLVM_OBJ_ROOT, True)
    ...
    def compile_for_binary(self, arg):
        ...
        exec_subprocess_call('%s %s' % (self.CMAKE, cmake_config_flags), LLVM_OBJ_ROOT, True)
    ...
    def install_prefix_for_binary(self):
        travis_fold_start("install")
        self.CPT_SRC_DIR = os.path.join(clangdir, 'tools', 'cling', 'tools', 'packaging')
        Cpt.set_vars_for_lit()

        box_draw("Filtering Cling's libraries and binaries")

        regex_array = []
        regex_filename = os.path.join(self.CPT_SRC_DIR, 'dist-files.txt')
            ...
            for file in files:
                ...
                for regex in regex_array:
                    ...
                    if re.search(regex, f):
                        ...
                        if not os.path.isdir(os.path.join(self.prefix, os.path.dirname(f))):
                            os.makedirs(os.path.join(self.prefix, os.path.dirname(f)))
                        shutil.copy(os.path.join(TMP_PREFIX, f), os.path.join(self.prefix, f))
                        break
...
cpt = Cpt(tar_required, CLING_SRC_DIR, VERSION, REVISION, gInCleanup, EXEEXT, SHLIBEXT, prefix, llvm_revision, CMAKE, EXTRA_CMAKE_FLAGS, dir, CPT_SRC_DIR)
...
if args['check_requirements']:
    ...
    if DIST == 'Ubuntu':
        ...
        if args["with_binary_llvm"] and not args["with_llvm_tar"]:
            if cpt.check_ubuntu('llvm-'+llvm_vers+'-dev') is False:
                llvm_binary_name = 'llvm-{0}-dev'.format(llvm_vers)
        for pkg in prerequisite:
            if cpt.check_ubuntu(pkg) is False:
                install_line += pkg + ' '
        yes = {'yes', 'y', 'ye', ''}
        no = {'no', 'n'}
...
''')
        ...
        for pkg in prerequisite:
            if cpt.check_redhat(pkg) is False:
                install_line += pkg + ' '
        yes = {'yes', 'y', 'ye', ''}
        no = {'no', 'n'}
...
if args['current_dev']:
    ...
    if args['with_binary_llvm']:
        compile = cpt.compile_for_binary
        install_prefix = cpt.install_prefix_for_binary
        cpt.fetch_clang(llvm_revision)
        clingDir = os.path.join(clangdir, 'tools', 'cling')
        CLING_SRC_DIR = os.path.join(clangdir, 'tools', 'cling')
        dir = CLING_SRC_DIR
        allow_clang_tool()
    else:
        cpt.fetch_llvm(llvm_revision)
        cpt.fetch_clang(llvm_revision)
    ...
    if TRAVIS_BUILD_DIR or APPVEYOR_BUILD_FOLDER:
        ...
        cpt.fetch_cling(CLING_BRANCH if CLING_BRANCH else 'master')
...
if args['tarball_tag']:
    ...
    if args["with_binary_llvm"]:
        ...
        cpt.fetch_llvm(llvm_revision)
        cpt.fetch_clang(llvm_revision)
    cpt.fetch_cling(args['tarball_tag'])
