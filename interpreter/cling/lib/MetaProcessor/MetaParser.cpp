...
namespace cling {
  ...
  bool MetaParser::isXCommand(MetaSema::ActionResult& actionResult,
    ...
    if (Tok.is(tok::ident) && (Tok.getIdent() == "x"
                               || Tok.getIdent() == "X")) {
      consumeToken();
      skipWhitespace();
