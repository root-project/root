...
bool HasUDT(const Decl* decl)
{
  ...
  assert(decl != nullptr && "HasUDT, 'decl' parameter is null");
...
void AppendConstructorSignature(const CXXConstructorDecl* ctorDecl, std::string& name)
{
  ...
  const FunctionProtoType* const ft = ctorDecl->hasWrittenPrototype() ?
                                      dyn_cast<FunctionProtoType>(aft) : nullptr;
...
{
  //Just in case asserts were deleted from ctor:
  assert(fInterpreter != nullptr && "DisplayClass, fCompiler is null");
...
{
  assert(identifierInfo != nullptr && "DisplayObjectLikeMacro, 'identifierInfo' parameter is null");
  assert(macroInfo != nullptr && "DisplayObjectLikeMacro, 'macroInfo' parameter is null");
  ...
  assert(fInterpreter->getCI() != nullptr && "DisplayObjectLikeMacro, compiler instance is null");
...
{
  assert(fInterpreter != nullptr && "DisplayTypedef, fInterpreter is null");
...
void DisplayClasses(llvm::raw_ostream& stream, const Interpreter* interpreter,
                    bool verbose)
{
  assert(interpreter != nullptr && "DisplayClasses, 'interpreter' parameter is null");
                  ...
                  const char* className, bool verbose)
{
  ...
  assert(className != nullptr && "DisplayClass, 'className' parameter is null");
