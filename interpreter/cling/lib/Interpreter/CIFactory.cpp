...
// https://gcc.gnu.org/onlinedocs/libstdc++/manual/using_dual_abi.html
    ...
    if (!FEOpts.Inputs.size() ||
      ...
      if (CompilerOpts.DefaultLanguage(&LangOpts)) {
        switch (CxxStdCompiledWith()) {
          case 23:
            assert(LangOpts.CPlusPlus23 && "Language version mismatch");
            LLVM_FALLTHROUGH;
          case 20:
            assert(LangOpts.CPlusPlus20 && "Language version mismatch");
            LLVM_FALLTHROUGH;
          case 17:
            assert(LangOpts.CPlusPlus17 && "Language version mismatch");
            LLVM_FALLTHROUGH;
          case 14:
            assert(LangOpts.CPlusPlus14 && "Language version mismatch");
            LLVM_FALLTHROUGH;
          case 11:
            assert(LangOpts.CPlusPlus11 && "Language version mismatch");
            break;
  ...
  createCIImpl(std::unique_ptr<llvm::MemoryBuffer> Buffer,
    ...
    CGOpts.ClearASTBeforeBackend = false;
    CGOpts.DisableFree = false;
...
namespace cling {
  ...
  void CIFactory::collectModule(CompilerInstance &CI) {
    setupCxxModules(CI);
  }
