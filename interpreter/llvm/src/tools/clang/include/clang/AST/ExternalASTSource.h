...
public:
  ...
  /// entity.
  virtual uint32_t getGeneration(const ASTContext &C) const;
...
struct LazyGenerationalUpdatePtr {
  ...
  struct LazyData {
    LazyData(const ASTContext *Context, ExternalASTSource *Source, T Value)
        : Context(Context), ExternalSource(Source), LastGeneration(0), LastValue(Value) {}
...
public:
  ...
  T get(Owner O) {
    if (LazyData *LazyVal = Value.template dyn_cast<LazyData*>()) {
      if (LazyVal->LastGeneration != LazyVal->ExternalSource->getGeneration(*LazyVal->Context)) {
        LazyVal->LastGeneration = LazyVal->ExternalSource->getGeneration(*LazyVal->Context);
