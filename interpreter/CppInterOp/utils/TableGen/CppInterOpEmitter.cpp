...
//   CppName       — name in Cpp:: namespace (may be shared by overloads)
...
StringMap<CTypeMapEntry> collectCTypeMaps(const RecordKeeper& Records) {
  ...
  for (const auto* Rec : Records.getAllDerivedDefinitions("CTypeMap")) {
    CTypeMapEntry E;
    StringRef TypeRef = Rec->getValueAsString("TypeRef");
...
/// TypeRef string. Strips trailing '&' from out-param types.
/// E.g. "std::vector<DeclRef>&" → "std::vector<DeclRef>".
static std::string getCppTmpType(StringRef TypeRef, StringRef /*ElemCType*/) {
  // Strip trailing '&' for out-params; the resulting type is used directly
  // as the C++ temporary.
  return TypeRef.rtrim('&').rtrim().str();
}
