...
# launch replace
launch () {
  if [ test `echo $CALLROOTEXE | grep -q -- -e` ] ; then
      echo Processing 'dt_wrap.C("'$1'",'$2')...';
  else 
  ($CALLROOTEXE -l -b -q -e '.L dt_RunDrawTest.C+' 'dt_wrap.C("'$1'",'$2')' 2>&1; return $?;) | \
     (eval grep -v -e $3 -e ExecutionContext) | \
