...
std::mt19937 gRNG{1};
std::normal_distribution<double> gGaus{0., 1.};
std::uniform_real_distribution<double> gUniform{1., 2.};
...
bool IsSameCont(const Cont &a, const Cont &b)
{
   ...
   if (size != std::distance(b.begin(), b.end()))
      return false;
...
bool IsSame(const std::unordered_set<T> &a, const std::unordered_set<T> &b)
{
   ...
   auto sortingFunction = [](const T &o1, const T &o2) {
      return std::string(o1.GetName()) < std::string(o2.GetName());
   };
...
bool IsSame<>(const TH1F &a, const TH1F &b)
{
   ...
   for (int i = 0; i < a.GetNbinsX(); ++i) {
      ...
      if (!AreBitwiseEqual(binca, bincb)) {
         if (gVerboseComparison)
            std::cout << "The content of bin " << i << "  of the histograms differ: " << binca << " " << bincb
                      << std::endl;
         return false;
      }
      ...
      if (!AreBitwiseEqual(binea, bineb)) {
         if (gVerboseComparison)
            std::cout << "The error of bin " << i << "  of the histograms differ: " << binea << " " << bineb
                      << std::endl;
         return false;
      }
   }
...
void readAndCheckFromFile(const T &obj, const char *objName, const char *filename)
{
   ...
   if (!IsSame(obj, *objFromFile)) {
      std::cerr << "Error: object " << objName << " read from file " << filename
                << " from file and in memory are not identical!\n";
   }
...
void randomizeAssoCont(Cont &cont)
{
   ...
   cont.clear();
   for (auto &el : v) {
      cont.insert(std::make_pair(el.first, el.second * gUniform(gRNG)));
   }
}
...
struct equal_to<TH1F> {
   bool operator()(const TH1F &a, const TH1F &b) const { return IsSame(a, b); }
};
template <>
struct equal_to<std::vector<TH1F>> {
   bool operator()(const std::vector<TH1F> &a, const std::vector<TH1F> &b) const { return IsSame(a, b); }
};
