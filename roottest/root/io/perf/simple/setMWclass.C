...
public:
   ...
   bool operator==(const simple &s) const { return this==&s; }
   bool operator< (const simple &s) const { return this < &s; }
   ClassDef(simple,2);
};
...
#include "TClass.h"
#ifdef __MAKECINT__
#pragma link C++ class set<simple>+;
#endif

void write(TBuffer &buf,int ntimes, int nelems) {
   Holder holder;
   for(int e=0; e<nelems; ++e) {
     holder.fContainer.insert(simple(e));
   }
   buf.SetWriteMode();
...
void setMWclass(int nread = 2, int nelems = 600) {
