...
static void
_ChangeProperty(HWND w, char *np, char *dp, int n, Atom_t type)
{
    HGLOBAL hMem;
    char *p;
...
void
W32ChangeProperty(HWND w, Atom_t property, Atom_t type,
...
int
_GetWindowProperty(GdkWindow *id, Atom_t property, Long_t long_offset, Long_t long_length,
                   Bool_t delete_it, Atom_t req_type, Atom_t *actual_type_return,
...
//                          GDK_ENTER_NOTIFY_MASK | GDK_LEAVE_NOTIFY_MASK;
   ...
   if(border > 0) {
        gdk_window_set_decorations (newWin,(GdkWMDecoration) GDK_DECOR_BORDER);
   }
   if (attr) {
...
GContext_t TGWin32::CreateGC(Drawable_t id, GCValues_t *gval)
{
   ...
   GdkGC *gc = gdk_gc_new_with_values(
       (GdkDrawable *)id, 
...
void TGWin32::ChangeGC(GContext_t gc, GCValues_t *gval)
{
   ...
   if ((mask & kGCTileStipXOrigin) || (mask & kGCTileStipYOrigin))
      gdk_gc_set_ts_origin((GdkGC *) gc, xgval.ts_x_origin, xgval.ts_y_origin);
   if ((mask & kGCClipXOrigin) || (mask & kGCClipYOrigin))
      gdk_gc_set_clip_origin((GdkGC *) gc, xgval.clip_x_origin, xgval.clip_y_origin);
   if (mask & kGCClipMask)
   ...
   if ((mask & kGCLineWidth) || (mask & kGCLineStyle) ||
      ...
      gdk_gc_set_line_attributes((GdkGC *) gc, xgval.line_width, xgval.line_style,
				            xgval.cap_style, xgval.join_style);
   if(mask & kGCSubwindowMode)
            ...
            Int_t depth)
{
   ...
   return (Pixmap_t) gdk_pixmap_create_from_data((GdkWindow *)id, (char *)bitmap,
                             ...
                             UInt_t width, UInt_t height)
{
   ...
   return (Pixmap_t) gdk_bitmap_create_from_data((GdkWindow *)id, (char *)bitmap,
                                           width, height);
}
...
//   GdkPixmap* rdata;
   char **rdata;
   rdata = (char **)gdk_pixmap_create_from_xpm(NULL, NULL, NULL, filename);
   ret_data = &rdata;
   return kTRUE;
   return kFALSE;
}
...
//______________________________________________________________________________
void TGWin32::SetDashes(GContext_t gc, Int_t offset, const char *dash_list, Int_t n)
{
...
void TGWin32::MapEvent(Event_t &ev, GdkEvent &xev, Bool_t tox)
{
   ...
   if (tox) {
      ...
      if (ev.fType == kGKeyPress)        xev.type = GDK_KEY_PRESS;
      if (ev.fType == kKeyRelease)       xev.type = GDK_KEY_RELEASE;
      if (ev.fType == kButtonPress)      xev.type = GDK_BUTTON_PRESS;
      if (ev.fType == kButtonRelease)    xev.type = GDK_BUTTON_RELEASE;
      if (ev.fType == kMotionNotify)     xev.type = GDK_MOTION_NOTIFY;
      if (ev.fType == kEnterNotify)      xev.type = GDK_ENTER_NOTIFY;
      if (ev.fType == kLeaveNotify)      xev.type = GDK_LEAVE_NOTIFY;
      if (ev.fType == kExpose)           xev.type = GDK_EXPOSE;
      if (ev.fType == kConfigureNotify)  xev.type = GDK_CONFIGURE;
      if (ev.fType == kMapNotify)        xev.type = GDK_MAP;
      if (ev.fType == kUnmapNotify)      xev.type = GDK_UNMAP;
      if (ev.fType == kDestroyNotify)    xev.type = GDK_DESTROY;
      if (ev.fType == kClientMessage)    xev.type = GDK_CLIENT_EVENT;
      if (ev.fType == kSelectionClear)   xev.type = GDK_SELECTION_CLEAR;
      if (ev.fType == kSelectionRequest) xev.type = GDK_SELECTION_REQUEST;
      if (ev.fType == kSelectionNotify)  xev.type = GDK_SELECTION_NOTIFY;
      ...
      if (xev.type == GDK_KEY_PRESS)         ev.fType = kGKeyPress;
      if (xev.type == GDK_KEY_RELEASE)       ev.fType = kKeyRelease;
      if (xev.type == GDK_BUTTON_PRESS)      ev.fType = kButtonPress;
      if (xev.type == GDK_BUTTON_RELEASE)    ev.fType = kButtonRelease;
      if (xev.type == GDK_MOTION_NOTIFY)     ev.fType = kMotionNotify;
      if (xev.type == GDK_ENTER_NOTIFY)      ev.fType = kEnterNotify;
      if (xev.type == GDK_LEAVE_NOTIFY)      ev.fType = kLeaveNotify;
      if (xev.type == GDK_EXPOSE)            ev.fType = kExpose;
      if (xev.type == GDK_CONFIGURE)         ev.fType = kConfigureNotify;
      if (xev.type == GDK_MAP)               ev.fType = kMapNotify;
      if (xev.type == GDK_UNMAP)             ev.fType = kUnmapNotify;
      if (xev.type == GDK_DESTROY)           ev.fType = kDestroyNotify;
      if (xev.type == GDK_SELECTION_CLEAR)   ev.fType = kSelectionClear;
      if (xev.type == GDK_SELECTION_REQUEST) ev.fType = kSelectionRequest;
      if (xev.type == GDK_SELECTION_NOTIFY)  ev.fType = kSelectionNotify;
      ...
      if (ev.fType == kButtonPress || ev.fType == kButtonRelease) {
         ...
         HWND tmpwin = ChildWindowFromPoint((HWND)GDK_DRAWABLE_XID(xev.button.window),tpoint);
         if(tmpwin)
            ev.fUser[0]   = (ULong_t)gdk_xid_table_lookup (tmpwin);
         else
      ...
      if (ev.fType == kMotionNotify) {
         ...
         tpoint.x = xev.button.x;
         tpoint.y = xev.button.y;
         HWND tmpwin = ChildWindowFromPoint((HWND)GDK_DRAWABLE_XID(xev.motion.window),tpoint);
         if(tmpwin)
            ev.fUser[0]   = (ULong_t)gdk_xid_table_lookup (tmpwin);
         else
...
//      gdk_window_set_override_redirect ((GdkWindow *) id, attr->fOverrideRedirect);
   if (attr && (attr->fMask & kWABackPixmap))
      gdk_window_set_back_pixmap ((GdkWindow *) id, (GdkPixmap *) attr->fBackgroundPixmap, 0);
   ...
   if(attr && (attr->fMask & kWABorderWidth)) { 
       if(attr->fBorderWidth > 0)
           gdk_window_set_decorations ((GdkWindow *) id,(GdkWMDecoration) GDK_DECOR_BORDER);
   }
}
                           ...
                           UChar_t *data, Int_t len)
{
   ...
   // window.
   gdk_property_change ((GdkWindow *) id, (GdkAtom) property, (GdkAtom) type,
			      8, GDK_PROP_MODE_REPLACE, data, len);
}
...
void TGWin32::WMDeleteNotify(Window_t id)
{
   // Tell WM to send message when window is closed via WM.
    ...
    W32ChangeProperty ((HWND)GDK_DRAWABLE_XID ((GdkWindow *)id),
		prop, XA_ATOM, 32, GDK_PROP_MODE_REPLACE, (unsigned char *)&gWM_DELETE_WINDOW, 1);
                        ...
                        Cursor_t cursor, Bool_t grab, Bool_t owner_events)
{
   ...
   if (grab) {
      gdk_pointer_grab((GdkWindow *) id, owner_events, (GdkEventMask)xevmask, 
           (GdkWindow *) confine,(GdkCursor *) cursor, GDK_CURRENT_TIME);
   } else {
...
void TGWin32::SetClassHints(Window_t id, char *className, char *resourceName)
{
   // Set the windows class and resource name.
	...
	if ((class_string = s = (char *)malloc((unsigned) (len_nm + len_cl + 2)))) { 
	    if (len_nm) {
...
//                  len_nm+len_cl+2);
		W32ChangeProperty((HWND)GDK_DRAWABLE_XID ((GdkWindow *)id), (Atom_t)XA_WM_CLASS,
			(Atom_t)XA_STRING, 8, GDK_PROP_MODE_REPLACE, (unsigned char *) class_string,
			len_nm+len_cl+2);
	    free(class_string);
	}
}
...
//______________________________________________________________________________
void TGWin32::SetMWMHints(Window_t id, UInt_t value, UInt_t funcs, UInt_t input)
{
                           ...
                           UInt_t winc, UInt_t hinc)
{
   ...
   flags = (GdkWindowHints)(GDK_HINT_MIN_SIZE | GDK_HINT_MAX_SIZE | GDK_HINT_RESIZE_INC);
...
//______________________________________________________________________________
void TGWin32::GetFontProperties(FontStruct_t font, Int_t &max_ascent, Int_t &max_descent)
{
...
//______________________________________________________________________________
void TGWin32::FillRectangle(Drawable_t id, GContext_t gc, Int_t x, Int_t y, UInt_t w, UInt_t h)
{
...
//______________________________________________________________________________
void TGWin32::DrawRectangle(Drawable_t id, GContext_t gc, Int_t x, Int_t y, UInt_t w, UInt_t h)
{
...
//______________________________________________________________________________
void TGWin32::DrawSegments(Drawable_t id, GContext_t gc, Segment_t *seg, Int_t nseg)
{
   // Draws multiple line segments. Each line is specified by a pair of points.

   gdk_draw_segments((GdkDrawable *) id, (GdkGC *) gc, (GdkSegment *) seg, nseg);
}
...
Window_t TGWin32::GetPrimarySelectionOwner()
{
   ...
   return (Window_t) gdk_selection_owner_get (gdk_atom_intern ("GDK_SELECTION_PRIMARY",0));
}
...
void TGWin32::SetPrimarySelectionOwner(Window_t id)
{
   ...
   // That is the window in which, for example some text is selected.
    gdk_selection_owner_set((GdkWindow *)id, clipboard_atom, GDK_CURRENT_TIME, 0);
}
...
//______________________________________________________________________________
void TGWin32::LookupString(Event_t *event, char *buf, Int_t buflen, UInt_t &keysym)
{
...
//______________________________________________________________________________
void TGWin32::TranslateCoordinates(Window_t src, Window_t dest, Int_t src_x,
                     Int_t src_y, Int_t &dest_x, Int_t &dest_y, Window_t &child)
{
   ...
   // returned as well.
	...
	point.y=src_y;
	MapWindowPoints(
		sw, // handle of window to be mapped from
    ...
    if(child == src) child = (Window_t) 0;
    dest_x = point.x;
    dest_y = point.y;
}

//______________________________________________________________________________
void TGWin32::GetWindowSize(Drawable_t id, Int_t &x, Int_t &y, UInt_t &w, UInt_t &h)
{
...
void TGWin32::FillPolygon(Window_t id, GContext_t gc, Point_t *points, Int_t npnt)
{
   ...
   gdk_draw_polygon((GdkWindow *) id, (GdkGC *) gc, 1, (GdkPoint *) points, npnt);
}

//______________________________________________________________________________
void TGWin32::QueryPointer(Window_t id, Window_t &rootw, Window_t &childw,
...
//______________________________________________________________________________
void TGWin32::SetClipRectangles(GContext_t gc, Int_t x, Int_t y, Rectangle_t *recs, Int_t n)
{
...
Region_t TGWin32::PolygonRegion(Point_t *points, Int_t np, Bool_t winding)
{
   ...
   return (Region_t) gdk_region_polygon(xy, np, winding ? GDK_WINDING_RULE : GDK_EVEN_ODD_RULE);
}
...
//______________________________________________________________________________
void TGWin32::IntersectRegion(Region_t rega, Region_t regb, Region_t result)
{
