...
// Event masks
//
static ULong_t gMouseMask = GDK_BUTTON_PRESS_MASK   | GDK_BUTTON_RELEASE_MASK |
                            GDK_ENTER_NOTIFY_MASK   | GDK_LEAVE_NOTIFY_MASK   |
                            GDK_POINTER_MOTION_MASK | GDK_KEY_PRESS_MASK      |
                            GDK_KEY_RELEASE_MASK;
static ULong_t gKeybdMask = GDK_BUTTON_PRESS_MASK  | GDK_KEY_PRESS_MASK |
                            GDK_ENTER_NOTIFY_MASK  | GDK_LEAVE_NOTIFY_MASK;
...
Bool_t TGWin32::Init()
{
   ...
   if (OpenDisplay() == -1) return kFALSE;
   return kTRUE;
}
...
void TGWin32::CloseWindow1()
{
   ...
   if (gCws->new_colors) {
      gdk_colormap_free_colors (fColormap, (GdkColor *)gCws->new_colors, gCws->ncolors);
...
void TGWin32::CopyWindowtoPixmap(GdkDrawable *pix, int xpos, int ypos )
{
   ...
   gdk_drawable_get_size (pix, &ww, &hh);
   gdk_window_copy_area(pix, gGCpxmp, xpos, ypos, gCws->drawing, 0, 0, ww, hh);
   gdk_flush();
}
...
void TGWin32::DrawPolyLine(int n, TPoint *xyt)
{
   ...
   if (n > 1) {
      ...
      gdk_draw_point(gCws->drawing,
                 gLineStyle == GDK_LINE_SOLID ? gGCline : gGCdash, px, py);
   }
   delete [] (GdkPoint *)xy;
}
...
//                    (GdkGC *) gGCtext, x, y, (const gchar *) text, strlen(text));
    if(text) {
        ...
        if((length == 1) && (text[0] < 0)) {
            gdk_draw_text_wc((GdkDrawable *) gCws->drawing, (GdkFont *)gTextFont, 
                        (GdkGC *) gGCtext, x, y2, (const GdkWChar *) text, 1);
        }
        else {
            gdk_draw_text((GdkDrawable *) gCws->drawing, (GdkFont *)gTextFont, 
                        (GdkGC *) gGCtext, x, y2, (const gchar *) text, strlen(text));
        }
    }
...
void TGWin32::GetGeometry(int wid, int &x, int &y, unsigned int &w, unsigned int &h)
{
  ...
  if (wid < 0) {
      ...
      gTws = &fWindows[wid];
      gdk_window_get_geometry(gTws->window, &x, &y, &width, &height, &depth);
...
Int_t TGWin32::OpenDisplay()
{
   ...
   gGCecho = gdk_gc_new_with_values((GdkWindow *)GDK_ROOT_PARENT(), &echov, 
   ...
   pixmp2 = gdk_bitmap_create_from_data(GDK_ROOT_PARENT (), // NULL
                                  null_cursor_bits, 16, 16);
   gNullCursor = gdk_cursor_new_from_pixmap(pixmp1,pixmp2,&fore,&back,0,0);
...
//                            GDK_ENTER_NOTIFY_MASK | GDK_LEAVE_NOTIFY_MASK;
   if(xval >= 0) attributes.x = xval;
   else  attributes.x = -1.0 * xval;
   if(yval >= 0) attributes.y = yval;
   else  attributes.y = -1.0 * yval;
...
void TGWin32::QueryPointer(int &ix, int &iy)
{
   ...
   root_return = gdk_window_get_pointer((GdkWindow *)gCws->window,
					  &root_x_return, &root_y_return, &mask_return);
...
Int_t TGWin32::RequestLocator(Int_t mode, Int_t ctyp, Int_t &x, Int_t &y)
{
   ...
   while (button_press == 0) {
      ...
      switch (ctyp) {
         ...
         case 2 :
             gdk_draw_line(gCws->window, gGCecho,
         ...
         case 3 :
             ...
             gdk_draw_arc(gCws->window, gGCecho,0,
...
void TGWin32::RescaleWindow(int wid, unsigned int w, unsigned int h)
{
   ...
   if (gTws->buffer) {
      ...
      for (i = 0; i < kMAXGC; i++) gdk_gc_set_clip_mask(gGClist[i], None);
      SetColor(gGCpxmp, 0);
...
//______________________________________________________________________________
void TGWin32::SetClipRegion(int wid, int x, int y, unsigned int w, unsigned int h)
{
...
void TGWin32::SetDoubleBufferON()
{
   ...
   if (!gTws->buffer) {
      gTws->buffer = gdk_pixmap_new(GDK_ROOT_PARENT (), //NULL,
                     gTws->width, gTws->height, gdk_visual_get_best_depth());
      SetColor(gGCpxmp, 0);
      gdk_draw_rectangle(gTws->buffer, gGCpxmp, 1, 0, 0, gTws->width, gTws->height);
      SetColor(gGCpxmp, 1);
   }
   for (int i = 0; i < kMAXGC; i++) gdk_gc_set_clip_mask(gGClist[i], None);
...
void TGWin32::SetDrawMode(EDrawMode mode)
{
   ...
   switch (mode) {
      case kCopy:
         for (i = 0; i < kMAXGC; i++) gdk_gc_set_function(gGClist[i], GDK_COPY);
         break;

      case kXor:
         for (i = 0; i < kMAXGC; i++) gdk_gc_set_function(gGClist[i], GDK_XOR);
         break;

      case kInvert:
         for (i = 0; i < kMAXGC; i++) gdk_gc_set_function(gGClist[i], GDK_INVERT);
         break;
   }
   fDrawMode = mode;
}
   ...
   switch (style) {
      ...
      case 3:         // hatch
         ...
         if (fasi != current_fasi) {
            ...
            switch (fasi) {
               case 1:
                  gFillPattern = gdk_bitmap_create_from_data(
                      GDK_ROOT_PARENT (), p1_bits, 16, 16);
                  break;
               case 2:
                  gFillPattern = gdk_bitmap_create_from_data(
                      GDK_ROOT_PARENT (), p2_bits, 16, 16);
                  break;
               case 3:
                  gFillPattern = gdk_bitmap_create_from_data(
                      GDK_ROOT_PARENT (), p3_bits, 16, 16);
                  break;
               case 4:
                  gFillPattern = gdk_bitmap_create_from_data(
                      GDK_ROOT_PARENT (), p4_bits, 16, 16);
                  break;
               case 5:
                  gFillPattern = gdk_bitmap_create_from_data(
                      GDK_ROOT_PARENT (), p5_bits, 16, 16);
                  break;
               case 6:
                  gFillPattern = gdk_bitmap_create_from_data(
                      GDK_ROOT_PARENT (), p6_bits, 16, 16);
                  break;
               case 7:
                  gFillPattern = gdk_bitmap_create_from_data(
                      GDK_ROOT_PARENT (), p7_bits, 16, 16);
                  break;
               case 8:
                  gFillPattern = gdk_bitmap_create_from_data(
                      GDK_ROOT_PARENT (), p8_bits, 16, 16);
                  break;
               case 9:
                  gFillPattern = gdk_bitmap_create_from_data(
                      GDK_ROOT_PARENT (), p9_bits, 16, 16);
                  break;
               case 10:
                  gFillPattern = gdk_bitmap_create_from_data(
                      GDK_ROOT_PARENT (), p10_bits, 16, 16);
                  break;
               case 11:
                  gFillPattern = gdk_bitmap_create_from_data(
                      GDK_ROOT_PARENT (), p11_bits, 16, 16);
                  break;
               case 12:
                  gFillPattern = gdk_bitmap_create_from_data(
                      GDK_ROOT_PARENT (), p12_bits, 16, 16);
                  break;
               case 13:
                  gFillPattern = gdk_bitmap_create_from_data(
                      GDK_ROOT_PARENT (), p13_bits, 16, 16);
                  break;
               case 14:
                  gFillPattern = gdk_bitmap_create_from_data(
                      GDK_ROOT_PARENT (), p14_bits, 16, 16);
                  break;
               case 15:
                  gFillPattern = gdk_bitmap_create_from_data(
                      GDK_ROOT_PARENT (), p15_bits, 16, 16);
                  break;
               case 16:
                  gFillPattern = gdk_bitmap_create_from_data(
                      GDK_ROOT_PARENT (), p16_bits, 16, 16);
                  break;
               case 17:
                  gFillPattern = gdk_bitmap_create_from_data(
                      GDK_ROOT_PARENT (), p17_bits, 16, 16);
                  break;
               case 18:
                  gFillPattern = gdk_bitmap_create_from_data(
                      GDK_ROOT_PARENT (), p18_bits, 16, 16);
                  break;
               case 19:
                  gFillPattern = gdk_bitmap_create_from_data(
                      GDK_ROOT_PARENT (), p19_bits, 16, 16);
                  break;
               case 20:
                  gFillPattern = gdk_bitmap_create_from_data(
                      GDK_ROOT_PARENT (), p20_bits, 16, 16);
                  break;
               case 21:
                  gFillPattern = gdk_bitmap_create_from_data(
                      GDK_ROOT_PARENT (), p21_bits, 16, 16);
                  break;
               case 22:
                  gFillPattern = gdk_bitmap_create_from_data(
                      GDK_ROOT_PARENT (), p22_bits, 16, 16);
                  break;
               case 23:
                  gFillPattern = gdk_bitmap_create_from_data(
                      GDK_ROOT_PARENT (), p23_bits, 16, 16);
                  break;
               case 24:
                  gFillPattern = gdk_bitmap_create_from_data(
                      GDK_ROOT_PARENT (), p24_bits, 16, 16);
                  break;
               case 25:
                  gFillPattern = gdk_bitmap_create_from_data(
                      GDK_ROOT_PARENT (), p25_bits, 16, 16);
                  break;
               default:
                  gFillPattern = gdk_bitmap_create_from_data(
                      GDK_ROOT_PARENT (), p2_bits, 16, 16);
...
void TGWin32::SetInput(int inp)
{
   // Set input on or off.
    if (inp == 1)
        EnableWindow((HWND)GDK_DRAWABLE_XID((GdkWindow *)gCws->window), TRUE);
    else
        EnableWindow((HWND)GDK_DRAWABLE_XID((GdkWindow *)gCws->window), FALSE);
...
void TGWin32::SetLineType(int n, int *dash)
{
   ...
   if (n <= 0) {
      ...
      gdk_gc_set_line_attributes(gGCline, gLineWidth,
                         (GdkLineStyle)gLineStyle, (GdkCapStyle)gCapStyle, 
...
void TGWin32::SetOpacity(Int_t percent)
{
   ...
   if (gdk_visual_get_best_depth() <= 8) return;
   if (percent == 0) return;
...
//______________________________________________________________________________
void TGWin32::CollectImageColors(ULong_t pixel, ULong_t *&orgcolors, Int_t &ncolors,
                               Int_t &maxcolors)
{
...
//*-*       14 :                            "Wingdings"            0           4
   ...
   switch (fontnumber/10) {
       case  1 : sprintf(fx11, "-*-times-medium-i-normal-*-15-*-*-*-*-*-iso8859-1");     break;
       case  2 : sprintf(fx11, "-*-times-bold-r-normal-*-15-*-*-*-*-*-iso8859-1");       break;
       case  3 : sprintf(fx11, "-*-times-bold-i-normal-*-15-*-*-*-*-*-iso8859-1");       break;
       case  4 : sprintf(fx11, "-*-arial-medium-r-normal-*-15-*-*-*-*-*-iso8859-1");     break;
       case  5 : sprintf(fx11, "-*-arial-medium-o-normal-*-15-*-*-*-*-*-iso8859-1");     break;
       case  6 : sprintf(fx11, "-*-arial-bold-r-normal-*-15-*-*-*-*-*-iso8859-1");       break;
       case  7 : sprintf(fx11, "-*-arial-bold-o-normal-*-15-*-*-*-*-*-iso8859-1");       break;
       case  8 : sprintf(fx11, "-*-courier-medium-r-normal-*-15-*-*-*-*-*-iso8859-1");   break;
       case  9 : sprintf(fx11, "-*-courier-medium-o-normal-*-15-*-*-*-*-*-iso8859-1");   break;
       case 10 : sprintf(fx11, "-*-courier-bold-r-normal-*-15-*-*-*-*-*-iso8859-1");     break;
       case 11 : sprintf(fx11, "-*-courier-bold-o-normal-*-15-*-*-*-*-*-iso8859-1");     break;
       case 12 : sprintf(fx11, "-*-symbol-medium-r-normal-*-15-*-*-*-*-*-iso8859-1");    break;
       case 13 : sprintf(fx11, "-*-times-medium-r-normal-*-15-*-*-*-*-*-iso8859-1");     break;
       case 14 : sprintf(fx11, "-*-wingdings-medium-r-normal-*-15-*-*-*-*-*-iso8859-1"); break;
       default : sprintf(fx11, "-*-arial-bold-r-normal-*-15-*-*-*-*-*-iso8859-1");       break;
   }
...
UInt_t TGWin32::ExecCommand(TGWin32Command *code)
{
  ...
  while (gdk_events_pending()) {
		...
		if(event != NULL) {
			...
			if (!event_window) return 0; //break;
			switch (event->type) {
			    case GDK_DELETE:
...
void TGWin32::UpdateWindow(int mode)
{
   ...
   if (gCws->double_buffer) {
      gdk_window_copy_area(gCws->window,gGCpxmp,0,0,gCws->drawing,0,0,gCws->width, gCws->height);
...
//______________________________________________________________________________
void TGWin32::WritePixmap(int wid, unsigned int w, unsigned int h, char *pxname)
{
...
//______________________________________________________________________________
void TGWin32::ImgPickPalette(GdkImage *image, Int_t &ncol, Int_t *&R, Int_t *&G, Int_t *&B)
{
...
void TGWin32::WriteGIF(char *name)
{
   ...
   ximage = gdk_image_get(gCws->drawing, 0, 0,
                      gCws->width, gCws->height);
...
//______________________________________________________________________________
void TGWin32::PutImage(int offset,int itran,int x0,int y0,int nx,int ny,
                     int xmin,int ymin,int xmax,int ymax, unsigned char *image)
{
   ...
   for (i = 0; i < 256; i++) {
      if (nlines[i] != 0) {
         ...
         gdk_draw_segments(gCws->drawing,gGCline,&lines[i][0],nlines[i]);
      }
   }
}
