...
TEST(SOFIE_HardSigmoid, GenerateHexfloatClampConstants)
{
   ...
   EXPECT_TRUE(code.find("std::fmax") != std::string::npos) << "Generated code should use std::fmax for clamping";

   EXPECT_TRUE(code.find("std::fmin") != std::string::npos) << "Generated code should use std::fmin for clamping";
}
...
TEST(SOFIE_HardSigmoid, GenerateFloatSuffix)
{
   ...
   EXPECT_TRUE(code.find("f))") != std::string::npos) << "Generated code missing 'f' suffix after beta constant";
}
...
TEST(SOFIE_HardSigmoid, NumericCorrectnessLinearRegion)
{
   ...
   for (const auto &[input, expected] : referenceData) {
      ...
      EXPECT_NEAR(computed, expected, tol) << "Linear region mismatch at x = " << input;
   }
}
...
TEST(SOFIE_HardSigmoid, NumericCorrectnessClamps)
{
   ...
   const std::vector<std::pair<float, float>> clampData = {
      {-10.0f, 0.0f}, {-3.0f, 0.0f}, {-2.5f, 0.0f}, {2.5f, 1.0f}, {3.0f, 1.0f}, {10.0f, 1.0f},
   };
   ...
   for (const auto &[input, expected] : clampData) {
      ...
      EXPECT_NEAR(computed, expected, tol) << "Clamp behavior mismatch at x = " << input;
   }
}
...
TEST(SOFIE_HardSigmoid, CustomAlphaBeta)
{
   ...
   const std::vector<std::pair<float, float>> testData = {
      {-5.0f, 0.0f}, {-3.0f, 0.0f}, {0.0f, 0.5f}, {3.0f, 1.0f}, {5.0f, 1.0f},
   };
   ...
   for (const auto &[input, expected] : testData) {
      ...
      EXPECT_NEAR(computed, expected, tol) << "Custom alpha/beta mismatch at x = " << input;
   }
}
