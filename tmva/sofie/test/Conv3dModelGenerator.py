...
### generate Conv3d model using Pytorch
...
class Net(nn.Module):
    ...
    def __init__(self, nc = 1, ng = 1, nl = 4, use_bn = False, use_maxpool = False, use_avgpool = False):
        ...
        if (self.use_bn): self.bn1 = nn.BatchNorm3d(4)
        if (self.use_maxpool): self.pool1 = nn.MaxPool3d(2)
        if (self.use_avgpool): self.pool1 = nn.AvgPool3d(2)
        if (self.nl > 1):
...
def main():
   ...
   parser = make_parser('parameters for the Conv network : batchSize , inputChannels, inputImageSize, depth, nLayers ',
                        pooling=True)
   ...
   name = model_name("Conv3dModel", bsize, use_bn, use_maxpool, use_avgpool)
