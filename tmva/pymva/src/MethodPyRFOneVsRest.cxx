...
ClassImp(MethodPyRFOneVsRest)
   ...
   MethodPyRFOneVsRest::MethodPyRFOneVsRest(const TString &jobName, const TString &methodTitle, DataSetInfo &dsi,
                                            const TString &theOption)
   : PyMethodBase(jobName, Types::kPyRFOneVsRest, methodTitle, dsi, theOption), n_estimators(10), criterion("gini"),
     max_depth("None"), min_samples_split(2), min_samples_leaf(1), min_weight_fraction_leaf(0), max_features("'auto'"),
     max_leaf_nodes("None"), bootstrap(kTRUE), oob_score(kFALSE), n_jobs(1), random_state("None"), verbose(0),
     warm_start(kFALSE), class_weight("None"), n_jobsOVR(1) // for OneVsRest Classifier
{
}

//_______________________________________________________________________
MethodPyRFOneVsRest::MethodPyRFOneVsRest(DataSetInfo &theData, const TString &theWeightFile)
   : PyMethodBase(Types::kPyRFOneVsRest, theData, theWeightFile), n_estimators(10), criterion("gini"),
     max_depth("None"), min_samples_split(2), min_samples_leaf(1), min_weight_fraction_leaf(0), max_features("'auto'"),
     max_leaf_nodes("None"), bootstrap(kTRUE), oob_score(kFALSE), n_jobs(1), random_state("None"), verbose(0),
     warm_start(kFALSE), class_weight("None"), n_jobsOVR(1) // for OneVsRest Classifier
{
...
void MethodPyRFOneVsRest::ProcessOptions()
{
   ...
   if (min_samples_split < 0) {
      Log() << kERROR << " MinSamplesSplit < 0... that does not work !! "
            << " I set it to 2 .. just so that the program does not crash" << Endl;
      min_samples_split = 2;
   }
   if (min_samples_leaf < 0) {
      Log() << kERROR << " MinSamplesLeaf < 0... that does not work !! "
            << " I set it to 1 .. just so that the program does not crash" << Endl;
      min_samples_leaf = 1;
   }

   if (min_weight_fraction_leaf < 0) {
      Log() << kERROR << " MinWeightFractionLeaf < 0... that does not work !! "
            << " I set it to 0 .. just so that the program does not crash" << Endl;
...
void MethodPyRFOneVsRest::Init()
{
   ...
   for (int i = 0; i < fNrowsTraining; i++) {
      ...
      if (e->GetClass() == TMVA::Types::kSignal)
         TrainDataClasses[i] = TMVA::Types::kSignal;
      else
         TrainDataClasses[i] = TMVA::Types::kBackground;
...
void MethodPyRFOneVsRest::Train()
{
   ...
   if (PyCallable_Check(fRFClassifierClass)) {
      ...
      fClassifier = PyObject_CallMethod(fClassifier, const_cast<char *>("fit"), const_cast<char *>("(OO)"), fTrainData,
                                        fTrainDataClasses);
   ...
   //   // OneVsRestClassifier using training dataset features and labels to train.
   //   fOVRClassifier = PyObject_CallMethod(fOVRClassifier, const_cast<char *>("fit"), const_cast<char *>("(OO)"),
   //   fTrainData, fTrainDataClasses);
   ...
   // fClassifier = PyObject_CallMethod(fClassifier, const_cast<char *>("fit"), const_cast<char *>("(OOO)"), fTrainData,
   // fTrainDataClasses, fTrainDataWeights);
...
void MethodPyRFOneVsRest::GetHelpMessage() const
{
   ...
   Log() << Endl;
   Log() << Endl;
   Log() << gTools().Color("bold") << "--- Performance tuning via configuration options:" << gTools().Color("reset")
         << Endl;
   Log() << Endl;
   Log() << "<None>" << Endl;
}
