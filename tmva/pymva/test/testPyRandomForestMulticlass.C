...
int testPyRandomForestMulticlass(){
   ...
   // Load data
   TMVA::DataLoader *dataloader = new TMVA::DataLoader("datasetTestPyRandomForestMulticlass");
   ...
   factory->BookMethod(dataloader, TMVA::Types::kPyRandomForest, "PyRandomForest",
      "!H:!V:VarTransform=None:NEstimators=10:Verbose=0");
   ...
   reader->BookMVA("PyRandomForest", "datasetTestPyRandomForestMulticlass/weights/testPyRandomForestMulticlass_PyRandomForest.weights.xml");
   ...
   for(UInt_t i=0; i<numEvents; i++){
      signal->GetEntry(i);
      meanMvaSignal += reader->EvaluateMulticlass("PyRandomForest")[0];
      background0->GetEntry(i);
      meanMvaBackground0 += reader->EvaluateMulticlass("PyRandomForest")[1];
      background1->GetEntry(i);
      meanMvaBackground1 += reader->EvaluateMulticlass("PyRandomForest")[2];
      background2->GetEntry(i);
      meanMvaBackground2 += reader->EvaluateMulticlass("PyRandomForest")[3];
   }
...
int main(){
   int err = testPyRandomForestMulticlass();
   return err;
}
