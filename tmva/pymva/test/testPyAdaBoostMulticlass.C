...
int testPyAdaBoostMulticlass(){
   ...
   TFile* outputFile = TFile::Open("ResultsTestPyAdaBoostMulticlass.root", "RECREATE");
   TMVA::Factory *factory = new TMVA::Factory("testPyAdaBoostMulticlass", outputFile,
      "!V:Silent:Color:!DrawProgressBar:AnalysisType=multiclass");

   // Load data
   TMVA::DataLoader *dataloader = new TMVA::DataLoader("datasetTestPyAdaBoostMulticlass");
   ...
   factory->BookMethod(dataloader, TMVA::Types::kPyAdaBoost, "PyAdaBoost",
      "!H:!V:VarTransform=None:NEstimators=50");
   ...
   reader->BookMVA("PyAdaBoost", "datasetTestPyAdaBoostMulticlass/weights/testPyAdaBoostMulticlass_PyAdaBoost.weights.xml");
   ...
   for(UInt_t i=0; i<numEvents; i++){
      signal->GetEntry(i);
      meanMvaSignal += reader->EvaluateMulticlass("PyAdaBoost")[0];
      background0->GetEntry(i);
      meanMvaBackground0 += reader->EvaluateMulticlass("PyAdaBoost")[1];
      background1->GetEntry(i);
      meanMvaBackground1 += reader->EvaluateMulticlass("PyAdaBoost")[2];
      background2->GetEntry(i);
      meanMvaBackground2 += reader->EvaluateMulticlass("PyAdaBoost")[3];
   }
   ...
   // to 1.0/num_classes, thought the separation is quite good. You can check this by looking at
   // the MVA value distributions using TMVAMultiClassGui.
   // Because of this, the thresholds for passing this test are relaxed to greater or less
   // 1.0/num_classes for signal and backgrounds respectively.

   std::cout << "Mean MVA response on signal: " << meanMvaSignal << std::endl;
   if(meanMvaSignal < 0.25){
      std::cout << "[ERROR] Mean response on signal is " << meanMvaSignal << " (<0.25)" << std::endl;
      return 1;
   }
   std::cout << "Mean MVA response on background 0: " << meanMvaBackground0 << std::endl;
   if(meanMvaBackground0 < 0.25){
      std::cout << "[ERROR] Mean response on background 0 is " << meanMvaBackground0 << " (<0.25)" << std::endl;
      return 1;
   }
   std::cout << "Mean MVA response on background 1: " << meanMvaBackground1 << std::endl;
   if(meanMvaBackground0 < 0.25){
      std::cout << "[ERROR] Mean response on background 1 is " << meanMvaBackground1 << " (<0.25)" << std::endl;
      return 1;
   }
   std::cout << "Mean MVA response on background 2: " << meanMvaBackground2 << std::endl;
   if(meanMvaBackground0 < 0.25){
      std::cout << "[ERROR] Mean response on background 2 is " << meanMvaBackground2 << " (<0.25)" << std::endl;
      return 1;
   }
...
int main(){
   int err = testPyAdaBoostMulticlass();
   return err;
}
