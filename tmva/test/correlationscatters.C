...
void correlationscatters( Bool_t useDecorr = kFALSE, TString fin = "TMVA.root", bool useTMVAStyle = kTRUE )
{
  if(!useTMVAStyle) {
    gROOT->Reset();
  ...
  TString extension = (useDecorr) ? "_decorr" : "";

  TString dirName = "CorrelationPlots" + extension;
  ...
  noPlots/=2; // there are scatter and profile plots
  cout << "*** found: " << noPlots << " plots" << endl;
  ...
  for (UInt_t itype = 0; itype < 2; itype++) {
    ...
    while ((key = (TKey*)next())) {
      ...
      if (hname.EndsWith( thename[itype] + extension ) && hname.BeginsWith( "scat_" )) { // found a new signal plot
	...
	if (countPad > noPad) {
	  ...
	  if (useDecorr) sprintf( fname, "plots/correlationscatter_decorr_c%i", countCanvas+1 );
	  else           sprintf( fname, "plots/correlationscatter_c%i", countCanvas+1 );
  ...
  if (countPad <= noPad) {
    c[countCanvas]->Update();
    sprintf( fname, "plots/correlationscatter_c%i", countCanvas+1 );
    TMVAGlob::imgconv( c[countCanvas], &fname[0] );
  }
