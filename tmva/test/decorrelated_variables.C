...
void decorrelated_variables( TString fin = "TMVA.root", Bool_t useTMVAStyle = kTRUE )
{
  if(!useTMVAStyle) {
    gROOT->Reset();
  ...
  decorrelated_input_variables->cd();
  ...
  // loop over all objects in "decorrelated_input_variables" directory
  ...
  while ((key = (TKey*)next())) {
    ...
    if (hname.Contains("__S")){ // found a new signal plot
      ...
      if ((c[countCanvas]==NULL) || (countPad>noPad)) {
        ...
        c[countCanvas] = new TCanvas( cn, "MVA Decorrelated Variables", 
      ...
      sig->SetTitle( TString("MVA decorrelated variable: ") + title );
      ...
      if (countPad > noPad) {
        ...
        sprintf( fname, "plots/decorrelated_variables_c%i", countCanvas+1 );
  ...
  if (countPad <= noPad) {
     c[countCanvas]->Update();
     sprintf( fname, "plots/decorrelated_variables_c%i", countCanvas );
     TMVAGlob::imgconv( c[countCanvas-1], &fname[0] );
  }
}
