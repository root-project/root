...
void TMVApplication( TString myMethodList = "" ) 
{
   ...
   TH1F *probHistFi;
   if (Use_Fisher)        probHistFi = new TH1F( "PROBA_MVA_Fisher", "PROBA_MVA_Fisher",  nbin, 0, 1 );
   ...
   int    nSelCuts = 0, nSelCutsD = 0, nSelCutsGA = 0;
   double effS     = 0.7;
   ...
   for (Long64_t ievt=0; ievt<theTree->GetEntries();ievt++) {
      ...
      if (Use_Fisher       )   probHistFi->Fill( reader->GetProba( "Fisher method" ) );
   }
   sw.Stop();
   ...
   if (Use_Fisher       )   probHistFi->Write();
   target->Close();
