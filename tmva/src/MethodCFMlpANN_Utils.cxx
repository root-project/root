// @(#)root/tmva $Id: MethodCFMlpANN_Utils.cxx,v 1.6 2006/05/22 10:33:48 andreas.hoecker Exp $ 
 ...
 * $Id: MethodCFMlpANN_Utils.cxx,v 1.6 2006/05/22 10:33:48 andreas.hoecker Exp $ 
 **********************************************************************************/
					    ...
					    Int_t *numcycle, Int_t /*det_len*/)
{
  ...
  fParam_1.nunap = 40;
  printf("--- TMVA::MethodCFMlpANN: total number of events for training: %i\n", fParam_1.nevl);
  printf("--- TMVA::MethodCFMlpANN: total number of events for testing : %i\n", fParam_1.nevt);
  printf("--- TMVA::MethodCFMlpANN: total number of training cycles    : %i\n", fParam_1.nblearn);
  ...
  i__1 = fParam_1.layerm;
  for (j = 1; j <= i__1; ++j) {
    printf("--- TMVA::MethodCFMlpANN: number of layers for neuron(%2i): %i\n",j, 
	   neur_1.neuron[j - 1]);
  }
  ...
  if (fParam_1.ichoi == 0) {
    printf("--- TMVA::MethodCFMlpANN: new training will be performed\n");
  } 
  else {
    printf("--- TMVA::MethodCFMlpANN: new training will be continued from a weight file\n");
  }
  ncoef = 0;
  ntemp = 0;
  for (i__ = 1; i__ <= max_nNodes_; ++i__) {
...
void TMVA::MethodCFMlpANN_Utils::Innit(char *det, Double_t *tout2, Double_t *tin2, Int_t )
{
  ...
  printf("--- TMVA::MethodCFMlpANN: elapsed time: %s\n", (const char*)timer.GetElapsedTime()  );
