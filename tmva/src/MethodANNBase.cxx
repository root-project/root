// @(#)root/tmva $Id: MethodANNBase.cxx,v 1.42 2006/09/30 00:16:01 andreas.hoecker Exp $
...
void TMVA::MethodANNBase::InitANNBase()
{
   ...
   if (FIXED_SEED) frgen = new TRandom3(1);   // fix output for debugging
...
void TMVA::MethodANNBase::WaitForKeyboard()
{
   ...
   string dummy;
   cout << "***Type anything to continue (q to quit): ";
   getline(cin, dummy);
   if (dummy == "q" || dummy == "Q") {
      PrintMessage("Quit");
      delete this;
...
void TMVA::MethodANNBase::PrintNetwork()
{
   ...
   for (Int_t i = 0; i < numLayers; i++) {
      ...
      cout << "Layer #" << i << " (" << numNeurons << " neurons):" << endl;
      PrintLayer(curLayer);
   }
}
...
void TMVA::MethodANNBase::PrintLayer(TObjArray* layer)
{
   ...
   for (Int_t j = 0; j < numNeurons; j++) {
      ...
      cout << "\tNeuron #" << j << " (LinksIn: " << neuron->NumPreLinks() 
...
//_______________________________________________________________________
void TMVA::MethodANNBase::WriteHistosToFile() const
{
