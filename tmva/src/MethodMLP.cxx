// @(#)root/tmva $Id: MethodMLP.cxx,v 1.25 2006/09/29 23:27:15 andreas.hoecker Exp $
...
TMVA::MethodMLP* TMVA::MethodMLP::m_This = 0;
...
{     
   ...
   for (Int_t i = 0; i < nEvents; i++) {
      ...
      // this goes with the old event model
      //Float_t* ptr = &(branchVar[0]);
      //TrainOneEventFast(index[i], ptr, type);
...
void TMVA::MethodMLP::GeneticMinimize()
{
  ...
  cout << "Estimator after optimization: " << estimator << endl;
}
...
void TMVA::MethodMLP::FCN( Int_t& npars, Double_t* grad, Double_t &f, Double_t* fitPars, Int_t iflag )
{
  ...
  nc++;
  if (f < minf) minf = f;
  for (Int_t ipar=0; ipar<fNumberOfWeights; ipar++) cout << fitPars[ipar] << " ";
  cout << endl;
  cout << "***** new estimator: " << f << "  min: " << minf << " --> ncalls: " << nc << endl;
}
