// @(#)root/tmva $Id: MethodCFMlpANN.cxx,v 1.28 2006/09/29 10:14:15 andreas.hoecker Exp $    
   ...
   if (HasTrainingTree()) {
      ...
      if (Verbose())
         cout << "--- " << GetName() << " <verbose>: " 
              << Data().GetNEvtSigTrain() << " signal and " << Data().GetNEvtBkgdTrain() << " background"
...
void TMVA::MethodCFMlpANN::ProcessOptions() 
{
   ...
   cout << "--- " << GetMethodName() << ": use " << fNcycles << " training cycles" << endl;
   cout << "--- " << GetMethodName() << ": use configuration (nodes per layer): in=";
   for (Int_t i=0; i<fNlayers-1; i++) cout << fNodes[i] << ":";
   cout << fNodes[fNlayers-1] << "=out" << endl;
}
...
void TMVA::MethodCFMlpANN::Train( void )
{
   ...
   if (!CheckSanity()) { 
      cout << "--- " << GetName() << "::Train: fatal error: sanity check failed" << endl;
      exit(1);
   }
...
#else
   cout << "--- " << GetName() << "::Train: sorry Train_nn is not yet implemented on Windows" << endl;
#endif  
...
Double_t TMVA::MethodCFMlpANN::GetMvaValue()
{
   ...
   if (!isOK) {
      cout << "--- " << GetName() << ": Problem in ::EvalANN (!isOK) for event " 
           << " ==> exit(1)"
           << endl;
      exit(1);
   }
...
void TMVA::MethodCFMlpANN::ReadWeightsFromStream( istream & istr )
{
   ...
   if (GetNvar() != nva) { // wrong file
      ...
      if (lclass != 2) { // wrong file
         ...
         if (istr.eof( )) {
            cout << "--- " << GetName() << ": Error in ::ReadWeightsFromStream: "
                 << "reach EOF prematurely: " << endl;
         }
         else {
   ...
   if (GetNvar() != fNeuronNN[0]) {
      cout << "--- " << GetName() << ": Error in ::ReadWeightsFromFile: mismatch in zeroth layer:"
           << GetNvar() << " " << fNeuronNN[0] << " ==> exit(1)" << endl;
      exit(1);
   }
}
...
void TMVA::MethodCFMlpANN::WriteNNWeightsToStream( std::ostream & o, Int_t nva, Int_t lclass, 
                                                   ...
                                                   const Double_t* wNN, const Double_t* wwNN, const Double_t* tempNN ) const
{
...
#define ww_ref(a_1,a_2)    wwNN[(a_2)*max_nLayers_ + a_1 - 7]
   ...
   // number of output classes must be 2
   if (lclass != 2) { // wrong file
      cout << "--- " << GetName() << ": Error in ::WriteNNWeightsToStream: "
           << "mismatch in number of classes" << endl;
      exit(1);
   }
   else {
...
//_______________________________________________________________________
void  TMVA::MethodCFMlpANN::WriteHistosToFile( void ) const
{
