...
public:
  RSampler(std::vector<RFlat2DMatrix> &datasets, std::string sampleType, bool shuffle = true, const std::size_t setSeed = 0)
    : fDatasets(datasets),
   ...
   void RandomSampler(RFlat2DMatrix &SampledTensor) {
         std::size_t rows = 0;
         for (std::size_t i = 0; i < fDatasets.size(); i++) {
            rows += fDatasets[i].GetRows();
         }
         
         RFlat2DMatrix ConcatTensor;
         fTensorOperators->ConcatinateTensors(ConcatTensor, fDatasets);
