...
using MatrixInput_t =
    std::pair<const TMatrixT<Double_t> &, const TMatrixT<Double_t> &>;
 ...
 */
//______________________________________________________________________________
template <typename AArchitecture> class TBatch {
private:
 ...
 */
template <typename Data_t, typename AArchitecture> class TBatchIterator {
private:
  TDataLoader<Data_t, AArchitecture> &fDataLoader;
  size_t fBatchIndex;

public:
  TBatchIterator(TDataLoader<Data_t, AArchitecture> &dataLoader,
                 size_t index = 0)
 ...
 * type to the accelerator device or the CPU. A TDataLoader object manages a
 * number
 ...
 * samples in the dataset. One epoch is the number of buffers required to
 * transfer
 * the complete training set. Using the begin() and end() member functions
 * allows
...
public:
  TDataLoader(const Data_t &data, size_t nSamples, size_t batchSize,
              size_t nInputFeatures, size_t nOutputFeatures,
              size_t nStreams = 1);
  ...
  /** Copy input matrix into the given host buffer. Function to be specialized
   * by
  ...
  void CopyOutput(HostBuffer_t &buffer, IndexIterator_t begin,
                  size_t batchSize);
...
TDataLoader<Data_t, AArchitecture>::TDataLoader(
  ...
  for (size_t i = 0; i < fNStreams; i++) {
    ...
    fDeviceBuffers.push_back(
        DeviceBuffer_t(inputMatrixSize + outputMatrixSize));
  }
...
TBatch<AArchitecture> TDataLoader<Data_t, AArchitecture>::GetBatch() {
  ...
  HostBuffer_t outputHostBuffer =
      hostBuffer.GetSubBuffer(inputMatrixSize, outputMatrixSize);

  DeviceBuffer_t inputDeviceBuffer =
      deviceBuffer.GetSubBuffer(0, inputMatrixSize);
  DeviceBuffer_t outputDeviceBuffer =
      deviceBuffer.GetSubBuffer(inputMatrixSize, outputMatrixSize);
