...
auto testEncodeInput(typename Architecture::Matrix_t &input,
                     ...
                     typename Architecture::Matrix_t &Weights) -> void {
  using Matrix_t = typename Architecture::Matrix_t;
...
auto testReconstructInput(typename Architecture::Matrix_t &compressedInput,
                          ...
                          typename Architecture::Matrix_t &fWeights) -> void {
  size_t m, n;
...
auto testUpdateParameter(typename Architecture::Matrix_t &x,
                         ...
                         double learningRate, size_t fBatchSize) -> void {
  using Matrix_t = typename Architecture::Matrix_t;
  size_t m, n;
  m = fWeights.GetNrows();
  n = fWeights.GetNcols();
  Architecture::UpdateParams(x, tildeX, y, z, fVBiases, fHBiases, fWeights,
                             VBiasError, HBiasError, learningRate, fBatchSize);
  for (size_t i = 0; i < m; i++) {
    for (size_t j = 0; j < n; j++) {
