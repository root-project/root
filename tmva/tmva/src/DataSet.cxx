...
Long64_t TMVA::DataSet::GetNClassEvents( Int_t type, UInt_t classNumber )
{
   try {
      ...
      Log() << kFATAL << Form("Dataset[%s] : ", fdsi->GetName()) << "No "
            << (type == 0 ? "training" : (type == 1 ? "testing" : "_unknown_type_")) << " events for class "
            << (ci == nullptr ? "_no_name_known_" : ci->GetName()) << " (index # " << classNumber << ")"
