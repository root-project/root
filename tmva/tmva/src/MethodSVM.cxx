...
TMVA::MethodSVM::MethodSVM(const TString &jobName, const TString &methodTitle, DataSetInfo &theData,
   ...
   : MethodBase(jobName, Types::kSVM, methodTitle, theData, theOption), fCost(0), fTolerance(0), fMaxIter(0),
     fNSubSets(0), fBparm(0), fGamma(0), fWgSet(nullptr), fInputData(nullptr), fSupportVectors(nullptr),
     fSVKernelFunction(nullptr), fMinVars(nullptr), fMaxVars(nullptr), fDoubleSigmaSquared(0), fOrder(0), fTheta(0),
     fKappa(0), fMult(0), fNumVars(0), fGammas(""), fGammaList(""), fDataSize(0), fLoss(0)
{
   fVarNames.clear();
...
TMVA::MethodSVM::MethodSVM(DataSetInfo &theData, const TString &theWeightFile)
   : MethodBase(Types::kSVM, theData, theWeightFile), fCost(0), fTolerance(0), fMaxIter(0), fNSubSets(0), fBparm(0),
     fGamma(0), fWgSet(nullptr), fInputData(nullptr), fSupportVectors(nullptr), fSVKernelFunction(nullptr),
     fMinVars(nullptr), fMaxVars(nullptr), fDoubleSigmaSquared(0), fOrder(0), fTheta(0), fKappa(0), fMult(0),
     fNumVars(0), fGammas(""), fGammaList(""), fDataSize(0), fLoss(0)
{
   fVarNames.clear();
   fNumVars = theData.GetVariableInfos().size();
   for( int i=0;i<fNumVars; i++){
