...
TMVA::RuleFit::RuleFit()
   : fNTreeSample(0), fNEveEffTrain(0), fMethodRuleFit(nullptr), fMethodBase(nullptr), fVisHistsUseImp(kTRUE),
     fLogger(new MsgLogger("RuleFit"))
{
   std::srand( randSEED ); // initialize random number generator used by std::random_shuffle
}
...
{
   if (fMethodRuleFit == nullptr)
      Log() << kFATAL << "RuleFit::SetTrainingEvents - MethodRuleFit not initialized" << Endl;
   UInt_t neve = el.size();
