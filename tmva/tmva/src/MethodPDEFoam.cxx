...
TMVA::MethodPDEFoam::MethodPDEFoam(const TString &jobName, const TString &methodTitle, DataSetInfo &dsi,
   ...
   : MethodBase(jobName, Types::kPDEFoam, methodTitle, dsi, theOption), fSigBgSeparated(kFALSE), fFrac(0.001),
     fDiscrErrCut(-1.0), fVolFrac(1.0 / 15.0), fnCells(999), fnActiveCells(500), fnSampl(2000), fnBin(5),
     fEvPerBin(10000), fCompress(kTRUE), fMultiTargetRegression(kFALSE), fNmin(100), fCutNmin(kTRUE), fMaxDepth(0),
     fKernelStr("None"), fKernel(kNone), fKernelEstimator(nullptr), fTargetSelectionStr("Mean"),
     fTargetSelection(kMean), fFillFoamWithOrigWeights(kFALSE), fUseYesNoCell(kFALSE), fDTLogic("None"),
     fDTSeparation(kFoam), fPeekMax(kTRUE), fXmin(), fXmax(), fFoam()
{
}
...
TMVA::MethodPDEFoam::MethodPDEFoam(DataSetInfo &dsi, const TString &theWeightFile)
   : MethodBase(Types::kPDEFoam, dsi, theWeightFile), fSigBgSeparated(kFALSE), fFrac(0.001), fDiscrErrCut(-1.0),
     fVolFrac(1.0 / 15.0), fnCells(999), fnActiveCells(500), fnSampl(2000), fnBin(5), fEvPerBin(10000),
     fCompress(kTRUE), fMultiTargetRegression(kFALSE), fNmin(100), fCutNmin(kTRUE), fMaxDepth(0), fKernelStr("None"),
     fKernel(kNone), fKernelEstimator(nullptr), fTargetSelectionStr("Mean"), fTargetSelection(kMean),
     fFillFoamWithOrigWeights(kFALSE), fUseYesNoCell(kFALSE), fDTLogic("None"), fDTSeparation(kFoam), fPeekMax(kTRUE),
     fXmin(), fXmax(), fFoam()
{
}
...
TMVA::MethodPDEFoam::~MethodPDEFoam( void )
{
   ...
   if (fKernelEstimator != nullptr) delete fKernelEstimator;
}
...
void TMVA::MethodPDEFoam::GetNCuts(PDEFoamCell *cell, std::vector<UInt_t> &nCuts)
{
   ...
   if (cell->GetDau0() != nullptr) GetNCuts(cell->GetDau0(), nCuts);
   if (cell->GetDau1() != nullptr) GetNCuts(cell->GetDau1(), nCuts);
}
...
TMVA::PDEFoamKernelBase* TMVA::MethodPDEFoam::CreatePDEFoamKernel()
{
   ...
   default: Log() << kFATAL << "Kernel: " << fKernel << " not supported!" << Endl; return nullptr;
   }
   return nullptr;
}
