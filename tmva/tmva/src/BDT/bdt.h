...
public:
  std::shared_ptr<AbstractNode> child_true;
  std::shared_ptr<AbstractNode> child_false;
...
public:
  ...
  std::vector<std::shared_ptr<AbstractNode>> nodes; // change to vector of pointers
  /*double inference(double event[]){
    return this->nodes.back().inference(event); // ->
  }*/
};
