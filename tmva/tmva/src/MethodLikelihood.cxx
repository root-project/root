...
TMVA::MethodLikelihood::MethodLikelihood(const TString &jobName, const TString &methodTitle, DataSetInfo &theData,
   ...
   : TMVA::MethodBase(jobName, Types::kLikelihood, methodTitle, theData, theOption), fEpsilon(1.e3 * DBL_MIN),
     fTransformLikelihoodOutput(kFALSE), fDropVariable(0), fHistSig(nullptr), fHistBgd(nullptr),
     fHistSig_smooth(nullptr), fHistBgd_smooth(nullptr), fDefaultPDFLik(nullptr), fPDFSig(nullptr), fPDFBgd(nullptr),
     fNsmooth(2), fNsmoothVarS(nullptr), fNsmoothVarB(nullptr), fAverageEvtPerBin(0), fAverageEvtPerBinVarS(nullptr),
     fAverageEvtPerBinVarB(nullptr), fKDEfineFactor(0), fInterpolateString(nullptr)
{
}
...
TMVA::MethodLikelihood::MethodLikelihood(DataSetInfo &theData, const TString &theWeightFile)
   : TMVA::MethodBase(Types::kLikelihood, theData, theWeightFile), fEpsilon(1.e3 * DBL_MIN),
     fTransformLikelihoodOutput(kFALSE), fDropVariable(0), fHistSig(nullptr), fHistBgd(nullptr),
     fHistSig_smooth(nullptr), fHistBgd_smooth(nullptr), fDefaultPDFLik(nullptr), fPDFSig(nullptr), fPDFBgd(nullptr),
     fNsmooth(2), fNsmoothVarS(nullptr), fNsmoothVarB(nullptr), fAverageEvtPerBin(0), fAverageEvtPerBinVarS(nullptr),
     fAverageEvtPerBinVarB(nullptr), fKDEfineFactor(0), fInterpolateString(nullptr)
...
void TMVA::MethodLikelihood::Train( void )
{
   ...
   for (UInt_t ivar=0; ivar<GetNvar(); ivar++) {
      ...
      if ((*fPDFSig)[ivar]->GetSmoothedHist() != nullptr)
         (*fHistSig_smooth)[ivar] = (*fPDFSig)[ivar]->GetSmoothedHist();
      if ((*fPDFBgd)[ivar]->GetSmoothedHist() != nullptr)
