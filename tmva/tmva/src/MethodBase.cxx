...
TMVA::MethodBase::MethodBase(const TString &jobName, Types::EMVA methodType, const TString &methodTitle,
   ...
   : IMethod(), Configurable(theOption), fTmpEvent(nullptr), fRanking(nullptr), fInputVars(nullptr),
     fAnalysisType(Types::kNoAnalysisType), fRegressionReturnVal(nullptr), fMulticlassReturnVal(nullptr),
     fDataSetInfo(dsi), fSignalReferenceCut(0.5), fSignalReferenceCutOrientation(1.),
     fVariableTransformType(Types::kSignal), fJobName(jobName), fMethodName(methodTitle), fMethodType(methodType),
     fTestvar(""), fTMVATrainingVersion(TMVA_VERSION_CODE), fROOTTrainingVersion(ROOT_VERSION_CODE),
     fConstructedFromWeightFile(kFALSE), fBaseDir(nullptr), fMethodBaseDir(nullptr), fFile(nullptr),
     fSilentFile(kFALSE), fModelPersistence(kTRUE), fWeightFile(""), fEffS(nullptr), fDefaultPDF(nullptr),
     fMVAPdfS(nullptr), fMVAPdfB(nullptr), fSplS(nullptr), fSplB(nullptr), fSpleffBvsS(nullptr), fSplTrainS(nullptr),
     fSplTrainB(nullptr), fSplTrainEffBvsS(nullptr), fVarTransformString("None"), fTransformationPointer(nullptr),
     fTransformation(dsi, methodTitle), fVerbose(kFALSE), fVerbosityLevelString("Default"), fHelp(kFALSE),
     fHasMVAPdfs(kFALSE), fIgnoreNegWeightsInTraining(kFALSE), fSignalClass(0), fBackgroundClass(0), fSplRefS(nullptr),
     fSplRefB(nullptr), fSplTrainRefS(nullptr), fSplTrainRefB(nullptr), fSetupCompleted(kFALSE)
{
   SetTestvarName();
...
TMVA::MethodBase::MethodBase(Types::EMVA methodType, DataSetInfo &dsi, const TString &weightFile)
   : IMethod(), Configurable(""), fTmpEvent(nullptr), fRanking(nullptr), fInputVars(nullptr),
     fAnalysisType(Types::kNoAnalysisType), fRegressionReturnVal(nullptr), fMulticlassReturnVal(nullptr),
     fDataSetInfo(dsi), fSignalReferenceCut(0.5), fVariableTransformType(Types::kSignal), fJobName(""),
     fMethodName("MethodBase"), fMethodType(methodType), fTestvar(""), fTMVATrainingVersion(0), fROOTTrainingVersion(0),
     fConstructedFromWeightFile(kTRUE), fBaseDir(nullptr), fMethodBaseDir(nullptr), fFile(nullptr), fSilentFile(kFALSE),
     fModelPersistence(kTRUE), fWeightFile(weightFile), fEffS(nullptr), fDefaultPDF(nullptr), fMVAPdfS(nullptr),
     fMVAPdfB(nullptr), fSplS(nullptr), fSplB(nullptr), fSpleffBvsS(nullptr), fSplTrainS(nullptr), fSplTrainB(nullptr),
     fSplTrainEffBvsS(nullptr), fVarTransformString("None"), fTransformationPointer(nullptr), fTransformation(dsi, ""),
     fVerbose(kFALSE), fVerbosityLevelString("Default"), fHelp(kFALSE), fHasMVAPdfs(kFALSE),
     fIgnoreNegWeightsInTraining(kFALSE), fSignalClass(0), fBackgroundClass(0), fSplRefS(nullptr), fSplRefB(nullptr),
     fSplTrainRefS(nullptr), fSplTrainRefB(nullptr), fSetupCompleted(kFALSE)
{
   fLogger->SetSource(GetName());
//    // constructor used for Testing + Application of the MVA,
//    // only (no training), using given WeightFiles
...
TMVA::MethodBase::~MethodBase( void )
{
   ...
   if (fSplTrainEffBvsS) { delete fSplTrainEffBvsS;
