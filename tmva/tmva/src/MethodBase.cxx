...
TMVA::MethodBase::MethodBase(const TString &jobName, Types::EMVA methodType, const TString &methodTitle,
                             DataSetInfo &dsi, const TString &theOption)
   : IMethod(), Configurable(theOption), fTmpEvent(0), fRanking(0), fInputVars(0),
     fAnalysisType(Types::kNoAnalysisType), fRegressionReturnVal(0), fMulticlassReturnVal(0), fDataSetInfo(dsi),
     fSignalReferenceCut(0.5), fSignalReferenceCutOrientation(1.), fVariableTransformType(Types::kSignal),
     fJobName(jobName), fMethodName(methodTitle), fMethodType(methodType), fTestvar(""),
     fTMVATrainingVersion(TMVA_VERSION_CODE), fROOTTrainingVersion(ROOT_VERSION_CODE),
     fConstructedFromWeightFile(kFALSE), fBaseDir(0), fMethodBaseDir(0), fFile(0), fSilentFile(kFALSE),
     fModelPersistence(kTRUE), fWeightFile(""), fEffS(0), fDefaultPDF(0), fMVAPdfS(0), fMVAPdfB(0), fSplS(0), fSplB(0),
     fSpleffBvsS(0), fSplTrainS(0), fSplTrainB(0), fSplTrainEffBvsS(0), fVarTransformString("None"),
     fTransformationPointer(0), fTransformation(dsi, methodTitle), fVerbose(kFALSE), fVerbosityLevelString("Default"),
     fHelp(kFALSE), fHasMVAPdfs(kFALSE), fIgnoreNegWeightsInTraining(kFALSE), fSignalClass(0), fBackgroundClass(0),
     fSplRefS(0), fSplRefB(0), fSplTrainRefS(0), fSplTrainRefB(0), fSetupCompleted(kFALSE)
{
   SetTestvarName();
...
TMVA::MethodBase::MethodBase(Types::EMVA methodType, DataSetInfo &dsi, const TString &weightFile)
   : IMethod(), Configurable(""), fTmpEvent(0), fRanking(0), fInputVars(0), fAnalysisType(Types::kNoAnalysisType),
     fRegressionReturnVal(0), fMulticlassReturnVal(0), fDataSetInfo(dsi), fSignalReferenceCut(0.5),
     fVariableTransformType(Types::kSignal), fJobName(""), fMethodName("MethodBase"), fMethodType(methodType),
     fTestvar(""), fTMVATrainingVersion(0), fROOTTrainingVersion(0), fConstructedFromWeightFile(kTRUE), fBaseDir(0),
     fMethodBaseDir(0), fFile(0), fSilentFile(kFALSE), fModelPersistence(kTRUE), fWeightFile(weightFile), fEffS(0),
     fDefaultPDF(0), fMVAPdfS(0), fMVAPdfB(0), fSplS(0), fSplB(0), fSpleffBvsS(0), fSplTrainS(0), fSplTrainB(0),
     fSplTrainEffBvsS(0), fVarTransformString("None"), fTransformationPointer(0), fTransformation(dsi, ""),
     fVerbose(kFALSE), fVerbosityLevelString("Default"), fHelp(kFALSE), fHasMVAPdfs(kFALSE),
     fIgnoreNegWeightsInTraining(kFALSE), fSignalClass(0), fBackgroundClass(0), fSplRefS(0), fSplRefB(0),
     fSplTrainRefS(0), fSplTrainRefB(0), fSetupCompleted(kFALSE)
{
   fLogger->SetSource(GetName());
   //    // constructor used for Testing + Application of the MVA,
   //    // only (no training), using given WeightFiles
...
TMVA::MethodBase::~MethodBase(void)
{
   ...
   if (fRegressionReturnVal)
      delete fRegressionReturnVal;
   if (fMulticlassReturnVal)
...
void TMVA::MethodBase::SetupMethod()
{
   ...
   if (fSetupCompleted)
      Log() << kFATAL << Form("Dataset[%s] : ", DataInfo().GetName()) << "Calling SetupMethod for the second time"
            << Endl;
   InitBase();
   DeclareBaseOptions();
   Init();
...
void TMVA::MethodBase::DeclareBaseOptions()
{
   ...
   DeclareOptionRef(
      fVarTransformString, "VarTransform",
      "List of variable transformations performed before training, e.g., \"D_Background,P_Signal,G,N_AllClasses\" for: "
      "\"Decorrelation, PCA-transformation, Gaussianisation, Normalisation, each for the given class of events "
      "('AllClasses' denotes all events of all classes, if no class indication is given, 'All' is assumed)\"");
   ...
   DeclareOptionRef(fIgnoreNegWeightsInTraining, "IgnoreNegWeightsInTraining",
                    "Events with negative weights are ignored in the training (but are included for testing and "
...
void TMVA::MethodBase::ProcessBaseOptions()
{
   ...
   TMVA::CreateVariableTransforms(fVarTransformString, DataInfo(), GetTransformationHandler(), Log());
   ...
   } else if (fVerbosityLevelString == "Debug")
      Log().SetMinType(kDEBUG);
   else if (fVerbosityLevelString == "Verbose")
      Log().SetMinType(kVERBOSE);
   else if (fVerbosityLevelString == "Info")
      Log().SetMinType(kINFO);
   else if (fVerbosityLevelString == "Warning")
      Log().SetMinType(kWARNING);
   else if (fVerbosityLevelString == "Error")
      Log().SetMinType(kERROR);
   else if (fVerbosityLevelString == "Fatal")
      Log().SetMinType(kFATAL);
   else if (fVerbosityLevelString != "Default") {
      Log() << kFATAL << "<ProcessOptions> Verbosity level type '" << fVerbosityLevelString << "' unknown." << Endl;
   }
...
void TMVA::MethodBase::DeclareCompatibilityOptions()
{
   ...
   DeclareOptionRef(fVariableTransformTypeString = "Signal", "VarTransformType",
                    "Use signal or background events to derive for variable transformation (the transformation is "
                    "applied on both types of, course)");
   AddPreDefVal(TString("Signal"));
   AddPreDefVal(TString("Background"));
   DeclareOptionRef(
      fTxtWeightsOnly = kTRUE, "TxtWeightFilesOnly",
...
std::map<TString, Double_t> TMVA::MethodBase::OptimizeTuningParameters(TString /* fomType */, TString /* fitType */)
{
   ...
   Log() << kWARNING << Form("Dataset[%s] : ", DataInfo().GetName())
         << "Parameter optimization is not yet implemented for method " << GetName() << Endl;
   Log() << kWARNING << Form("Dataset[%s] : ", DataInfo().GetName())
...
void TMVA::MethodBase::TrainMethod()
{
   ...
   Log() << kINFO //<<Form("Dataset[%s] : ",DataInfo().GetName())
         << "Elapsed time for training with " << nEvents << " events: " << traintimer.GetElapsedTime() << "         "
         << Endl;
   ...
   if ((!DoRegression()) && (fModelPersistence))
      MakeClass();
...
void TMVA::MethodBase::GetRegressionDeviation(UInt_t tgtNum, Types::ETreeType type, Double_t &stddev,
                                              Double_t &stddev90Percent) const
{
   if (!DoRegression())
      Log() << kFATAL << Form("Dataset[%s] : ", DataInfo().GetName())
            << "Trying to use GetRegressionDeviation() with a classification job" << Endl;
   Log() << kINFO << Form("Dataset[%s] : ", DataInfo().GetName()) << "Create results for "
         << (type == Types::kTraining ? "training" : "testing") << Endl;
   ResultsRegression *regRes =
      (ResultsRegression *)Data()->GetResults(GetMethodName(), Types::kTesting, Types::kRegression);
   bool truncate = false;
...
void TMVA::MethodBase::AddRegressionOutput(Types::ETreeType type)
{
   ...
   if (nEvents >= totalProgressDraws)
      drawProgressEvery = nEvents / totalProgressDraws;
   ...
   for (Int_t ievt = 0; ievt < nEvents; ievt++) {
      ...
      // Only draw the progress bar once in a while, doing this every event causes the evaluation to be ridiculously
      // slow
      if (ievt % drawProgressEvery == 0 || ievt == nEvents - 1)
         timer.DrawProgressBar(ievt);
   }

   Log() << kINFO << Form("Dataset[%s] : ", DataInfo().GetName()) << "Elapsed time for evaluation of " << nEvents
         << " events: " << timer.GetElapsedTime() << "       " << Endl;
...
void TMVA::MethodBase::AddMulticlassOutput(Types::ETreeType type)
{
   ...
   Log() << kINFO << Form("Dataset[%s] : ", DataInfo().GetName()) << "Multiclass evaluation of " << GetMethodName()
         << " on " << (type == Types::kTraining ? "training" : "testing") << " sample" << Endl;
...
{
   return GetMvaValue() * GetSignalReferenceCutOrientation() >
                GetSignalReferenceCut() * GetSignalReferenceCutOrientation()
             ? kTRUE
             : kFALSE;
}
////////////////////////////////////////////////////////////////////////////////
...
{
   return mvaVal * GetSignalReferenceCutOrientation() > GetSignalReferenceCut() * GetSignalReferenceCutOrientation()
             ? kTRUE
             : kFALSE;
}
...
std::vector<Double_t> TMVA::MethodBase::GetMvaValues(Long64_t firstEvt, Long64_t lastEvt, Bool_t logProgress)
{
   ...
   Long64_t nEvents = Data()->GetNEvents();
   if (firstEvt > lastEvt || lastEvt > nEvents)
      lastEvt = nEvents;
   if (firstEvt < 0)
      firstEvt = 0;
   ...
   if (logProgress)
      Log() << kHEADER << Form("[%s] : ", DataInfo().GetName()) << "Evaluation of " << GetMethodName() << " on "
            << (Data()->GetCurrentType() == Types::kTraining ? "training" : "testing") << " sample (" << nEvents
            << " events)" << Endl;
   ...
   if (logProgress) {
      Log() << kINFO //<<Form("Dataset[%s] : ",DataInfo().GetName())
            << "Elapsed time for evaluation of " << nEvents << " events: " << timer.GetElapsedTime() << "       "
            << Endl;
   }
...
void TMVA::MethodBase::AddClassifierOutputProb(Types::ETreeType type)
{
   ...
   Log() << kDEBUG << Form("Dataset[%s] : ", DataInfo().GetName()) << "Elapsed time for evaluation of " << nEvents
         << " events: " << timer.GetElapsedTime() << "       " << Endl;
}
...
TMVA::RegressionKeyFigures TMVA::MethodBase::TestRegression(Types::ETreeType type)
{
   ...
   Log() << kINFO << "Elapsed time for evaluation of " << nevt << " events: " << timer.GetElapsedTime() << "       "
         << Endl;
...
{
   ResultsMulticlass *resMulticlass =
      dynamic_cast<ResultsMulticlass *>(Data()->GetResults(GetMethodName(), Types::kTesting, Types::kMulticlass));
   if (!resMulticlass)
      Log() << kFATAL << Form("Dataset[%s] : ", DataInfo().GetName())
...
void TMVA::MethodBase::TestClassification()
{
   ...
   if (0 == mvaRes && !(GetMethodTypeName().Contains("Cuts"))) {
      Log() << Form("Dataset[%s] : ", DataInfo().GetName()) << "mvaRes " << mvaRes << " GetMethodTypeName "
            << GetMethodTypeName() << " contains " << !(GetMethodTypeName().Contains("Cuts")) << Endl;
   ...
   gTools().ComputeStat(GetEventCollection(Types::kTesting), mvaRes->GetValueVector(), fMeanS, fMeanB, fRmsS, fRmsB,
                        fXmin, fXmax, fSignalClass);
   ...
   if (mvaProb)
      Log() << kINFO << "Also filling probability and rarity histograms (on request)..." << Endl;
   ...
   if (mvaRes->GetSize() != GetNEvents()) {
      Log() << kFATAL
            << TString::Format("Inconsistent result size  %lld with number of events %u ", mvaRes->GetSize(),
                               GetNEvents())
            << Endl;
...
void TMVA::MethodBase::WriteStateToStream(std::ostream &tf) const
{
   ...
   tf << prefix << "#GEN -*-*-*-*-*-*-*-*-*-*-*- general info -*-*-*-*-*-*-*-*-*-*-*-" << std::endl
      << prefix << std::endl;
   ...
   TString analysisType(((const_cast<TMVA::MethodBase *>(this)->GetAnalysisType() == Types::kRegression)
                            ? "Regression"
                            : "Classification"));

   tf << prefix << "Analysis type  : "
      << "[" << ((GetAnalysisType() == Types::kRegression) ? "Regression" : "Classification") << "]" << std::endl;
   tf << prefix << std::endl;
...
void TMVA::MethodBase::WriteStateToXML(void *parent) const
{
   ...
   AddInfoItem(gi, "TMVA Release",
               GetTrainingTMVAVersionString() + " [" + gTools().StringFromInt(GetTrainingTMVAVersionCode()) + "]");
   AddInfoItem(gi, "ROOT Release",
               GetTrainingROOTVersionString() + " [" + gTools().StringFromInt(GetTrainingROOTVersionCode()) + "]");
   AddInfoItem(gi, "Creator", userInfo->fUser);
   TDatime dt;
   AddInfoItem(gi, "Date", dt.AsString());
   ...
   TString analysisType(
      (aType == Types::kRegression) ? "Regression" : (aType == Types::kMulticlass ? "Multiclass" : "Classification"));
   ...
   if (fMVAPdfS)
      fMVAPdfS->AddXMLTo(pdfs);
   if (fMVAPdfB)
      fMVAPdfB->AddXMLTo(pdfs);
...
void TMVA::MethodBase::WriteStateToFile() const
{
   ...
   // writing xml file
   TString xmlfname(tfname);
   xmlfname.ReplaceAll(".txt", ".xml");
   Log() << kINFO //<<Form("Dataset[%s] : ",DataInfo().GetName())
         << "Creating xml weight file: " << gTools().Color("lightblue") << xmlfname << gTools().Color("reset") << Endl;
...
void TMVA::MethodBase::ReadStateFromFile()
{
   ...
   Log() << kINFO //<<Form("Dataset[%s] : ",DataInfo().GetName())
         << "Reading weight file: " << gTools().Color("lightblue") << tfname << gTools().Color("reset") << Endl;

   if (tfname.EndsWith(".xml")) {
#if ROOT_VERSION_CODE >= ROOT_VERSION(5, 29, 0)
      void *doc = gTools().xmlengine().ParseFile(
         tfname,
         gTools()
            .xmlenginebuffersize()); // the default buffer size in TXMLEngine::ParseFile is 100k. Starting with
                                     // ROOT 5.29 one can set the buffer size, see: http://savannah.cern.ch/bugs/?78864.
                                     // This might be necessary for large XML files
#else
   ...
   if (!fTxtWeightsOnly) {
      // ---- read the ROOT file
      TString rfname(tfname);
      rfname.ReplaceAll(".txt", ".root");
      Log() << kINFO << Form("Dataset[%s] : ", DataInfo().GetName())
            << "Reading root weight file: " << gTools().Color("lightblue") << rfname << gTools().Color("reset") << Endl;
...
#else
   Log() << kFATAL << Form("Dataset[%s] : ", DataInfo().GetName())
         << "Method MethodBase::ReadStateFromXMLString( const char* xmlstr = " << xmlstr
...
void TMVA::MethodBase::ReadStateFromXML(void *methodNode)
{
      ...
      if (nodeName == "GeneralInfo") {
         ...
         while (antypeNode) {
            ...
            if (name == "AnalysisType") {
               ...
               val.ToLower();
               if (val == "regression")
                  SetAnalysisType(Types::kRegression);
               else if (val == "classification")
                  SetAnalysisType(Types::kClassification);
               else if (val == "multiclass")
                  SetAnalysisType(Types::kMulticlass);
               else
                  Log() << kFATAL << Form("Dataset[%s] : ", DataInfo().GetName()) << "Analysis type " << val
                        << " is not known." << Endl;
            }
            ...
            if (name == "TMVA Release" || name == "TMVA") {
               ...
               Log() << kDEBUG << Form("[%s] : ", DataInfo().GetName())
         ...
         Log() << kWARNING << Form("Dataset[%s] : ", DataInfo().GetName()) << "Unparsed XML node: '" << nodeName << "'"
               << Endl;
      }
   ...
   if (GetTransformationHandler().GetCallerName() == "")
...
void TMVA::MethodBase::ReadStateFromStream(std::istream &fin)
{
   ...
   GetLine(fin, buf);
   while (!TString(buf).BeginsWith("Method"))
      GetLine(fin, buf);
   TString namestr(buf);
   ...
   Log() << kINFO << Form("Dataset[%s] : ", DataInfo().GetName()) << "Read method \"" << GetMethodName()
         << "\" of type \"" << GetMethodTypeName() << "\"" << Endl;
   ...
   GetLine(fin, buf);
   while (!TString(buf).BeginsWith("#OPT"))
      GetLine(fin, buf);
   ...
   if (IsNormalised()) {
      VariableNormalizeTransform *norm = (VariableNormalizeTransform *)GetTransformationHandler().AddTransformation(
   ...
   if (GetTransformationHandler().GetTransformationList().GetSize() > 0) {
      fin.getline(buf, 512);
      while (!TString(buf).BeginsWith("#MAT"))
         fin.getline(buf, 512);
      if (varTrafo) {
         TString trafo(fVariableTransformTypeString);
         trafo.ToLower();
   ...
   if (HasMVAPdfs()) {
      ...
      fin.getline(buf, 512);
      while (!TString(buf).BeginsWith("#MVAPDFS"))
         fin.getline(buf, 512);
      if (fMVAPdfS != 0) {
         delete fMVAPdfS;
         fMVAPdfS = 0;
   ...
   // Now read weights
   fin.getline(buf, 512);
   while (!TString(buf).BeginsWith("#WGT"))
      fin.getline(buf, 512);
   fin.getline(buf, 512);
...
void TMVA::MethodBase::ReadVarsFromStream(std::istream &istr)
{
   ...
   if (readNVar != DataInfo().GetNVariables()) {
      Log() << kFATAL << Form("Dataset[%s] : ", DataInfo().GetName()) << "You declared " << DataInfo().GetNVariables()
            << " variables in the Reader"
            << " while there are " << readNVar << " variables declared in the file" << Endl;
   }
   ...
   for (; varIt != DataInfo().GetVariableInfos().end(); ++varIt, ++varIdx) {
      ...
      if (varIt->GetExpression() == varInfo.GetExpression()) {
         ...
         Log() << kFATAL << "The expression declared to the Reader needs to be checked (name or order are wrong)"
               << Endl;
      }
   }
}
...
void TMVA::MethodBase::ReadSpectatorsFromXML(void *specnode)
{
   ...
   if (readNSpec != DataInfo().GetNSpectators(kFALSE)) {
      Log() << kFATAL << Form("Dataset[%s] : ", DataInfo().GetName()) << "You declared "
            << DataInfo().GetNSpectators(kFALSE) << " spectators in the Reader"
            << " while there are " << readNSpec << " spectators declared in the file" << Endl;
   }
...
TDirectory *TMVA::MethodBase::BaseDir() const
{
   if (fBaseDir != 0)
      return fBaseDir;
   Log() << kDEBUG << Form("Dataset[%s] : ", DataInfo().GetName()) << " Base Directory for " << GetMethodName()
   ...
   if (!sdir) {
      Log() << kDEBUG << Form("Dataset[%s] : ", DataInfo().GetName()) << " Base Directory for " << GetMethodTypeName()
...
TString TMVA::MethodBase::GetWeightFileName() const
{
   ...
   TString wFileName =
      GetJobName() + "_" + GetMethodName() + suffix + "." + gConfig().GetIONames().fWeightFileExtension + ".xml";
   if (wFileDir.IsNull())
      return wFileName;
   // add weight file directory of it is not null
   return (wFileDir + (wFileDir[wFileDir.Length() - 1] == '/' ? "" : "/") + wFileName);
}
////////////////////////////////////////////////////////////////////////////////
...
void TMVA::MethodBase::WriteEvaluationHistosToFile(Types::ETreeType treetype)
{
   ...
   if (!results)
      Log() << kFATAL << Form("Dataset[%s] : ", DataInfo().GetName())
            << "<WriteEvaluationHistosToFile> Unknown result: " << GetMethodName()
            << (treetype == Types::kTraining ? "/kTraining" : "/kTesting") << "/kMaxAnalysisType" << Endl;
...
Bool_t TMVA::MethodBase::GetLine(std::istream &fin, char *buf)
{
   ...
   if (line.BeginsWith("Analysis type")) {
      ...
      s >> analysisType;
      if (analysisType == "regression" || analysisType == "Regression")
         SetAnalysisType(Types::kRegression);
      else if (analysisType == "classification" || analysisType == "Classification")
         SetAnalysisType(Types::kClassification);
      else if (analysisType == "multiclass" || analysisType == "Multiclass")
         SetAnalysisType(Types::kMulticlass);
      else
         Log() << kFATAL << "Analysis type " << analysisType << " from weight-file not known!" << std::endl;

      Log() << kINFO << Form("Dataset[%s] : ", DataInfo().GetName()) << "Method was trained for "
            << (GetAnalysisType() == Types::kRegression
                   ? "Regression"
                   : (GetAnalysisType() == Types::kMulticlass ? "Multiclass" : "Classification"))
            << Endl;
   }
...
void TMVA::MethodBase::CreateMVAPdfs()
{
   ...
   if (mvaRes == 0 || mvaRes->GetSize() == 0) {
      Log() << kERROR << Form("Dataset[%s] : ", DataInfo().GetName())
...
Double_t TMVA::MethodBase::GetProba(const Event *ev)
{
   ...
   Double_t sigFraction = DataInfo().GetTrainingSumSignalWeights() /
                          (DataInfo().GetTrainingSumSignalWeights() + DataInfo().GetTrainingSumBackgrWeights());
...
Double_t TMVA::MethodBase::GetEfficiency(const TString &theString, Types::ETreeType type, Double_t &effSerr)
{
   ...
   if (!list || list->GetSize() < 2)
      computeArea = kTRUE; // the area is computed
   else if (list->GetSize() > 2) {
      Log() << kFATAL << Form("Dataset[%s] : ", DataInfo().GetName()) << "<GetEfficiency> Wrong number of arguments"
            << " in string: " << theString << " | required format, e.g., Efficiency:0.05, or empty string" << Endl;
      delete list;
      return -1;
   }
   ...
   if (results->DoesExist("MVA_EFF_S") == 0) {
      ...
      for (UInt_t ievt = 0; ievt < Data()->GetNEvents(); ievt++) {
         ...
         if (sign > 0 && maxbin > fNbinsH)
            continue; // can happen... event doesn't count
         if (sign < 0 && maxbin < 1)
            continue; // can happen... event doesn't count
         if (sign > 0 && maxbin < 1)
            maxbin = 1;
         ...
         if (sign > 0)
            for (Int_t ibin = 1; ibin <= maxbin; ibin++)
               theHist->AddBinContent(ibin, theWeight);
         else if (sign < 0)
            for (Int_t ibin = maxbin + 1; ibin <= fNbinsH; ibin++)
               theHist->AddBinContent(ibin, theWeight);
         else
            Log() << kFATAL << Form("Dataset[%s] : ", DataInfo().GetName()) << "<GetEfficiency> Mismatch in sign"
                  << Endl;
      }
      ...
      TH1 *inveff_BvsS = new TH1D(GetTestvarName() + "_invBeffvsSeff", GetTestvarName(), fNbins, 0, 1);
      ...
      for (Int_t bini = 1; bini <= fNbins; bini++) {
         ...
         if (Use_Splines_for_Eff_)
            ...
            effB = eff_b->GetBinContent(eff_b->FindBin(cut));
   ...
   if (computeArea) {
      ...
      for (Int_t bini = 1; bini <= nbins_; bini++) {
         ...
         // find signal efficiency that corresponds to required background efficiency
         if ((effB - effBref) * (effB_ - effBref) <= 0)
            break;
         effS_ = effS;
         effB_ = effB;
      }
      ...
      effSerr = 0;
      if (nevtS > 0)
         effSerr = TMath::Sqrt(effS * (1.0 - effS) / nevtS);
...
Double_t TMVA::MethodBase::GetTrainingEfficiency(const TString &theString)
{
   ...
   if (list->GetSize() != 2) {
      Log() << kFATAL << Form("Dataset[%s] : ", DataInfo().GetName())
            ...
            << " in string: " << theString << " | required format, e.g., Efficiency:0.05" << Endl;
      delete list;
      return -1;
   }
   // that will be the value of the efficiency retured (does not affect
   ...
   if (results->GetHist("MVA_S")->GetNbinsX() != results->GetHist("MVA_B")->GetNbinsX() ||
       results->GetHist("MVA_HIGHBIN_S")->GetNbinsX() != results->GetHist("MVA_HIGHBIN_B")->GetNbinsX()) {
      Log() << kFATAL << Form("Dataset[%s] : ", DataInfo().GetName())
            << "<GetTrainingEfficiency> Binning mismatch between signal and background histos" << Endl;
      return -1.0;
   }
   ...
   if (results->DoesExist("MVA_TRAIN_S") == 0) {
      ...
      TH1 *mva_s_tr =
         new TH1D(GetTestvarName() + "_Train_S", GetTestvarName() + "_Train_S", fNbinsMVAoutput, fXmin, sxmax);
      TH1 *mva_b_tr =
         new TH1D(GetTestvarName() + "_Train_B", GetTestvarName() + "_Train_B", fNbinsMVAoutput, fXmin, sxmax);
      ...
      TH1 *mva_eff_tr_s =
         new TH1D(GetTestvarName() + "_trainingEffS", GetTestvarName() + " (signal)", fNbinsH, xmin, xmax);
      TH1 *mva_eff_tr_b =
         new TH1D(GetTestvarName() + "_trainingEffB", GetTestvarName() + " (background)", fNbinsH, xmin, xmax);
      ...
      for (Int_t ievt = 0; ievt < Data()->GetNEvents(); ievt++) {
         ...
         if (sign > 0)
            for (Int_t ibin = 1; ibin <= maxbin; ibin++)
               theEffHist->AddBinContent(ibin, theWeight);
         else
            for (Int_t ibin = maxbin + 1; ibin <= fNbinsH; ibin++)
               theEffHist->AddBinContent(ibin, theWeight);
      }
      ...
      for (Int_t bini = 1; bini <= fNbins; bini++) {
         ...
         if (Use_Splines_for_Eff_)
            ...
            effB = mva_eff_tr_b->GetBinContent(mva_eff_tr_b->FindBin(cut));
...
std::vector<Float_t> TMVA::MethodBase::GetMulticlassTrainingEfficiency(std::vector<std::vector<Float_t>> &purity)
{
   Data()->SetCurrentType(Types::kTraining);
   ResultsMulticlass *resMulticlass =
      dynamic_cast<ResultsMulticlass *>(Data()->GetResults(GetMethodName(), Types::kTraining, Types::kMulticlass));
   if (!resMulticlass)
      Log() << kFATAL << "unable to create pointer in GetMulticlassTrainingEfficiency, exiting." << Endl;
...
Double_t TMVA::MethodBase::GetMaximumSignificance(Double_t SignalEvents, Double_t BackgroundEvents,
                                                  Double_t &max_significance_value) const
{
   ...
   if (SignalEvents <= 0 || BackgroundEvents <= 0) {
      Log() << kFATAL << Form("Dataset[%s] : ", DataInfo().GetName()) << "<GetMaximumSignificance> "
            << "Number of signal or background events is <= 0 ==> abort" << Endl;
   }

   Log() << kINFO << Form("Dataset[%s] : ", DataInfo().GetName())
         << "Using ratio SignalEvents/BackgroundEvents = " << SignalEvents / BackgroundEvents << Endl;
   ...
   Log() << kINFO << Form("Dataset[%s] : ", DataInfo().GetName()) << "Optimal cut at      : " << max_significance
         << Endl;
   Log() << kINFO << Form("Dataset[%s] : ", DataInfo().GetName()) << "Maximum significance: " << max_significance_value
         << Endl;
...
void TMVA::MethodBase::Statistics(Types::ETreeType treeType, const TString &theVarName, Double_t &meanS,
                                  Double_t &meanB, Double_t &rmsS, Double_t &rmsB, Double_t &xmin, Double_t &xmax)
{
...
void TMVA::MethodBase::MakeClass(const TString &theClassFileName) const
{
   ...
   Log() << kINFO //<<Form("Dataset[%s] : ",DataInfo().GetName())
         << "Creating standalone class: " << gTools().Color("lightblue") << classFileName << gTools().Color("reset")
         << Endl;
   ...
   if (GetAnalysisType() == Types::kMulticlass) {
      fout << "   virtual std::vector<double> GetMulticlassValues( const std::vector<double>& inputValues ) const = 0;"
           << std::endl;
   } else {
   ...
   fout << "         std::cout << \"Problem in class \\\"\" << fClassName << \"\\\": empty input vector\" << std::endl;"
        << std::endl;
   fout << "         fStatusIsClean = false;" << std::endl;
   fout << "      }" << std::endl;
   fout << std::endl;
   fout << "      if (theInputVars.size() != fNvars) {" << std::endl;
   fout << "         std::cout << \"Problem in class \\\"\" << fClassName << \"\\\": mismatch in number of input "
           "values: \""
        << std::endl;
   ...
   fout << "            std::cout << \"Problem in class \\\"\" << fClassName << \"\\\": mismatch in input variable "
           "names\" << std::endl"
        << std::endl;
   fout << "                      << \" for variable [\" << ivar << \"]: \" << theInputVars[ivar].c_str() << \" != \" "
   ...
   if (GetAnalysisType() == Types::kMulticlass) {
      fout << "   std::vector<double> GetMulticlassValues( const std::vector<double>& inputValues ) const override;"
           << std::endl;
   } else {
   ...
   if (GetAnalysisType() == Types::kMulticlass) {
      fout << "   std::vector<double> GetMulticlassValues__( const std::vector<double>& inputValues ) const;"
           << std::endl;
   } else {
   ...
   if (GetAnalysisType() == Types::kMulticlass) {
      fout << "inline std::vector<double> " << className
           << "::GetMulticlassValues( const std::vector<double>& inputValues ) const" << std::endl;
   } else {
      fout << "inline double " << className << "::GetMvaValue( const std::vector<double>& inputValues ) const"
           << std::endl;
   }
   fout << "{" << std::endl;
   ...
   fout << "      std::cout << \"Problem in class \\\"\" << fClassName << \"\\\": cannot return classifier response\""
        << std::endl;
...
void TMVA::MethodBase::PrintHelpMessage() const
{
   ...
   if (!o) {
      ...
      Log() << gTools().Color("bold") << "H e l p   f o r   M V A   m e t h o d   [ " << GetName()
            << " ] :" << gTools().Color("reset") << Endl;
   } else {
...
{
   UInt_t a = GetTrainingTMVAVersionCode() & 0xff0000;
   a >>= 16;
   UInt_t b = GetTrainingTMVAVersionCode() & 0x00ff00;
   b >>= 8;
...
{
   UInt_t a = GetTrainingROOTVersionCode() & 0xff0000;
   a >>= 16;
   UInt_t b = GetTrainingROOTVersionCode() & 0x00ff00;
   b >>= 8;
...
Double_t TMVA::MethodBase::GetKSTrainingVsTest(Char_t SorB, TString opt)
{
   ...
   if (mvaRes != NULL) {
      ...
      if (!mva_s || !mva_b || !mva_s_tr || !mva_b_tr)
         return -1;
