...
TMVA::RuleEnsemble::RuleEnsemble(RuleFit *rf)
   : fLearningModel(kFull), fImportanceCut(0),
     fLinQuantile(0.025) // default quantile for killing outliers in linear terms
     ,
     fOffset(0), fAverageSupport(0.8), fAverageRuleSigma(0.4) // default value - used if only linear model is chosen
     ,
     fRuleFSig(0), fRuleNCave(0), fRuleNCsig(0), fRuleMinDist(1e-3) // closest allowed 'distance' between two rules
     ,
     fNRulesGenerated(0), fEvent(nullptr), fEventCacheOK(true), fRuleMapOK(true), fRuleMapInd0(0), fRuleMapInd1(0),
     fRuleMapEvents(nullptr), fLogger(new MsgLogger("RuleFit"))
{
   Initialize( rf );
}
...
TMVA::RuleEnsemble::RuleEnsemble(const RuleEnsemble &other)
   : fAverageSupport(1), fEvent(nullptr), fRuleMapEvents(nullptr), fRuleFit(nullptr), fLogger(new MsgLogger("RuleFit"))
{
   Copy( other );
}
...
TMVA::RuleEnsemble::RuleEnsemble()
   : fLearningModel(kFull), fImportanceCut(0),
     fLinQuantile(0.025) // default quantile for killing outliers in linear terms
     ,
     fOffset(0), fImportanceRef(1.0), fAverageSupport(0.8),
     fAverageRuleSigma(0.4) // default value - used if only linear model is chosen
     ,
     fRuleFSig(0), fRuleNCave(0), fRuleNCsig(0), fRuleMinDist(1e-3) // closest allowed 'distance' between two rules
     ,
     fNRulesGenerated(0), fEvent(nullptr), fEventCacheOK(true), fRuleMapOK(true), fRuleMapInd0(0), fRuleMapInd1(0),
     fRuleMapEvents(nullptr), fRuleFit(nullptr), fLogger(new MsgLogger("RuleFit"))
{
}
