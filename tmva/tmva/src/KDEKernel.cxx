...
TMVA::KDEKernel::KDEKernel(EKernelIter kiter, const TH1 *hist, Float_t lower_edge, Float_t upper_edge,
   ...
   : fSigma(1.), fIter(kiter), fLowerEdge(lower_edge), fUpperEdge(upper_edge), fFineFactor(FineFactor),
     fKernel_integ(nullptr), fKDEborder(kborder), fLogger(new MsgLogger("KDEKernel"))
{
   if (hist == nullptr) {
