...
TMVA::DataSetFactory::DataSetFactory()
   : fVerbose(kFALSE), fVerboseLevel(TString("Info")), fScaleWithPreselEff(0), fCurrentTree(nullptr), fCurrentEvtIdx(0),
     fInputFormulas(0), fLogger(new MsgLogger("DataSetFactory", kINFO))
{
}
   ...
   for( UInt_t cls = 0; cls < dsi.GetNClasses(); ++cls ){
      ...
      if (splitMode.Contains( "RANDOM" )){
         ...
         if( sizeTraining > UInt_t(requestedTraining) ){
            ...
            eventVectorTraining.erase(
               std::remove(eventVectorTraining.begin(), eventVectorTraining.end(), (void *)nullptr),
               eventVectorTraining.end());
         }
         ...
         if( sizeTesting > UInt_t(requestedTesting) ){
            ...
            // now remove and erase all events with pointer==NULL
            eventVectorTesting.erase(std::remove(eventVectorTesting.begin(), eventVectorTesting.end(), (void *)nullptr),
                                     eventVectorTesting.end());
         }
      }
      else { // erase at end
