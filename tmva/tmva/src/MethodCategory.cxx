...
TMVA::MethodCategory::MethodCategory(const TString &jobName, const TString &methodTitle, DataSetInfo &theData,
                                     const TString &theOption)
   : TMVA::MethodCompositeBase(jobName, Types::kCategory, methodTitle, theData, theOption), fCatTree(nullptr),
...
TMVA::MethodCategory::MethodCategory(DataSetInfo &dsi, const TString &theWeightFile)
   : TMVA::MethodCompositeBase(Types::kCategory, dsi, theWeightFile), fCatTree(nullptr), fDataSetManager(nullptr)
{
}
...
void TMVA::MethodCategory::ReadWeightsFromXML( void* wghtnode )
{
   ...
   for (UInt_t i=0; i<nSubMethods; i++) {
      ...
      MethodBase* method = dynamic_cast<MethodBase*>( ClassifierFactory::Instance().Create( methodType.Data(),
                                                                                            dsi, "none" ) );
      if (method == nullptr) Log() << kFATAL << "Could not create sub-method " << method << " from XML." << Endl;
