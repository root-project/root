...
{
  fmGamma.clear();
  for( std::vector<float>::const_iterator iter = params.begin(); iter != params.end(); ++iter ){
  ...
  case kProd:
    {
      ...
      // kProd before returning so it can be used again. Described in "An Introduction to
      // Support Vector Machines and Other Kernel-based Learning
  ...
  case kSum:
    {
      ...
      // and evaluating the value for each, setting kernel back to
      // kSum before returning so it can be used again. Described in "An Introduction to
      // Support Vector Machines and Other Kernel-based Learning
      // Methods" by Cristianini and Shawe-Taylor, Section 3.3.2
      Float_t kernelVal = 0;
      for(UInt_t i = 0; i<fKernelsList.size(); i++){
