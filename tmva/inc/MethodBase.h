// @(#)root/tmva $Id: MethodBase.h,v 1.47 2006/10/04 22:29:27 andreas.hoecker Exp $   
...
namespace TMVA {
   ...
   public:
      ...
      TTree*   GetTrainingTree() const { 
         if (GetPreprocessingMethod()!=Types::kNone) {
            cout << "Trying to access correlated Training tree in method " << GetMethodName() << endl;
            exit(1);
         }
      ...
      TTree*   GetTestTree() const {
         if (GetPreprocessingMethod()!=Types::kNone) {
            cout << "Trying to access correlated Test tree in method " << GetMethodName() << endl;
            exit(1);
         }
   ...
   private:
      ...
      Bool_t    fVerbose;
      Bool_t    fHelp;
      Bool_t    LooseOptionCheckingEnabled() { return fLooseOptionCheckingEnabled; }
   ...
   protected:
      ...
      void ParseOptions( Bool_t silent = kFALSE);

      void PrintOptions(ostream&) const;
...
void TMVA::MethodBase::AssignOpt(const TString& name, T& valAssign) const 
{
   ...
   else std::cout << "Error: Option \"" << name 
