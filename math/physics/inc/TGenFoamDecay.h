...
/*!
  ...
  @brief  Adaptive MC generator of decay and integrator over LIPS. The generator decays a central blob of a given 4-momentum into particles of specific masses. 
...
This class is a multipurpose MC generator for decay, which also integrates functions over phase space. It uses FOAM adaptive integrator.

To use this class, make your own generator class that inherits from TGenFoamDecay and redefine Integrand() method, filling it with an integrand function that will be integrated over LIPS (Lorent Invariant Phase Space)  and possibly kinematics cuts (return 0.0). If you do not have any specific integrand, then the best choice is to make Integrand() return 1.0.
...
1. Create your generator class derived from TGenFoamDecay and implement Integrand() method - return 1.0 if you do not have any specific integrand.
...
If you prepared the integrand to be integrated over LIPS, you can print the integral final value by calling the Finalize() method or getting the numerical value by GetIntegMC().

### Troubleshooting and suggestions

- By default, the weight of the event is 1.0; however, see OptRej parameter. Sometimes FOAM, even for OptRej=1 weight are not precisely 1.0, so you can remove them or use weights, e.g., in histograms.
- If you do not have an integrand function (e.g., matrix element), then the optimal choice for Integrand() method is to return 1.0 (uniform distribution in the phase space).
- If the integrand has 'small' support in phase space and you get some nonsense result/errors, you probably should increase (say 10x or more) nSampl and nCells parameters of FOAM. It will increase the probability that adaptive MC will have a chance to spot support of the function. However, the exploration phase will last longer. If FOAM still cannot spot the support of the integral, then you can think of using a special Monte Carlo generator for non-spherical decay.
- FOAM has many various setting that can be tested when you deal with demanding integrand functions, see TFoam documentation or the Foam paper: 
 * S. Jadach, Foam: A General-Purpose Cellular Monte Carlo Event Generator, Comput.Phys.Commun. 152 (2003) 55-100
- Kinematic cuts can be effectively implemented in Integrand() method - just simply return 0.0 when kinematic cuts are met.
- Foam produces a lot of diagnostic output. It is good to read it at the beginning of implementation. However, in production code or when the generator is embedded in more extensive software, it is better to turn them off. To suppress them set Chat to 0.
