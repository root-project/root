...
bool equal(double d1, double d2, double stol = 10000) { 
   ...
   if ( std::abs(d1) > 0) 
      return  ( std::abs( d1-d2) < eps * std::max(d1,d2) ); 
   else 
...
bool stressSymPosInversion(int n, bool selftest ) { 
   ...
   iret |= ( equal(m1,m2) && equal(m1,m3) && equal(m1,m4) );
   if (!iret) 
...
bool stressInversion(int n = 1000000) { 
   bool ok = stressSymPosInversion(n, doSelfTest); 
   std::cout << "Test inversion of positive defined matrix ....... "; 
   if (ok) std::cout << "OK \n"; 
