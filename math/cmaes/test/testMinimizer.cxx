...
int testNewMinimizer( const ROOT::Math::IMultiGenFunction & func, const double * x0, const double * s0, const std::string & minimizer, const std::string & algoType) { 
   ...
   if (minimizer == "GSL_BFGS") { 
      std::cout << "DO cmaes from last point\n";
      gNCall = 0; 
      iret |= DoNewMinimization(func, &xmin.front(), s0, "cmaes","",minval,edm,&xmin[0]); 
      int pr = std::cout.precision(18);
...
int testRosenBrock() { 
   ...
   iret |= testNewMinimizer(fRB,xRB,s0,"cmaes","");
...
//    iret |= testOldMinimizer(RosenBrock,"cmaes",2);
...
//    std::cout << "FUNC " << func(x) << std::endl;
   ...
   iret |= testNewMinimizer(func,x0,s0, "cmaes","");
...
int testTrigoFletcher() { 
   ...
   iret |= testNewMinimizer(fTrigo,xTrigo,sTrigo,"cmaes","");
...
//   minimum    : F(1,1,1,1)  =   0.
   ...
   iret |= testNewMinimizer(f, x0, s0,"cmaes","");
