...
public:
   VectorTest(int n1, int n2) : nGen(n1), n2Loop(n2) {}
   ...
   int check(std::string name, double s1, double s2, double s3, double scale = 1)
   {
      ...
      std::cout << s1 << "\t" << s2 << "\t" << s3 << "\n";
      std::cout << "Rel Diff 1-2:\t" << (s1 - s2) / std::fabs(s1) << " Diff 1-3:\t" << (s1 - s3) / std::fabs(s1)
                << std::endl;
   ...
   inline double addXY(const SVector<double, 2> &v) { return v(0) + v(1); }
   template <class V>
   ...
   inline double dotProd(const XYVector &v1, const XYVector &v2) { return v1.Dot(v2); }

   inline double dotProd(const SVector<double, 2> &v1, const SVector<double, 2> &v2) { return Dot(v1, v2); }

#ifdef HAVE_CLHEP
   inline double addXY(const Hep2Vector &v) { return v.x() + v.y(); }
   inline double getSum(const Hep2Vector &v1, const Hep2Vector &v2) { return v1.x() + v1.y() + v2.x() + v2.y(); }
#endif
...
#ifdef HAVE_CLHEP
   inline double dPhi(Hep2Vector &v1, Hep2Vector &v2) { return std::abs(v1.phi() - v2.phi()); }
#endif
