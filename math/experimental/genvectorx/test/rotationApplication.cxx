...
struct correctedTicks {
   double operator()(double ticks, const V & /*v */, const XYZVector & /* ans */) { return ticks; }
};
...
struct correctedTicks<DisplacementVector3D<CylindricalEta3D<double>>> {
   double operator()(double ticks, const DisplacementVector3D<CylindricalEta3D<double>> &v, const XYZVector &ans)
   {
...
struct correctedTicks<PositionVector3D<CylindricalEta3D<double>>> {
   double operator()(double ticks, const PositionVector3D<CylindricalEta3D<double>> &v, const XYZVector &ans)
   {
      double t1 = correctTicks(ticks, v.z(), v.r());
      double t2 = correctTicks(ticks, ans.z(), ans.r());
      return t1 > t2 ? t1 : t2;
...
int testApplication(const R &r, const V &v, const XYZVector &answer, double t)
{
...
#ifdef DEBUG
   std::cout << ">>>>> Testing application of " << RotationTraits<R>::name() << " " << r << "\non "
             << CoordinateTraits<typename V::CoordinateType>::name() << v << " ticks = " << ticks;
#endif
#ifdef TRACE2
   ...
   if (compare3D(rv, r * v, 10) != 0) {
      std::cout << "Inconsistency between R(v) and R*v for R = " << RotationTraits<R>::name() << " " << r
   ...
   if (closeEnough(v.r(), rv.r(), "r", ticks) != 0) {
      std::cout << "Radius change  between R(v) and R*v for R = " << RotationTraits<R>::name() << " " << r
                << "\nacting on " << CoordinateTraits<CoordType>::name() << v << "\n";
      ret |= 17;
   }
#ifdef TRACE2
...
struct TestRotation {
   ...
   TestRotation(std::vector<XYZ> const &xyz_, std::vector<double> const &phi_) : xyz(xyz_), phi(phi_) {}
};
...
Rotation3D rrr(TestRotation const &t)
{
   ...
   for (unsigned int i = 0; i < t.xyz.size(); ++i) {
      switch (t.xyz[i]) {
      case X: r = r * RotationX(t.phi[i]); break;
      case Y: r = r * RotationY(t.phi[i]); break;
      case Z: r = r * RotationZ(t.phi[i]); break;
      }
   }
#ifdef TRACE1
   std::cout << " done\n";
...
XYZVector ans(TestRotation const &t, XYZVector const &v_in)
{
   ...
   for (int i = t.xyz.size() - 1; i >= 0; --i) {
      switch (t.xyz[i]) {
      case X: v = rxv(t.phi[i], v); break;
      case Y: v = ryv(t.phi[i], v); break;
      case Z: v = rzv(t.phi[i], v); break;
      }
   }
   return v;
}
...
#endif
   ret |= doTest<R, DisplacementVector3D<Cartesian3D<double>>>(testRotation, testVector, ticks);
#ifdef DEBUG
   std::cout << ">>>>> DisplacementVector3D< Polar3D<double> \n";
#endif
   ret |= doTest<R, DisplacementVector3D<Polar3D<double>>>(testRotation, testVector, ticks);
#ifdef DEBUG
   std::cout << ">>>>> DisplacementVector3D< CylindricalEta3D<double> \n";
#endif
   ret |= doTest<R, DisplacementVector3D<CylindricalEta3D<double>>>(testRotation, testVector, ticks);
#ifdef DEBUG
   std::cout << ">>>>> PositionVector3D< Cartesian3D<double> \n";
#endif
   ret |= doTest<R, PositionVector3D<Cartesian3D<double>>>(testRotation, testVector, ticks);
#ifdef DEBUG
   std::cout << ">>>>> PositionVector3D< Polar3D<double> \n";
#endif
   ret |= doTest<R, PositionVector3D<Polar3D<double>>>(testRotation, testVector, ticks);
#ifdef DEBUG
   std::cout << ">>>>> PositionVector3D< CylindricalEta3D<double> \n";
#endif
   ret |= doTest<R, PositionVector3D<CylindricalEta3D<double>>>(testRotation, testVector, ticks);
#ifdef DEBUG
   std::cout << ">>>>> ForeignVector\n";
#endif
   ret |= doTest<R, ForeignVector>(testRotation, testVector, ticks);
#ifdef DEBUG
   std::cout << ">>>>> LorentzVector<PxPyPzE4D<double> >\n";
#endif
   ret |= doTestL<R, PxPyPzE4D<double>>(testRotation, testVector, ticks);
#ifdef TRACE1
...
int doTestOfAxial(XYZVector const &testVector)
{
   ...
   std::cout << ">>>>> DisplacementVector3D< Cartesian3D<double> \n";
#endif
   ret |= doTestA<R, DisplacementVector3D<Cartesian3D<double>>>(testVector, ticks);
#ifdef DEBUG
   std::cout << ">>>>> DisplacementVector3D< Polar3D<double> \n";
#endif
   ret |= doTestA<R, DisplacementVector3D<Polar3D<double>>>(testVector, ticks);
#ifdef DEBUG
   std::cout << ">>>>> DisplacementVector3D< CylindricalEta3D<double> \n";
#endif
   ret |= doTestA<R, DisplacementVector3D<CylindricalEta3D<double>>>(testVector, ticks);
#ifdef DEBUG
   std::cout << ">>>>> PositionVector3D< Cartesian3D<double> \n";
#endif
   ret |= doTestA<R, PositionVector3D<Cartesian3D<double>>>(testVector, ticks);
#ifdef DEBUG
   std::cout << ">>>>> PositionVector3D< Polar3D<double> \n";
#endif
   ...
   std::cout << ">>>>> PositionVector3D< CylindricalEta3D<double> \n";
#endif
   ret |= doTestA<R, PositionVector3D<CylindricalEta3D<double>>>(testVector, ticks);
#ifdef DEBUG
   std::cout << ">>>>> ForeignVector\n";
#endif
...
int rotationApplication(bool forceRun = false)
{
   ...
   for (std::vector<TestRotation>::const_iterator n = testRotations.begin(); n != testRotations.end(); ++n) {
      for (std::vector<XYZVector>::const_iterator m = testVectors.begin(); m != testVectors.end(); ++m) {
         ret |= exerciseTestCase(*n, *m);
      }
   }
   for (std::vector<XYZVector>::const_iterator vp = testVectors.begin(); vp != testVectors.end(); ++vp) {
