...
public:
   ...
   */
   ...
   /**
      copy constructor
   */
   Rotation3D(Rotation3D const &r) { *this = r; }
   ...
   */
   Rotation3D &operator=(AxisAngle const &a) { return operator=(Rotation3D(a)); }

   /**
      Assign from EulerAngles
   */
   Rotation3D &operator=(EulerAngles const &e) { return operator=(Rotation3D(e)); }
   ...
   */
   Rotation3D &operator=(Quaternion const &q) { return operator=(Rotation3D(q)); }

   /**
      Assign from an axial rotation
   */
   Rotation3D &operator=(RotationZ const &r) { return operator=(Rotation3D(r)); }
   Rotation3D &operator=(RotationY const &r) { return operator=(Rotation3D(r)); }
   Rotation3D &operator=(RotationX const &r) { return operator=(Rotation3D(r)); }
   ...
   */
   ...
   {
      SetComponents(m(0, 0), m(0, 1), m(0, 2), m(1, 0), m(1, 1), m(1, 2), m(2, 0), m(2, 1), m(2, 2));
      return *this;
   }
   ...
   */
   void GetComponents(Scalar &xx, Scalar &xy, Scalar &xz, Scalar &yx, Scalar &yy, Scalar &yz, Scalar &zx, Scalar &zy,
                      Scalar &zz) const
   {
      xx = fM[kXX];
      xy = fM[kXY];
   ...
   */
   ...
   {
      return Rotation3D(fM[kXX] * r.fM[kXX] + fM[kXY] * r.fM[kYX] + fM[kXZ] * r.fM[kZX],
   ...
   */
   ...
   bool operator!=(const Rotation3D &rhs) const { return !operator==(rhs); }
