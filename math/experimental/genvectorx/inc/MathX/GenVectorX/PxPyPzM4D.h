...
public:
   ...
   */
   PxPyPzM4D(Scalar px, Scalar py, Scalar pz, Scalar m) : fX(px), fY(py), fZ(pz), fM(m)
   {
   ...
   */
   ...
   explicit PxPyPzM4D(const CoordSystem &v) : fX(v.X()), fY(v.Y()), fZ(v.Z()), fM(v.M())
   {
   }
   ...
   /**
      copy constructor
    */
   PxPyPzM4D(const PxPyPzM4D &v) : fX(v.fX), fY(v.fY), fZ(v.fZ), fM(v.fM) {}
   ...
   */
   ...
   /**
      Exact equality
   */
   bool operator==(const PxPyPzM4D &rhs) const { return fX == rhs.fX && fY == rhs.fY && fZ == rhs.fZ && fM == rhs.fM; }
