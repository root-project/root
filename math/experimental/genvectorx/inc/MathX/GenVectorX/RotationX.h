...
public:
   ...
   */
   explicit RotationX(Scalar angle) : fAngle(angle), fSin(math_sin(angle)), fCos(math_cos(angle)) { Rectify(); }
   ...
   */
   ...
   bool operator!=(const RotationX &rhs) const { return !operator==(rhs); }
 ...
 */
template <class R>
inline typename RotationX::Scalar Distance(const RotationX &r1, const R &r2)
{
...
inline std::ostream &operator<<(std::ostream &os, const RotationX &r)
{
