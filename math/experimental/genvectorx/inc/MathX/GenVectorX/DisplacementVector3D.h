...
public:
   ...
   */
   DisplacementVector3D(Scalar a, Scalar b, Scalar c) : fCoordinates(a, b, c) {}
   ...
   explicit DisplacementVector3D(const DisplacementVector3D<OtherCoords, Tag> &v) : fCoordinates(v.Coordinates())
   {
   }
   ...
   */
   template <class OtherCoords>
   explicit DisplacementVector3D(const PositionVector3D<OtherCoords, Tag> &p) : fCoordinates(p.Coordinates())
   {
   }
   ...
   */
   ...
   explicit DisplacementVector3D(const ForeignVector &v) : fCoordinates(Cartesian3D<Scalar>(v.x(), v.y(), v.z()))
   {
   }
   ...
   */
   ...
   /**
     Exact equality
    */
   bool operator==(const DisplacementVector3D &rhs) const { return fCoordinates == rhs.fCoordinates; }
   bool operator!=(const DisplacementVector3D &rhs) const { return !(operator==(rhs)); }
   ...
   */
    ...
    */
   DisplacementVector3D operator-() const { return operator*(Scalar(-1)); }
...
inline DisplacementVector3D<CoordSystem, U>
...
template <
   ...
   typename std::enable_if<std::is_arithmetic<typename DisplacementVector3D<T, U>::Scalar>::value>::type * = nullptr>
