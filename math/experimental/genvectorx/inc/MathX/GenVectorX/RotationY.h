...
public:
   ...
   */
   explicit RotationY(Scalar angle) : fAngle(angle), fSin(math_sin(angle)), fCos(math_cos(angle)) { Rectify(); }
   ...
   */
   ...
   bool operator!=(const RotationY &rhs) const { return !operator==(rhs); }
 ...
 */
template <class R>
inline typename RotationY::Scalar Distance(const RotationY &r1, const R &r2)
{
...
inline std::ostream &operator<<(std::ostream &os, const RotationY &r)
{
