...
Quaternion Quaternion::operator*(const RotationX &rx) const
{
   ...
   Quaternion q(rx);
   return Quaternion(U() * q.U() - I() * q.I(), I() * q.U() + U() * q.I(), J() * q.U() + K() * q.I(),
                     K() * q.U() - J() * q.I());
}
...
Quaternion Quaternion::operator*(const RotationY &ry) const
{
   ...
   Quaternion q(ry);
   return Quaternion(U() * q.U() - J() * q.J(), I() * q.U() - K() * q.J(), J() * q.U() + U() * q.J(),
                     K() * q.U() + I() * q.J());
}
...
Quaternion Quaternion::operator*(const RotationZ &rz) const
{
   ...
   Quaternion q(rz);
   return Quaternion(U() * q.U() - K() * q.K(), I() * q.U() + J() * q.K(), J() * q.U() - I() * q.K(),
                     K() * q.U() + U() * q.K());
}

Quaternion operator*(RotationX const &r, Quaternion const &q)
{
   return Quaternion(r) * q; // TODO: improve performance
}

Quaternion operator*(RotationY const &r, Quaternion const &q)
{
   return Quaternion(r) * q; // TODO: improve performance
}

Quaternion operator*(RotationZ const &r, Quaternion const &q)
{
   return Quaternion(r) * q; // TODO: improve performance
}
