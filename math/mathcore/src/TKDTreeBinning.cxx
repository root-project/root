...
TKDTreeBinning::TKDTreeBinning(UInt_t dataSize, UInt_t dataDim, Double_t *data, UInt_t nBins, bool adjustBinEdges)
   : fData(0), fBinMinEdges(std::vector<Double_t>()), fBinMaxEdges(std::vector<Double_t>()),
     fDataBins((TKDTreeID *)nullptr), fDim(dataDim), fDataSize(dataSize),
     fDataThresholds(std::vector<std::pair<Double_t, Double_t>>(fDim, std::make_pair(0., 0.))), fIsSorted(kFALSE),
     fIsSortedAsc(kFALSE), fBinsContent(std::vector<UInt_t>())
{
...
///
TKDTreeBinning::TKDTreeBinning(UInt_t dataSize, UInt_t dataDim, const std::vector<double> &data, UInt_t nBins,
                               bool adjustBinEdges)
   : fData(0), fBinMinEdges(std::vector<Double_t>()), fBinMaxEdges(std::vector<Double_t>()),
     fDataBins((TKDTreeID *)nullptr), fNBins(nBins), fDim(dataDim), fDataSize(dataSize),
     fDataThresholds(std::vector<std::pair<Double_t, Double_t>>(fDim, std::make_pair(0., 0.))), fIsSorted(kFALSE),
     fIsSortedAsc(kFALSE), fBinsContent(std::vector<UInt_t>())
{
   if (adjustBinEdges) SetBit(kAdjustBinEdges);
