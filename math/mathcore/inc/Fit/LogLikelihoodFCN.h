...
public:
   ...
   */
   LogLikelihoodFCN (const std::shared_ptr<UnBinData> & data, const std::shared_ptr<IModelFunction> & func, int weight = 0, bool extended = false, unsigned executionPolicy = ROOT::Fit::kSerial) :
      BaseFCN( data, func),
   ...
   */
   LogLikelihoodFCN (const UnBinData & data, const IModelFunction & func, int weight = 0, bool extended = false, unsigned executionPolicy = ROOT::Fit::kSerial) :
