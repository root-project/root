...
TBranch::TBranch()
   : TNamed(), TAttFill(0, 1001), fCompress(0), fBasketSize(32000), fEntryOffsetLen(1000), fWriteBasket(0),
     fEntryNumber(0), fOffset(0), fMaxBaskets(10), fNBaskets(0), fSplitLevel(0), fNleaves(0), fReadBasket(0),
     fReadEntry(-1), fFirstBasketEntry(-1), fNextBasketEntry(-1), fCurrentBasket(nullptr), fEntries(0), fFirstEntry(0),
     fTotBytes(0), fZipBytes(0), fBranches(), fLeaves(), fBaskets(fMaxBaskets), fBasketBytes(nullptr),
     fBasketEntry(nullptr), fBasketSeek(nullptr), fTree(nullptr), fMother(nullptr), fParent(nullptr), fAddress(nullptr),
     fDirectory(nullptr), fFileName(""), fEntryBuffer(nullptr), fTransientBuffer(nullptr), fBrowsables(nullptr),
     fSkipZip(kFALSE), fReadLeaves(&TBranch::ReadLeavesImpl), fFillLeaves(&TBranch::FillLeavesImpl)
{
...
TBranch::TBranch(TTree *tree, const char *name, void *address, const char *leaflist, Int_t basketsize, Int_t compress)
   : TNamed(name, leaflist), TAttFill(0, 1001), fCompress(compress), fBasketSize((basketsize < 100) ? 100 : basketsize),
     fEntryOffsetLen(0), fWriteBasket(0), fEntryNumber(0), fOffset(0), fMaxBaskets(10), fNBaskets(0), fSplitLevel(0),
     fNleaves(0), fReadBasket(0), fReadEntry(-1), fFirstBasketEntry(-1), fNextBasketEntry(-1), fCurrentBasket(nullptr),
     fEntries(0), fFirstEntry(0), fTotBytes(0), fZipBytes(0), fBranches(), fLeaves(), fBaskets(fMaxBaskets),
     fBasketBytes(nullptr), fBasketEntry(nullptr), fBasketSeek(nullptr), fTree(tree), fMother(nullptr),
     fParent(nullptr), fAddress((char *)address), fDirectory(fTree->GetDirectory()), fFileName(""),
     fEntryBuffer(nullptr), fTransientBuffer(nullptr), fBrowsables(nullptr), fSkipZip(kFALSE),
     fReadLeaves(&TBranch::ReadLeavesImpl), fFillLeaves(&TBranch::FillLeavesImpl)
{
...
TBranch::TBranch(TBranch *parent, const char *name, void *address, const char *leaflist, Int_t basketsize,
   ...
   : TNamed(name, leaflist), TAttFill(0, 1001), fCompress(compress), fBasketSize((basketsize < 100) ? 100 : basketsize),
     ...
     fNleaves(0), fReadBasket(0), fReadEntry(-1), fFirstBasketEntry(-1), fNextBasketEntry(-1), fCurrentBasket(nullptr),
     fEntries(0), fFirstEntry(0), fTotBytes(0), fZipBytes(0), fBranches(), fLeaves(), fBaskets(fMaxBaskets),
     fBasketBytes(nullptr), fBasketEntry(nullptr), fBasketSeek(nullptr), fTree(parent ? parent->GetTree() : nullptr),
     fMother(parent ? parent->GetMother() : nullptr), fParent(parent), fAddress((char *)address),
     fDirectory(fTree ? fTree->GetDirectory() : nullptr), fFileName(""), fEntryBuffer(nullptr),
     fTransientBuffer(nullptr), fBrowsables(nullptr), fSkipZip(kFALSE), fReadLeaves(&TBranch::ReadLeavesImpl),
