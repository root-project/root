...
TBranchElement::TBranchElement()
   : TBranch(), fClassName(), fParentName(), fClonesName(), fCollProxy(nullptr), fCheckSum(0), fClassVersion(0), fID(0),
     fType(0), fStreamerType(-1), fMaximum(0), fSTLtype(ROOT::kNotSTL), fNdata(1), fBranchCount(nullptr),
     fBranchCount2(nullptr), fInfo(nullptr), fObject(nullptr), fOnfileObject(nullptr), fInit(kFALSE),
     fInitOffsets(kFALSE), fTargetClass(), fCurrentClass(), fParentClass(), fBranchClass(), fClonesClass(),
     fBranchOffset(nullptr), fBranchID(-1), fReadActionSequence(nullptr), fFillActionSequence(nullptr),
     fIterators(nullptr), fWriteIterators(nullptr), fPtrIterators(nullptr)
{
   fNleaves = 0;
...
TBranchElement::TBranchElement(TTree *tree, const char *bname, TStreamerInfo *sinfo, Int_t id, char *pointer,
                               Int_t basketsize, Int_t splitlevel, Int_t btype)
   : TBranch(), fClassName(sinfo->GetName()), fParentName(), fClonesName(), fCollProxy(nullptr),
     fCheckSum(sinfo->GetCheckSum()), fClassVersion(sinfo->GetClass()->GetClassVersion()), fID(id), fType(0),
     fStreamerType(-1), fMaximum(0), fSTLtype(ROOT::kNotSTL), fNdata(1), fBranchCount(nullptr), fBranchCount2(nullptr),
     fInfo(sinfo), fObject(nullptr), fOnfileObject(nullptr), fInit(kTRUE), fInitOffsets(kFALSE),
     fTargetClass(fClassName), fCurrentClass(), fParentClass(), fBranchClass(sinfo->GetClass()), fClonesClass(),
     fBranchOffset(nullptr), fBranchID(-1), fReadActionSequence(nullptr), fFillActionSequence(nullptr),
     fIterators(nullptr), fWriteIterators(nullptr), fPtrIterators(nullptr)
{
...
TBranchElement::TBranchElement(TBranch *parent, const char *bname, TStreamerInfo *sinfo, Int_t id, char *pointer,
                               Int_t basketsize, Int_t splitlevel, Int_t btype)
   : TBranch(), fClassName(sinfo->GetName()), fParentName(), fClonesName(), fCollProxy(nullptr),
     fCheckSum(sinfo->GetCheckSum()), fClassVersion(sinfo->GetClass()->GetClassVersion()), fID(id), fType(0),
     fStreamerType(-1), fMaximum(0), fSTLtype(ROOT::kNotSTL), fNdata(1), fBranchCount(nullptr), fBranchCount2(nullptr),
   ...
   if (id < 0) {
      ...
      if (fBranchClass.GetClass()) {
         ...
         if (fBranchClass.GetClass()->IsTObject()) {
            ...
            hasCustomStreamer = !fBranchClass.GetClass()->GetCollectionProxy() &&
                                (fBranchClass.GetClass()->GetStreamer() != nullptr ||
                                 fBranchClass.GetClass()->TestBit(TClass::kHasCustomStreamerMember));
         }
...
TBranchElement::TBranchElement(TTree *tree, const char *bname, TClonesArray *clones, Int_t basketsize, Int_t splitlevel,
                               Int_t compress)
   : TBranch(), fClassName("TClonesArray"), fParentName(),
     fInfo((TStreamerInfo *)TClonesArray::Class()->GetStreamerInfo()), fTargetClass(fClassName), fCurrentClass(),
     fParentClass(), fBranchClass(TClonesArray::Class()), fBranchID(-1), fReadActionSequence(nullptr),
     fFillActionSequence(nullptr), fIterators(nullptr), fWriteIterators(nullptr), fPtrIterators(nullptr)
{
...
TBranchElement::TBranchElement(TBranch *parent, const char *bname, TClonesArray *clones, Int_t basketsize,
                               Int_t splitlevel, Int_t compress)
   : TBranch(), fClassName("TClonesArray"), fParentName(),
     fInfo((TStreamerInfo *)TClonesArray::Class()->GetStreamerInfo()), fTargetClass(fClassName), fCurrentClass(),
     fParentClass(), fBranchClass(TClonesArray::Class()), fBranchID(-1), fReadActionSequence(nullptr),
     fFillActionSequence(nullptr), fIterators(nullptr), fWriteIterators(nullptr), fPtrIterators(nullptr)
...
TBranchElement::TBranchElement(TTree *tree, const char *bname, TVirtualCollectionProxy *cont, Int_t basketsize,
                               Int_t splitlevel, Int_t compress)
   : TBranch(), fClassName(cont->GetCollectionClass()->GetName()), fParentName(), fTargetClass(fClassName),
     fCurrentClass(), fParentClass(), fBranchClass(cont->GetCollectionClass()), fBranchID(-1),
     fReadActionSequence(nullptr), fFillActionSequence(nullptr), fIterators(nullptr), fWriteIterators(nullptr),
...
TBranchElement::TBranchElement(TBranch *parent, const char *bname, TVirtualCollectionProxy *cont, Int_t basketsize,
                               Int_t splitlevel, Int_t compress)
   : TBranch(), fClassName(cont->GetCollectionClass()->GetName()), fParentName(), fTargetClass(fClassName),
     fCurrentClass(), fParentClass(), fBranchClass(cont->GetCollectionClass()), fBranchID(-1),
     fReadActionSequence(nullptr), fFillActionSequence(nullptr), fIterators(nullptr), fWriteIterators(nullptr),
...
TLeaf* TBranchElement::FindLeaf(const char *name)
{
   ...
   if (leaf == nullptr && GetListOfLeaves()->GetEntries() == 1) {
      ...
      if (br->IsA() != TBranchElement::Class()) return nullptr;
   ...
   if (fInfo) {
      ...
      if (!fInit) {
         ...
         if (GetID() > -1) {
            ...
            if (fOnfileObject == nullptr && (fType == 31 || fType == 41 || (0 <= fType && fType <= 2)) &&
                fInfo->GetNelement() && fInfo->GetElement(0)->GetType() == TStreamerInfo::kCacheNew) {
               Int_t arrlen = 1;
               if (fType==31 || fType==41) {
...
void TBranchElement::ReadLeavesCollection(TBuffer& b)
{
   ...
   if( proxy->HasPointers() && fSplitLevel > TTree::kSplitCollectionOfPointers )
   {
      ...
      if (!fNdata || *(void **)proxy->At(0) != nullptr) i = fNdata;
...
void TBranchElement::SetAddress(void* addr)
{
   ...
   if (fType == 3) {
      ...
      if (newType && (newType != GetCollectionProxy()->GetCollectionClass())) {
         ...
         } else if (newProxy && (oldProxy->HasPointers() == newProxy->HasPointers()) &&
                    (oldProxy->GetValueClass() != nullptr) && (newProxy->GetValueClass() != nullptr)) {
...
void TBranchElement::SetReadActionSequence()
{
   ...
   // Get the action sequence we need to copy for reading.
   TStreamerInfoActions::TActionSequence *original = nullptr;
   TStreamerInfoActions::TActionSequence *transient = nullptr;
   if (fType == 41) {
...
void TBranchElement::SetReadLeavesPtr()
{
   if (TestBit(kDecomposedObj)) {
      ...
      // top-level branch.
      Bool_t hasCustomStreamer = !fBranchClass.GetClass()->GetCollectionProxy() &&
                                 (fBranchClass.GetClass()->GetStreamer() != nullptr ||
                                  fBranchClass.GetClass()->TestBit(TClass::kHasCustomStreamerMember));
      if (hasCustomStreamer) {
...
void TBranchElement::SetupAddressesImpl()
{
   ...
   if( fType == 41 && fSplitLevel >= TTree::kSplitCollectionOfPointers )
   {
      ...
      if (!parent->GetAddress()) parent->SetAddress(nullptr);
      return;
   }
   ...
   for (Int_t elemID = 0; elemID < ndata; ++elemID) {
      ...
      if (elem->IsA() == TStreamerBase::Class()) {
         ...
         if ((btype == 31) || (btype == 41)) {
            ...
            if (unroll < 0) {
               ...
               TBranchElement *branch =
                  new TBranchElement(this, branchname, sinfo, elemID, nullptr, basketsize, 0, btype);
                 ...
                 splitSTLP && fType != 4 )
         {
            ...
            TBranchElement *branch =
               new TBranchElement(this, branchname, sinfo, elemID, nullptr, basketsize, splitSTLP, btype);
