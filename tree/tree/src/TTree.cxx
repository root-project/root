...
- `leaflist` is the concatenation of all the variable names and types
  ...
  types is given below:
   ...
   - `f` : a 24 bit floating point with truncated mantissa (`Float16_t`, stored as 3 bytes, occupies 4 bytes in memory): By default, in disk, only 21 bits are used: 1 for the sign, 8 for the exponent and 12 for the mantissa. Can be customized with suffix `[min,max(,nbits)] `where `nbits` is for the mantissa.
   - `D` : a 64 bit floating point (`Double_t`)
   - `d` : a 24 bit floating point with truncated mantissa (`Double32_t`, stored as 3 bytes, occupies 8 bytes in memory): By default, in disk, 1 bit is used for the sign, 8 for the exponent and 15 for the mantissa. Can be customized with suffix `[min,max(,nbits)]` where `nbits` is for the mantissa.
