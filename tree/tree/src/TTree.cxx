...
TTree::TTree()
   : TNamed(), TAttLine(), TAttFill(), TAttMarker(), fEntries(0), fTotBytes(0), fZipBytes(0), fSavedBytes(0),
     fFlushedBytes(0), fWeight(1), fTimerInterval(0), fScanField(25), fUpdate(0), fDefaultEntryOffsetLen(1000),
     fNClusterRange(0), fMaxClusterRange(0), fMaxEntries(0), fMaxEntryLoop(0), fMaxVirtualSize(0),
     fAutoSave(-300000000), fAutoFlush(-30000000), fEstimate(1000000), fClusterRangeEnd(nullptr), fClusterSize(nullptr),
     fCacheSize(0), fChainOffset(0), fReadEntry(-1), fTotalBuffers(0), fPacketSize(100), fNfill(0), fDebug(0),
     fDebugMin(0), fDebugMax(9999999), fMakeClass(0), fFileNumber(0), fNotify(nullptr), fDirectory(nullptr),
     fBranches(), fLeaves(), fAliases(nullptr), fEventList(nullptr), fEntryList(nullptr), fIndexValues(), fIndex(),
     fTreeIndex(nullptr), fFriends(nullptr), fPerfStats(nullptr), fUserInfo(nullptr), fPlayer(nullptr),
     fClones(nullptr), fBranchRef(nullptr), fFriendLockStatus(0), fTransientBuffer(nullptr), fCacheDoAutoInit(kTRUE),
     fCacheUserSet(kFALSE), fIMTEnabled(ROOT::IsImplicitMTEnabled()), fNEntriesSinceSorting(0)
{
...
{
   if (aname == nullptr) return nullptr;
   ...
   if (slash) {
      ...
      return GetLeafImpl(nullptr, aname);
   }
}
...
TTreeCache *TTree::GetReadCache(TFile *file, Bool_t create /* = kFALSE */ )
{
   ...
   if (pe && pe->GetTree() != this) pe = nullptr;
   if (create && !pe) {
...
TTreeFriendLeafIter::TTreeFriendLeafIter(const TTree *tree, Bool_t dir)
   : fTree(const_cast<TTree *>(tree)), fLeafIter(nullptr), fTreeIter(nullptr), fDirection(dir)
{
}
...
TTreeFriendLeafIter::TTreeFriendLeafIter(const TTreeFriendLeafIter &iter)
   : TIterator(iter), fTree(iter.fTree), fLeafIter(nullptr), fTreeIter(nullptr), fDirection(iter.fDirection)
{
}
