...
ROOT::Internal::GetFieldRange(const ROOT::RFieldBase &field, ROOT::Internal::RPageSource &pageSource)
{
   std::uint64_t arraySize = 1;
   DescriptorId_t columnId = ROOT::kInvalidDescriptorId;

   {
      ...
      columnId = fnGetPrincipalColumnId(field.GetOnDiskId());
      ...
      arraySize = std::max(arraySize, desc.GetFieldDescriptor(field.GetOnDiskId()).GetNRepetitions());
   }

   return ROOT::RNTupleGlobalRange(0, pageSource.GetNElements(columnId) / arraySize);
}
