...
std::string ROOT::Experimental::Internal::ParseS3Url(std::string_view uri)
{
   ...
   if (ROOT::StartsWith(schemeLower, "ntpl+s3+https://")) {
      ...
      throw ROOT::RException(R__FAIL("invalid S3 URI (expected ntpl+s3+http:// or ntpl+s3+https://): " + uriStr));
   }
   ...
   if (hostAndPath.empty())
      throw ROOT::RException(R__FAIL("S3 URI has no host: " + uriStr));
...
ROOT::Experimental::Internal::RPageSinkS3::RPageSinkS3(std::string_view ntupleName, std::string_view uri,
   ...
   : RPageSinkS3(ntupleName, ParseS3Url(uri), options, RFromBaseUrl{})
{
}

ROOT::Experimental::Internal::RPageSinkS3::RPageSinkS3(std::string_view ntupleName, std::string baseUrl,
                                                       const ROOT::RNTupleWriteOptions &options, RFromBaseUrl)
   : RPagePersistentSink(ntupleName, options), fBaseUrl(std::move(baseUrl)), fConnection(fBaseUrl)
{
                                                         ...
                                                         const ROOT::RNTupleWriteOptions &opts) const
{
   ...
   return std::unique_ptr<ROOT::Internal::RPageSink>(
      new RPageSinkS3(name, std::move(cloneBaseUrl), opts, RFromBaseUrl{}));
}
