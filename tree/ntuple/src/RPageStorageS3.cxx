...
/// Bounds the header and footer sizes taken from the anchor, so that summing them cannot overflow.
/// The checksum proves they arrived intact, not that they are sensible.
constexpr std::uint64_t kMaxEnvelopeSize = 1024 * 1024 * 1024;
} // anonymous namespace
...
ROOT::Experimental::Internal::RPageSourceS3::RPageSourceS3(std::string_view ntupleName, std::string_view baseUrl,
   ...
   : RPageSource(ntupleName, options), fBaseUrl(baseUrl), fConnection(fBaseUrl), fClusterConnection(fBaseUrl)
{
   fConnection.SetCredentialsFromEnvironment();
...
ROOT::Experimental::Internal::RPageSourceS3::~RPageSourceS3()
{
   ...
   // the members below (in particular fConnection) are destroyed.
...
{
   // Resolve the template the writer recorded rather than assuming the default one. Same substitution
   // as RPageSinkS3::CloneAsHidden().
   std::string url = fAnchor.GetUrlTemplate();

   auto pos = url.find("${baseurl}");
   if (pos != std::string::npos)
      url.replace(pos, std::strlen("${baseurl}"), fBaseUrl);
...
std::size_t ROOT::Experimental::Internal::RPageSourceS3::GetObjectPrefix(ROOT::Internal::RCurlConnection &connection,
                                                                         const std::string &url, unsigned char *buffer,
                                                                         std::size_t size)
{
   connection.SetUrl(url).ThrowOnError();
   ...
   // more than the object holds is how its size is learned without a preceding HEAD. Unlike GetObject(),
...
void ROOT::Experimental::Internal::RPageSourceS3::LoadStructureImpl()
{
   ...
   auto anchorSize = GetObjectPrefix(fConnection, fBaseUrl, anchorBuffer.get(), kAnchorReadSize);
   ...
   if (anchorSize == kAnchorReadSize) {
      ...
      anchorSize = GetObjectPrefix(fConnection, fBaseUrl, anchorBuffer.get(), kMaxAnchorSize);
      if (anchorSize == kMaxAnchorSize) {
         throw ROOT::RException(R__FAIL("S3 anchor at " + fBaseUrl + " is implausibly large (over " +
   ...
   // The envelope sizes are used to size a buffer and to offset into it; bound them so that summing them
   // below cannot overflow.
   if (fAnchor.GetNBytesHeader() > kMaxEnvelopeSize || fAnchor.GetNBytesFooter() > kMaxEnvelopeSize ||
       fAnchor.GetLenHeader() > kMaxEnvelopeSize || fAnchor.GetLenFooter() > kMaxEnvelopeSize) {
      throw ROOT::RException(R__FAIL("S3 anchor at " + fBaseUrl +
                                     " declares an implausible header or footer size "
                                     "(limit is " +
                                     std::to_string(kMaxEnvelopeSize) + " bytes); refusing to read it"));
   }
...

   {
      ...
      GetObject(fConnection, MakeObjectUrl(fAnchor.GetHeaderObjId()),
                reinterpret_cast<unsigned char *>(fStructureBuffer.fPtrHeader), fAnchor.GetNBytesHeader());
      GetObject(fConnection, MakeObjectUrl(fAnchor.GetFooterObjId()),
                reinterpret_cast<unsigned char *>(fStructureBuffer.fPtrFooter), fAnchor.GetNBytesFooter());
      // fNRead counts byte ranges read, so the three GETs count and the preceding HEAD does not.
...
void ROOT::Experimental::Internal::RPageSourceS3::LoadPageListImpl(const RNTupleLocator &locator, unsigned char *buffer)
{
   ...
   GetObject(fConnection, MakeObjectUrl(objId), buffer, locator.GetNBytesOnStorage());
}
                                                                     ...
                                                                     RSealedPage &sealedPage)
{
   ...
   GetObject(fConnection, MakeObjectUrl(objId),
             const_cast<unsigned char *>(reinterpret_cast<const unsigned char *>(sealedPage.GetBuffer())),
             sealedPage.GetBufferSize());
}
...
ROOT::Experimental::Internal::RPageSourceS3::LoadClusters(std::span<ROOT::Internal::RCluster::RKey> clusterKeys)
{
   ...
   for (const auto &clusterKey : clusterKeys) {
...

      {
         ...
         // cluster pool's I/O thread, hence fClusterConnection: the caller may be using fConnection.
         Detail::RNTupleAtomicTimer timer(fCounters->fTimeWallRead, fCounters->fTimeCpuRead);
