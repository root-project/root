...
/// a buffer length, no matter how large the internal MaxEnvelopeSize read option is set.
...
void ROOT::Experimental::Internal::RPageSourceS3::LoadStructureImpl()
{
   ...
   // The limit is an internal read option so that it can be raised for a data set with an unusually
   // large schema; it is capped at what the format itself can express, so that the sum cannot overflow
   // no matter what it is set to.
   const auto maxEnvelopeSize =
      std::min(ROOT::Internal::RNTupleReadOptionsManip::GetMaxEnvelopeSize(fOptions), kFormatMaxEnvelopeSize);
   if (fAnchor.GetNBytesHeader() > maxEnvelopeSize || fAnchor.GetNBytesFooter() > maxEnvelopeSize ||
      ...
      throw ROOT::RException(R__FAIL("S3 anchor at " + fBaseUrl + " declares a header or footer larger than " +
                                     std::to_string(maxEnvelopeSize) + " bytes"));
   }
