                                                            ...
                                                            const Internal::RNTupleProcessorProvenance &provenance)
{
   ...
   if (!fieldIdx) {
      try {
         ...
         if (auto fieldNamePrefix = provenance.GetFieldNamePrefix();
             !fieldNamePrefix.empty() && onDiskFieldName.find(fieldNamePrefix) == 0) {
            onDiskFieldName = onDiskFieldName.substr(provenance.GetFieldNamePrefix().size() + 1);
         }
   ...
   const Internal::RNTupleProcessorProvenance &provenance, bool updateFields)
{
   ...
   for (const auto &fieldIdx : fieldIdxs) {
      ...
      if (std::includes(fieldProvenance.cbegin(), fieldProvenance.cend(), auxProvenance.cbegin(), auxProvenance.cend()))
...
{
   auto auxProvenance = provenance.Evolve(fAuxiliaryProcessor->GetProcessorName());
   if (IsAuxiliaryField(fieldName, auxProvenance.GetFieldNamePrefix())) {
