...
private:
   ...
   ROOT::Internal::RCurlConnection fMainConnection;
   ...
   /// the prefetch path needs a handle of its own rather than sharing fMainConnection.
   ROOT::Internal::RCurlConnection fClusterConnection;
   /// Anchor metadata, fetched and parsed in LoadStructureImpl
   RNTupleAnchorS3 fAnchor;
   /// The anchor's URL template with ${baseurl} already substituted, so that resolving an object ID only
   /// has to replace ${objid}. Set from the anchor in LoadStructureImpl and carried into clones.
   std::string fResolvedUrlTemplate;
   ...
   void GetObject(ROOT::Internal::RCurlConnection &connection, const std::string &url, unsigned char *buffer,
                  std::size_t size);
   /// Download at most `size` bytes of `url`, returning how many arrived. Used to read an object of
   /// unknown length, where a reply shorter than the request is the answer rather than an error.
   std::size_t GetShortObject(ROOT::Internal::RCurlConnection &connection, const std::string &url,
