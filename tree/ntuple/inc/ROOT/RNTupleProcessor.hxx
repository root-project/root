...
protected:
   ...
   bool AddOrUpdateEntryField(std::string_view fieldName, std::shared_ptr<void> valuePtr = nullptr)
   {
...
public:
   ...
   template <typename T>
   RNTupleProcessorOptionalPtr<T> RegisterField(std::string_view fieldName)
   {
      if (!fEntry)
         Initialize();
      ...
      if (!AddOrUpdateEntryField(fieldName)) {
         throw RException(R__FAIL("cannot register field with name \"" + std::string(fieldName) +
