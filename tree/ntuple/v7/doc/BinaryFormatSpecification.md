...
Type names are stored according to the following normalization rules
  ...
  - Integer template arguments are written in standard decimal notation; only if integer values are larger than 32bit,
    a suffix `ll` or `ull` (if the value does not fit in a `long long`) is added.
