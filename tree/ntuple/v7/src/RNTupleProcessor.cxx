...
{
   // TODO(fdegeus): this check is not complete, e.g. the situation where the auxiliary processor is a chain of joins
                                                                ...
                                                                std::string_view fieldNamePrefix)
{
   ...
   if (!fJoinFieldValues.empty()) {
      for (unsigned i = 0; i < fJoinFieldValues.size(); ++i) {
         auto ptr = fEntry->GetPtr<void>(fJoinFieldTokens[i]);
         fJoinFieldValues[i] = ptr.get();
      }
   }
}
...
ROOT::NTupleSize_t ROOT::Experimental::RNTupleJoinProcessor::LoadEntry(ROOT::NTupleSize_t entryNumber)
{
   ...
   if (!HasJoinTable()) {
      for (auto &auxProcessor : fAuxiliaryProcessors) {
         // TODO handle aux processor with fewer entries
   ...
   for (unsigned i = 0; i < fJoinTables.size(); ++i) {
      const auto entryIdx = fJoinTables[i]->GetEntryIndex(fJoinFieldValues);
