...
ROOT::Experimental::Detail::RPageSourceFriend::RPageSourceFriend(std::string_view ntupleName, std::vector<std::string> locationVec, const RNTupleReadOptions &options) : RPageSource{ntupleName, options}
{
...
void ROOT::Experimental::Detail::RPageSourceFriend::CompareFileMetaData()
{
   ...
   for (std::size_t i = 1; i < fSources.size(); ++i) {
      ...
      if (desc.GetNClusters() != fSources.at(i)->GetDescriptor().GetNClusters()) {
         R__WARNING_HERE("NTuple") << "The meta-data (number of clusters) of the files do not match. A nullptr was returned, please make sure all files have the same number of clusters!";
         fUnsafe = true;
         return;
      }
      ...
      for (std::size_t i = 0; i < desc.GetNClusters(); ++i) {
         if (!desc.GetClusterDescriptor(i).BeFriendAble(fSources.at(i)->GetDescriptor().GetClusterDescriptor(i))) {
            fUnsafe = true;
            R__WARNING_HERE("NTuple") << "The meta-data of the clusters of the files do not match. A nullptr was returned, please make sure all the clusters in all files have the same number of entries!";
            return;
         }
      }
   }
   ...
   // Checks if 2 RPageSources have a common field name. From a runtime perspective, sorting and then comparing adjacent fieldNames would be faster. But for better readability of code another implementation was chosen.
   for (std::size_t i = 0; i < fSources.size(); ++i) {
      for (std::size_t j = i+1; j < fSources.size(); ++j) {
         if (fSources.at(i)->GetDescriptor().CommonFieldNameExists(fSources.at(j)->GetDescriptor())) {
            fUnsafe = true;
            R__WARNING_HERE("NTuple") << "The files contain multiple fields with the same field name! A nullptr was returned, please make sure all fields have unique names!";
            return;
         }
      }
   }
