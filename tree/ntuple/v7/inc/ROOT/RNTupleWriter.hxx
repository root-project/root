...
public:
   ...
   /// a light check whether the entry comes from the ntuple's own model
   /// \return The number of uncompressed bytes written.
   std::size_t Fill(Detail::RRawPtrWriteEntry &entry) { return fFillContext.Fill(entry); }
   ...
   void FillNoFlush(Detail::RRawPtrWriteEntry &entry, RNTupleFillStatus &status)
   {
   ...
   std::unique_ptr<REntry> CreateEntry() const { return fFillContext.CreateEntry(); }
