...
struct RNTupleProcessorEntryLoader {
   ...
   {
      processor.FreezeEntry(/*setEntryPointers=*/true);
...
TEST_F(RNTupleChainProcessorTest, LoadUnfrozenEntry)
{
   using ROOT::Experimental::Internal::RNTupleProcessorEntryLoader;
   ...
   try {
      RNTupleProcessorEntryLoader::LoadUnfrozenEntry(*proc, 0);
      FAIL() << "should not be able to load unfrozen entries";
   } catch (const ROOT::RException &err) {
      EXPECT_THAT(err.what(), testing::HasSubstr("cannot load processor entry, because it has not been frozen yet"));
   }
}
