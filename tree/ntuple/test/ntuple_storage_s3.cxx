...
{
   EXPECT_EQ("http://localhost:9000/mybucket/path", ParseS3Url("ntpl+s3+http://localhost:9000/mybucket/path"));
}

TEST(RPageSinkS3, ParseS3UrlHttps)
{
   EXPECT_EQ("https://s3.cern.ch/mybucket/path", ParseS3Url("ntpl+s3+https://s3.cern.ch/mybucket/path"));
}

TEST(RPageSinkS3, ParseS3UrlInvalid)
{
   // Non-S3 schemes, the bare s3:// (left to ROOT's S3 file handler), the old s3+http(s):// forms,
   // and ntpl+s3:// without a transport are all rejected.
   EXPECT_THROW(ParseS3Url("http://example.com"), ROOT::RException);
   EXPECT_THROW(ParseS3Url("daos://pool/container"), ROOT::RException);
   EXPECT_THROW(ParseS3Url(""), ROOT::RException);
   EXPECT_THROW(ParseS3Url("s3://bucket/path"), ROOT::RException);
   EXPECT_THROW(ParseS3Url("s3+https://host/bucket/path"), ROOT::RException);
   EXPECT_THROW(ParseS3Url("ntpl+s3://host/bucket/path"), ROOT::RException);
   // A scheme followed only by slashes has no host either (trailing slashes are stripped before the
   // emptiness check).
   EXPECT_THROW(ParseS3Url("ntpl+s3+http:///"), ROOT::RException);
}
...
TEST(RPageSinkS3, ParseS3UrlTrailingSlash)
{
   ...
   EXPECT_EQ("http://localhost:9000/bucket/path", ParseS3Url("ntpl+s3+http://localhost:9000/bucket/path/"));
   EXPECT_EQ("https://s3.cern.ch/bucket", ParseS3Url("ntpl+s3+https://s3.cern.ch/bucket/"));
}
...
TEST(RPageSinkS3, ParseS3UrlCaseInsensitiveScheme)
{
   ...
   EXPECT_EQ("http://Host:9000/MyBucket/Path", ParseS3Url("NTPL+S3+HTTP://Host:9000/MyBucket/Path"));
   EXPECT_EQ("https://Host/MyBucket/Path", ParseS3Url("Ntpl+S3+Https://Host/MyBucket/Path"));
}
...
TEST(RPageSinkS3, ParseS3UrlAwsAndCeph)
{
   ...
   EXPECT_EQ("https://s3.eu-west-1.amazonaws.com/bucket/data", // AWS path-style, regional
             ParseS3Url("ntpl+s3+https://s3.eu-west-1.amazonaws.com/bucket/data"));
   EXPECT_EQ("https://bucket.s3.eu-west-1.amazonaws.com/data", // AWS virtual-hosted style
             ParseS3Url("ntpl+s3+https://bucket.s3.eu-west-1.amazonaws.com/data"));
   EXPECT_EQ("https://s3.cern.ch/bucket/data", // Ceph RGW (CERN)
             ParseS3Url("ntpl+s3+https://s3.cern.ch/bucket/data"));
}

TEST(RPageSinkS3, ParseS3UrlRejectsUnsupportedComponents)
{
   EXPECT_THROW(ParseS3Url("ntpl+s3+https://KEY:SECRET@host/bucket/path"), ROOT::RException); // userinfo
   EXPECT_THROW(ParseS3Url("ntpl+s3+http://host/bucket/path?versionId=1"), ROOT::RException); // query
   EXPECT_THROW(ParseS3Url("ntpl+s3+http://host/bucket/path#section"), ROOT::RException);     // fragment
   EXPECT_THROW(ParseS3Url("ntpl+s3+http://"), ROOT::RException);                             // no host
}
...
TEST(RPageSinkS3Wire, WriteIssuesExpectedPuts)
{
   ...
   setenv("S3_ACCESS_KEY", "dummykey", 1);
   setenv("S3_SECRET_KEY", "dummysecret", 1);
   setenv("S3_REGION", "us-east-1", 1);
   ...
   unsetenv("S3_ACCESS_KEY");
   unsetenv("S3_SECRET_KEY");
   unsetenv("S3_REGION");
