...
TEST(RNTuple, TypeNameNormalizationById)
{
   ...
   EXPECT_EQ("CustomStruct", ROOT::Internal::GetRenormalizedTypeName(typeid(class CustomStruct)));

   EXPECT_EQ("std::map<std::int32_t,std::int32_t>",
             ROOT::Internal::GetRenormalizedTypeName(typeid(std::map<int, int>)));

   EXPECT_EQ("std::pair<std::uint64_t,std::array<CustomStruct,2>>",
             ROOT::Internal::GetRenormalizedTypeName(typeid(const std::pair<size_t, std::array<::CustomStruct, 2>>)));
}
