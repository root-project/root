...
using ROOT::Internal::ROnDiskPageMapHeap;
...
public:
   ...
   std::vector<std::unique_ptr<RCluster>> LoadClusters(std::span<RCluster::RKey> clusterKeys) final
   {
      ...
      for (auto key : clusterKeys) {
         ...
         auto pageMap = std::make_unique<ROOT::Internal::ROnDiskPageMap>();
...
{
   auto cluster = new ROnDiskPageMapHeap(nullptr);
   delete cluster;

   cluster = new ROnDiskPageMapHeap(std::make_unique<unsigned char[]>(1));
   delete cluster;
}
...
TEST(Cluster, Basics)
{
   ...
   auto pageMap = std::make_unique<ROnDiskPageMapHeap>(std::unique_ptr<unsigned char[]>(memory));
...
TEST(Cluster, AdoptPageMaps)
{
   ...
   auto pageMap1 = std::make_unique<ROnDiskPageMapHeap>(std::unique_ptr<unsigned char[]>(mem1));
   ...
   auto pageMap2 = std::make_unique<ROnDiskPageMapHeap>(std::unique_ptr<unsigned char[]>(mem2));
