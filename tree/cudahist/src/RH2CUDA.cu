                            ...
                            Double_t *cells, Double_t *w, UInt_t bufferSize, HistStats *stats)
{
   ...
   for (int i = tid; i < bufferSize; i += stride) {
      auto binx = FindFixBin(cells[i], binEdges, nCells, xMin, xMax);
      auto biny = FindFixBin(cells[i], binEdges, nCells, xMin, xMax);
...
// RH2CUDA::RH2CUDA(Int_t _ncells) : RH2CUDA() {
RH2CUDA::RH2CUDA(Int_t _nCells, Double_t _xLow, Double_t _xHigh, const Double_t *_binEdges) : RH2CUDA()
{
   fNcells = _nCells;
   fXmin = _xLow;
   fXmax = _xHigh;
...
void RH2CUDA::GetStats(UInt_t size)
{
...
void RH2CUDA::Fill(Double_t x, Double_t w)
{
   fCells.push_back(x);
...
void RH2CUDA::Fill(Double_t x)
{
   Fill(x, 1.0);
}

void RH2CUDA::Fill(const char *namex, Double_t w)
{
   Fatal(":Fill(const char *namex, Double_t w)", "Cuda version not implemented yet");
}

// Copy back results on GPU to CPU.
Int_t RH2CUDA::RetrieveResults(Double_t *histResult, Double_t *stats)
{
