...
void SetEmptyBranchesHelper(TTree *inputTree, TTree &outputTree, RBranchSet &outputBranches,
                            ...
                            const std::type_info &typeInfo, const std::optional<int> &basketSize);
...
public:
   ...
   void SetEmptyBranches(TTree *inputTree, TTree &outputTree, std::index_sequence<S...>)
   {
      ...
      (SetEmptyBranchesHelper(inputTree, outputTree, outputBranches, fInputBranchNames[S], fOutputBranchNames[S],
                              typeid(ColTypes), fOptions.fBasketSize),
       ...);
   }
...
public:
   ...
   void Finalize()
   {
      ...
      if (!fOutputFile->Get(fullTreeName.c_str())) {
         ...
         if (auto treeDS = dynamic_cast<ROOT::Internal::RDF::RTTreeDS *>(fInputDataSource))
            inputTree = treeDS->GetTree();
         using ind_t = std::index_sequence_for<ColTypes...>;
         SetEmptyBranches(inputTree, *outTree, ind_t{});
