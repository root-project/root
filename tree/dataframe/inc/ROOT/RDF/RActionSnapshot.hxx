...
public:
   RActionSnapshot(Helper &&h, const ColumnNames_t &columns, std::shared_ptr<PrevNode> pd,
   ...
   ~RActionSnapshot() { fLoopManager->Deregister(this); }
   ...
   */
   ...
   {
      RColumnReadersUntypedSnapshotInfo info{RActionBase::GetColumnNames(), RActionBase::GetColRegister(),
                                             *fLoopManager};
      fValues[slot] = GetUntypedColumnReaders(slot, r, info);
   ...
   std::unique_ptr<RActionBase> MakeVariedAction(std::vector<void *> && /*results*/) final
   {
      ...
      return std::unique_ptr<RActionBase>(nullptr);
   }
   ...
   {
      return std::make_unique<RActionSnapshot>(fHelper.CallMakeNew(newResult), GetColumnNames(), fPrevNodePtr,
                                               GetColRegister());
   }
