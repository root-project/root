...
BookVariationJit(const std::vector<std::string> &colNames, std::string_view variationName,
                 ...
                 std::shared_ptr<RDFDetail::RNodeBase> *upcastNodeOnHeap, bool isSingleColumn);
...
std::shared_ptr<RDFDetail::RNodeBase> UpcastNode(std::shared_ptr<RDFDetail::RNodeBase> ptr);
...
void AddDSColumnsHelper(const std::string &colName, RLoopManager &lm, RDataSource &ds, RColumnRegister &colRegister)
{
   ...
   std::vector<std::unique_ptr<RDFDetail::RColumnReaderBase>> colReaders;
...
void JitFilterHelper(F &&f, const char **colsPtr, std::size_t colsSize, std::string_view name,
                     std::weak_ptr<RDFDetail::RJittedFilter> *wkJittedFilter, std::shared_ptr<PrevNode> *prevNodeOnHeap,
                     RColumnRegister *colRegister) noexcept
{
   ...
   using F_t = RDFDetail::RFilter<Callable_t, PrevNode>;
   ...
   jittedFilter->SetFilter(
      std::unique_ptr<RDFDetail::RFilterBase>(new F_t(std::forward<F>(f), cols, *prevNodeOnHeap, *colRegister, name)));
                    ...
                    const ColumnNames_t &, RColumnRegister &, RLoopManager &lm)
{
   return std::unique_ptr<RDFDetail::RDefineBase>(
      new RDFDetail::RDefinePerSample<std::decay_t<F>>(name, dummyType, std::forward<F>(f), lm));
}
...
void JitDefineHelper(F &&f, const char **colsPtr, std::size_t colsSize, std::string_view name, RLoopManager *lm,
                     std::weak_ptr<RDFDetail::RJittedDefine> *wkJittedDefine, RColumnRegister *colRegister,
                     std::shared_ptr<RDFDetail::RNodeBase> *prevNodeOnHeap) noexcept
{
   ...
   // use unique_ptr<RDFDetail::RDefineBase> instead of make_unique<NewCol_t> to reduce jit/compile-times
   std::unique_ptr<RDFDetail::RDefineBase> newCol{
...
template <>
struct RMinReturnType<RDFDetail::RInferredType, false> {
   using type = double;
};
...
struct RNeedJittingHelper<RDFDetail::RInferredType, Rest...> {
...
template <>
struct RNeedJittingHelper<RDFDetail::RInferredType> {
   static constexpr bool value = true;
};
...
template <typename T>
using MinReturnType_t = typename ROOT::Internal::RDF::RMinReturnType<T>::type;
