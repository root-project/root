...
struct SnapshotHelperArgs {
   ...
   std::weak_ptr<ROOT::Detail::RDF::RLoopManager> fLoopManager;
            ...
            const RColumnRegister &colRegister)
{
   ...
   if (snapHelperArgs->fToNTuple) {
      ...
      if (!ROOT::IsImplicitMTEnabled()) {
         ...
         actionPtr.reset(
            new Action_t(Helper_t(filename, treename, colNames, outputColNames, options, lmPtr, std::move(isDefine)),
...
{
#ifdef R__USE_IMT
   // Do not pollute the column readers with TTreeReaderValues that won't work
   // because each thread has a different instance of TTreeReader anyway
   if (ROOT::IsImplicitMTEnabled() && ds.GetLabel() == "TTreeDS")
      return;
#endif
