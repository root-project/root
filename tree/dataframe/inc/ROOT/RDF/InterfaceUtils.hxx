...
void JitFilterHelper(F &&f, std::initializer_list<const char *> colsList, std::string_view name,
                     ...
                     RBookedDefines *defines) noexcept
{
   ...
   const ColumnNames_t cols(colsList.begin(), colsList.end());
...
// If colsList has no elements, build a RDefinePerSample (it has no input columns), otherwise a RDefine.
template <typename RDefineTypeTag, typename F>
void JitDefineHelper(F &&f, std::initializer_list<const char *> colsList, std::string_view name, RLoopManager *lm,
...
void CallBuildAction(std::shared_ptr<PrevNodeType> *prevNodeOnHeap, std::initializer_list<const char *> colsList,
