...
TEST(RDFSnapshotRNTuple, Compression)
{
   ...
   // Through RNTupleWriteOptions provided by RSnapshotOptions
   {
      RSnapshotOptions opts;
      opts.fOutputFormat = ROOT::RDF::ESnapshotOutputFormat::kRNTuple;
      opts.fNTupleWriteOpts.SetCompression(101);
      ...
      auto reader = RNTupleReader::Open("ntuple", fileGuard.GetPath());
      auto compSettings = *reader->GetDescriptor().GetClusterDescriptor(0).GetColumnRange(0).GetCompressionSettings();
      EXPECT_EQ(101, compSettings);
   }
   // When both are set to different compression settings, an error should be thrown
   {
      RSnapshotOptions opts;
      opts.fOutputFormat = ROOT::RDF::ESnapshotOutputFormat::kRNTuple;
      ...
      try {
         df.Snapshot("ntuple", fileGuard.GetPath(), "x", opts);
         FAIL() << "passing conflicting compression settings should throw";
      } catch (const std::runtime_error &err) {
         EXPECT_THAT(err.what(),
                     testing::HasSubstr(
                        "Snapshot: fCompressionAlgorithm and/or fCompressionLevel and the compression in "
                        "fNTupleWriteOpts in RSnapshotOptions have been set to something other than the default"));
      }
   }
}
