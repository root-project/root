...
TEST(RDFSnapshot, Snapshot_RNTupleTemplated)
{
   auto filename = "Snapshot_RNTuple.root";

   auto df = ROOT::RDataFrame(25ull).Define("x", [] { return 10; });
   auto sdf = df.SnapshotRNTuple<int>("ntuple", filename, {"x"});

   std::vector<std::string> expectedColumns = {"x"};
   EXPECT_EQ(expectedColumns, sdf->GetColumnNames());
}

TEST(RDFSnapshot, Snapshot_RNTupleJITted)
{
   auto filename = "Snapshot_RNTuple.root";

   auto df = ROOT::RDataFrame(25ull).Define("x", [] { return 10; });
   auto sdf = df.SnapshotRNTuple("ntuple", filename, {"x"});
...
// TODO:
// * test snapshotting from existing on-disk fields
...
//          "Branch arr hangs from a non-split branch. A copy is being performed in order to properly read the
//          content.");
// #else
...
//       auto out_df =
//          ROOT::RDataFrame("t", "df_readwritetclonesarray.root").Snapshot("t", "df_readwriteclonesarray3.root",
//          {"arr"});
//       RVec<TH1D> hvec;
// #ifndef NDEBUG
//       ROOT_EXPECT_WARNING(
...
//    ROOT::TestSupport::CheckDiagsRAII diagRAII{kSysError, "TFile::TFile", "file /definitely/not/a/valid/path/f.root
//    can not be opened No such file or directory"}; EXPECT_THROW(df.Snapshot("t", out_fname, {"rdfslot_"}),
//    std::runtime_error);
// }
...
//          " "main tree has a TTreeIndex on the friend tree 't'. You can also unset the bit manually if you know what
//          you " "are doing; note that you risk associating wrong TTree entries of the friend with those of the main
//          TTree!";
...
//       const auto expected =
//          "Tree 't' has the kEntriesReshuffled bit set and cannot have friends nor can be added as a friend unless the
//          " "main tree has a TTreeIndex on the friend tree 't2'. You can also unset the bit manually if you know what
//          you " "are doing; note that you risk associating wrong TTree entries of the friend with those of the main
//          TTree!";
...
//    // multi-thread execution might have scrambled events w.r.t. the original file, so we just check overall
//    properties const auto nEvents = fixedSizeArr->size(); EXPECT_EQ(nEvents, kNEvents);
//    // TODO check more!
// }
...
// {
//    {
//       auto df = ROOT::RDataFrame(3).Define("x", [](ULong64_t e) { return ROOT::RVecD(e, double(e)); },
//       {"rdfentry_"}); df.Snapshot<ROOT::RVecD>("t", "test_snapshotRVecWriteRVecFromFile.root", {"x"});
//    }
...
//    // but the address printed changes every time
//    ROOT::TestSupport::CheckDiagsRAII diagRAII;
//    diagRAII.requiredDiag(kSysError, "TFile::TFile", "file /definitely/not/a/valid/path/f.root can not be opened No
//    such file or directory"); diagRAII.optionalDiag(kSysError, "TReentrantRWLock::WriteUnLock", "Write lock already
//    released for", /*wholeStringNeedsToMatch=*/false); EXPECT_THROW(df.Snapshot("t", out_fname, {"rdfslot_"}),
//    std::runtime_error);
// }
