...
TEST(RDFSnapshotRNTuple, LazyNotTriggered)
{
   ...
   EXPECT_FALSE(std::filesystem::exists("lazysnapshotnottriggered_shouldnotbecreated.root"));
}
...
TEST_F(RDFSnapshotRNTupleTest, VectorFields)
{
   ...
   auto nnloMax = sdf->Define("nnloMax",
                              [](ROOT::RVec<ROOT::RVec<float>> nnlo) {
...
TEST_F(RDFSnapshotRNTupleTest, ComplexFields)
{
   ...
   auto electronsPtMax =
      sdf->Define("electronsPtMax", [](ROOT::RVec<float> electronPt) { return ROOT::VecOps::Max(electronPt); },
                  {"electrons.pt"})
         .Max("electronsPtMax")
         .GetValue();
   ...
   auto jetsElectronsPtMax =
      sdf2
         ->Define("jetsElectronsPtMax",
                  [](ROOT::RVec<ROOT::RVec<float>> jetsElectronsPt) {
                     auto innerMax = ROOT::VecOps::Map(
                        jetsElectronsPt, [](ROOT::RVec<float> electronsPt) { return ROOT::VecOps::Max(electronsPt); });
                     return ROOT::VecOps::Max(innerMax);
                  },
                  {"jets_electrons.pt"})
...
TEST(RDFSnapshotRNTuple, UpdateSameName)
{
   ...
   try {
      ...
      EXPECT_STREQ(err.what(), "Snapshot: RNTuple \"ntuple\" already present in file "
                               "\"RDFSnapshotRNTuple_update_same_name.root\". If you want to delete the original "
                               "ntuple and write another, please set RSnapshotOptions::fOverwriteIfExists to true.");
   }
...
TEST(RDFSnapshotRNTuple, DisallowToTTreeTemplated)
{
   const auto treename = "tree";
   const auto filename = "RDFSnapshotRNTuple_disallow_to_ttree_templated.root";
   ...
   try {
      ...
      EXPECT_STREQ(err.what(), "Snapshotting from TTree to RNTuple is not yet supported. The current recommended way "
                               "to convert TTrees to RNTuple is through ROOT::Experimental::RNTupleImporter.");
   }
}

TEST(RDFSnapshotRNTuple, DisallowToTTreeJITted)
{
   const auto treename = "tree";
   const auto filename = "RDFSnapshotRNTuple_disallow_to_ttree_jitted.root";
