...
protected:
   ...
   RDFManyBranchTypesTTree() : fNSlots(GetParam().first ? std::min(4u, std::thread::hardware_concurrency()) : 1u)
   {
      ...
      t.Branch("lorentzVectors", &lorentzVectors, 32000, 0);
...
TEST_P(RDFManyBranchTypesTTree, SnapshotEmptyTTree)
{
   ...
   const auto outputFileNameActual{"rdfmanybranchtypesttree_snapshotemptytree_actual.root"};
   {
      ...
      auto snapdf_expected = df.Snapshot(outputTreeNameExpected, outputFileNameExpected, df.GetColumnNames(), opts);
      ...
      auto snap_actual = df.Snapshot(outputTreeNameActual, outputFileNameActual, df.GetColumnNames(), opts);
