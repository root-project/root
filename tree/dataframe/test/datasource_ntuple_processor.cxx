...
using ROOT::Experimental::RDF::RNTupleProcessorDS;
...
class RNTupleProcessorDSTest : public ::testing::Test {
protected:
   std::string fFileName = "RNTupleProcessorDS_test.root";
...
TEST_F(RNTupleProcessorDSTest, ColTypeNames)
{
   RNTupleProcessorDS ds(RNTupleProcessor::Create({fNtplName, fFileName}));
   ...
   try {
      ...
      EXPECT_THAT(err.what(), testing::HasSubstr("RNTupleProcessorDS: There is no column with name \"Address\""));
   }
}

TEST_F(RNTupleProcessorDSTest, NFiles)
{
   RNTupleProcessorDS ds(RNTupleProcessor::Create({fNtplName, fFileName}));
...
TEST_F(RNTupleProcessorDSTest, Basic)
{
   auto df = ROOT::Experimental::RDF::FromRNTupleProcessor(RNTupleProcessor::Create({fNtplName, fFileName}));
   auto pt_max = df.Max<float>("pt");
   auto pt_sum = df.Sum<float>("pt");
   EXPECT_EQ(42.f, *pt_max);
   EXPECT_EQ(42.f, *pt_sum);
   EXPECT_EQ(*pt_sum, *pt_max);
}

TEST_F(RNTupleProcessorDSTest, CardinalityColumn)
{
   auto df = ROOT::Experimental::RDF::FromRNTupleProcessor(RNTupleProcessor::Create({fNtplName, fFileName}));
...
TEST_F(RNTupleProcessorDSTest, ProjectedCardinalityColumn)
{
   auto df = ROOT::Experimental::RDF::FromRNTupleProcessor(RNTupleProcessor::Create({fNtplName, fFileName}));
...
{
   auto df = ROOT::Experimental::RDF::FromRNTupleProcessor(RNTupleProcessor::Create({name, fname}));
   ...
   auto sumVecElectronPt1 = df.Aggregate(
      [](float &acc, const ROOT::RVec<Electron> &ve) {
         for (const auto &e : ve)
            acc += e.pt;
      },
      [](float a, float b) { return a + b; }, "VecElectron");
   auto sumVecElectronPt2 = df.Aggregate(
      [](float &acc, const ROOT::RVec<float> &pts) {
         for (auto pt : pts)
            acc += pt;
      },
      [](float a, float b) { return a + b; }, "VecElectron.pt");
   ...
   EXPECT_FLOAT_EQ(128. * 137.0, sumVecElectronPt2.GetValue());
}

static void ChainTest(const std::string &name, const std::string &fname)
{
   auto proc1 = RNTupleProcessor::Create({name, fname});
   auto df1 = ROOT::RDataFrame(std::make_unique<RNTupleProcessorDS>(std::move(proc1)));
   EXPECT_DOUBLE_EQ(42.0, df1.Sum<float>("pt").GetValue());

   auto proc2 = RNTupleProcessor::CreateChain({{name, fname}, {name, fname}});
   auto df2 = ROOT::RDataFrame(std::make_unique<RNTupleProcessorDS>(std::move(proc2)));
   EXPECT_DOUBLE_EQ(84.0, df2.Sum<float>("pt").GetValue());

   std::vector<ROOT::Experimental::RNTupleOpenSpec> openSpecs(1000, {name, fname});
   auto proc1000 = RNTupleProcessor::CreateChain(openSpecs);
   auto df1000 = ROOT::RDataFrame(std::make_unique<RNTupleProcessorDS>(std::move(proc1000)));
   EXPECT_DOUBLE_EQ(42000.0, df1000.Sum<float>("pt").GetValue());

   FileRAII guardFile1("RNTupleProcessorDS_test_chain_1.root");
   FileRAII guardFile2("RNTupleProcessorDS_test_chain_2.root");
   FileRAII guardFile3("RNTupleProcessorDS_test_chain_3.root");
   ...
   auto proc3 = RNTupleProcessor::CreateChain(
      {{"chain", guardFile1.GetPath()}, {"chain", guardFile2.GetPath()}, {"chain", guardFile3.GetPath()}});
   auto df3 = ROOT::Experimental::RDF::FromRNTupleProcessor(std::move(proc3));
   // TODO Address NFiles in the processor
...
TEST_F(RNTupleProcessorDSTest, Chain)
{
   ChainTest(fNtplName, fFileName);
}

TEST(RNTupleProcessorDS, Join)
{
   FileRAII guardFile1("RNTupleProcessorDS_test_join_1.root");
   FileRAII guardFile2("RNTupleProcessorDS_test_join_2.root");
...
// TEST_F(RNTupleProcessorDSTest, ReadMT)
// {
//    IMTRAII _;
...
// TEST_F(RNTupleProcessorDSTest, ChainMT)
// {
//    IMTRAII _;
...
// TEST_F(RNTupleProcessorDSTest, ChainTailScheduling)
// {
//    IMTRAII _;

//    FileRAII guardFile1("RNTupleProcessorDS_test_chain_tail_scheduling_1.root");
//    FileRAII guardFile2("RNTupleProcessorDS_test_chain_tail_scheduling_2.root");
//    FileRAII guardFile3("RNTupleProcessorDS_test_chain_tail_scheduling_3.root");
...
//    auto df = ROOT::RDataFrame(std::make_unique<RNTupleProcessorDS>(
...
TEST_F(RNTupleProcessorDSTest, ModifyColumnValues)
{
   auto df = ROOT::Experimental::RDF::FromRNTupleProcessor(RNTupleProcessor::Create({fNtplName, fFileName}));
   auto dfCorrected =
...
TEST(RNTupleProcessorDS, CollectionFieldTypes)
{
   // NB: The other tests already cover std::vector and std::array (and nestings thereof).
   FileRAII fileGuard{"RNTupleProcessorDS_test_collection_field_types.root"};
   {
      auto model = RNTupleModel::Create();
   ...
   RNTupleProcessorDS ds(RNTupleProcessor::Create({"ntuple", fileGuard.GetPath()}));
...
TEST_F(RNTupleProcessorDSTest, AlternativeColumnTypes)
{
   auto df = ROOT::Experimental::RDF::FromRNTupleProcessor(RNTupleProcessor::Create({fNtplName, fFileName}));
   ...
   try {
      ...
      auto dfInvalid = ROOT::Experimental::RDF::FromRNTupleProcessor(RNTupleProcessor::Create({fNtplName, fFileName}));
      ...
      EXPECT_THAT(err.what(),
                  testing::HasSubstr("RNTupleProcessorDS: invalid type \"std::pair<float,float>\" for column "
   ...
   try {
      ...
      EXPECT_THAT(err.what(),
                  testing::HasSubstr("RNTupleProcessorDS: invalid type \"std::vector<std::uint64_t>\" for column "
                                     "\"jets\" with on-disk type \"std::vector<float>\""));
   }
}
...
class RNTupleProcessorDSArraysDataset : public ::testing::Test {
protected:
   std::string fFileName = "rntupleProcessords_arrays_dataset.root";
   std::string fNtplName = "ntuple";
   ...
   void SetUp() override
   {
      ...
      for (int i = 1; i <= 5; i++) {
         ...
         *col3_rvec_arr = rvecVal;
         ...
         // std::array<ROOT::RVecF, 3> col5_member_2{ROOT::RVecF{1.f}, ROOT::RVecF{2.f, 3.f},
         // ROOT::RVecF{4.f, 5.f, 6.f}}; ROOT::RVec<std::array<float, 3>> col5_member_3{col5_member_1, col5_member_1,
         // col5_member_1, col5_member_1}; *col5_class_with_arrays =
...
TEST_F(RNTupleProcessorDSArraysDataset, Read)
{
...
// TEST_F(RNTupleProcessorDSArraysDataset, ReadMT)
// {
//    IMTRAII _;
...
TEST_F(RNTupleProcessorDSArraysDataset, UseArrays)
{
...
// TEST_F(RNTupleProcessorDSArraysDataset, UseArraysMT)
// {
//    IMTRAII _;
...
TEST_F(RNTupleProcessorDSArraysDataset, UseArraySizeColumn)
{
...
// TEST_F(RNTupleProcessorDSArraysDataset, UseArraySizeColumnMT)
// {
//    IMTRAII _;
