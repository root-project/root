...

   {
      ...
      // Test that the Masked column reader works
      {
         ...
         for (unsigned int systematicIndex : {0, 1, 100}) {
            ...
            const double sum = std::transform_reduce(expect.begin(), expect.end(), 0, std::plus<>(), [](int v) { return v % 6 == 0 ? v : 0; });
...
TEST(RDFVarySnapshot, SnapshotVirtualClass)
{
   ...
   auto result = rdf.Define("tf", [](ULong64_t entry){ return TwoFloats(entry); }, {"rdfentry_"})
      .Vary("tf", [](TwoFloats const & tf){
            TwoFloats up, down;
            up.a = tf.a;
            up.b = tf.a + 1.f;
            down.a = tf.a;
            down.b = tf.a - 1.f;
            return ROOT::RVec<TwoFloats>{up, down};
         }, {"tf"}, {"Up", "Down"})
      .Filter([](TwoFloats const & tf){ return 2. * tf.a == tf.b || static_cast<unsigned int>(tf.a) % 2 == 0; }, {"tf"})
...

   {
      ...
      TwoFloats *tf, *tf_Up, *tf_Down;
      ASSERT_EQ(tree->SetBranchAddress<TwoFloats>("tf", &tf), TTree::kMatch);
      ASSERT_EQ(tree->SetBranchAddress<TwoFloats>("tf__tf_Up", &tf_Up), TTree::kMatch);
      ASSERT_EQ(tree->SetBranchAddress<TwoFloats>("tf__tf_Down", &tf_Down), TTree::kMatch);
