...
TEST(RDFVarySnapshot, SnapshotVirtualClass)
{
   ...
   auto result =
      rdf.Define("tf", [](ULong64_t entry) { return TwoFloats(entry); }, {"rdfentry_"})
         .Vary("tf",
               [](TwoFloats const &tf) {
                  TwoFloats up, down;
                  up.a = tf.a;
               ...
               {"tf"}, {"Up", "Down"})
         .Filter([](TwoFloats const &tf) { return 2. * tf.a == tf.b || static_cast<unsigned int>(tf.a) % 2 == 0; },
                 {"tf"})
...

   {
      ...
      TwoFloats *tf = nullptr, *tf_Up = nullptr, *tf_Down = nullptr;
