...
ROOT::Internal::RDF::RBranchData::RBranchData(std::string inputBranchName, std::string outputBranchName, bool isDefine, const std::type_info *typeID)
      ...
      fIsDefine{isDefine}
{
   ...
   if (auto datatype = dynamic_cast<TDataType *>(dictionary); datatype) {
      fTypeData.emplace<FundamentalType>(datatype->Size());
   } else if (auto tclass = dynamic_cast<TClass *>(dictionary); tclass) {
      fTypeData.emplace<EmptyDynamicType>(nullptr, tclass);
   }
}
...
void *ROOT::Internal::RDF::RBranchData::EmptyInstance()
{
   ...
   if (!dynamic.fEmptyInstance) {
      auto *dictionary = TDictionary::GetDictionary(*fInputTypeID);
      assert(dynamic_cast<TDataType *>(dictionary) == nullptr); // TDataType should be handled by writing into the local buffer
...
void ROOT::Internal::RDF::RBranchData::ClearBranchContents()
{
   ...
   if (auto fundamental = std::get_if<FundamentalType>(&fTypeData); fundamental) {
      ...
      void * emptyInstancePtr = EmptyInstance();
      // TTree expects pointer to pointer, to figure out who allocates the object:
      fOutputBranch->SetAddress(&emptyInstancePtr);
   }
}
