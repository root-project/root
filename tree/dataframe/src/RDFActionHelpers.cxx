                             ...
                             const std::optional<int> &basketSize)
{
   ...
   if (sizeLeaf && !outputBranches.Get(sizeLeafName)) {
      ...
      const auto bufSize = basketSize.has_value() ? basketSize.value() : sizeLeaf->GetBranch()->GetBasketSize();
   ...
   const auto bufSize = basketSize.has_value() ? basketSize.value() : inputBranch->GetBasketSize();
   auto *outputBranch =
...
void ROOT::Internal::RDF::SetEmptyBranchesHelper(TTree *inputTree, TTree &outputTree,
                                                 ...
                                                 const std::optional<int> &basketSize)
{
   const auto bufSize = basketSize.has_value() ? basketSize.value() : 32000;
