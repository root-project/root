...
ROOT::Internal::RDF::RTTreeDS::GetColumnReaders(unsigned int /*slot*/, std::string_view colName,
                                                const std::type_info &tid)
{
   ...
   if (tid == typeid(void))
      return std::make_unique<ROOT::Internal::RDF::RTreeOpaqueColumnReader>(*fTreeReader, colName);
   ...
   if (auto &&[toConvert, innerTypeName, collType] = GetCollectionInfo(typeName); toConvert)
      return std::make_unique<ROOT::Internal::RDF::RTreeUntypedArrayColumnReader>(*fTreeReader, colName, innerTypeName,
                                                                                  collType);
   else
      return std::make_unique<ROOT::Internal::RDF::RTreeUntypedValueColumnReader>(*fTreeReader, colName, typeName);
}
