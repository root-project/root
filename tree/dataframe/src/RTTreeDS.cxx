                                                  ...
                                                  TTreeReader *treeReader)
{
   ...
   if (ti == typeid(void)) {
      ...
      if (auto &&[toConvert, innerTypeName, collType] = GetCollectionInfo(typeName); toConvert)
         colReader = std::make_unique<ROOT::Internal::RDF::RTreeUntypedArrayColumnReader>(*treeReader, col, innerTypeName, collType);
      else
   ...
   if (TDirectory * treeDir = treeReader->GetTree()->GetDirectory(); treeDir) {
      ...
      const std::string bitmaskMapName = std::string{"R_rdf_branchToBitmaskMapping_"} + treeReader->GetTree()->GetName();
      if (Map_t const * columnMaskMap = treeDir->Get<Map_t>(bitmaskMapName.c_str()); columnMaskMap) {
         if (auto it = columnMaskMap->find(std::string(col)); it != columnMaskMap->end()) {
            colReader = std::make_unique<RMaskedColumnReader>(*treeReader, std::move(colReader), it->second.first, it->second.second);
         }
      }
   }
