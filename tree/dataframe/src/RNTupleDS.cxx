...
RNTupleDS::GetColumnReaders(unsigned int slot, std::string_view name, const std::type_info &tid)
{
   ...
   const auto requestedType = Internal::GetRenormalizedTypeName(ROOT::Internal::GetDemangledTypeName(tid));

   RFieldBase *field;
   // If the field corresponding to the provided name is not a cardinality column and the requested type is different
   // from the proto field we that was created when the data source was constructed, we first have to create an
