...
#include <ROOT/RNTupleDSFields.hxx>
...
class RNTupleProcessorColumnReader : public ROOT::Detail::RDF::RColumnReaderBase {
   ...
   Long64_t fLastLoadedEntry = ROOT::kInvalidNTupleIndex;

public:
   RNTupleProcessorColumnReader(RNTupleProcessorOptionalPtr<void> &valuePtr) : fValuePtr(std::move(valuePtr)) {}
   ~RNTupleProcessorColumnReader() override = default;

   void *GetImpl(Long64_t entry) final
   {
      assert(entry == fLastLoadedEntry);
      return fValuePtr.GetPtr().get();
   }

   void SetEntryLoaded(Long64_t entry) { fLastLoadedEntry = entry; }
};
} // namespace ROOT::Experimental::Internal::RDF
                                                           ...
                                                           bool convertToRVec)
{
   ...
   if (const auto cardinalityField = dynamic_cast<const ROOT::RCardinalityField *>(valueField.get())) {
      ...
      if (cardinalityField->As32Bit()) {
         valueField = std::make_unique<ROOT::Internal::RDF::RRDFCardinalityField<std::uint32_t>>(field.GetFieldName());
      } else if (cardinalityField->As64Bit()) {
         valueField = std::make_unique<ROOT::Internal::RDF::RRDFCardinalityField<std::uint64_t>>(field.GetFieldName());
      } else {
                                                              ...
                                                              const std::type_info &tid)
{
   ...
   if (fProcessor->GetEntry().FindFieldIndex(name, requestedType)) {
      ...
      auto reader = std::make_unique<Internal::RDF::RNTupleProcessorColumnReader>(valuePtr);
      fColumnReaders.emplace_back(reader.get());
   ...
   if (dynamic_cast<const ROOT::RCardinalityField *>(&field)) {
      if (requestedType == "bool") {
         newField = std::make_unique<ROOT::Internal::RDF::RRDFCardinalityField<bool>>(strName);
      } else if (requestedType == "char") {
         newField = std::make_unique<ROOT::Internal::RDF::RRDFCardinalityField<char>>(strName);
      } else if (requestedType == "std::int8_t") {
         newField = std::make_unique<ROOT::Internal::RDF::RRDFCardinalityField<std::int8_t>>(strName);
      } else if (requestedType == "std::uint8_t") {
         newField = std::make_unique<ROOT::Internal::RDF::RRDFCardinalityField<std::uint8_t>>(strName);
      } else if (requestedType == "std::int16_t") {
         newField = std::make_unique<ROOT::Internal::RDF::RRDFCardinalityField<std::int16_t>>(strName);
      } else if (requestedType == "std::uint16_t") {
         newField = std::make_unique<ROOT::Internal::RDF::RRDFCardinalityField<std::uint16_t>>(strName);
      } else if (requestedType == "std::int32_t") {
         newField = std::make_unique<ROOT::Internal::RDF::RRDFCardinalityField<std::int32_t>>(strName);
      } else if (requestedType == "std::uint32_t") {
         newField = std::make_unique<ROOT::Internal::RDF::RRDFCardinalityField<std::uint32_t>>(strName);
      } else if (requestedType == "std::int64_t") {
         newField = std::make_unique<ROOT::Internal::RDF::RRDFCardinalityField<std::int64_t>>(strName);
      } else if (requestedType == "std::uint64_t") {
         newField = std::make_unique<ROOT::Internal::RDF::RRDFCardinalityField<std::uint64_t>>(strName);
      } else {
...
{
   assert(fNSlots == 0);
   // FIXME(fdegeus): Set to actual nSlots once MT support lands
   if (nSlots > 1)
      Warning("RNTupleProcessorDS", "MT is not yet supported. Using single-threaded processing instead.");
   fNSlots = 1;
}

bool ROOT::Experimental::RDF::RNTupleProcessorDS::SetEntry(unsigned int /* slot */, ULong64_t entry)
{
   if (fProcessor->LoadEntry(entry) == ROOT::kInvalidNTupleIndex)
      return false;

   for (auto colReader : fColumnReaders) {
      colReader->SetEntryLoaded(entry);
   }
...
{
   const auto &ntupleID = fProcessor->GetOptions().GetProcessorName();
