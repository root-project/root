...
struct GH20033SameBranchName
   : public ::testing::TestWithParam<
        std::tuple<bool, bool, bool, bool, std::string, std::string, std::string, std::string, std::string>> {
   ...
   {
      auto &&[_1, _2, _3, _4, stepZeroScan, stepOneScan, stepTwoScan, stepThreeScan, stepFourScan] = GetParam();
...
TEST_P(GH20033SameBranchName, Test)
{
   ...
   auto &&[stepOneTChain, stepTwoTChain, stepThreeTChain, stepFourTChain, stepZeroScan, stepOneScan, stepTwoScan,
           stepThreeScan, stepFourScan] = GetParam();
   ...
   if (!stepZeroScan.empty())
      scanExpression += "step0chai.value:";
   ...
   if (!stepOneScan.empty()) {
      if (stepOneTChain)
         scanExpression += "step1chai.value:";
      else
         scanExpression += std::string(stepOneToDiskName) + ".value:";
   }

   if (!stepTwoScan.empty()) {
      if (stepTwoTChain)
         scanExpression += "step2chai.value:";
      else
         scanExpression += std::string(stepTwoToDiskName) + ".value:";
   }

   if (!stepThreeScan.empty()) {
      if (stepThreeTChain)
         scanExpression += "step3chai.value:";
      else
         scanExpression += std::string(stepThreeToDiskName) + ".value:";
   }

   if (!stepFourScan.empty()) {
      if (stepFourTChain)
         scanExpression += "step4chai.value:";
      else
         scanExpression += std::string(stepFourToDiskName) + ".value:";
   }

   if (scanExpression.empty())
      GTEST_SKIP() << "Skipping test with empty TTree::Scan expression";

   if (scanExpression.back() == ':')
      scanExpression.pop_back();

   // Build the expected TTree::Scan value
   ...
   expectedScan += "*    Row   *";
   if (!stepZeroScan.empty())
      expectedScan += " step0chai.value *";
   if (!stepOneScan.empty())
      expectedScan += " " + std::string(stepOneTChain ? "step1chai" : stepOneToDiskName) + ".value *";
   if (!stepTwoScan.empty())
      expectedScan += " " + std::string(stepTwoTChain ? "step2chai" : stepTwoToDiskName) + ".value *";
   if (!stepThreeScan.empty())
      expectedScan += " " + std::string(stepThreeTChain ? "step3chai" : stepThreeToDiskName) + ".value *";
   if (!stepFourScan.empty())
      expectedScan += " " + std::string(stepFourTChain ? "step4chai" : stepFourToDiskName) + ".value *";
   expectedScan += "\n";
...
INSTANTIATE_TEST_SUITE_P(
   ...
   ([](const testing::TestParamInfo<GH20033SameBranchName::ParamType> &paramInfo) {
      auto &&[stepOneTChain, stepTwoTChain, stepThreeTChain, stepFourTChain, stepZeroScan, stepOneScan, stepTwoScan,
              stepThreeScan, stepFourScan] = paramInfo.param;
      // googletest only accepts ASCII alphanumeric characters for labels
      std::string label{};
      if (stepOneTChain)
         label += "StepOneTChain_";
      ...
      if (!stepFourScan.empty())
         label += "StepFourScan_" + stepFourScan;
      else
         label += "StepFourNoScan";
