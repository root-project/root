...
ROOT::Detail::TBranchProxy::TBranchProxy()
   : fDirector(nullptr), fInitialized(false), fIsMember(false), fIsClone(false), fIsaPointer(false),
     fHasLeafCount(false), fBranchName(""), fParent(nullptr), fDataMember(""), fClassName(""), fClass(nullptr),
     fElement(nullptr), fMemberOffset(0), fOffset(0), fArrayLength(1), fBranch(nullptr), fBranchCount(nullptr),
     fLastTree(nullptr), fRead(-1), fWhere(nullptr), fCollection(nullptr), fCurrentTreeNumber(-1){
                                                                              // Constructor.
                                                                           };

ROOT::Detail::TBranchProxy::TBranchProxy(TBranchProxyDirector *boss, const char *top, const char *name)
   : fDirector(boss), fInitialized(false), fIsMember(false), fIsClone(false), fIsaPointer(false), fHasLeafCount(false),
     fBranchName(top), fParent(nullptr), fDataMember(""), fClassName(""), fClass(nullptr), fElement(nullptr),
     fMemberOffset(0), fOffset(0), fArrayLength(1), fBranch(nullptr), fBranchCount(nullptr), fLastTree(nullptr),
     fRead(-1), fWhere(nullptr), fCollection(nullptr), fCurrentTreeNumber(-1)
{
   // Constructor.
...
ROOT::Detail::TBranchProxy::TBranchProxy(TBranchProxyDirector *boss, const char *top, const char *name,
   ...
   : fDirector(boss), fInitialized(false), fIsMember(true), fIsClone(false), fIsaPointer(false), fHasLeafCount(false),
     fBranchName(top), fParent(nullptr), fDataMember(membername), fClassName(""), fClass(nullptr), fElement(nullptr),
     fMemberOffset(0), fOffset(0), fArrayLength(1), fBranch(nullptr), fBranchCount(nullptr), fLastTree(nullptr),
     fRead(-1), fWhere(nullptr), fCollection(nullptr), fCurrentTreeNumber(-1)
{
   // Constructor.
...
ROOT::Detail::TBranchProxy::TBranchProxy(TBranchProxyDirector *boss, Detail::TBranchProxy *parent,
                                         const char *membername, const char *top, const char *name)
   : fDirector(boss), fInitialized(false), fIsMember(true), fIsClone(false), fIsaPointer(false), fHasLeafCount(false),
     fBranchName(top), fParent(parent), fDataMember(membername), fClassName(""), fClass(nullptr), fElement(nullptr),
     fMemberOffset(0), fOffset(0), fArrayLength(1), fBranch(nullptr), fBranchCount(nullptr), fLastTree(nullptr),
     fRead(-1), fWhere(nullptr), fCollection(nullptr), fCurrentTreeNumber(-1)
{
   // Constructor.
...
ROOT::Detail::TBranchProxy::TBranchProxy(TBranchProxyDirector *boss, TBranch *branch, const char *membername)
   : fDirector(boss), fInitialized(false), fIsMember(membername != nullptr && membername[0]), fIsClone(false),
     fIsaPointer(false), fHasLeafCount(false), fBranchName(branch->GetName()), fParent(nullptr),
     fDataMember(membername), fClassName(""), fClass(nullptr), fElement(nullptr), fMemberOffset(0), fOffset(0),
     fArrayLength(1), fBranch(nullptr), fBranchCount(nullptr), fLastTree(nullptr), fRead(-1), fWhere(nullptr),
     fCollection(nullptr), fCurrentTreeNumber(-1)
{
   // Constructor.
