...
// First loop on all leaves to generate dimension declarations
   ...
   for (l=0;l<nleaves;l++) {
      TLeaf *leaf = (TLeaf*)leaves->UncheckedAt(l);
      strncpy(blen,leaf->GetName(),1023); blen[1023]=0;
      bname = &blen[0];
      while (*bname) {
         if (*bname == '.') *bname='_';
         if (*bname == ',') *bname='_';
...
// second loop on all leaves to generate type declarations
   ...
   for (l=0;l<nleaves;l++) {
      ...
      TBranch *branch = leaf->GetBranch();
      branchname[0] = 0;
      strncpy(branchname,branch->GetName(),1023); branchname[1023]=0;
      strncpy(aprefix,branch->GetName(),1023); aprefix[1023]=0;
      ...
      if (leafcount) {
         ...
         if (len<=0) len = 1;
         strncpy(blen,leafcount->GetName(),1023); blen[1023]=0;
         bname = &blen[0];
         while (*bname) {
            if (*bname == '.') *bname='_';
            if (*bname == ',') *bname='_';
      ...
      if (branch->IsA() == TBranchElement::Class()) {
         ...
         if (bre->GetStreamerType() > 60) {
            ...
            char brename[256];
            strncpy(brename,bre->GetName(),255); brename[255]=0;
            char *bren = brename;
      ...
      if (leafcount) {
         ...
         if (bre && bre->GetBranchCount2()) {
            ...
            strncpy(b2len,l2->GetName(),1023); b2len[1023]=0;
            bname = &b2len[0];
            while (*bname) {
               if (*bname == '.') *bname='_';
               if (*bname == ',') *bname='_';
