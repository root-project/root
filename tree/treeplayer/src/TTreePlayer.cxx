...
// First loop on all leaves to generate dimension declarations
   ...
   for (l=0;l<nleaves;l++) {
      TLeaf *leaf = (TLeaf*)leaves->UncheckedAt(l);
      strlcpy(blen,leaf->GetName(),1023); 
      bname = &blen[0];
      while (*bname) {
         if (*bname == '.') *bname='_';
         if (*bname == ',') *bname='_';
...
// second loop on all leaves to generate type declarations
   ...
   for (l=0;l<nleaves;l++) {
      ...
      TBranch *branch = leaf->GetBranch();
      branchname[0] = 0;
      strlcpy(branchname,branch->GetName(),1023); 
      strlcpy(aprefix,branch->GetName(),1023); 
      ...
      if (leafcount) {
         ...
         if ( twodim || dimInName ) {
            ...
            if (dimInName) {
               strlcpy(dimensions,dimInName,dimlen-1); 
