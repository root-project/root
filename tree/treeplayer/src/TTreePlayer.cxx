...
// First loop on all leaves to generate dimension declarations
   ...
   for (l=0;l<nleaves;l++) {
      TLeaf *leaf = (TLeaf*)leaves->UncheckedAt(l);
      strlcpy(blen,leaf->GetName(),1023); blen[1023]=0;
      bname = &blen[0];
      while (*bname) {
         if (*bname == '.') *bname='_';
         if (*bname == ',') *bname='_';
...
// second loop on all leaves to generate type declarations
   ...
   for (l=0;l<nleaves;l++) {
      ...
      TBranch *branch = leaf->GetBranch();
      branchname[0] = 0;
      strlcpy(branchname,branch->GetName(),1023); branchname[1023]=0;
      strlcpy(aprefix,branch->GetName(),1023); aprefix[1023]=0;
      if (!branches.FindObject(branch)) branches.Add(branch);
      else leafStatus[l] = 1;
      if ( branch->GetNleaves() > 1) {
         // More than one leaf for the branch we need to distinguish them
         strcat(branchname,".");
         strcat(branchname,leaf->GetTitle()); branchname[1023]=0;
         if (leafcount) {
      ...
      if (leafcount) {
         ...
         if (len<=0) len = 1;
         strlcpy(blen,leafcount->GetName(),1023); blen[1023]=0;
         bname = &blen[0];
         while (*bname) {
            if (*bname == '.') *bname='_';
            if (*bname == ',') *bname='_';
      ...
      if (branch->IsA() == TBranchElement::Class()) {
         ...
         if (bre->GetStreamerType() > 60) {
            ...
            char brename[256];
            strlcpy(brename,bre->GetName(),255); brename[255]=0;
            char *bren = brename;
      ...
      if (leafcount) {
         //len = leafcount->GetMaximum();
         //strlcpy(blen,leafcount->GetName(),1023);
         //bname = &blen[0];
         ...
         if ( twodim || dimInName ) {
            ...
            if (dimInName) {
               strlcpy(dimensions,dimInName,dimlen); dimensions[dimlen]=0;
         ...
         if (bre && bre->GetBranchCount2()) {
            ...
            strlcpy(b2len,l2->GetName(),1023); b2len[1023]=0;
            bname = &b2len[0];
            while (*bname) {
               if (*bname == '.') *bname='_';
               if (*bname == ',') *bname='_';
   ...
   if (mustInit.Last()) {
      ...
      while( (obj = next()) ) {
         if (obj->InheritsFrom(TBranch::Class())) {
            strlcpy(branchname,((TBranch*)obj)->GetName(),1023);
         } else if (obj->InheritsFrom(TLeaf::Class())) {
            strlcpy(branchname,((TLeaf*)obj)->GetName(),1023); 
         }
         branchname[1023]=0;
         bname = branchname;
         while (*bname) {
...
// First loop on all leaves to generate type declarations
   ...
   for (l=0;l<nleaves;l++) {
      ...
      if ( branch->GetNleaves() > 1) {
         ...
         if (leafcount) strlcpy(branchname,branch->GetName(),1023);
         else           strlcpy(branchname,leaf->GetTitle(),1023);
         branchname[1023]=0;
      }
      char *twodim = (char*)strstr(leaf->GetTitle(),"][");
      bname = branchname;
