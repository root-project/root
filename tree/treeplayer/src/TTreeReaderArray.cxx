...
void ROOT::Internal::TTreeReaderArrayBase::CreateProxy()
{
   ...
   if (!myLeaf) {
      ...
      auto matchingArrayInfo = [](TDictionary *requestedDict, const StreamerElementArrayInfo &info) {
         ...
         // This part of the logic currently supports the specific use cases where there is only one streamer element
         // corresponding to the array data member that we are trying to partially read into a collection. Without
         // further use cases surfacing, we do not support more than one streamer element
...
ROOT::Internal::TTreeReaderArrayBase::StreamerElementArrayInfo
ROOT::Internal::TTreeReaderArrayBase::FillStreamerElementArrayInfo(TStreamerElement *element)
{
   StreamerElementArrayInfo arrInfo{};
   if (!element)
      return arrInfo;
   ...
   if (branch->IsA() == TBranchElement::Class()) {
      ...
      if (brElement->GetType() == 4 || brElement->GetType() == 3) {
         ...
         if (ExpectedTypeRet == 0) {
            ...
            arrInfo = FillStreamerElementArrayInfo(brElement->GetInfo()->GetElement(brElement->GetID()));
            return nullptr;
         ...
         if (id >= 0) {
            TStreamerElement *element = (TStreamerElement *)streamerInfo->GetElements()->At(id);
            // Fill information about the data member being an n-dim array
            arrInfo = FillStreamerElementArrayInfo(element);
            if (element->IsA() == TStreamerSTL::Class()) {
