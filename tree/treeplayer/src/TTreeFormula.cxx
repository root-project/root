...
TTreeFormula::TTreeFormula()
   : ROOT::v5::TFormula(), fQuickLoad(kFALSE), fNeedLoading(kTRUE), fDidBooleanOptimization(kFALSE),
     fDimensionSetup(nullptr)
...
TTreeFormula::TTreeFormula(const char *name, const char *expression, TTree *tree)
   : ROOT::v5::TFormula(), fTree(tree), fQuickLoad(kFALSE), fNeedLoading(kTRUE), fDidBooleanOptimization(kFALSE),
     fDimensionSetup(nullptr)
{
...
TTreeFormula::TTreeFormula(const char *name, const char *expression, TTree *tree,
   ...
   : ROOT::v5::TFormula(), fTree(tree), fQuickLoad(kFALSE), fNeedLoading(kTRUE), fDidBooleanOptimization(kFALSE),
     fDimensionSetup(nullptr), fAliasesUsed(aliases)
{
   Init(name,expression);
}
   ...
   if (leaf == nullptr) {
      ...
      if (cl && branchEl->GetBranchCount()) {
         ...
         if (useLeafCollectionObject) {
            ...
            if (cl->GetCollectionProxy()->GetValueClass() == nullptr && cl->GetCollectionProxy()->GetType() > 0) {
         ...
         } else if (!useLeafCollectionObject && elemCl && elemCl->GetCollectionProxy() &&
                  ...
                  case TMethodCall::kOther:
                     {
                        ...
                        leafinfo = new TFormLeafInfoMethod(cl,method);
                        cl = (return_type == "void") ? nullptr : TClass::GetClass(return_type.Data());
                     }
                     break;
                  default:
            ...
            *current = 0;
            ...
            if (cl && cl->GetCollectionProxy()) {
               if (numberOfVarDim>1) {
                  ...
                  R__ASSERT(maininfo);
                  leafinfo = new TFormLeafInfoMultiVarDimCollection(cl, 0, (TStreamerElement *)nullptr, maininfo);
                  previnfo->fNext = leafinfo;
                  previnfo = leafinfo;
                  leafinfo = new TFormLeafInfoCollection(cl,0,cl);
...
TLeaf* TTreeFormula::GetLeafWithDatamember(const char* topchoice, const char* nextchoice, Long64_t readentry) const
{
   ...
   while ((leafcur = (TLeaf*)nextleaf())) {
      ...
      cl = nullptr;
      if (leafcur->InheritsFrom(TLeafObject::Class()) && leafcur->GetBranch()->GetListOfBranches()->Last() == nullptr) {
      ...
      if (clonesinfo) { delete clonesinfo;
         clonesinfo = nullptr;
      }
...
TClass* TTreeFormula::EvalClass(Int_t oper) const
{
   ...
   switch(fLookupType[oper]) {
      ...
      case kMethod:
         return nullptr; // kMethod is deprecated so let's no waste time implementing this.
      case kTreeMember:
      case kDataMember: {
...
void* TTreeFormula::EvalObject(int instance)
{
   ...
   switch (fLookupType[0]) {
      ...
      case kLengthFunc:
      case kIteration:
      case kEntryList: return nullptr;
   }
...
//         case kParameter:    { pos++; tab[pos-1] = fParams[(oper & kTFOperMask)]; continue; }
         ...
         if (newaction == kDefinedVariable) {
            ...
            switch (lookupType) {
               ...
               case kTreeMember: { TREE_EVAL_INIT_LOOP;
                  tab[pos++] = ((TFormLeafInfo *)fDataMembers.UncheckedAt(code))
                                  ->GetTypedValue<T>((TLeaf *)nullptr, real_instance);
                  continue;
               }
