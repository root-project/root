...
class TestReducerMerge:
    ...
    def test_distributed_stddev(self, connection):
        ...
        treename = "tree"
        filename = "test_distributed_stddev_dask.root"
        ROOT.RDataFrame(100).Define("v", "rdfentry_").Snapshot(treename, filename)

        df = Dask.RDataFrame(treename, filename, daskclient=connection)
    ...
    def test_distributed_stats(self, connection):
        ...
        # Create dataset with fixed series of entries
        treename = "tree"
        filename = "test_distributed_stats_dask.root"
        ROOT.RDataFrame(100).Define("v", "static_cast<double>(rdfentry_)").Snapshot(treename, filename)

        df = (
            Dask.RDataFrame(treename, filename, daskclient=connection)
