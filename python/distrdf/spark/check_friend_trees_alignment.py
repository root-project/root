...
TREENAMES = [f"distrdf_check_friend_trees_alignment_spark_tree_{i}.root" for i in range(1, 7)]
FILENAMES = [f"distrdf_check_friend_trees_alignment_spark_file_{i}.root" for i in range(1, 7)]
...
class TestSparkFriendTreesAlignment:
    ...
    def test_sum_friend_column(self, connection):
        ...
        for nparts in range(1, 11):
            df = Spark.RDataFrame(chain, sparkcontext=connection, npartitions=nparts)
            s1 = df.Sum("x")
            s2 = df.Sum("friend.x")
            s1val = s1.GetValue()
            s2val = s2.GetValue()
