...
## \notebook -nodraw
##
## Test performance of all ROOT random generators (TRandom, TRandom1, TRandom2 and TRandom3).
## This script tests the generator TRandom3 against some ref values,
## and creates a timing table against TRandom, TRandom1 and TRandom2.
##
## E.g.: on an "Intel Xeon Quad-core Harpertown (E5410) 2.33 GHz" running
## "Linux SLC4 64 bit", and having the gcc 3.4 compiler.
##
## ~~~
...
## Rannor............  118.000  220.000  120.000  124.000 # FIXME
...
## ~~~
##
## Note that this tutorial has to be executed in interpreted mode.
##
## ~~~{.py}
##  IP[0]:  %run testrandom.py
## ~~~
##
## \macro_output
## \macro_code
...
TRandomMixMax = ROOT.TRandomMixMax
TRandomMixMax17 = ROOT.TRandomMixMax17
TRandomMixMax256 = ROOT.TRandomMixMax256
TRandomMT64 = ROOT.TRandomMT64
TRandomRanlux48 = ROOT.TRandomRanlux48
TRandom1 = ROOT.TRandom1 
TRandom2 = ROOT.TRandom2 
TRandom3 = ROOT.TRandom3 
TRandomGen = ROOT.TRandomGen 
TStopwatch = ROOT.TStopwatch 

TF1 = ROOT.TF1 
# handy check
try :
   TUnuran = ROOT.TUnuran 
except AttributeError:
   print("TUnuran is not part of your ROOT-installation-package.")
   print("Get ROOT binaries installation > 6.30 or try to built it by yourself")
   print("   using math-more library.")
   sys.exit()

TUnuranContDist = ROOT.TUnuranContDist 
TFile = ROOT.TFile 
...
def l(function, *args):
   #myvars = [x for x in locals() if not x.startswith("_")]
   myvars = [x for x in locals() if not x.startswith("_")]
   print(myvars)
...
# void 
def printf(FormatString, *args):
   print( FormatString % args, end='')
   return 
 
   s = FormatString.split("%")
   N = len(s) - 1 
   string = "Error at Formating String."
   if len(args) < len(s) - 1 :
      tmp = ""
      ...
      string = tmp%args 
      for i in range(len(args)+1, len(s)):
          string += "%"+s[i] 
      
   else:
     string = FormatString%args[:N] 
   print(string, end="") 

# void
def testAll() :
   #Note: 
   #      Because a non-deallocating problem raised at r2.Rannor(x,y),
   #      we will disable the garbage collector a while till the end of testAll().

   # Enabling garbage collector manually.
   #global gc
   #gc.enable() # garmanually.
   #Note:
   #      RANNOR causes problem at highly elevated N numbers.
   #      For N = 2000, RANNOR test functions properly. 
   #      For N = 20000, RANNOR test functions badly. 
   #      Other Random-TESTS function normally either large(E106) 
   #      or lower(E103) N numbers. 

   global N, cpn, N1, x, y, rsave
   global r0, r1, r2, r3, r4, r5, r6, r7, r8, r9, r10
   global sw
   N = 2000000 # High performance.
   N = 200000 # High performance.
   #N = 20000   # Lower performance.
   #N = 2000  # Lower performance.
   cpn = 1000000000./N
   N1 = Int_t(N/10)
   cpn1 = Float_t(cpn*10) # for TRandom1
   x,y = [Double_t() for _ in range(2)]
   global gRandom
   rsave = gRandom
   r0 = TRandom()
   r1 = TRandom1()
   ...
   r4 = TRandomMixMax()
   r5 = TRandomMixMax17()
   r6 = TRandomGen( ROOT.Math.MixMaxEngine(256,0) )()
   r7 = TRandomMixMax256()
   r8 = TRandomGen( ROOT.Math.MixMaxEngine(256,4) )()
   r9 = TRandomMT64()
   r10 = TRandomRanlux48()
   ...
   NR = 1000
   rn = [Double_t() for _ in range(NR)]
   c_rn = (c_double * len(rn))(*rn)
   sw.Start()
   #for (i=0;i<N;i+=NR
   for i in range(0,N,NR):
      r0.RndmArray(NR,c_rn)
   printf("RndmArray......... %8.3f",sw.CpuTime()*cpn)


   sw.Start()
   #for (i=0;i<N1;i+=NR
   for i in range(0,N1,NR):
      r1.RndmArray(NR, c_rn)
   printf(" %8.3f",sw.CpuTime()*cpn1)
...
def testRandom3() :
   ...
   # running using a seed of 4357 ROOT 5.13.07 and checked with GSL 1.8
   RefValue = [ 
   ...
   file = TFile("random3.root","RECREATE")
   file.SetCompressionLevel(0)
   r.Write("r")
   del file

   file = TFile("random3.root")
   rs = file.Get("r") # (TRandom3) 
   #for (i=0;i<1000;i++)
   for i in range(0, 1000):
...
def testRANNOR():
   ...
   #N = 200000 # High performance.
   #N = 20000   # Lower performance.
   N = 1000  # Lower performance. But functions properly! No crashes, no leaks.
   cpn = 1000000000./N
   N1 = Int_t(N/10)
   cpn1 = Float_t(cpn*10) # for TRandom1
   x,y = [Double_t() for _ in range(2)]
   ...
   #Note: IMPORTANT
   #      In case you are wondering about the double-definition of Random-objects
   #      or why the separation of RANNOR teset as an independent test from testAll().
   #      Well, RANNOR-test crashes for 1000000-N-loop. It goes well for N<1000 but 
   #      no for N>10000.   
   #      After using ctypes.c_double() as an input or using definitions on 
   #      ROOT.gInterpreter with ProcessLine(the nested root loop of python), 
   #      it crashes; it really does, by all means of creativity process trying 
   #      to tackle this, it really crashes.
   #      We are leaving here the python-definitions used to prove the efforts at
   #      trying in. 
   #      Two principal errors(or bugs, whatever you like to call it) happens:
   #
   #         1) cling JIT session error: Cannot allocate memory
   #         2) Fatal Python error: 
   #            none_dealloc: deallocating None: 
   #            bug likely caused by a refcount error in a C extension
   #            Python runtime state: initialized
   #      We don't not yet the problem. But is related to the automatically
   #      deallocation of Python.
   #      Hope we can fix this.
   #      For now, so, let's leave it with N = 1000 for RANNOR-test.
   ...
   for i in range(0,N,2):
      ...
      pass
      #r0.Rannor(x,y)
      #if i < 61408:  #in between.
      #if i < 100000: #Problem.
      #if i < 10000:  #No-problem.
      #FIXME:
      if i < 1000:
         r0.Rannor( x, y) # bad
   printf("Rannor............ %8.3f",sw.CpuTime()*cpn)

   ##Alternative Definitions to test Rannor.
   #ProcessLine("""
   ...
   #Double_t y_ref = *y;
   #""")
   #def loop(N, RandomClass="r0", RandomMethod="Rannor"):
   #   Declaration = f"{RandomClass}->{RandomMethod}( *x, *y ) ;"
   #   i = 0 
   #   while(True):
   #      if i < N: 
   #         #ROOT.r0.Rannor( ROOT.x, ROOT.y) # good N<1000; bad N>10000
   #         
   #         #ProcessLine("""
   #         #r0->Rannor( *x, *y );
   #         #""")
   ...
   #loop(N=2000000) #Persists the issue.
   ##loop(N=20000)     # Bad.
   ##loop(N=2000)     # Good.
   ##loop(N=20)     # Good.
