...
void Read()
{
   ...
   // Access to the processor's fields is done by first registering them through RNTupleProcessor::RegisterField(). The
   // returned value can be used to read the current entry's value for that particular field. Fields from the primary
   // ntuple are registered by their original name.
   auto px = processor->RegisterField<float>("vpx");
   // Fields from auxiliary ntuples are registered by prepending the name of the auxiliary ntuple.
   auto py = processor->RegisterField<float>(kAuxNTupleName + ".vpy");
