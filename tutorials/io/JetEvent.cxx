...
JetEvent::JetEvent() {
  if (!fgTracks)
    fgTracks = new TClonesArray("Track", 100);
  if (!fgJets)
    fgJets = new TClonesArray("Jet", 10);
  if (!fgHitsA)
    fgHitsA = new TClonesArray("Hit", 10000);
  if (!fgHitsB)
    fgHitsB = new TClonesArray("Hit", 1000);
  fJets = fgJets;
  fTracks = fgTracks;
  fHitsA = fgHitsA;
  fHitsB = fgHitsB;
...
void JetEvent::Build(Int_t jetm, Int_t trackm, Int_t hitam, Int_t hitbm) {
  ...
  fVertex.SetXYZ(gRandom->Gaus(0, 0.1), gRandom->Gaus(0, 0.2),
                 gRandom->Gaus(0, 10));

  Int_t njets = (Int_t)gRandom->Gaus(jetm, 1);
  if (njets < 1)
    njets = 1;
  for (Int_t j = 0; j < njets; j++) {
    ...
    Int_t ntracks = (Int_t)gRandom->Gaus(trackm, 3);
    if (ntracks < 1)
      ntracks = 1;
