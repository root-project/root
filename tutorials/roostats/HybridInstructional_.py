...
PValueToSignificance = RooStats.PValueToSignificance
...
''' 
ROOT.gInterpreter.Declare( long_declaration )
# After this, BinCountTestStat can be found in ROOT.BinCountTestStat 
...
class BinCountTestStat(TestStatistic):
   ...
   def __init__(self, columnName):
      super(TestStatistic).__init__()
      self._type = TestStatistic
      self.fColumnName = TString(columnName) 
   ...
   def Evaluate(self, data, nullPOI=nullptr):
      if not (data is RooAbsData) :
         #raise TypeError
         print("data is not a RooAbsData")
         return 
      # Essential method in BinCountTestStat 
      value = 0.0
   ...
   def GetVarName(self):
      return self.fColumnName #"tmp" #TString("tmp") # self.fColumnName
   def __del__(self):
      del self
...
"""
# snippet of code with output:
"""
In [264]: %run HybridInstructional.py
input_line_287:50:43: error: cannot initialize a parameter of type 'void *' with an rvalue of type 'const TString *'
        cOk = retconv->ToMemory(pyresult, &ret);
                                          ^~~~
/root_install/include/CPyCppyy/API.h:102:50: note: passing argument to parameter 'address' here
    virtual bool ToMemory(PyObject* value, void* address, PyObject* ctxt = nullptr);
                                                 ^
---------------------------------------------------------------------------
TypeError                                 Traceback (most recent call last)
File roostats/HybridInstructional.py:135
--> 135 class BinCountTestStat(TestStatistic):
    136 #def BinCountTestStat(columnName): 
    137    #public:
    139    def __init__(self):
    140       super(TestStatistic).__init__()

TypeError: no python-side overrides supported (failed to compile the dispatcher code)

Note: The problem is in GetVarName() method. If no definition is given in, goes well. However we will such a method
...
def HybridInstructional():
   ...
   print(f"-----------------------------------------")
   print(f"Part 2")
   print("Hybrid p-value from direct integration = ", 1 - cdf.getVal())
   print("Z_Gamma Significance  = ", PValueToSignificance(1 - cdf.getVal()))
   ...
   w.var("s").setVal(0.0) # importantnot
   ...
   w.var("s").setVal(50.0) # importantnot
   ...
   w.var("x").setVal(150.)
   w.var("y").setVal(100.)
   dataXY =  RooDataSet("dXY", "dXY", w.set("obsXY"))
   dataXY.add(w.set("obsXY"))
   
   # now we need new model configs, with PDF="model"
   b_modelXY = ModelConfig("B_modelXY", w)
   ...
   # create the alternate (signal+background) ModelConfig with s=50
   sb_modelXY = ModelConfig("S+B_modelXY", w)
   ...
   ropl = RatioOfProfiledLikelihoodsTestStat(b_modelXY.GetPdf(), sb_modelXY.GetPdf(), sb_modelXY.GetSnapshot())
   ropl.SetSubtractMLE(False)
   
   # profile likelihood where alternate is best fit value of signal yield
   # $\lambda(0) = -log L(s=0,\hat\hat{b})/L(\hat{s},\hat{b})$
   profll = ProfileLikelihoodTestStat(b_modelXY.GetPdf())
   
   # just use the maximum likelihood estimate of signal yield
   # $MLE = \hat{s}$
   mlets = MaxLikelihoodEstimateTestStat(sb_modelXY.GetPdf(), w.var("s"))
   ...
   # HYBRID CALCULATOR
   hc3 = HybridCalculator(dataXY, sb_modelXY, b_modelXY)
   ...
   c.cd(4)
   c.GetPad(4).SetLogy()
   p3 =  HypoTestPlot(r3, 50) # 50 bins
   p3.Draw()
   
   c.SaveAs("HybridInstructional.1.zbi.png")
