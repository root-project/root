...
class HypoTestInvOptions:
   ...
   resultFileName = TString() # file with results (by default is built automatically using the workspace input file name)
...
class HypoTestInvTool_plus(RooStats.HypoTestInvTool):
   ...
   def AnalyzeResult(self, r, calculatorType, testStatType,
      useCLs, npoints, fileNameBase):
      ...
      if (r != ROOT.kNone and self.mWriteResult):
         ...
         if ulDist:
            ulDist.Write()
         Info("StandardHypoTestInvDemo", "HypoTestInverterResult has been written in the file %s", self.mResultFileName.Data())
   ...
   def RunInverter(self, w, modelSBName,
      modelBName, dataName, Type,
      testStatType, useCLs, npoints,
      poimin, poimax, ntoys,
      ...
      Info("StandardHypoTestInvDemo", "Using {} as minimizer for computing the test statistic".format(
      ...
      if doFit:
         ...
         fitres = sbModel.GetPdf().fitTo(
            data, InitialHesse(False), Hesse(False), Minimizer(str(self.mMinimizerType), "Migrad"), Strategy(0),
            PrintLevel(self.mPrintLevel), Constrain(constrainParams), Save(True), Offset(RooStats.IsNLLOffset()))
   
         if (fitres.status() != 0):
             Warning("StandardHypoTestInvDemo",
             "Fit to the model failed - try with strategy 1 and perform first an Hesse computation")
             fitres = sbModel.GetPdf().fitTo(
             data, InitialHesse(True), Hesse(False), Minimizer(self.mMinimizerType, "Migrad"), Strategy(1),
             PrintLevel(self.mPrintLevel + 1), Constrain(constrainParams), Save(True), Offset(RooStats.IsNLLOffset()))
         ...
         sbModel.SetSnapshot(sbModel.GetParametersOfInterest())
         print(f"StandardHypoTestInvo: snapshot of S+B Model ", sbModel.GetName(),
      ...
      # Asimov data generated with nominal values
         ...
         hc =  AsymptoticCalculator(data, bModel, sbModel,
         True) # for using Asimov data generated with nominal values
      else:
         Error("StandardHypoTestInvDemo", "Invalid - calculator type = {Type}supported values are only :\n\t\t\t 0 "+\
      ...
      if (testStat == 0):
         Error("StandardHypoTestInvDemo", "Invalid - test statistic type = {testStatType} supported values are only :\n\t\t\t 0 (SLR) " + \
      ...
      if (toymcs and (Type == 0 or Type == 1)) :
         ...
         if data.isWeighted() and not self.mGenerateBinned:
            Info("StandardHypoTestInvDemo", ("Data set is weighted, nentries = {} and sum of weights = {:8.1f} but toy" + \
         ...
         if self.mGenerateBinned and sbModel.GetObservables().getSize() > 2:
            Warning("StandardHypoTestInvDemo", ("generate binned is activated but the number of observable is {}. Too much " + \
            "memory could be needed for allocating all the bins").format( 
      ...
      if (Type == 1):
         ...
         if bModel.GetNuisanceParameters() or sbModel.GetNuisanceParameters():
            ...
            nuisParams = \
               ibModel.GetNuisanceParameters() if (bModel.GetNuisanceParameters()) else ( sbModel.GetNuisanceParameters())
      ...
      if self.mRebuild:
         ...
         print(f"\n\n")
         print(f"Rebuild the upper limit distribution by re-generating new set of pseudo-experiment and re-compute " + \
         ...
         if (self.mRebuildParamValues == 0 or self.mRebuildParamValues == 1):
            ...
            if (self.mRebuildParamValues == 0):
               ...
               Minimizer(self.mMinimizerType, "Migrad"), Strategy(0), PrintLevel(self.mPrintLevel),
         ...
         if limDist:
            ...
            print(f"expected upper limit  (median of limit distribution) ", 
            limDist.InverseCDF(0.5))
            print(f"expected -1 sig limit (0.16% quantile of limit dist) ", 
            limDist.InverseCDF(ROOT.Math.normal_cdf(-1)))
            print(f"expected +1 sig limit (0.84% quantile of limit dist) ", 
            limDist.InverseCDF(ROOT.Math.normal_cdf(1)))
            print(f"expected -2 sig limit (.025% quantile of limit dist) ", 
            limDist.InverseCDF(ROOT.Math.normal_cdf(-2)))
            print(f"expected +2 sig limit (.975% quantile of limit dist) ", 
            limDist.InverseCDF(ROOT.Math.normal_cdf(2)))
...
#-------------------------------------------------- 
def StandardHypoTestInvDemo(infile = 0, wsName = "combined",
                             modelSBName = "ModelConfig", modelBName = "",
                             dataName = "obsData", calculatorType = 0, testStatType = 0,
                             useCLs = True, npoints = 6, poimin = 0, poimax = 5,
                             ntoys = 1000, useNumberCounting = False, nuisPriorName = 0):
