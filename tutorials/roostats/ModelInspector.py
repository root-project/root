...
class ModelInspectorGUI (TGMainFrame):
   ...
   def DoSlider(self):
      ...
      if not simPdf:
         ...
         for it in ( self.fSliderMap):
            ...
            self.fLabelMap[it.first].SetText(Form("{:s} = {:.3}f [{:.3}f,{:.3}f]".format( param.GetName(), it.first.GetPointerPosition(), \
         ...
         for tt in channelCat:
            ...
            #"""
            self.fData.plotOn(self.fPlot, MarkerSize(1), \
            ...
            for it in (self.fSliderMap ):
               ...
               self.fLabelMap[it.first].SetText(Form("{:s} = {:.3f} [{:.3f},{:.3f}]".format( param.GetName(), \
            ...
            #normCount = pdftmp.expectedEvents(RooArgSet(obs))
            normCount = pdftmp.expectedEvents(obstmp)
            pdftmp.plotOn(self.fPlot, LineColor(kGreen), LineWidth(2), Normalization(normCount, RooAbsReal.NumEvent))
            ...
            if not self.fFitRes:
               ...
               global gpdftmp
               gpdftmp = pdftmp
               pdftmp.plotOn(self.fPlot, LineColor(kBlue), LineWidth(2), Normalization(normCount, RooAbsReal.NumEvent))
               #pdftmp.plotOn(self.fPlot)
            else:
               pdftmp.plotOn(self.fPlot, Normalization(normCount, RooAbsReal.NumEvent), \
                              VisualizeError(self.fFitRes, self.fMC.GetNuisanceParameters()), FillColor(kYellow))
               pdftmp.plotOn(self.fPlot, LineColor(kBlue), LineWidth(2), Normalization(normCount, RooAbsReal.NumEvent))
               msglevel = RooMsgService.instance().globalKillBelow()
               RooMsgService.instance().setGlobalKillBelow(RooFit.WARNING)
               self.fData.plotOn(self.fPlot, MarkerSize(1), \
   ...
   def __init__(self, w, mc, data):
      ...
      for param in parameters:
         ...
         hlabel = \
         TGLabel(hframek, Form("{:s} = {:.3f} +{:.3f}".format(param.GetName(), param.getVal(), param.getError() ) ))

         hsliderk =  TGTripleHSlider(hframek, 0, kDoubleScaleBoth, ETestCmdId.HSId1.value, kHorizontalFrame, \
         self.GetDefaultFrameBackground(), kFALSE, kFALSE, kFALSE, kFALSE)
...
#---------------------------------------------------------------------------------------------------- 
def ModelInspector(infile = "", workspaceName = "combined", \
...
ModelInspector(infile = "", workspaceName = "combined", \
                   modelConfigName = "ModelConfig", dataName = "obsData")
