...
def AddModel(wks):
    ...
    sigma1 = RooRealVar("sigma1", "Width of Gaussian", 12.0, 2, 100)
    ...
    sigma1_z = RooRealVar("sigma1_z", "Width of Gaussian", 10.0, 6, 100)
    ...
    fzjj = RooRealVar("fzjj", "fraction of zjj background events", 0.4, 0.0, 1)

    # Set the expected fraction of signal to 20%.
    fsigExpected = RooRealVar("fsigExpected", "expected fraction of signal events", 0.2, 0.0, 1)
    ...
    mu = RooRealVar("mu", "signal strength in units of SM expectation", 1, 0.0, 2)
    ...
    ratioSigEff = RooRealVar("ratioSigEff", "ratio of signal efficiency to nominal signal efficiency", 1.0, 0.0, 2)
    ...
    # full model
    model = RooAddPdf(
        "model", "sig+zjj+qcd background shapes", RooArgList(sigModel, zjjModel, qcdModel), RooArgList(fsig, fzjj)
    )
