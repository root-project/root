...
BayesianNumericalOptions = Struct( \
   # interval CL
   confLevel = 0.95, \
   # integration Type (default is adaptive (numerical integration)
   ...
   integrationType = "", \
   # number of toys used for the MC integrations - for Vegas should be probably set to an higher value
   nToys = 10000, \
   # flag to compute interval by scanning posterior (it is more robust but maybe less precise)
   scanPosterior = False, \
   # plot posterior function after having computed the interval
   plotPosterior = True, \
   # number of points for scanning the posterior (if scanPosterior = False it is used only for
   # plotting). Use by default a low value to speed-up tutorial
   nScanPoints = 50, \
   # type of interval (0 is shortest, 1 central, 2 upper limit)
   intervalType = 1, \
   # force a different value of POI for doing the scan (default is given value)
   maxPOI = -999, \
   # force integration of nuisance parameters to be within nSigma of their error (do first
...
def StandardBayesianNumericalDemo(infile = "", workspaceName = "combined", \
   ...
   if nSigmaNuisance > 0:
      ...
      assert(pdf)
      res( \
         pdf.fitTo(data, Save(true), Minimizer(MinimizerOptions.DefaultMinimizerType().c_str()), \
         Hesse(true), PrintLevel(MinimizerOptions.DefaultPrintLevel() - 1)) \
      )
...
StandardBayesianNumericalDemo(infile = "", workspaceName = "combined", \
                                  modelConfigName = "ModelConfig", dataName = "obsData")
