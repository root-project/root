...
HypoTestOptions = Struct( \
   ...
   noSystematics = False , \
   # ratio Ntoys Null/ntoys ALT
   nToysRatio = 4 , \
   # change poi snapshot value for S+B model (needed for expected p0 values)
   poiValue = -1 , \
   printLevel = 0, \
...
modelSBName = "ModelConfig", modelBName = "", \
# 0 freq, 1 hybrid, 2 asymptotic \
dataName = "obsData", calcType = 0, \
# 0 LEP, 1 TeV, 2 LHC, 3 LHC - one sided \
testStatType = 3, \
ntoys = 5000, useNC = False, nuisPriorName = 0) : 
   ...
   ropl = \
   RatioOfProfiledLikelihoodsTestStat(bModel.GetPdf(), sbModel.GetPdf(), sbModel.GetSnapshot())
   ...
   if (calcType  != 2) :
      ...
      for i in range(5) :
         ...
         sig = -2 + i
         print(" Expected p -value and significance at " ,sig , " sigma = " , htExp.NullPValue(), \
               " significance ", htExp.Significance(), "sigma ")
         ...
         for i in range(5):
            ...
            print(" Expected p -value and significance at " , sig , " sigma = " , pval , " significance ", \
                  ROOT.Math.normal_quantile_c(pval, 1) , " sigma ")
...
nuisPriorName = 0 
# Running ...  
StandardHypoTestDemo(infile, workspaceName, modelSBName, modelBName, dataName, calcType, testStatType, ntoys, useNC, nuisPriorName) 
