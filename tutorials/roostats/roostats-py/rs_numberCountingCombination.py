...
def rs_numberCountingCombination_expected():
   ...
   b = [ 100., 100. ]# expected background
   b_c = ( ctypes.c_double *len(b))(*b)
   db = [ .0100, .0100 ]# fractional background uncertainty
   ...
   # because this is a
   plc = ProfileLikelihoodCalculator(wspace.data("ExpectedNumberCountingData"), wspace.pdf("TopLevelPdf"), poi, 0.05, \
   nullParams)
...
def rs_numberCountingCombination_observed():
   ...
   mainMeas = [ 123., 117. ] # observed main measurement
   ...
   dbMeas = [ .011, .0095 ] # observed fractional background uncertainty
   ...
   # Step 5, Create a calculator for doing the hypothesis test.
   # because this is a
   plc = ProfileLikelihoodCalculator(wspace.data("ObservedNumberCountingData"), wspace.pdf("TopLevelPdf"), poi, 0.05,
   nullParams)
...
def rs_numberCountingCombination_observedWithTau():
   ...
   mainMeas = [ 123., 117. ] # observed main measurement
   sideband = [ 11123., 9876. ] # observed sideband
   tau = [ 100., 100. ] # ratio of bkg in sideband to bkg in main measurement, from experimental design.
   ...
   # Step 5, Create a calculator for doing the hypothesis test.
   # because this is a
   plc = ProfileLikelihoodCalculator(wspace.data("ObservedNumberCountingDataWithSideband"), wspace.pdf("TopLevelPdf"),
   poi, 0.05, nullParams)
