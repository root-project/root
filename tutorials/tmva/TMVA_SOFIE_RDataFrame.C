...
void TMVA_SOFIE_RDataFrame(){
   ...
   std::string inputFile = "http://root.cern.ch/files/" + inputFileName;
    ...
    auto h1 = df1.DefineSlot("DNN_Value", SofieFunctor<7,TMVA_SOFIE_Higgs_trained_model::Session>(nthreads), {"m_jj", "m_jjj", "m_lv", "m_jlv", "m_bb", "m_wbb", "m_wwbb"})
                   .Histo1D({"h_sig", "", 100, 0, 1},"DNN_Value");

    ROOT::RDataFrame df2("bkg_tree", inputFile);
    auto h2 = df2.DefineSlot("DNN_Value", SofieFunctor<7,TMVA_SOFIE_Higgs_trained_model::Session>(nthreads), {"m_jj", "m_jjj", "m_lv", "m_jlv", "m_bb", "m_wbb", "m_wwbb"})
