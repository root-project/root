...
TFile *cernbuild(Int_t getFile=0, Int_t print=1)
{
   ...
   dir.Append("/tree/");
   dir.ReplaceAll("/./","/");
   FILE *fp = fopen(TString::Format("%scernstaff.dat", dir.Data()), "r");
   ...
   while (fgets(line, 80, fp)) {
      sscanf(&line[0],"%d %d %d %d %d %d %d  %d %d %s %s",
             &Category, &Flag, &Age, &Service, &Children, &Grade, &Step,
   ...
   delete hfile;
   if (getFile) {
      //cwe come here when the script is executed outside $ROOTSYS/tutorials/tree
