...
void rf611_weightedfits(int acceptancemodel = 2)
{
   ...
   std::array<std::string, 3> paramLabels{"c_{0}", "c_{1}", "N_{tot}"};
   for (std::size_t i = 0; i < 3; ++i) {
      std::string const &iLabel = std::to_string(i);
      std::string const &pLabel = paramLabels[i];
      std::string const &xLabel = "Pull (" + pLabel + "^{fit}-" + pLabel + "^{gen})/#sigma(" + pLabel + ")";
      // using the inverse Hessian matrix
      hc0pull[i] =
         new TH1D(("hc0pull" + iLabel).c_str(), (methodLabels[i] + ";" + xLabel + ";").c_str(), 20, -5.0, 5.0);
      // using the correction with the Hessian matrix with squared weights
      hc1pull[i] =
         new TH1D(("hc1pull" + iLabel).c_str(), (methodLabels[i] + ";" + xLabel + ";").c_str(), 20, -5.0, 5.0);
      // asymptotically correct approach
      hntotpull[i] =
         new TH1D(("hntotpull" + iLabel).c_str(), (methodLabels[i] + ";" + xLabel + ";").c_str(), 20, -5.0, 5.0);
   }
