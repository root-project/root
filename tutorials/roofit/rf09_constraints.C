...
void rf09_constraints()
{
  ...
  // METHOD 1 -- Multiply constraint term with regular p.d.f using RooProdPdf
  ...
  RooFitResult* r1 = modelc.fitTo(*d,Minos(kFALSE),Save()) ;

  // Fit modelc with constraint term on parameter f
  RooFitResult* r2 = modelc.fitTo(*d,Minos(kFALSE),Constrain(f),Save()) ;

  // METHOD 2 -- Specify constraint externally when doing fitTo()
  ...
  RooFitResult* r3 = model.fitTo(*d,Minos(kFALSE),ExternalConstraints(fconstext),Save()) ;
