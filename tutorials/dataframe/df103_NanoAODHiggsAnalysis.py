...
##    its invariant mass.
##
## Another aim of this version of the tutorial is to show a way to blend C++ and Python code.
## All the functions that make computations on data to define new columns or filter existing
## ones in a precise way, better suited to be written in C++, have been moved to a header that
## is then declared to the ROOT C++ interpreter. The functions that instead create nodes of
## the computational graph (e.g. Filter, Define) remain inside the main Python script.
##
## \macro_image
## \macro_code
## \macro_output
...
def reco_higgs_to_2el2mu(df):
    ...
    "Z_mass",
    "compute_z_masses_2el2mu(Electron_pt, Electron_eta, Electron_phi, Electron_mass, Muon_pt, Muon_eta, Muon_phi, Muon_mass)"
    )
    ...
    "H_mass",
    "compute_higgs_mass_2el2mu(Electron_pt, Electron_eta, Electron_phi, Electron_mass, Muon_pt, Muon_eta, Muon_phi, Muon_mass)"
    )
...
def selection_2el2mu(df):
    ...
    df_el_track = df_el_sip3d.Filter("All(Electron_sip3d_el<4) && All(abs(Electron_dxy)<0.5) && All(abs(Electron_dz)<1.0)",
...
def selection_4el(df):
    ...
    df_pv = df_sip3d.Filter("All(Electron_sip3d<4) && All(abs(Electron_dxy)<0.5) && "
...
def df103_NanoAODHiggsAnalysis():
    ...
    df_sig_4l = ROOT.ROOT.RDataFrame(
        "Events",
    ...
    df_data_doublemu = ROOT.ROOT.RDataFrame(
        "Events",
        doublemu_files
    )
    ...
    df_data_doubleel = ROOT.ROOT.RDataFrame(
        "Events",
        doubleel_files
    )
    ...
    df_h_sig_2el2mu = df_sig_2el2mu_reco\
        .Define("weight", "add_weight_higgs_sig_2el2mu()")\
    ...
    df_h_bkg_2el2mu = df_bkg_2el2mu_reco\
        .Define("weight", "add_weight_higgs_bkg_2el2mu()")\
