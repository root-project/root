...
auto Select = [](ROOT::Experimental::TDataFrame& dataFrame) {
   ...
   .Filter([](int ik, std::array_view<float> rstart, std::array_view<float> rend) {
      return rend[ik-1] - rstart[ik-1] > 22; },
           { "ik", "rstart", "rend"})
   .Filter([](int ipi, std::array_view<float> rstart, std::array_view<float> rend) {
...
Double_t fdm5(Double_t *xx, Double_t *par)
{
   ...
   Double_t res = dxbin*(par[0]*pow(x-0.13957, par[1])
                         + par[2] / 2.5066/par[4]*exp(-xp3/2/par[4]/par[4]));
   return res;
}
...
Double_t fdm2(Double_t *xx, Double_t *par)
{
   ...
   Double_t res = dxbin*(par[0]*pow(x-0.13957, 0.25)
                         + par[1] / 2.5066/sigma*exp(-xp3/2/sigma/sigma));
   return res;
}
