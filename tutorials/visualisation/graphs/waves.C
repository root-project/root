...
Double_t interference(Double_t *x, Double_t *par)
{
   ...
   Double_t amplitude = par[0] * (cos(tpi_l * sqrt(x_p2 + ym_p2)) + par[3] * cos(tpi_l * sqrt(x_p2 + yp_p2)));
...
void waves(Double_t d = 3, Double_t lambda = 1, Double_t amp = 10)
{
   ...
   for (Int_t i = 0; i < colNum; i++) {
      ...
      palette[i] =
         TColor::GetColor((Float_t)TMath::Power(level, 0.3), (Float_t)TMath::Power(level, 0.3), (Float_t)0.5 * level);
      // palette[i] = 1001+i;
   }
