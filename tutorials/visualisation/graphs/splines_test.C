...
void splines_test(Int_t np = 23, Double_t a = -0.5, Double_t b = 31)
{
   ...
   dx = xx2 - xx1;
   dy = yy2 - yy1;
   line5 = new TLine(xx1 + dx * 0.3, yy1 + dy * 1.02, xx1 + dx * 0.38, yy1 + dy * 1.02);
   line5->SetLineColor(kRed);
   line5->SetLineWidth(2);
   text5 = new TText(xx1 + dx * 0.4, yy1 + dy * 1.03, "quintic spline");
   text5->SetTextAlign(12);
   text5->SetTextSize(0.04);
   line3 = new TLine(xx1 + dx * 0.67, yy1 + dy * 1.02, xx1 + dx * 0.75, yy1 + dy * 1.02);
   line3->SetLineColor(kGreen);
   line3->SetLineWidth(2);
   text3 = new TText(xx1 + dx * 0.77, yy1 + dy * 1.03, "third spline");
   ...
   for (Int_t nnp = 2; nnp <= np; ++nnp) {
      ...
      if (spline5)
         delete spline5;
      spline5 = new TSpline5("Test", xx, f, nnp, "b1e1b2e2", f->Derivative(a), f->Derivative(b),
      ...
      if (spline3)
         delete spline3;
      spline3 = new TSpline3("Test", xx, yy, nnp, "b1e1", f->Derivative(a), f->Derivative(b));
