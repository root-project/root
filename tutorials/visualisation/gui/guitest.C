...
Int_t mb_button_id[13] = {kMBYes,   kMBNo,     kMBOk,    kMBApply, kMBRetry,  kMBIgnore, kMBCancel,
                          kMBClose, kMBYesAll, kMBNoAll, kMBNewer, kMBAppend, kMBDismiss};
...
TileFrame::TileFrame(const TGWindow *p)
{
   ...
   fFrame = new TGCompositeFrame(p, 10, 10, kHorizontalFrame, TGFrame::GetWhitePixel());
...
void TileFrame::HandleMouseWheel(Event_t *event)
{
   ...
   if (event->fCode == kButton4 || event->fCode == kButton5) {
      ...
      if (fCanvas->GetContainer()->GetHeight())
         page = Int_t(Float_t(fCanvas->GetViewPort()->GetHeight() * fCanvas->GetViewPort()->GetHeight()) /
...
TestMainFrame::TestMainFrame(const TGWindow *p, UInt_t w, UInt_t h)
{
   ...
   for (int i = 0; i < 256; ++i)
      fCanvasWindow->AddFrame(
         new TGFrame(fCanvasWindow->GetContainer(), 32, 32, 0, TColor::RGB2Pixel(0, 0, (i + 1) & 255)),
   ...
   fStatusFrame = new TGCompositeFrame(fMain, 60, 20, kHorizontalFrame | kSunkenFrame);
   ...
   fStatusFrame->AddFrame(fTestButton, new TGLayoutHints(kLHintsTop | kLHintsLeft, 2, 0, 2, 2));
   ...
   fStatusFrame->AddFrame(fColorSel, new TGLayoutHints(kLHintsTop | kLHintsLeft, 2, 0, 2, 2));

   fMain->AddFrame(fStatusFrame, new TGLayoutHints(kLHintsBottom | kLHintsExpandX, 0, 0, 1, 0));
...
void TestMainFrame::HandleMenu(Int_t id)
{
   ...
   case M_TEST_DLG: new TestDialog(gClient->GetRoot(), fMain, 400, 200); break;

   case M_TEST_MSGBOX: new TestMsgBox(gClient->GetRoot(), fMain, 400, 200); break;

   case M_TEST_SLIDER: new TestSliders(gClient->GetRoot(), fMain, 400, 200); break;

   case M_TEST_SHUTTER: new TestShutter(gClient->GetRoot(), fMain, 400, 200); break;

   case M_TEST_DIRLIST: new TestDirList(gClient->GetRoot(), fMain, 400, 200); break;

   case M_TEST_FILELIST: new TestFileList(gClient->GetRoot(), fMain, 400, 200); break;

   case M_TEST_PROGRESS: new TestProgress(gClient->GetRoot(), fMain, 600, 300); break;

   case M_TEST_NUMBERENTRY: new EntryTestDlg(gClient->GetRoot(), fMain); break;
   ...
   case M_TEST_FONTDIALOG: {
      ...
      if (prop.fName != "")
         printf("Selected font: %s, size %d, italic %s, bold %s, color 0x%lx, align %u\n", prop.fName.Data(),
                prop.fSize, prop.fItalic ? "yes" : "no", prop.fBold ? "yes" : "no", prop.fColor, prop.fAlign);
   } break;
...
TestDialog::TestDialog(const TGWindow *p, const TGWindow *main, UInt_t w, UInt_t h, UInt_t options)
{
   ...
   fEc1->GetCanvas()->Connect("ProcessedEvent(Int_t,Int_t,Int_t,TObject*)", "TestDialog", this,
...
void TestDialog::HandleButtons(Int_t id)
{
   ...
   case 103: // add tabs
      ...
      fTab->Layout();
      break;
   case 81: fRad2->SetState(kButtonUp); break;
   case 82: fRad1->SetState(kButtonUp); break;
   case 92: fListBox->SetMultipleSelections(fCheckMulti->GetState()); break;
   default: break;
   }
}
...
void TestDialog::HandleEmbeddedCanvas(Int_t event, Int_t x, Int_t y, TObject *sel)
{
...
TestMsgBox::TestMsgBox(const TGWindow *p, const TGWindow *main, UInt_t w, UInt_t h, UInt_t options)
   : fRedTextGC(TGButton::GetDefaultGC())
{
   ...
   fL2 = new TGLayoutHints(kLHintsTop | kLHintsRight | kLHintsExpandX, 2, 5, 0, 2);
   ...
   fL4 = new TGLayoutHints(kLHintsTop | kLHintsLeft, 0, 0, 5, 0);
...
void TestMsgBox::DoTest()
{
   ...
   new TGMsgBox(gClient->GetRoot(), fMain, fTbtitle->GetString(), fTbmsg->GetString(), icontype, buttons, &retval);
   fMain->Disconnect("CloseWindow()");
   fMain->Connect("CloseWindow()", "TestMsgBox", this, "CloseWindow()");
}
...
TestSliders::TestSliders(const TGWindow *p, const TGWindow *main, UInt_t w, UInt_t h)
{
   ...
   fMain->SetMWMHints(kMWMDecorAll | kMWMDecorResizeH | kMWMDecorMaximize | kMWMDecorMinimize | kMWMDecorMenu,
...
void TestSliders::DoText(const char * /*text*/)
{
   ...
   switch (id) {
   case HId1: fHslider1->SetPosition(atoi(fTbh1->GetString())); break;
   case VId1: fVslider1->SetPosition(atoi(fTbv1->GetString())); break;
   case HId2: fHslider2->SetPosition(atoi(fTbh2->GetString())); break;
   case VId2: fVslider2->SetPosition(atoi(fTbv2->GetString()), atoi(fTbv2->GetString()) + 2); break;
   default: break;
   }
}
...
TestShutter::TestShutter(const TGWindow *p, const TGWindow *main, UInt_t w, UInt_t h)
{
...
TestDirList::TestDirList(const TGWindow *p, const TGWindow *main, UInt_t w, UInt_t h)
{
   ...
   fContents->Connect("Clicked(TGListTreeItem*,Int_t)", "TestDirList", this, "OnDoubleClick(TGListTreeItem*,Int_t)");
#ifdef R__WIN32
...
void TestDirList::OnDoubleClick(TGListTreeItem *item, Int_t btn)
{
   ...
   if ((btn != kButton1) || !item || (Bool_t)item->GetUserData())
      return;
...
TestProgress::TestProgress(const TGWindow *p, const TGWindow *main, UInt_t w, UInt_t h)
{
...
EntryTestDlg::EntryTestDlg(const TGWindow *p, const TGWindow *main)
{
   ...
   for (int i = 0; i < 13; i++) {
      ...
      fNumericEntries[i] = new TGNumberEntry(fF[i], numinit[i], 12, i + 20, (TGNumberFormat::EStyle)i);
