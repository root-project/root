...
DNDMainFrame::DNDMainFrame(const TGWindow *p, int w, int h) : TGMainFrame(p, w, h), fGraph(0), fHist1D(0), fHist2D(0)

{
   ...
   hf->AddFrame(fStatus, new TGLayoutHints(kLHintsExpandX | kLHintsCenterY, 10, 10, 10, 10));
   ...
   if (!gSystem->AccessPathName(link.Data(), kReadPermission)) {
      ...
      if (img) {
         ...
         img->Scale(16, 16);
         pic = gClient->GetPicturePool()->GetPicture("rose512", img->GetPixmap(), img->GetMask());
         delete img;
      } else
         pic = gClient->GetPicture("psp_t.xpm");
      link.Prepend("file://");
...
void DNDMainFrame::DataDropped(TGListTreeItem *, TDNDData *data)
{
   ...
   if (data) {
      ...
      if (data->fDataType == gRootObj) {
         ...
         sprintf(tmp, "Received DND data : Type = \"%s\"; Length = %d bytes;", obj->ClassName(), data->fDataLength);
         ...
         if (img1 && img2) {
            img1->Merge(img2);
            pic = gClient->GetPicturePool()->GetPicture("doc_lnk", img1->GetPixmap(), img1->GetMask());
            delete img2;
            delete img1;
         } else
