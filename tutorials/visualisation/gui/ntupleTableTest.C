...
/// the window are made. For more information about the use of TTreeTableInterface and TGTable, see their documentation.
///
/// \macro_code
///
...
public:
   TableTest(const TGWindow *p, UInt_t ntrows, UInt_t ntcols, UInt_t w = 100, UInt_t h = 100);
   ~TableTest() override;
...
TableTest::TableTest(const TGWindow *p, UInt_t ntrows, UInt_t ntcols, UInt_t w, UInt_t h)
   : TGMainFrame(p, w, h), fNTableRows(ntrows), fNTableColumns(ntcols), fTable(nullptr)
{
   ...
   fInterface = new TTreeTableInterface(ntuple, varexp.Data(), select.Data(), options.Data());
