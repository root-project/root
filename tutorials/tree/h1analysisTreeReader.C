...
// Selection function to select D* and D0.
	...
	if (entry == 0) Info("Process()", "Now processing tree pointer %8lld + %lld\n", (Long64_t)myTreeReader->GetTree(), fChainOffset);
   fProcessed++;
...
void h1analysisTreeReader::Init(TTree* myTree){
	Info("Init()", "Initiating tree %s\n", myTree ? myTree->GetName() : "UNKNOWN");

	fChainOffset = myTree->GetChainOffset();

	//myTreeReader->SetTree(myTree->GetTree());
}
...
//   get branch pointers
   ...
   Info("Notify","processing file: %s (%lld)",myTreeReader->GetTree()->GetCurrentFile()->GetName(), fProcessed);
