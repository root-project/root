...
// How many parameters does the background model have?
// Update this number if you change the background function
constexpr int nbgdpar=3;
...
Double_t signal(Double_t *x, Double_t *par) {
   ...
   return par[0]*TMath::Gaus(x[0], par[2], par[1], true);
}

// Sum of background and peak function. We pass x and the fit parameters
// down to the signal and background functions. Note that `nbgdpar` needs
...
void FitRoutine(TCanvas* c1, TH1* histo, float fitxmin, float fitxmax, TString filename){
   ...
   TF1 backFcn("backFcn",background,fitxmin,fitxmax,3);
   backFcn.SetLineColor(kRed);
   TF1 signalFcn("signalFcn",signal,fitxmin,fitxmax,3);
   ...
   const double binwidth = (histo->GetXaxis()->GetXmax()-histo->GetXaxis()->GetXmin())/histo->GetNbinsX();
