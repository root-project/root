...
void draw_axes()
{
   ...
   draw0->SetTitle("vertical").SetTicksInvert().SetTicksSize(0.02_normal);

   auto draw1 = canvas->Draw<RAxisDrawable>(RPadPos(x1, 0.9_normal), false, w1);
   draw1->SetTitle("horizontal").SetTitleCenter();

   auto draw2 = canvas->Draw<RAxisDrawable>(RPadPos(x1, 0.7_normal), false, w1);
   draw2->SetTitle("both side ticks").SetTicksBoth().SetTicksSize(0.02_normal).SetTitleCenter();

   auto draw3 = canvas->Draw<RAxisDrawable>(RPadPos(x1, 0.5_normal), false, w1);
   draw3->SetTitle("center labels").SetLabelsCenter();
   ...
   draw4->SetMinMax(TDatime(2020,11,12,9,0,0).Convert(), TDatime(2020,11,12,12,0,0).Convert())
          .SetTimeDisplay("%d/%m/%y %H:%M").SetTitle("time display").LabelsAttr().SetTextSize(0.01).SetTextColor(RColor::kRed);
   ...
   draw5->SetLabels(labels).SetTitle("labels, swap ticks side").SetTitleLeft().SetTicksInvert();

   auto draw6 = canvas->Draw<RAxisDrawable>(RPadPos(0.5_normal,0.9_normal), true, -0.8_normal);
   draw6->SetMinMax(0, 10).SetTitle("vertical negative length").SetEndingArrow();

   auto draw7 = canvas->Draw<RAxisDrawable>(RPadPos(x2, 0.9_normal), false, w2);
   draw7->SetMinMax(1, 100).SetLog(10).SetTitle("log10 scale").SetTitleCenter().TitleAttr().SetFont(12).SetTextColor(RColor::kGreen);

   auto draw8 = canvas->Draw<RAxisDrawable>(RPadPos(x2, 0.7_normal), false, w2);
   draw8->SetMinMax(0.125, 128).SetLog(2).SetTitle("log2 scale").SetTitleCenter();

   auto draw9 = canvas->Draw<RAxisDrawable>(RPadPos(x2, 0.5_normal), false, w2);
   draw9->SetMinMax(1, 100).SetLog(2.7182).SetTitle("ln scale").SetTitleCenter();

   auto draw10 = canvas->Draw<RAxisDrawable>(RPadPos(x2+w2, 0.3_normal), false, -w2);
   draw10->SetTitle("horizontal negative length").SetTitleCenter().SetEndingArrow();

   auto draw11 = canvas->Draw<RAxisDrawable>(RPadPos(x2, 0.1_normal), false, w2);
   draw11->SetReverse().SetTitle("horizontal reverse").SetTitleCenter().SetEndingArrow();

   auto draw12 = canvas->Draw<RAxisDrawable>(RPadPos(0.97_normal, 0.1_normal), true, 0.8_normal);
   draw12->SetTitle("vertical axis with arrow").SetTitleCenter()
