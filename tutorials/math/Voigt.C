...
void plotTwoTGraphs(Double_t x[], Double_t y1[], Double_t y2[], const Int_t nPoints
		    ...
		    , std::string plotTitle1, std::string plotTitle2, std::string plotTitle3, std::string plotTitle4
		    , std::string pdfTitle
...
void Voigt(){
  ...
  Double_t yMaximum = ROOT::Math::VoigtRelativistic::evaluate(median,median,sigma,width); //y maximum is at x=median (and non relativistic = relativistic at median so choice of function does not matter).
  for (Int_t i=0;i<nPoints;i++) {
    ...
    if (i!=0){ //calculate the voigt dumping functions with varying sigma. Skip sigma=0 as voigt is undefined here.
      y_nonRelVoigtDumpingFunction[i] = TMath::Voigt(median-median,currentX,width) / TMath::BreitWigner(median,median,width);
      y_relVoigtDumpingFunction[i] = ROOT::Math::VoigtRelativistic::dumpingFunction(median,currentX,width);;
    }
    
    if (y_nonRelVoigt[i]<yMinimum){yMinimum = y_nonRelVoigt[i];}
    if (y_relVoigt[i]<yMinimum){yMinimum = y_relVoigt[i];}
  }

  plotTwoTGraphs(x, y_nonRelVoigt, y_relVoigt, nPoints
		 ...
		 , "Comparing Voigt", "M = " + std::to_string(int(round(median))) + " GeV", "#Gamma = " + std::to_string(int(round(width))) + " GeV", "#sigma = " + std::to_string(int(round(sigma))) + " GeV" //Plot Title entry four lines)
		 , "Voigt_M"+std::to_string(int(round(median)))+"_Gamma" + std::to_string(int(round(width))) + "_sigma" + std::to_string(int(round(sigma))) +".pdf)" //PDF file title.
		 );
  
    plotTwoTGraphs(x, y_nonRelVoigtDumpingFunction, y_relVoigtDumpingFunction, nPoints
		   , 200, xMaximum //xAxis limits
		   , 0, 1.2 //yAxis limits, expand for aesthetics.
		   ,"NonRel Voigt DF", "Rel Voigt DF" //Legend entries
		   , "Voigt Dumping Functions", "M = " + std::to_string(int(round(median))) + " GeV", "#Gamma = " + std::to_string(int(round(width))) + " GeV", "" //Plot Title entry three lines)
		   , "VoigtDF_M"+std::to_string(int(round(median)))+"_Gamma" + std::to_string(int(round(width))) +".pdf)" //PDF file title.
