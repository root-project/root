...
pad1 = ROOT.TPad( 'pad1', 'The pad with the function',  0.03, 0.62, 0.50, 0.92, 21 )
pad2 = ROOT.TPad( 'pad2', 'The pad with the histogram', 0.51, 0.62, 0.98, 0.92, 21 )
pad3 = ROOT.TPad( 'pad3', 'The pad with the histogram', 0.03, 0.02, 0.97, 0.57, 21 )
pad1.Draw()
pad2.Draw()
pad3.Draw()
#
...
#
example = ROOT.TFile( 'hsimple.root' )
example.ls()

# Draw a global picture title
title = ROOT.TPaveLabel( 0.1, 0.94, 0.9, 0.98,
...
hpx = ROOT.gROOT.FindObject( 'hpx' )
hpx.SetFillColor( 45 )
hpx.DrawCopy()
label1 = ROOT.TPaveLabel( -3.5, 700, -1, 800, 'Default option' )
...
pad2.cd()
hpx.DrawCopy( 'lego1' )
label2 = ROOT.TPaveLabel( -0.72, 0.74, -0.22, 0.88, 'option Lego1' )
label2.SetFillColor( 42 )
label2.Draw()
label2a = ROOT.TPaveLabel( -0.93, -1.08, 0.25, -0.92, 'Click on lego to rotate' )
...
hpx.Draw( 'e1p' )
label3 = ROOT.TPaveLabel( 2, 600, 3.5, 650, 'option e1p' )
...
pave = ROOT.TPaveText( -3.78, 500, -1.2, 750 )
