...
std::size_t CallbackData(char *data, std::size_t size, std::size_t nmemb, void *userdata)
{
   ...
   RTransferState *transfer = reinterpret_cast<RTransferState *>(userdata);
...
std::string GetUserAgentString()
{
   ...
   auto curlVersionInfo = curl_version_info(CURLVERSION_NOW);
...
ROOT::Internal::RCurlConnection::RCurlConnection(const std::string &url)
{
   ...
   if (kInitCode != CURLE_OK) {
      throw RException(R__FAIL("cannot initialize curl library: " + GetCurlErrorString(kInitCode)));
   }
