...
TEST(RRawFileDavix, Idle)
{
   ...
   RRawFileDavix("http://root-eos.web.cern.ch/files/davix.test", options);
}
...
TEST(RRawFileDavix, Basics)
{
   std::string line;
   RRawFile::ROptions options;
   std::unique_ptr<RRawFileDavix> f(new RRawFileDavix("http://root-eos.web.cern.ch/files/davix.test", options));
   f->Readln(line);
   EXPECT_STREQ("Hello, World", line.c_str());
   EXPECT_EQ(13u, f->GetSize());

   std::unique_ptr<RRawFileDavix> f2(new RRawFileDavix("http://root-eos.web.cern.ch/files/davix.test.404", options));
