...
def _recursiveTreePrinter(tree, indent):
    ...
    for branch in listOfBranches:  # Print loop
        rec = [branch.GetName(), '"' + branch.GetTitle() + '"', str(branch.GetTotBytes())]
...
LONG_TEMPLATE = (
    isSpecial(ANSI_BOLD, "{0:{classWidth}}") + "{1:{timeWidth}}" + "{2:{nameWidth}}{3:{titleWidth}}{4:{cycleWidth}}"
)
...
def _rootLsPrintLongLs(keyList, indent, treeListing, recursiveListing):
    ...
    ```
    $ rootls -l https://root-eos.web.cern.ch/files/tutorials/hsimple.root
    ...
    for previouskey, currentkey, nextkey in zip_longest([None] + keyList[:-1], keyList, keyList[1:]):
        ...
        if (
            recursiveListing
            and isDirectoryKey(currentkey)
            and len(ROOT.gDirectory.Get(currentkey.GetName()).GetListOfKeys())
        ):
...
def _rootLsPrintSimpleLs(keyList, indent, oneColumn, recursiveListing):
    ...
    if max_element_width >= term_width:
        ...
        while True:
            col_widths = [
                max(len(key.GetName()) + min_chars_between for j, key in enumerate(keyList) if j % ncol == i)
                for i in range(ncol)
            ]
...
def _rootLsPrint(keyList, indent, oneColumn, longListing, treeListing, recursiveListing):
...
def _rootLsProcessFile(
    fileName, pathSplitList, manySources, indent, oneColumn, longListing, treeListing, recursiveListing
):
