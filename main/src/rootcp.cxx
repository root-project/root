...
static void CopyNode(const RootSource &src, const RootCpDestination &dest, NodeIdx_t nodeIdx, const RootCpArgs &args)
{
   ...
   if (cl->InheritsFrom("TObject")) {
      ...
      obj->Delete();
   } else if (cl == TClass::GetClass("ROOT::RNTuple")) {
      // TODO: RNTuple
...
int main(int argc, char **argv)
{
   ...
   for (const auto &srcFname : args.fSources) {
      ...
      // According to rootcp.py, this is necessary for fast copy. TODO: verify
      gROOT->GetListOfFiles()->Remove(src.fObjectTree.fFile.get());
