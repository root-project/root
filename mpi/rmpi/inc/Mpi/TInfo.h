...
namespace ROOT {
   ...
   class TInfo : public TObject {
     ...
     public:
       Binding(MPI_Info &finf, TString key)
           : fInternalInfo(finf), fInternalKey(key) {}
       Binding(const Binding &obj)
           : fInternalInfo(obj.fInternalInfo), fInternalKey(obj.fInternalKey) {}
       ~Binding() {}
       ...
       template <class T> Binding &operator=(const T &value) {
         ...
         ROOT_MPI_CHECK_CALL(MPI_Info_set,
                             (fInternalInfo,
                              const_cast<Char_t *>(fInternalKey.Data()),
   ...
   public:
     ...
     TInfo(const TInfo &info)
         : TObject(info), fInfo(info.fInfo), fBinder(fInfo, "") {}
