...
// then the aproximation that I am using to get for FLOPs  is measuring the time in every rank to get the maximum in all
// proccesses called t,
// in parallel I am calling a number of proccesses called p,
// To get the total FLOPs used in all proccesses the formula is  FLOPs=(p-1)*FLOP/t where FLOP is the theorical formula
// for matrix operation and is p-1 because
// I am using reduce operation.
// An important note is that using message compression the time can change and it can modified the value of the FLOPs
// measured.
// Read the paper http://wgropp.cs.illinois.edu/bib/papers/pdata/1999/pvmmpi99/mpptest.pdf to understand the
// difficulties measuring performance.
// Reference
// Gropp, W., & Lusk, E. (1999, September). Reproducible measurements of MPI performance characteristics. In European
// Parallel Virtual Machine/Message Passing Interface Users’ Group Meeting (pp. 11-18). Springer Berlin Heidelberg.
...
template <class T>
void FillMatrix(T *mdata, Int_t n)
{
...
void reduce()
{
   ...
   for (auto &n : msize) {
      ...
      if (COMM_WORLD.GetRank() == 0) {
         ...
         std::cout << left << fixed << "MaxTime(sec) = " << setw(10) << maxtime << setw(20)
                   << Form(" matrix[%d][%d] ", n, n) << " MFlops(Aprox)=" << setw(16) << flops[counter]
                   << " Total jobs = " << COMM_WORLD.GetSize() << std::endl;
         counter++;
      }
   }
