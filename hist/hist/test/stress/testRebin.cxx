...
TEST(StressHistorgram, Test2DRebin)
{
   ...
   TH2D *h2d = new TH2D("h2d", "Original Histogram", xrebin * TMath::Nint(r.Uniform(1, 5)), minRange, maxRange,
   ...
   TH2D *h3 = new TH2D("test2DRebin", "test2DRebin", h2d->GetNbinsX() / xrebin, h2d2->GetXaxis()->GetXmin(),
...
TEST(StressHistorgram, Test2DRebinProfile)
{
   ...
   Int_t xrebin = TMath::Nint(r.Uniform(minRebin, maxRebin));
   Int_t yrebin = TMath::Nint(r.Uniform(minRebin, maxRebin));
   TProfile2D *h2d = new TProfile2D("p2d", "Original Profile Histogram", xrebin * TMath::Nint(r.Uniform(1, 5)),
                                    minRange, maxRange, yrebin * TMath::Nint(r.Uniform(1, 5)), minRange, maxRange);
   ...
   TProfile2D *h3 = new TProfile2D("test2DRebinProfile", "test2DRebin", h2d->GetNbinsX() / xrebin,
                                   h2d2->GetXaxis()->GetXmin(), h2d2->GetXaxis()->GetXmax(), h2d->GetNbinsY() / yrebin,
                                   h2d2->GetYaxis()->GetXmin(), h2d2->GetYaxis()->GetXmax());
   r.SetSeed(seed);
   for (Int_t i = 0; i < nEvents; ++i)
      h3->Fill(r.Uniform(minRange * .9, maxRange * 1.1), r.Uniform(minRange * .9, maxRange * 1.1), r.Uniform(0, 10));
...
TEST(StressHistorgram, Test3DRebin)
{
   ...
   // make the bins of the orginal histo not an exact divider to leave an extra bin
   TH3D *h3d = new TH3D("h3d", "Original Histogram", xrebin * TMath::Nint(r.Uniform(1, 5)), minRange, maxRange,
                        yrebin * TMath::Nint(r.Uniform(1, 5)), minRange, maxRange,
   ...
   // range of rebinned histogram may be different than original one
   TH3D *h3 = new TH3D("test3DRebin", "test3DRebin", h3d->GetNbinsX() / xrebin, h3d2->GetXaxis()->GetXmin(),
