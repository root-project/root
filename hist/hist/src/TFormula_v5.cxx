...
Bool_t TFormula::AnalyzeFunction(TString &chaine, Int_t &err, Int_t offset)
{
   ...
   ROOT::v5::TFormulaPrimitive *prim = ROOT::v5::TFormulaPrimitive::FindFormula(
      cbase, args_paran > 0 ? cbase.Data() + args_paran + 1 : (const char *)nullptr);
...
void TFormula::ClearFormula(Option_t * /*option*/ )
{
   ...
   if (fExpr)   { delete [] fExpr;
      fExpr = nullptr;
   }
   if (fNames)  { delete [] fNames;
      fNames = nullptr;
   }
   if (fOper)   { delete [] fOper;
      fOper = nullptr;
   }
   ...
   //
   //MI change
   if (fPredefined)    { delete [] fPredefined;
      fPredefined = nullptr;
   }
   if (fOperOffset)    { delete [] fOperOffset;
      fOperOffset = nullptr;
   }
   if (fExprOptimized) { delete [] fExprOptimized;
      fExprOptimized = nullptr;
   }
   if (fOperOptimized) { delete [] fOperOptimized;
      fOperOptimized = nullptr;
   }
...
void  TFormula::MakePrimitive(const char *expr, Int_t pos)
{
   ...
   ROOT::v5::TFormulaPrimitive *prim =
      ROOT::v5::TFormulaPrimitive::FindFormula(cbase, paran > 0 ? cbase.Data() + paran + 1 : (const char *)nullptr);
   if (prim) {
