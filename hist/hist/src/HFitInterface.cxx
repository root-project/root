...
void ExamineRange(const TAxis *axis, std::pair<double, double> range, int &hxfirst, int &hxlast)
{
   ...
   if (hxfirst < hxlast) {
      if (axis->GetBinCenter(hxfirst) < xlow)
         hxfirst++;
      if (axis->GetBinCenter(hxlast) > xhigh)
         hxlast--;
   }
}
...
#ifdef DEBUG
   std::cout << "THFitInterface: ifirst = " << hxfirst << " ilast =  " << hxlast << " total bins  " << n << std::endl;
#endif
   ...
   for (binx = hxfirst; binx <= hxlast; ++binx) {
      ...
      for (biny = hyfirst; biny <= hylast; ++biny) {
         ...
         for (binz = hzfirst; binz <= hzlast; ++binz) {
            ...
            if (!HFitInterface::AdjustError(fitOpt, error, value))
               continue;
            ...
            if (ndim == hdim - 1) {
               ...
               if (hdim == 2)
                  dv.Add(x, x[1], yaxis->GetBinWidth(biny) / error);
               if (hdim == 3)
                  dv.Add(x, x[2], zaxis->GetBinWidth(binz) / error);
            } else {
...
void InitExpo(const ROOT::Fit::BinData &data, TF1 *f1)
{
   ...
   if (valxmin <= 0 && valxmax > 0)
      valxmin = valxmax;
   else if (valxmax <= 0 && valxmin > 0)
      valxmax = valxmin;
   else if (valxmin <= 0 && valxmax <= 0) {
      valxmin = 1;
      valxmax = 1;
   }
...
void Init2DGaus(const ROOT::Fit::BinData &data, TF1 *f1)
{
   ...
   double constant =
      0.5 * (valmax + binwidthx * allcha / (sqrtpi * rmsx)) * (valmax + binwidthy * allcha / (sqrtpi * rmsy));
...
#ifdef DEBUG
   std::cout << "2D Gaussian initial par values" << constant << "   " << meanx << "   " << rmsx << meany << "   "
             << rmsy << std::endl;
#endif
}
...
void DoFillData(BinData &dv, const TGraph *gr, BinData::ErrorType type, TF1 *func)
{
   ...
   if (func) {
      double a1, a2;
      func->GetRange(a1, a2);
      std::cout << "func range " << a1 << "  " << a2 << std::endl;
   }
#endif

   // Unlike histograms, graphs may have array of points created in non-ascending order along the X axis. This breaks
   // fitting algorithm. We create a "remap" for the TGraph point indexes that provides ascending order of X values for
   // the BinData
   ...
   for (int j = 0; j < nPoints; ++j) {
      ...
      // (like in a case of a graph)
         ...
         if (!HFitInterface::AdjustError(fitOpt, errorY))
            continue;
...
void FillData(SparseData &dv, const TH1 *h1, TF1 * /*func*/)
{
   ...
   for (int i = 0; i < ncells; ++i) {
      ...
      //              , h1->GetBinContent(i));
      if (!(h1->IsBinOverflow(i) || h1->IsBinUnderflow(i)) && h1->GetBinContent(i)) {
         int x, y, z;
...
void FillData(SparseData &dv, const THnBase *h1, TF1 * /*func*/)
{
   ...
   for (ULong64_t i = 0; i < nEntries; i++) {
      ...
      for (int j = 0; j < dim && insertBox; ++j) {
         ...
         if ((axis->GetBinLowEdge(coord[j]) < axis->GetXmin()) || (axis->GetBinUpEdge(coord[j]) > axis->GetXmax())) {
            insertBox = false;
         }
...
void FillData(BinData &dv, const TGraph2D *gr, TF1 *func)
{
   ...
   for (int i = 0; i < nPoints; ++i) {
      ...
      if (useRangeX && (x[0] < xmin || x[0] > xmax))
         continue;
      if (useRangeY && (x[1] < ymin || x[1] > ymax))
         continue;
      ...
      if (!HFitInterface::AdjustError(fitOpt, errorZ))
         continue;
