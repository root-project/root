// @(#)root/hist:$Name:  $:$Id: TSpectrum.h,v 1.4 2001/06/07 08:38:47 brun Exp $
...
public:
   ...
   virtual const char *Background1(float *spectrum,int size,int niter);
   virtual const char *Background2(float **spectrum,int sizex,int sizey,int niter);
   virtual const char *Deconvolution1(float *source,float *resp,int size,int niter);
   virtual const char *Deconvolution2(float **source,float **resp,int sizex,int sizey,int niter);
