...
TEST(RBinIndexMultiDimRange, Constructor)
{
   const RBinIndexMultiDimRange invalid;
   EXPECT_TRUE(invalid.GetRanges().empty());

   const auto index0 = RBinIndex(0);
   const auto range0 = CreateBinIndexRange(index0, index0, 0);
   const RBinIndexMultiDimRange multiRange0({range0});
   ...
   const RBinIndexMultiDimRange multiRange001(ranges);
...
TEST(RBinIndexMultiDimRange, Equality)
{
   const auto index0 = RBinIndex(0);
   const auto range0 = CreateBinIndexRange(index0, index0, 0);
   const auto range01 = CreateBinIndexRange(index0, RBinIndex(1), 1);
   ...
   const RBinIndexMultiDimRange invalid;
   const RBinIndexMultiDimRange multiRange({range0});
   const RBinIndexMultiDimRange multiRange0({range0});
   const RBinIndexMultiDimRange multiRange001({range0, range01});
...
TEST(RBinIndexMultiDimRange, Invalid)
{
   const RBinIndexMultiDimRange invalid;
...
TEST(RBinIndexMultiDimRange, Empty)
{
   const auto index0 = RBinIndex(0);
   const auto empty = CreateBinIndexRange(index0, index0, 0);
   ASSERT_EQ(empty.begin(), empty.end());
   const RBinIndexMultiDimRange emptyMulti({empty});
   EXPECT_EQ(emptyMulti.begin(), emptyMulti.end());
   EXPECT_EQ(std::distance(emptyMulti.begin(), emptyMulti.end()), 0);
}

TEST(RBinIndexMultiDimRange, Normal)
{
   const auto index0 = RBinIndex(0);
   const auto range01 = CreateBinIndexRange(index0, RBinIndex(1), 0);
   const RBinIndexMultiDimRange normal({range01});
...
TEST(RBinIndexMultiDimRange, Full)
{
   const auto underflow = RBinIndex::Underflow();
   const RBinIndex invalid;
   const auto full = CreateBinIndexRange(underflow, invalid, /*nNormalBins=*/10);
   const RBinIndexMultiDimRange fullMulti({full});
...
TEST(RBinIndexMultiDimRange, Multi)
{
   const auto index0 = RBinIndex(0);
   ...
   const RBinIndex invalid;
   const auto full = CreateBinIndexRange(underflow, invalid, /*nNormalBins=*/10);
   const RBinIndexMultiDimRange multi({normal, full});
...
TEST(RBinIndexMultiDimRange, MultiEmpty)
{
   ...
   const RBinIndexMultiDimRange multi({normal, empty, normal});
