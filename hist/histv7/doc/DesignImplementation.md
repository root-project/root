...
Classes are **not** templated to improve performance, in particular not on the axis type.
...
Many member functions have two overloads: one accepting a function parameter pack and one accepting a `std::tuple` or `std::array` (see below).

### Arguments with Different Types

For functions that take arguments with different types expect a `std::tuple`.
...
In this case, the function has a `std::size N` template argument and accepts a `std::array`.
An example is `template <std::size_t N> const T &GetBinContent(const std::array<BinIndex, N> &args)`
...
```
This will copy the arguments, which is fine in this case because `BinIndex` is small (see below).
...
template <std::size_t N> void SetBinContent(const std::array<BinIndex, N> &args, const T &content);
```
...
Small objects are passed by value instead of by reference (`BinIndex`, `Weight`).
