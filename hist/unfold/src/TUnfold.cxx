...
Double_t TUnfold::DoUnfold(void)
{
   ...
   if(fConstraint != kEConstraintNone) {
      ...
      TMatrixDSparse *temp1 = MultiplyMSparseMSparseTranspVector(Eepsilon, Eepsilon, nullptr);
...
(const TMatrixDSparse *A,Int_t *rankPtr) const
{
   ...
   if((rNumEl>=0)&&(nF>0)&&((nD2==0)||D2inv)) {
      ...
      if(minusBD2inv && F) {
         TMatrixDSparse *minusBD2invBt = MultiplyMSparseMSparseTranspVector(minusBD2inv, B, nullptr);
   ...
   TMatrixDSparse *r =
      (rNumEl >= 0) ? CreateSparseMatrix(A->GetNrows(), A->GetNrows(), rNumEl, rEl_row, rEl_col, rEl_data) : nullptr;
   delete [] rEl_data;
   delete [] rEl_col;
   delete [] rEl_row;
