...
/////////////////////////////////////////////////////////////////////////////
   ...
   if (dimension == 1 ) {
      ...
      if (strstr(option,"old")) {
         ...
         npeaks=Search1General(source,size,sigma,threshold,kTRUE,3);
      }
      TH1 *hnew = (TH1*)hin->Clone("markov");
      for (i=0;i<size;i++) hnew->SetBinContent(i+1,source[i]);
...
//END_HTML
   ...
   if (size<2*number_of_iterations+1) return "Too Large Clipping Window";
...
//_______________________________________________________________________________
	const char *TSpectrum::Background1General(float *spectrum,int size,int number_of_iterations,int direction,int filter_order,bool compton)
        {
/////////////////////////////////////////////////////////////////////////////
                ...
                if(filter_order==BACK1_ORDER2){
                	for(i=1;i<=number_of_iterations;i++){
                		for(j=i;j<size-i;j++){
                			...
                			b=(working_space[size+j-i]+working_space[size+j+i])/2.0;
		                	if(b<a)
				                a=b;
                			working_space[j]=a;
		                }
        	...
        	for(i=0,b2=0;i<size;i++){
	        	...
	        	if(TMath::Abs(a-b)>=1){
			        ...
			        if(b1<0)
				        b1=0;
        			yb1=spectrum[b1];
        			for(b2=b1+1,c=0,priz=0;priz==0&&b2<size;b2++){
...
/////////////////////////////////////////////////////////////////////////////
	...
	float men[7]={4,35,21,231,429,143,1105};
        if (size <= 0) return "Wrong Parameters";
        if(points<3||points>15||points%2==0) return("Incorrect Number of Smoothing Points");
        float *working_space = new float[size];
	k=points/2-1;
	b=men[k];
  	for(i=0;i<size;i++){
...
//_______________________________________________________________________________
 const char *TSpectrum::Deconvolution1(float *source,const float *resp,int size,int number_of_iterations)
{
/////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////
	...
	const char *TSpectrum::Deconvolution1HighResolution(float *source,const float *resp,int size,int number_of_iterations,int number_of_repetitions,double boost)
	{
/////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////
	...
	int i,j,k,m,lindex,posit,imin,imax,jmin,jmax,lh_gold,iter,repet;
...
//read source vector
	...
	for(m=1;m<size;m++){
		...
		for(j=1;j<=m;j++){
			...
			ldb+=working_space[m+1-j]*working_space[3*size+j-1];
			ldc+=working_space[m+1-j]*working_space[3*size+j-1+2*size];
		}
		a-=lda;
		...
		for(j=1;j<=m;j++){
			working_space[3*size+j-1+size]=working_space[3*size+j-1]-working_space[3*size+m+size]*working_space[3*size+m-j];
			working_space[3*size+j-1+3*size]=working_space[3*size+j-1+2*size]-working_space[3*size+m+3*size]*working_space[3*size+m-j];
		}
		for(i=0;i<=m;i++){
			working_space[3*size+i]=working_space[3*size+i+size];
			working_space[3*size+i+2*size]=working_space[3*size+i+3*size];
		}
	}
	for(i=0;i<size;i++)
...
//_______________________________________________________________________________
	const char *TSpectrum::Deconvolution1Unfolding(float *source,const float **resp,int sizex,int sizey,int number_of_iterations)
	{
/////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////
	...
	double lda,ldb,ldc,area;
        ...
        if(sizex<sizey) return "Sizex must be greater than sizey)";
        if (number_of_iterations<=0) return "Number of iterations must be positive";
        double *working_space=new double [sizex*sizey+2*sizey*sizey+4*sizex];
...
/*read source vector*/
	for(i=0;i<sizex;i++)
		working_space[sizex*sizey+2*sizey*sizey+2*sizex+i]=source[i];
/*create matrix at*a + at*y */
	for(i=0;i<sizey;i++){
		...
		for(k=0;k<sizex;k++){
			ldb=working_space[sizex*i+k];
			ldc=working_space[sizex*sizey+2*sizey*sizey+2*sizex+k];
			lda=lda+ldb*ldc;
		}
		working_space[sizex*sizey+2*sizey*sizey+3*sizex+i]=lda;
	}
/*move vector at*y*/
	for(i=0;i<sizey;i++)
		working_space[sizex*sizey+2*sizey*sizey+2*sizex+i]=working_space[sizex*sizey+2*sizey*sizey+3*sizex+i];
/*create matrix at*a*at*a + vector at*a*at*y */
	for(i=0;i<sizey;i++){
		for(j=0;j<sizey;j++){
			...
			working_space[sizex*sizey+sizey*sizey+sizey*i+j]=lda;
		}
		lda=0;
		for(k=0;k<sizey;k++){
			ldb=working_space[sizex*sizey+sizey*i+k];
...
/*initialization in resulting vectore */
	...
	for(lindex=0;lindex<number_of_iterations;lindex++){
		for(i=0;i<sizey;i++){
			lda=0;
			for(j=0;j<sizey;j++){
				ldb=working_space[sizex*sizey+sizey*sizey+sizey*i+j];
				ldc=working_space[sizex*sizey+2*sizey*sizey+j];
				lda=lda+ldb*ldc;
			}
			ldb=working_space[sizex*sizey+2*sizey*sizey+2*sizex+i];
			if(lda!=0){
				lda=ldb/lda;
			}
			else
		...
		for(i=0;i<sizey;i++)
			working_space[sizex*sizey+2*sizey*sizey+i]=working_space[sizex*sizey+2*sizey*sizey+3*sizex+i];
	}
/*write back resulting spectrum*/
	for(i=0;i<sizex;i++){
                if(i<sizey)
...
//_____________________________________________________________________________
   ...
   Int_t TSpectrum::Search1General(float *spectrum,int size,float sigma,int threshold,bool markov,int aver_window)
   {
...
/////////////////////////////////////////////////////////////////////////////
	...
	float s,f,si4=0,fi4=0,sold,fold=0,norma,filter[PEAK_WINDOW];
        if(sigma<0){
...
stav1:		        	stav=2;
			        ...
			        if(stav!=0){
				        b=sumai/suma;
                                        a=(-spectrum[(int)(b-3.0*sigma+0.5)]+2*spectrum[(int)(b+0.5)]-spectrum[(int)(b+3.0*sigma+0.5)])/2;
...
//////////////////////////////////////////////////////////////////////////////
	...
	double TSpectrum::Deramp(double i,double i0,double sigma,double t,double s,double b)
	{
//////////////////////////////////////////////////////////////////////////////
//   AUXILIARY FUNCION                                                      //
//                                                                          //
...
//////////////////////////////////////////////////////////////////////////////
	...
	double TSpectrum::Deri0(double i,double amp,double i0,double sigma,double t,double s,double b)
	{
//////////////////////////////////////////////////////////////////////////////
//   AUXILIARY FUNCION                                                      //
//                                                                          //
...
//////////////////////////////////////////////////////////////////////////////
	...
	double TSpectrum::Derderi0(double i,double amp,double i0,double sigma)
	{
//////////////////////////////////////////////////////////////////////////////
//   AUXILIARY FUNCION                                                      //
//                                                                          //
...
//////////////////////////////////////////////////////////////////////////////
	...
	double TSpectrum::Dersigma(int num_of_fitted_peaks,double i,const double* parameter,double sigma,double t,double s,double b)
	{
//////////////////////////////////////////////////////////////////////////////////
//   AUXILIARY FUNCION                                                          //
//                                                                              //
...
//////////////////////////////////////////////////////////////////////////////////
	...
	double TSpectrum::Derdersigma(int num_of_fitted_peaks,double i,const double* parameter,double sigma)
	{
//////////////////////////////////////////////////////////////////////////////////
//   AUXILIARY FUNCION                                                          //
//                                                                              //
...
//////////////////////////////////////////////////////////////////////////////////
	...
	double TSpectrum::Dert(int num_of_fitted_peaks,double i,const double* parameter,double sigma,double b)
	{
//////////////////////////////////////////////////////////////////////////////////
//   AUXILIARY FUNCION                                                          //
//                                                                              //
...
//////////////////////////////////////////////////////////////////////////////////
	...
	double TSpectrum::Ders(int num_of_fitted_peaks,double i,const double* parameter,double sigma)
	{
//////////////////////////////////////////////////////////////////////////////////
//   AUXILIARY FUNCION                                                          //
//                                                                              //
...
//////////////////////////////////////////////////////////////////////////////////
	...
	double TSpectrum::Derb(int num_of_fitted_peaks,double i,const double* parameter,double sigma,double t,double b)
	{
//////////////////////////////////////////////////////////////////////////////////
//   AUXILIARY FUNCION                                                          //
//                                                                              //
...
//////////////////////////////////////////////////////////////////////////////////
	...
	double TSpectrum::Shape(int num_of_fitted_peaks,double i,const double* parameter,double sigma,double t,double s,double b,double a0,double a1,double a2)
	{
//////////////////////////////////////////////////////////////////////////////////
//   AUXILIARY FUNCION                                                          //
//                                                                              //
...
//////////////////////////////////////////////////////////////////////////////////
	...
	double TSpectrum::Ourpowl(double a,int pw){//power function
		double c;
		c=1;
		if(pw>0)
			c=c*a*a;
...
/////////////////////////////////////////////////////////////////////////////
        ...
        int i,j,k,shift=2*p->number_of_peaks+7,peak_vel,rozmer,iter,pw,regul_cycle,flag;
        ...
        if(p->statistic_type!=FIT1_OPTIM_CHI_COUNTS&&p->statistic_type!=FIT1_OPTIM_CHI_FUNC_VALUES&&p->statistic_type!=FIT1_OPTIM_MAX_LIKELIHOOD)
                return("WRONG TYPE OF STATISTIC");
        if(p->alpha_optim!=FIT1_ALPHA_HALVING&&p->alpha_optim!=FIT1_ALPHA_OPTIMAL)
                return("WRONG OPTIMIZATION ALGORITHM");
        if(p->power!=FIT1_FIT_POWER2&&p->power!=FIT1_FIT_POWER4&&p->power!=FIT1_FIT_POWER6&&p->power!=FIT1_FIT_POWER8&&p->power!=FIT1_FIT_POWER10&&p->power!=FIT1_FIT_POWER12)
                return("WRONG POWER");
        if(p->fit_taylor!=FIT1_TAYLOR_ORDER_FIRST&&p->fit_taylor!=FIT1_TAYLOR_ORDER_SECOND)
	...
	if(rozmer>=p->xmax-p->xmin+1)
        ...
        //filling vectors
	        ...
	        for(i=p->xmin;i<=p->xmax;i++){
		        yw=source[i];
        		ywm=yw;
	        	f=Shape(p->number_of_peaks,(double)i,working_space,working_space[peak_vel],working_space[peak_vel+1],working_space[peak_vel+3],working_space[peak_vel+2],working_space[peak_vel+4],working_space[peak_vel+5],working_space[peak_vel+6]);
		        ...
		        for(j=0,k=0;j<p->number_of_peaks;j++){
			        if(p->fix_amp[j]==false){
       				        a=Deramp((double)i,working_space[2*j+1],working_space[peak_vel],working_space[peak_vel+1],working_space[peak_vel+3],working_space[peak_vel+2]);
                			if(ywm!=0){
	                			c=Ourpowl(a,pw);
                                                if(p->statistic_type==FIT1_OPTIM_CHI_FUNC_VALUES){
        			...
        			if(p->fix_position[j]==false){
       	        			a=Deri0((double)i,working_space[2*j],working_space[2*j+1],working_space[peak_vel],working_space[peak_vel+1],working_space[peak_vel+3],working_space[peak_vel+2]);
                                        if(p->fit_taylor==FIT1_TAYLOR_ORDER_SECOND)
                                                d=Derderi0((double)i,working_space[2*j],working_space[2*j+1],working_space[peak_vel]);
        			        if(ywm!=0){
	        			        c=Ourpowl(a,pw);
                                                if(TMath::Abs(a)>0.00000001&&p->fit_taylor==FIT1_TAYLOR_ORDER_SECOND){
	        	...
	        	if(p->fix_sigma==false){
       		        	a=Dersigma(p->number_of_peaks,(double)i,working_space,working_space[peak_vel],working_space[peak_vel+1],working_space[peak_vel+3],working_space[peak_vel+2]);
                                if(p->fit_taylor==FIT1_TAYLOR_ORDER_SECOND)
               			        d=Derdersigma(p->number_of_peaks,(double)i,working_space,working_space[peak_vel]);
                		if(ywm!=0){
	                		c=Ourpowl(a,pw);
                                        if(TMath::Abs(a)>0.00000001&&p->fit_taylor==FIT1_TAYLOR_ORDER_SECOND){
                                                d=d*TMath::Abs(yw-f)/(2*a*ywm);
        		...
        		if(p->fix_t==false){
       	        		a=Dert(p->number_of_peaks,(double)i,working_space,working_space[peak_vel],working_space[peak_vel+2]);
        	        	if(ywm!=0){
	        	        	c=Ourpowl(a,pw);
                                        if(p->statistic_type==FIT1_OPTIM_CHI_FUNC_VALUES){
       		        		        b=a*(yw*yw-f*f)/(ywm*ywm);
        		...
        		if(p->fix_b==false){
       	        		a=Derb(p->number_of_peaks,(double)i,working_space,working_space[peak_vel],working_space[peak_vel+1],working_space[peak_vel+2]);
        	        	if(ywm!=0){
	        	        	c=Ourpowl(a,pw);
                                        if(p->statistic_type==FIT1_OPTIM_CHI_FUNC_VALUES){
       		        		        b=a*(yw*yw-f*f)/(ywm*ywm);
        		...
        		if(p->fix_s==false){
       	        		a=Ders(p->number_of_peaks,(double)i,working_space,working_space[peak_vel]);
        	        	if(ywm!=0){
	        	        	c=Ourpowl(a,pw);
                                        if(p->statistic_type==FIT1_OPTIM_CHI_FUNC_VALUES){
       		        		        b=a*(yw*yw-f*f)/(ywm*ywm);
	        ...
	        do{
		        if(p->alpha_optim==FIT1_ALPHA_OPTIMAL){
			        ...
			        for(pi=0.1;flag==0&&pi<=100;pi+=0.1){
		        		...
		        		if(chi2<chi_min&&p->statistic_type!=FIT1_OPTIM_MAX_LIKELIHOOD||chi2>chi_min&&p->statistic_type==FIT1_OPTIM_MAX_LIKELIHOOD){
		        ...
		        iter+=1;
        		regul_cycle+=1;
	        }while((chi>chi_opt&&p->statistic_type!=FIT1_OPTIM_MAX_LIKELIHOOD||chi<chi_opt&&p->statistic_type==FIT1_OPTIM_MAX_LIKELIHOOD)&&regul_cycle<FIT1_NUM_OF_REGUL_CYCLES);
        	for(j=0;j<rozmer;j++){
        ...
        chi_er=chi_cel/b;
	for(i=0,j=0;i<p->number_of_peaks;i++){
		p->area[i]=Area(working_space[2*i],working_space[peak_vel],working_space[peak_vel+1],working_space[peak_vel+2]);
		if(p->fix_amp[i]==false){
			...
			if(working_space[3*shift+j]!=0)
        		...
        		if(p->area[i]>0){
	        		a=Derpa(working_space[peak_vel],working_space[peak_vel+1],working_space[peak_vel+2]);
                                b=working_space[4*shift+j];//temp_xk[j]
                                if(b==0)
                                        b=1;
                                else
                                        b=1/b;
        			p->area_err[i]=sqrt(TMath::Abs(a*a*b*chi_er));
	        	}
		        else
			        p->area_err[i]=0;
			j+=1;
		...
		if(p->fix_position[i]==false){
			...
			if(working_space[3*shift+j]!=0)//temp[j]
				p->position_err[i]=sqrt(TMath::Abs(working_space[2*shift+j]))/sqrt(TMath::Abs(working_space[3*shift+j]));//der[j]/temp[j]
			j+=1;
		}
		else{
	...
	if(p->fix_sigma==false){
		...
		if(working_space[3*shift+j]!=0)//temp[j]
			p->sigma_err=sqrt(TMath::Abs(working_space[2*shift+j]))/sqrt(TMath::Abs(working_space[3*shift+j]));//der[j]/temp[j]
		j+=1;
	}
	else{
	...
	if(p->fix_t==false){
		...
		if(working_space[3*shift+j]!=0)//temp[j]
			p->t_err=sqrt(TMath::Abs(working_space[2*shift+j]))/sqrt(TMath::Abs(working_space[3*shift+j]));//der[j]/temp[j]
		j+=1;
	}
	else{
		p->t_calc=p->t_init;
	...
	if(p->fix_b==false){
		...
		if(working_space[3*shift+j]!=0)//temp[j]
			p->b_err=sqrt(TMath::Abs(working_space[2*shift+j]))/sqrt(TMath::Abs(working_space[3*shift+j]));//der[j]/temp[j]
		j+=1;
	}
	else{
		p->b_calc=p->b_init;
	...
	if(p->fix_s==false){
		...
		if(working_space[3*shift+j]!=0)//temp[j]
			p->s_err=sqrt(TMath::Abs(working_space[2*shift+j]))/sqrt(TMath::Abs(working_space[3*shift+j]));//der[j]/temp[j]
		j+=1;
	}
	else{
		p->s_calc=p->s_init;
	...
	if(p->fix_a1==false){
		...
		if(working_space[3*shift+j]!=0)//temp[j]
			p->a1_err=sqrt(TMath::Abs(working_space[2*shift+j]))/sqrt(TMath::Abs(working_space[3*shift+j]));//der[j]/temp[j]
		j+=1;
	}
	else{
		p->a1_calc=p->a1_init;
	...
	if(p->fix_a2==false){
		...
		if(working_space[3*shift+j]!=0)//temp[j]
			p->a2_err=sqrt(TMath::Abs(working_space[2*shift+j]))/sqrt(TMath::Abs(working_space[3*shift+j]));//der[j]/temp[j]
		j+=1;
	}
	else{
		p->a2_calc=p->a2_init;
...
//////////////////////////////////////////////////////////////////////////////////
	...
	const char* TSpectrum::Fit1Stiefel(float *source, TSpectrumOneDimFit* p,int size)
	{
/////////////////////////////////////////////////////////////////////////////
/*	ONE-DIMENSIONAL FIT FUNCTION				           */
...
/*									   */
/////////////////////////////////////////////////////////////////////////////
        int i,j,k,shift=2*p->number_of_peaks+7,peak_vel,rozmer,iter,regul_cycle,flag;
        double a,b,alpha,chi_opt,yw,ywm,f,chi2,chi_min,chi=0,pi,pmin=0,chi_cel=0,chi_er;
        if (size <= 0) return "Wrong Parameters";
        if(p->number_of_peaks<=0)
                return("INVALID NUMBER OF PEAKS, MUST BE POSITIVE");
        if(p->number_of_iterations<=0)
	...
	for(i=0;i<rozmer;i++)
        ...
        for(iter=0;iter<p->number_of_iterations;iter++){
	        ...
	        for(i=p->xmin;i<=p->xmax;i++){
        		...
        		for(j=0,k=0;j<p->number_of_peaks;j++){
	        		if(p->fix_amp[j]==false){
		        		working_space[2*shift+k]=Deramp((double)i,working_space[2*j+1],working_space[peak_vel],working_space[peak_vel+1],working_space[peak_vel+3],working_space[peak_vel+2]);
			        	k+=1;
        			}
	        		if(p->fix_position[j]==false){
		        		working_space[2*shift+k]=Deri0((double)i,working_space[2*j],working_space[2*j+1],working_space[peak_vel],working_space[peak_vel+1],working_space[peak_vel+3],working_space[peak_vel+2]);
			        	k+=1;
        			}
	        	}
		        if(p->fix_sigma==false){
			        working_space[2*shift+k]=Dersigma(p->number_of_peaks,(double)i,working_space,working_space[peak_vel],working_space[peak_vel+1],working_space[peak_vel+3],working_space[peak_vel+2]);
        			k+=1;
	        	}
		        if(p->fix_t==false){
			        working_space[2*shift+k]=Dert(p->number_of_peaks,(double)i,working_space,working_space[peak_vel],working_space[peak_vel+2]);
        			k+=1;
	        	}
		        if(p->fix_b==false){
			        working_space[2*shift+k]=Derb(p->number_of_peaks,(double)i,working_space,working_space[peak_vel],working_space[peak_vel+1],working_space[peak_vel+2]);
        			k+=1;
	        	}
		        if(p->fix_s==false){
			        working_space[2*shift+k]=Ders(p->number_of_peaks,(double)i,working_space,working_space[peak_vel]);
        			k+=1;
	        	}
		        if(p->fix_a0==false){
...
lab10:		ip=1;
	...
	void TSpectrum::Fourier(float *working_space,int num,int hartley,int direction,int zt_clear)
	{
//////////////////////////////////////////////////////////////////////////////////
//   AUXILIARY FUNCION                                                          //
//                                                                              //
...
lab65:		j=j+k;
	...
	void TSpectrum::BitReverseHaar(float *working_space,int shift,int num,int start)
	{
//////////////////////////////////////////////////////////////////////////////////
//   AUXILIARY FUNCION                                                          //
//                                                                              //
...
lab10:		ip=1;
	...
	int TSpectrum::GeneralExe(float *working_space,int zt_clear,int num,int degree,int type)
	{
//////////////////////////////////////////////////////////////////////////////////
//   AUXILIARY FUNCION                                                          //
//                                                                              //
...
//////////////////////////////////////////////////////////////////////////////////
	...
	for(m=1;m<=iter;m++){
		...
		mnum=num/nump;
		mnum2=mnum/2;
		if(m>degree&&(type==TRANSFORM1_FOURIER_HAAR||type==TRANSFORM1_WALSH_HAAR||type==TRANSFORM1_COS_HAAR||type==TRANSFORM1_SIN_HAAR))
			mp2step*=2;
		if(ring>1)
			ring=ring/2;
		for(mp=0;mp<nump;mp++){
	...
	int TSpectrum::GeneralInv(float *working_space,int num,int degree,int type)
	{
//////////////////////////////////////////////////////////////////////////////////
//   AUXILIARY FUNCION                                                          //
//                                                                              //
...
//////////////////////////////////////////////////////////////////////////////////
	...
	wpwr=2.0*pi/a;
	mp2step=1;
	if(type==TRANSFORM1_FOURIER_HAAR||type==TRANSFORM1_WALSH_HAAR||type==TRANSFORM1_COS_HAAR||type==TRANSFORM1_SIN_HAAR){
		for(i=0;i<iter-degree;i++)
			mp2step*=2;
	}
	ring=1;
	for(m=1;m<=iter;m++){
		...
		if(m<=iter-degree&&(type==TRANSFORM1_FOURIER_HAAR||type==TRANSFORM1_WALSH_HAAR||type==TRANSFORM1_COS_HAAR||type==TRANSFORM1_SIN_HAAR))
			mp2step/=2;
		for(i=0;i<num;i++){
			val1=working_space[num+i];
			working_space[i]=val1;
...
/////////////////////////////////////////////////////////////////////////////
	...
	if(direction==TRANSFORM1_FORWARD){
	        	...
	        	working_space[0]=working_space[0]/sqrt(2.0);
        		for(i=0;i<size/2;i++){
		        	dest[i]=working_space[i];
        		}
                        break;
	        	...
	        	working_space[size/2-1]=working_space[size/2]/sqrt(2.0);
        		for(i=0;i<size/2;i++){
		        	dest[i]=working_space[i];
        		}
                        break;
			...
			for(i=0;i<size;i++){
					...
					working_space[i]=val;
                        ...
                        if(type==TRANSFORM1_FOURIER_WALSH||type==TRANSFORM1_FOURIER_HAAR||type==TRANSFORM1_WALSH_HAAR){
				for(i=0;i<j;i++)
			...
			for(i=0,l=0;i<size;i++,l=(l+k)%size){
				...
				working_space[size+i+2*size]=working_space[l+i/j+2*size];
			}
			for(i=0;i<size;i++){
        		...
        		for(i=0;i<size;i++){
		        	dest[i]=working_space[i];
                        ...
                        if(type==TRANSFORM1_FOURIER_WALSH||type==TRANSFORM1_FOURIER_HAAR){
                		for(i=0;i<size;i++){
	        	...
	        	for(i=2;i<=(size/2);i++){
			        ...
			        working_space[size-i+1+size]=-working_space[i-1+size];
        		}
        		...
        		for(i=0;i<size;i++){
		        	working_space[i]=source[i];
                        ...
                        size=2*size;
        		working_space[size/2]=working_space[size/2-1]*sqrt(2.0);
        		for(i=size/2-1;i>0;i--){
        			a=pi*(double)i/(double)size;
		        	working_space[i+size]=-(double)working_space[i-1]*cos(a);
        			working_space[i]=(double)working_space[i-1]*sin(a);
        		}
	        	for(i=2;i<=(size/2);i++){
		        	working_space[size-i+1]=working_space[i-1];
			        working_space[size-i+1+size]=-working_space[i-1+size];
			...
			for(i=0,l=0;i<size;i++,l=(l+k)%size){
				...
				working_space[size+l+i/j+2*size]=working_space[i+2*size];
			}
			for(i=0;i<size;i++){
				working_space[i]=working_space[size+i];
				working_space[i+2*size]=working_space[size+i+2*size];
				...
				for(i=0;i<size;i++){
					...
					a=pi*(double)(i%j)/(double)(2*j);
					if(i%j==0){
						working_space[2*size+k+i%j]=working_space[i]*sqrt(2.0);
						working_space[4*size+2*size+k+i%j]=0;
					}
					else{
						b=sin(a);
						a=cos(a);
						working_space[4*size+2*size+k+i%j]=-(double)working_space[i]*b;
						working_space[2*size+k+i%j]=(double)working_space[i]*a;
					}
				}
				for(i=0;i<size;i++){
					k=i/j;
					...
					if(i%j==0){
						...
						working_space[2*size+k+2*j-i%j]=working_space[2*size+k+i%j];
						working_space[4*size+2*size+k+2*j-i%j]=-working_space[4*size+2*size+k+i%j];
					}
				}
        			for(i=0;i<2*size;i++){
	        			working_space[i]=working_space[2*size+i];
		        		working_space[4*size+i]=working_space[4*size+2*size+i];
			        }
				...
				for(i=0;i<size;i++){
					...
					a=pi*(double)(i%j)/(double)(2*j);
					if(i%j==0){
						working_space[2*size+k+j+i%j]=working_space[j+k/2-i%j-1]*sqrt(2.0);
						working_space[4*size+2*size+k+j+i%j]=0;
					}
					else{
						b=sin(a);
						a=cos(a);
						working_space[4*size+2*size+k+j+i%j]=-(double)working_space[j+k/2-i%j-1]*b;
						working_space[2*size+k+j+i%j]=(double)working_space[j+k/2-i%j-1]*a;
					}
				}
				for(i=0;i<size;i++){
					k=i/j;
					...
					if(i%j==0){
						...
						working_space[2*size+k+i%j]=working_space[2*size+k+2*j-i%j];
						working_space[4*size+2*size+k+i%j]=-working_space[4*size+2*size+k+2*j-i%j];
					}
				}
				for(i=0;i<2*size;i++){
					working_space[i]=working_space[2*size+i];
...
//////////FILTER1_ZONAL FUNCTION - CALCULATES DIFFERENT 1-D ORTHOGONAL TRANSFORMS, SETS GIVEN REGION TO FILTER COEFFICIENT AND TRANSFORMS IT BACK//////
	...
	for(i=0,new_area=0;i<size;i++){
        ...
        else if(type==TRANSFORM1_FOURIER_WALSH||type==TRANSFORM1_FOURIER_HAAR){
        	for(i=0,old_area=0;i<size;i++){
