...
TGedEditor::TGedEditor(TCanvas *canvas, UInt_t width, UInt_t height)
   : TGMainFrame(gClient->GetRoot(), width, height), fCan(nullptr), fTab(nullptr), fTabContainer(nullptr),
     fModel(nullptr), fPad(nullptr), fCanvas(nullptr), fClass(nullptr), fGlobal(kTRUE)
{
...
void TGedEditor::SetModel(TVirtualPad* pad, TObject* obj, Int_t event, Bool_t force)
{
   ...
   if (mapTabs) { // selected object is different class
      ...
      if (seltab == nullptr || fTab->SetTab(seltab->GetString(), kFALSE) == kFALSE) fTab->SetTab(0, kFALSE);
   }
      ...
      if (!exclfr && frame->AcceptModel(fModel)){
         ...
         if (frame->GetExtraTabs()) {
            ...
            while ((subf = (TGedFrame::TGedSubFrame*)next())) {
               ...
               if (fVisibleTabs.FindObject(ti) == nullptr) fVisibleTabs.Add(ti);
            }
         }
...
void  TGedEditor::ExcludeClassEditor(TClass* cl, Bool_t recurse)
{
   ...
   if (pair) {
      if (recurse && pair->Value() == nullptr) pair->SetValue((TObject *)(Long_t)1); // hack, reuse TObject as Bool_t
   } else {
