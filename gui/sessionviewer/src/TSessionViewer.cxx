...
const char *xpm_names[] = {"monitor01.xpm", "monitor02.xpm", "monitor03.xpm", "monitor04.xpm", nullptr};

const char *conftypes[] = {"Config files", "*.conf", "All files", "*.*", nullptr, nullptr};

const char *pkgtypes[] = {"Package files", "*.par", "All files", "*.*", nullptr, nullptr};

const char *macrotypes[] = {"C files", "*.[C|c]*", "All files", "*", nullptr, nullptr};

const char *kFeedbackHistos[] = {"PROOF_PacketsHist",  "PROOF_EventsHist",  "PROOF_NodeHist", "PROOF_LatencyHist",
                                 "PROOF_ProcTimeHist", "PROOF_CpuTimeHist", nullptr};
...
const char *xpm_toolbar[] = {"fileopen.xpm",
                             "filesaveas.xpm",
                             "",
                             "connect.xpm",
                             "disconnect.xpm",
...
ToolBarData_t tb_data[] = {{"", "Open Config File", kFALSE, kFileLoadConfig, nullptr},
...
TSessionServerFrame::TSessionServerFrame(TGWindow *p, Int_t w, Int_t h)
   : TGCompositeFrame(p, w, h), fFrmNewServer(nullptr), fTxtName(nullptr), fTxtAddress(nullptr), fNumPort(nullptr),
     fLogLevel(nullptr), fTxtConfig(nullptr), fTxtUsrName(nullptr), fSync(nullptr), fViewer(nullptr), fBtnAdd(nullptr),
     fBtnConnect(nullptr)
{
}
...
void TSessionServerFrame::Build(TSessionViewer *gui)
{
   ...
   fFrmNewServer->AddFrame(fTxtName = new TGTextEntry(fFrmNewServer, (const char *)nullptr, 1), new TGLayoutHints());
   ...
   fFrmNewServer->AddFrame(new TGLabel(fFrmNewServer, "Server name:"),
                           new TGLayoutHints(kLHintsLeft, 3, 3, 3, 3));
   fFrmNewServer->AddFrame(fTxtAddress = new TGTextEntry(fFrmNewServer, (const char *)nullptr, 2), new TGLayoutHints());
   ...
   fFrmNewServer->AddFrame(new TGLabel(fFrmNewServer, "Configuration File:"),
                           new TGLayoutHints(kLHintsLeft, 3, 3, 3, 3));
   fFrmNewServer->AddFrame(fTxtConfig = new TGTextEntry(fFrmNewServer, (const char *)nullptr, 4), new TGLayoutHints());
   ...
   fFrmNewServer->AddFrame(new TGLabel(fFrmNewServer, "User Name:"),
            new TGLayoutHints(kLHintsLeft, 3, 3, 3, 3));
   fFrmNewServer->AddFrame(fTxtUsrName = new TGTextEntry(fFrmNewServer, (const char *)nullptr, 6), new TGLayoutHints());
...
void TSessionServerFrame::OnBtnDeleteClicked()
{
   ...
   m.Form("Are you sure to delete the server \"%s\"",
          desc->fName.Data());
   Int_t result;
   new TGMsgBox(fClient->GetRoot(), this, "", m.Data(), nullptr, kMBOk | kMBCancel, &result);
...
TSessionFrame::TSessionFrame(TGWindow *p, Int_t w, Int_t h)
   : TGCompositeFrame(p, w, h), fTab(nullptr), fFA(nullptr), fFB(nullptr), fFC(nullptr), fFD(nullptr), fFE(nullptr),
     fCommandTxt(nullptr), fCommandBuf(nullptr), fInfoTextView(nullptr), fClearCheck(nullptr), fBtnShowLog(nullptr),
     fBtnNewQuery(nullptr), fBtnGetQueries(nullptr), fLBPackages(nullptr), fBtnAdd(nullptr), fBtnRemove(nullptr),
     fBtnUp(nullptr), fBtnDown(nullptr), fBtnShow(nullptr), fBtnShowEnabled(nullptr), fChkMulti(nullptr),
     fChkEnable(nullptr), fBtnUpload(nullptr), fBtnEnable(nullptr), fBtnClear(nullptr), fBtnDisable(nullptr),
     fDSetView(nullptr), fDataSetTree(nullptr), fBtnUploadDSet(nullptr), fBtnRemoveDSet(nullptr),
     fBtnVerifyDSet(nullptr), fBtnRefresh(nullptr), fTxtParallel(nullptr), fLogLevel(nullptr), fApplyLogLevel(nullptr),
     fApplyParallel(nullptr), fViewer(nullptr)
{
...
TEditQueryFrame::TEditQueryFrame(TGWindow *p, Int_t w, Int_t h)
   : TGCompositeFrame(p, w, h, kVerticalFrame), fFrmMore(nullptr), fBtnMore(nullptr), fTxtQueryName(nullptr),
     fTxtChain(nullptr), fTxtSelector(nullptr), fTxtOptions(nullptr), fNumEntries(nullptr), fNumFirstEntry(nullptr),
     fTxtParFile(nullptr), fTxtEventList(nullptr), fViewer(nullptr), fQuery(nullptr), fChain(nullptr)
{
}
...
void TEditQueryFrame::Build(TSessionViewer *gui)
{
   ...
   AddFrame(fTxtQueryName = new TGTextEntry(this, (const char *)nullptr, 1),
   ...
   AddFrame(fTxtChain = new TGTextEntry(this, (const char *)nullptr, 2),
   ...
   AddFrame(fTxtSelector = new TGTextEntry(this, (const char *)nullptr, 3),
   ...
   fFrmMore->AddFrame(fTxtOptions = new TGTextEntry(fFrmMore, (const char *)nullptr, 4),
                      new TGTableLayoutHints(1, 2, 0, 1, 0, 17, 0, 0, 8));
   ...
   fFrmMore->AddFrame(fTxtEventList = new TGTextEntry(fFrmMore, (const char *)nullptr, 6),
                      new TGTableLayoutHints(1, 2, 3, 4, 0, 17, 5, 0, 0));
   // add "Browse" button
...
TSessionQueryFrame::TSessionQueryFrame(TGWindow *p, Int_t w, Int_t h)
   : TGCompositeFrame(p, w, h), fBtnSubmit(nullptr), fBtnFinalize(nullptr), fBtnStop(nullptr), fBtnAbort(nullptr),
     fBtnShowLog(nullptr), fBtnRetrieve(nullptr), fBtnSave(nullptr), fInfoTextView(nullptr), fModified(0), fFiles(0),
     fFirst(0), fEntries(0), fPrevTotal(0), fPrevProcessed(0), fLabInfos(nullptr), fLabStatus(nullptr), fTotal(nullptr),
     fRate(nullptr), fStatus(kStopped), fTab(nullptr), fFA(nullptr), fFB(nullptr), fFC(nullptr), fFD(nullptr),
     frmProg(nullptr), fECanvas(nullptr), fStatsCanvas(nullptr), fViewer(nullptr), fDesc(nullptr)
{
}
...
void TSessionQueryFrame::UpdateButtons(TQueryDescription *desc)
{
   ...
   switch (desc->fStatus) {
      ...
      case TQueryDescription::kSessionQueryCompleted:
         fBtnSubmit->SetEnabled(submit_en);
         fBtnFinalize->SetEnabled(kTRUE);
         if (((desc->fResult == nullptr) || (desc->fResult && (desc->fResult->IsFinalized() ||
...
TSessionViewer::TSessionViewer(const char *name, Int_t x, Int_t y, UInt_t w, UInt_t h)
   : TGMainFrame(gClient->GetRoot(), w, h), fSessionHierarchy(nullptr), fSessionItem(nullptr)
{
   // only one session viewer allowed
   if (gSessionViewer)
...
void TSessionViewer::ReadConfiguration(const char *filename)
{
   ...
   if (fSessionItem)
      ...
      fSessionItem = fSessionHierarchy->AddItem(nullptr, "Sessions", fBaseIcon, fBaseIcon);
...
{
   delete fUserGroup;
   if (gSessionViewer == this) gSessionViewer = nullptr;
}
                                       ...
                                       Int_t x, Int_t y)
{
   ...
   if (fActDesc->fConnected && fActDesc->fAttached) {
      ...
      fPopupSrv->DisableEntry(kSessionGetQueries);
      fPopupSrv->DisableEntry(kSessionShowStatus);
      if (entry->GetParent() != nullptr) fSessionMenu->EnableEntry(kSessionDelete);
...
void TSessionViewer::CleanupSession()
{
   ...
   Int_t result;
   new TGMsgBox(fClient->GetRoot(), this, "", m.Data(), nullptr, kMBYes | kMBNo | kMBCancel, &result);
   if (result == kMBYes) {
