...
static const char *gFiletypes[] = {"All files", "*", "Text files", "*.txt", "ROOT macros", "*.C", nullptr, nullptr};
...
void TGTextEdit::Print(Option_t *) const
{
   ...
   if (p) {
      ...
      new TGMsgBox(fClient->GetDefaultRoot(), this, "Editor", msg.Data(), kMBIconAsterisk, kMBOk, nullptr);
   } else {
      msg.Form("Could not execute: %s -P%s\n", gPrintCommand, gPrinter);
      new TGMsgBox(fClient->GetDefaultRoot(), this, "Editor", msg.Data(), kMBIconExclamation, kMBOk, nullptr);
   }
}
                          ...
                          Bool_t caseSensitive)
{
   ...
   if (!fText->Search(&pos, fCurrent, string, direction, caseSensitive)) {
      ...
      if (!fText->Search(&pos, fCurrent, string, direction, caseSensitive)) { //try again
         ...
         gVirtualX->Bell(20);
         new TGMsgBox(fClient->GetDefaultRoot(), fCanvas, "TextEdit", msg.Data(), kMBIconExclamation, kMBOk, nullptr);
         return kFALSE;
      }
      return kTRUE;
   }
               ...
               switch (parm1) {
                  ...
                  case kM_FILE_OPEN:
                     if (!IsSaved()) {
                        ...
                        if (retval == kMBCancel)
                           return kTRUE;
                        if (retval == kMBYes)
                           if (!SaveFile(nullptr)) return kTRUE;
                     }
                     Clear();
                  ...
                  case kM_SEARCH_FINDAGAIN:
                     ...
                     if (!Search(fSearch->fBuffer, fSearch->fDirection,
                        ...
                        new TGMsgBox(fClient->GetDefaultRoot(), this, "Editor", msg2.Data(), kMBIconExclamation, kMBOk,
                                     nullptr);
                     }
                     break;
                  case kM_SEARCH_GOTO:
