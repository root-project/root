...
void TGFileBrowser::Add(TObject *obj, const char *name, Int_t check)
{
   if (fListLevel && !strcmp(fListLevel->GetText(), "Classes") &&
      ...
      if (obj && obj->InheritsFrom("TApplication")) fListLevel = nullptr;
...
void TGFileBrowser::AddRemoteFile(TObject *obj)
{
   ...
   if (R_ISDIR(type) || fFilter == nullptr || (fFilter && filename.Index(*fFilter) != kNPOS)) {
...
void TGFileBrowser::BrowseObj(TObject *obj)
{
   ...
   if (obj == gROOT) {
      ...
      if (gROOT->GetListOfFiles() && !gROOT->GetListOfFiles()->IsEmpty()) Selected(nullptr);
   }
}
                                   ...
                                   Option_t *opt)
{
   ...
   if (strstr(opt, "SetRootDir")) {
      if (!fListTree->FindChildByName(nullptr, entry)) item = fRootDir = fListTree->AddItem(nullptr, entry);
   }
      ...
      //
      if (!fListTree->FindChildByName(nullptr, entry)) item = fListTree->AddItem(nullptr, entry);
   }
   if (item && path) {
      TString infos = path;
   ...
   if (isdir) {
      ...
      if (files) {
         ...
         while ((file=(TSystemFile*)nextf())) {
            ...
            if (!file->IsDirectory() && (fFilter == nullptr || (fFilter && fname.Index(*fFilter) != kNPOS))) {
               if (!fShowHidden && fname.BeginsWith("."))
                  continue;
               size = modtime = 0;
