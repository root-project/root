   ...
   switch (entry->fType) {
      ...
      case kMenuEntry:
         if ((entry->fStatus & kMenuActiveMask) && entry->fType != kMenuLabel) {
            ...
            if (entry->fPic != nullptr) entry->fPic->Draw(fId, fSelGC, 8, entry->fEy + 1);
            ...
            if (entry->fPic != nullptr) entry->fPic->Draw(fId, fNormGC, 8, entry->fEy + 1);
...
void TGPopupMenu::DeleteEntry(Int_t id)
{
   ...
   while ((ptr = (TGMenuEntry *) next()))
      if (ptr->fEntryId == id) {
         ...
         delete ptr;
         Reposition();
         if (fCurrent == ptr) fCurrent = nullptr;
         return;
      }
}
