 ...
 //     intext = 0,
   ...
   while (p && (p = p->fPNext)) {
      ...
      switch (p->fType) {
         ...
         case Html_EndTABLE: p = nullptr; break;
         ...
         case Html_TH:
            ...
            if ((cp = p->MarkupArg("rowspan", nullptr)) && (j = atoi(cp)) > 0 && cols < HTML_MAX_COLUMNS) {
               rspans[cols] = j;
               rspanstart = 1;
            } else {
               rspanstart = 0;
  ...
  // Figure out how wide to draw the table
   ...
   for (iRow = 1; iRow <= pTable->fNRow; iRow++) {
      ...
      for (iCol = 1; iCol <= pTable->fNCol; iCol++) {
         ...
         // subsequent rows
         if (apElem[iCol] == nullptr || (iRow != pTable->fNRow && lastRow[iCol] > iRow)) continue;

         // Align the contents of the cell vertically.
         switch (valign[iCol]) {
         case VAlign_Unknown:
         case VAlign_Center: dy = (rowBottom - ymax[iCol]) / 2; break;
         case VAlign_Top:
         case VAlign_Baseline: dy = 0; break;
         case VAlign_Bottom: dy = rowBottom - ymax[iCol]; break;
            }
            if (dy) {
