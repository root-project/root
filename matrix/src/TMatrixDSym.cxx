...
TMatrixDSym TMatrixDSym::GetSub(Int_t row_lwb,Int_t row_upb,Option_t *option) const
{
  ...
  const Double_t *ap = this->GetElements()+(row_lwb-fRowLwb)*fNrows+(row_lwb-fRowLwb);
...
void TMatrixDSym::SetSub(Int_t row_lwb,const TMatrixDSym &source)
{
  ...
  const Double_t *bp = source.GetElements();
        Double_t *ap = this->GetElements()+(row_lwb-fRowLwb)*fNrows+(row_lwb-fRowLwb);
...
TMatrixDSym &TMatrixDSym::operator=(const TMatrixDSym &source)
{
  ...
  if (this != &source) {
    ...
    memcpy(this->GetElements(),source.fElements,fNelems*sizeof(Double_t));
  }
  return *this;
}
...
void TMatrixDSym::Streamer(TBuffer &R__b)
{
  ...
  if (R__b.IsReading()) {
    ...
    if (R__v > 1) {
      Clear();
      TMatrixDSym::Class()->ReadBuffer(R__b,this,R__v,R__s,R__c);
      if (fNelems <= kSizeMax) {
    ...
    R__b.CheckByteCount(R__s,R__c,TMatrixDSym::IsA());
    //====end of old versions
  } else {
    TMatrixDSym::Class()->WriteBuffer(R__b,this);
  }
}
