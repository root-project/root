// @(#)root/matrix:$Name:  $:$Id: TVectorF.cxx,v 1.1 2004/01/25 20:33:32 brun Exp $
...
void TVectorF::ResizeTo(Int_t lwb,Int_t upb)
{
  ...
  if (IsValid()) {
    ...
    if (nrows_copy > 0) {
      ...
      if (bot > 0)
        memset(elements_new,0,bot*sizeof(Float_t));
      if (top > 0)
        memset(elements_new+bot+nrows_copy,0,top*sizeof(Float_t));
    }
...
TVectorF TVectorF::GetSub(Int_t row_lwb,Int_t row_upb,Option_t *option) const
{
  ...
  const Float_t *ap = this->GetElements()+(row_lwb-fRowLwb);
...
void TVectorF::SetSub(Int_t row_lwb,const TVectorF &source)
{
  ...
  const Float_t *bp = source.GetElements();
        Float_t *ap = this->GetElements()+(row_lwb-fRowLwb);
...
{
  Assert(IsValid());
  memset(this->GetElements(),0,fNrows*sizeof(Float_t));
  return *this;
}
...
Float_t TVectorF::Norm1() const
{
  ...
  Float_t norm = 0;
  const Float_t *ep = this->GetElements();
  const Float_t * const fp = ep+fNrows;
  while (ep < fp)
    norm += TMath::Abs(*ep++);
...
TVectorF &TVectorF::operator=(const TVectorF &source)
{
  ...
  if (this != &source && AreCompatible(*this,source)) {
    ...
    memcpy(fElements,source.GetElements(),fNrows*sizeof(Float_t));
  }
  return *this;
}
...
TVectorF &TVectorF::operator=(const TMatrixFRow_const &mr)
{
  ...
  const Float_t *rp = mr.GetPtr();              // Row ptr
        Float_t *ep = this->GetElements();      // Vector ptr
  const Float_t * const fp = ep+fNrows;
  while (ep < fp) {
    *ep++ = *rp;
     rp += inc;
...
TVectorF &TVectorF::operator*=(const TMatrixF &a)
{
  ...
  const Float_t *mp = a.GetElements();     // Matrix row ptr
        Float_t *tp = this->GetElements(); // Target vector ptr
#ifdef CBLAS
...
TVectorF &TVectorF::operator*=(const TMatrixFSym &a)
{
  ...
  const Float_t *mp1 = a.GetElements(); // Matrix row ptr
...
Bool_t operator==(const TVectorF &v1,const TVectorF &v2)
{
  ...
  return (memcmp(v1.GetElements(),v2.GetElements(),v1.GetNrows()*sizeof(Float_t)) == 0);
}
...
Float_t operator*(const TVectorF &v1,const TVectorF &v2)
{
  ...
  const Float_t *v1p = v1.GetElements();
  const Float_t *v2p = v2.GetElements();
...
void Compare(const TVectorF &v1,const TVectorF &v2)
{
  ...
  Float_t difmax = -1;
  const Float_t *mp1 = v1.GetElements();    // Vector element pointers
  const Float_t *mp2 = v2.GetElements();
...
Bool_t VerifyVectorValue(const TVectorF &v,Float_t val,
                         const Int_t verbose,Float_t maxDevAllow)
{
...
Bool_t VerifyVectorIdentity(const TVectorF &v1,const TVectorF &v2,
                            const Int_t verbose,const Float_t maxDevAllow)
{
