                       ...
                       const char *confdir, Int_t loglevel, const char *)
{
   ...
   fSlaveInfo = nullptr;
   fChains         = new TList;
   fAvailablePackages = nullptr;
   fEnabledPackages = nullptr;
   ...
   if (dynconf.Length() > 0) {
      ...
      fLastPollWorkers_s = time(nullptr);
      // Extract parameters
      Int_t from = 0;
      TString p;
   ...
   if (TestBit(TProof::kIsClient)) {
      ...
      if ((fLogFileW = fopen(fLogFileName.Data(), "w")) == nullptr)
         Error("Init", "could not create temporary logfile %s", fLogFileName.Data());
      if ((fLogFileR = fopen(fLogFileName.Data(), "r")) == nullptr)
...
Int_t TProofLite::SetupWorkers(Int_t opt, TList *startedWorkers)
{
               ...
               *msg >> ord;
               ...
               if ((wrk = (TSlave *) started.FindObject(ord))) {
                  ...
                  if (wrk->IsValid()) {
                     ...
                     wrk->SetupServ(TSlave::kSlave, nullptr);
                  }
   ...
   if (!gROOT->IsBatch() && !fProgressDialog) {
      if ((fProgressDialog =
         ...
         if (fProgressDialog->LoadPlugin() == -1) fProgressDialog = nullptr;
   }
...
Int_t TProofLite::CreateSandbox()
{
   ...
   TString stag;
   stag.Form("%s-%d-%d", gSystem->HostName(), (int)time(nullptr), gSystem->GetPid());
   SetName(stag.Data());
                                               ...
                                               const char *selec)
{
   ...
   TProofQueryResult *pqr = new TProofQueryResult(seqnum, opt, fPlayer->GetInputList(), nent, fst, dset, selec,
                                                  (dset ? dset->GetEntryList() : nullptr));
                             ...
                             Long64_t nentries, Long64_t first)
{
   ...
   TProofQueryResult *pq = MakeQueryResult(nentries, opt, first, nullptr, selec);
...
Int_t TProofLite::Remove(const char *ref, Bool_t all)
{
   ...
   if (fQMgr) {
      ...
      if (fQMgr->LockSession(queryref, &lck) == 0) {
         ...
         // Remove query
         fQMgr->RemoveQuery(queryref, nullptr);
