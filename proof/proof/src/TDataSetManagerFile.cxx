                                       ...
                                       const char *dsName, UInt_t option)
{
   ...
   // convert * to "nothing"
   if (group && (strcmp(group, "*") == 0 || !group[0])) group = nullptr;
   if (user && (strcmp(user, "*") == 0 || !user[0])) user = nullptr;
   ...
   if (notCommonUser) {
      ...
      if (user && group && strchr(user, '*') && strchr(group, '*')) {
         ...
         if (printing && !forcescan && fUseCache && CheckLocalCache(group, user, nullptr, option) == 0) {
                                       ...
                                       const char *dsName, UInt_t option)
{
   ...
   // Do the scan
   Int_t result =
      TDataSetManager::ScanDataSet(dataset, fopt, sopt, 0, dbg, &fNTouchedFiles, &fNOpenedFiles, &fNDisappearedFiles,
...
Long_t TDataSetManagerFile::GetModTime(const char *uri)
{
   ...
   Long_t modTime;
   if (gSystem->GetPathInfo(path.Data(), (Long_t *)nullptr, (Long_t *)nullptr, (Long_t *)nullptr, &modTime)) {
      return -1;
   }
