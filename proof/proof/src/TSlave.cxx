...
TSlave::TSlave(const char *url, const char *ord, Int_t perf, const char *image, TProof *proof, Int_t stype,
               const char *workdir, const char *msd, Int_t)
   : fImage(image), fProofWorkDir(workdir), fWorkDir(workdir), fPort(-1), fOrdinal(ord), fPerfIdx(perf), fProtocol(0),
     fSocket(nullptr), fProof(proof), fInput(nullptr), fBytesRead(0), fRealTime(0), fCpuTime(0),
     fSlaveType((ESlaveType)stype), fStatus(TSlave::kInvalid), fParallel(0), fMsd(msd)
{
