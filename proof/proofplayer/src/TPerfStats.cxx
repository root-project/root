...
TPerfStats::TPerfStats(TList *input, TList *output)
   : fTrace(nullptr), fPerfEvent(nullptr), fPacketsHist(nullptr), fProcPcktHist(nullptr), fEventsHist(nullptr),
     fLatencyHist(nullptr), fProcTimeHist(nullptr), fCpuTimeHist(nullptr), fBytesRead(0), fTotCpuTime(0.),
     fTotBytesRead(0), fTotEvents(0), fNumEvents(0), fSlaves(0), fDoHist(kFALSE), fDoTrace(kFALSE),
     fDoTraceRate(kFALSE), fDoSlaveTrace(kFALSE), fDoQuota(kFALSE), fMonitorPerPacket(kFALSE), fMonSenders(3),
                             ...
                             Double_t cputime, Long64_t bytesRead)
{
   ...
   if (!fMonSenders.IsEmpty() && fMonitorPerPacket) {
      TQueryResult *qr = (gProofServ && gProofServ->GetProof()) ? gProofServ->GetProof()->GetQueryResult() : nullptr;
      if (!gProofServ || !gProofServ->GetSessionTag() || !gProofServ->GetProof() || !qr) {
         Error("PacketEvent", "some required object are undefined (%p %p %p %p)", gProofServ,
               (gProofServ ? gProofServ->GetSessionTag() : nullptr), (gProofServ ? gProofServ->GetProof() : nullptr),
...
void TPerfStats::WriteQueryLog()
{
   ...
   if (!fMonSenders.IsEmpty()) {
      TQueryResult *qr = (gProofServ && gProofServ->GetProof()) ? gProofServ->GetProof()->GetQueryResult() : nullptr;
      if (!gProofServ || !gProofServ->GetSessionTag() || !gProofServ->GetProof() || !qr) {
         Error("WriteQueryLog", "some required object are undefined (%p %p %p %p)", gProofServ,
               (gProofServ ? gProofServ->GetSessionTag() : nullptr), (gProofServ ? gProofServ->GetProof() : nullptr),
               ((gProofServ && gProofServ->GetProof()) ? qr : nullptr));
         return;
      }
