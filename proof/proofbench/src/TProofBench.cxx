...
TProofBench::TProofBench(const char *url, const char *outfile, const char *proofopt)
   : fUnlinkOutfile(kFALSE), fProofDS(nullptr), fOutFile(nullptr), fNtries(4), fHistType(nullptr), fNHist(16),
     fReadType(nullptr), fDataSet("BenchDataSet"), fNFilesWrk(2), fReleaseCache(kTRUE),
     fDataGenSel(kPROOF_BenchSelDataGenDef), fRunCPU(nullptr), fRunDS(nullptr), fDS(nullptr), fDebug(kFALSE),
...
{
   if (SetOutFile(nullptr) != 0) Warning("CloseOutFile", "problems closing '%s'", fOutFileName.Data());
}
...
Int_t TProofBench::RunCPU(Long64_t nevents, Int_t start, Int_t stop, Int_t step)
{
   ...
   // Close the file
   if (SetOutFile(nullptr) != 0) Warning("RunCPU", "problems closing '%s'", fOutFileName.Data());
...
Int_t TProofBench::RunCPUx(Long64_t nevents, Int_t start, Int_t stop)
{
   ...
   // Close the file
   if (SetOutFile(nullptr) != 0) Warning("RunCPUx", "problems closing '%s'", fOutFileName.Data());
                              ...
                              Int_t start, Int_t stop, Int_t step)
{
   ...
   // Close the file
   if (SetOutFile(nullptr) != 0) Warning("RunDataSet", "problems closing '%s'", fOutFileName.Data());
...
Int_t TProofBench::RunDataSetx(const char *dset, Int_t start, Int_t stop)
{
   ...
   // Close the file
   if (SetOutFile(nullptr) != 0) Warning("RunDataSetx", "problems closing '%s'", fOutFileName.Data());
