...
TProofBenchRunCPU::TProofBenchRunCPU(TPBHistType *histtype, Int_t nhists, TDirectory *dirproofbench, TProof *proof,
                                     TProofNodes *nodes, Long64_t nevents, Int_t ntries, Int_t start, Int_t stop,
                                     Int_t step, Int_t draw, Int_t debug)
   : TProofBenchRun(proof, kPROOF_BenchSelCPUDef), fHistType(histtype), fNHists(nhists), fNEvents(nevents),
     fNTries(ntries), fStart(start), fStop(stop), fStep(step), fDraw(draw), fDebug(debug),
     fDirProofBench(dirproofbench), fNodes(nodes), fListPerfPlots(nullptr), fCanvas(nullptr),
     fProfile_perfstat_event(nullptr), fHist_perfstat_event(nullptr), fProfile_perfstat_evtmax(nullptr),
     fNorm_perfstat_evtmax(nullptr), fProfile_queryresult_event(nullptr), fNorm_queryresult_event(nullptr),
     fProfile_cpu_eff(nullptr), fProfLegend(nullptr), fNormLegend(nullptr), fName(0)
{
