...
static TString gh1src("http://root-eos.web.cern.ch/files/h1");
...
static TString geventsrc("http://root-eos.web.cern.ch/files/data");
...
Int_t PT_H1AssertFiles(const char *h1src)
{
   ...
   if (!strncmp(h1src,"download",8)) {
      ...
      Int_t i = 0;
      for (i = 0; i < 4; i++) {
         TString src = TString::Format("http://root-eos.web.cern.ch/files/h1/%s", gh1file[i]);
...
Int_t PT_EventAssertFiles(const char *eventsrc, Int_t nf = 10)
{
   ...
   if (!strncmp(eventsrc,"download",8)) {
      ...
      Int_t i = 0;
      for (i = 0; i < nf; i++) {
         TString src = TString::Format("http://root-eos.web.cern.ch/files/data/event_%d.root", i+1);
...
Int_t PT_AdminFunc(void *, RunTimes &tt)
{
   ...
   // Test 'cp' and 'md5sum;
   if (mgr->Cp("http://root-eos.web.cern.ch/files/h1/dstarmb.root", "~/") != 0) {
      // Cp failure
