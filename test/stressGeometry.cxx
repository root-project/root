...
// from http://root-test.web.cern.ch/files
//
...
void stressGeometry(const char *exp="*", Bool_t generate_ref=kFALSE, Bool_t vecgeom=kFALSE) {
   ...
   for (i=0; i<NG; i++) {
      ...
      TGeoManager::Import(Form("http://root-test.web.cern.ch/files/%s",fname.Data()));
      if (!gGeoManager) return;
      if (vecgeom) TVirtualGeoConverter::Instance()->ConvertGeometry();
      
      fname = TString::Format("files/%s_ref_%d.root", exps[i],versions[i]);

      if (gen_ref || !TFile::Open(Form("http://root-test.web.cern.ch/files/%s_ref_%d.root",exps[i],versions[i]),"CACHEREAD")) {
...
void ReadRef(Int_t kexp) {
   ...
   if (!gen_ref)
      fname = TString::Format("http://root-test.web.cern.ch/files/%s_ref_%d.root", exps[kexp],versions[kexp]);
   else
