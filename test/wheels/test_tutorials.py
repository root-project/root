...
SKIP_TUTORIALS = {
    "ntpl011_global_temperatures.C",  # requires RCanvas, not supported yet in ROOT wheels
...
for sub in subdirs:
    ...
    for f in sub_path.rglob("*.py"):
        # RHist is not supported in ROOT wheels, also skip all histv7 tutorials
        if any(skip in f.name for skip in SKIP_TUTORIALS) or f.parent.name == "histv7":
