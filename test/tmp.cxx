...
private:
  ...
  Bool_t ComparePDF(RooAbsPdf& rPDF1,RooAbsPdf& rPDF2,const RooArgSet& rAllObservables,RooAbsData& rTestData)
  {
    ...
    *pVars1 = *pVars2;
    ...
    RooArgSet * par1 = rPDF1.getParameters(rTestData); 
    RooArgSet * par2 = rPDF2.getParameters(rTestData); 
    RooStats::RemoveConstantParameters(par1);
    RooStats::RemoveConstantParameters(par2);
 
   // print fit variables 
    if (_verb > 1) par2->Print("v"); 

    RooFitResult* r2 = rPDF2.fitTo(rTestData,Save(), RooFit::Minimizer("Minuit"), RooFit::PrintLevel(_verb), RooFit::Offset(true));
    //RooFitResult* r2 = rPDF2.fitTo(rTestData,Save(), RooFit::PrintLevel(_verb));
    
    // print fit variables 
    if (_verb > 1) par1->Print("v"); 

    if (gMinuit) { delete gMinuit; gMinuit=0; }

    // do the fit
    RooFitResult* r1 = rPDF1.fitTo(rTestData,Save(), RooFit::Minimizer("Minuit"), RooFit::PrintLevel(_verb), RooFit::Offset(true));
    //RooFitResult* r1 = rPDF1.fitTo(rTestData,Save(), RooFit::PrintLevel(_verb));
