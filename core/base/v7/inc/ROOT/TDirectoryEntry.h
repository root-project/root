...
// Forward declaration for TDirectoryEntry::operator bool()
...
public:
  ...
  explicit TDirectoryEntry(const std::shared_ptr<T>& ptr):
    ...
    fObj(ptr),
    fThrower(&TDirectoryEntry::Thrower<T>) {}
  ...
  std::type_index GetTypeInfo() const { return fTypeInfo; }
  ...
  explicit operator bool() const { return *this != TDirectoryEntry(); }

private:
  template<class T>
  void Thrower() const { throw static_cast<T*>(fObj.get()); }

  time_point_t fDate = clock_t::now(); ///< Time of last change
  std::type_index fTypeInfo;
  TClass* fType; ///< The type of the object
  std::shared_ptr<void> fObj;
  void (TDirectoryEntry::*fThrower)() const;
};
...
{
  try { (this->*fThrower)(); }
  catch (U* ptr) { return std::shared_ptr<U>(fObj, static_cast<U*>(ptr)); }
  catch (...) {}
