...
TFileCollection::TFileCollection(const char *name, const char *title, const char *textfile, Int_t nfiles,
                                 Int_t firstfile)
   : TNamed(name, title), fList(nullptr), fMetaDataList(nullptr), fDefaultTree(), fTotalSize(0), fNFiles(0),
     fNStagedFiles(0), fNCorruptFiles(0)
{
   fList = new THashList();
   fList->SetOwner();
...
TFileCollection *TFileCollection::GetFilesOnServer(const char *server)
{
   ...
   while ((fi = (TFileInfo *)nxf())) {
      ...
      if ((xu = fi->FindByUrl(srv.Data()))) {
         ...
         TFileInfo *nfi =
            new TFileInfo(xu->GetUrl(), fi->GetSize(), fi->GetUUID() ? fi->GetUUID()->AsString() : nullptr,
...
TMap *TFileCollection::GetFilesPerServer(const char *exclude, Bool_t curronly)
{
   ...
   while ((fi = (TFileInfo *)nxf())) {
      ...
      while ((xurl = (curronly) ? curl : fi->NextUrl())) {
         ...
         // Create a new TFileInfo object
         TFileInfo *nfi =
            new TFileInfo(xurl->GetUrl(kTRUE), fi->GetSize(), fi->GetUUID() ? fi->GetUUID()->AsString() : nullptr,
                          fi->GetMD5() ? fi->GetMD5()->AsString() : nullptr);
         if (fi->GetMetaDataList()) {
            TIter nxm(fi->GetMetaDataList());
            TFileInfoMeta *md = nullptr;
