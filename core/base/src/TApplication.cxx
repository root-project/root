...
namespace {
enum EUrl {kURLforClass, kURLforNameSpace};
////////////////////////////////////////////////////////////////////////////////
/// The function generates a URL address for class or namespace (name).
...
///
/// \param[in] name the name of the class or the namespace
/// \param[in] scopeTtype the enumerator for class or namespace

static TString UrlGenerator(const TString &name, EUrl scopeType)
{
   TString url = TString::Format("https://root.cern/doc/master/%s%s.html",
   (scopeType == kURLforClass ? "class" : "namespace"), name.Data());
   return url;
}
}
...
/// class (className), but modified with respect to Doxygen syntax - spacing
/// around special symbols and adding the missing scopes ("std::").
/// "FormatMethodForDoxygen" works for functions defined inside namespaces
...
static TString FormatMethodForDoxygen(const TString &className, TFunction *func)
{
...
namespace {
////////////////////////////////////////////////////////////////////////////////
/// The function checks if a member function of a class is defined as inline.
...
static TString FormatReturnTypeForDoxygen(const TString &className, TFunction *func)
{
   ...
   classNameRE.Append(className);
   classNameRE.Append("::\\b");
   TPRegexp returnFix(classNameRE);
   returnFix.Substitute(returnType, "");
   // We check is if the method is deffined as inline (inliceCheck).
   Int_t inlineCheck = (func->ExtraProperty() & kIsInlined);
   // inlineCheck is not zero only if the function is defined as inline.
   if (inlineCheck == 0) {
      ...
      Int_t virtualCheck = (func->Property() & kIsVirtual);
      if (virtualCheck != 0) {
         // If the function is virtual, we append "virtual" before the return type.
         TString buffer("virtual ");
         buffer.Append(returnType);
         returnType.Clear();
         returnType.Append(buffer);
      }
      returnType.ReplaceAll(" *", "*");
   }
...
namespace {
////////////////////////////////////////////////////////////////////////////////
/// The function generates in a URL for "enumeratorName" defined in "className".
/// It returns a TString with the URL used in the online reference guide,
/// generated with Doxygen. The URL generation is specific. It could be
/// separated into 3 parts - URL for "className", part for the enumeration
/// and a part for the enumerator.
///
/// \param[in] className the name of the class
/// \param[in] enumeratorName the name of the enumerator from the class
...
static TString URLforEnumerator(const TString &className, const TString &enumeratorName, TDataMember *dataMember, EUrl scopeType)
{
   ...
   // If we have anonymous enumeration we need other syntax ("className::@1@1").
   if ((classEnumeration.Contains("(anonymous)"))) {
      // FIXME: need to invetigate the numbering scheme.
      md5EnumClass.Append(className);
      md5EnumClass.Append("::@1@1");
   } else {
      // If the enumeration is not anonymous we put "className::Enumeration" in a TString,
      // which will be crypted with MD5 later.
      md5EnumClass.Append(classEnumeration);
      TString delim("::");
      // We extract the part after "::" (this is the enumerator name).
      TObjArray enumOnly(*classEnumeration.Tokenize(delim));
      TString enumOnlyName = (enumOnly.Last())->GetName();
...
/// Destructor) of "className". Doxygen uses specific syntax - the first part
/// of the URL is only for the class and it is generated with the help of
/// "UrlGenerator". Then we append "#a" and a sequence of numbers and letters.
/// This sequence is text, encrypted with MD5. The original text is:
/// "Return type className::methodNamemethodName(method arguments)".
/// For constructor/destructor of the class the return type is not appended.
///
/// \param[in] className the name of the class
...
/// guide, generated with Doxygen, for a certain class (".help className"),
/// method of a class (".help className::Method") or a namespace
/// (".help NameSpace").
///
...
void TApplication::Help(const char *line)
{
   ...
   TString strippedCommand = command.Strip(TString::kBoth);
   // If the user chooses ".help" or ".?".
   if ((strippedCommand == ".help") || (strippedCommand == ".?")) {
      gInterpreter->ProcessLine(line);
   //   " << metaString << "help ClassName\t\t- Opens Doxygen online reference guide for ClassName\n"
   //   "   " << metaString << "help ClassName::Method\t- Opens Doxygen online reference guide for Method\n"
   //   "   " << metaString << "help NameSpace\t\t- Opens Doxygen online reference guide for NameSpace\n"
   ...
   // If the user wants to use the extended ".help className" command to access
   // the online reference guide.
   if (!strippedCommand.BeginsWith(".help ")) {
   ...
   if (!cl) {
      // If the user wants to open a browser with the URL for "className" only, but types the class incorrectly.
   ...
   if (cl->GetListOfEnums()->FindObject(memberName)) {
      ...
      // with respect to the "scopeType".
      OpenInBrowser(URLforEnumeration(className, memberName, scopeType));
      return;
    }

   // We check if "memberName" is enumerator defined inside a class.
   if (TDataMember *enumerator = (TDataMember*)cl->GetListOfDataMembers()->FindObject(memberName)) {
      // We generate the URL with "URLforEnumerator" and with respect to the "scopeType".
      OpenInBrowser(URLforEnumerator(className, memberName, enumerator, scopeType));
      return;
   }
   // We check if "memberName" is enumerator defined in one the base classes of "className".
