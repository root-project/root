...
TFileInfo::TFileInfo(const char *in, Long64_t size, const char *uuid, const char *md5, TObject *meta)
   : fCurrentUrl(nullptr), fUrlList(nullptr), fSize(-1), fUUID(nullptr), fMD5(nullptr), fMetaDataList(nullptr),
     fIndex(-1)
{
...
TFileInfo::TFileInfo(const TFileInfo &fi)
   : TNamed(fi.GetName(), fi.GetTitle()), fCurrentUrl(nullptr), fUrlList(nullptr), fSize(fi.fSize), fUUID(nullptr),
     fMD5(nullptr), fMetaDataList(nullptr), fIndex(fi.fIndex)
{
   if (fi.fUrlList) {
      fUrlList = new TList;
      fUrlList->SetOwner();
