...
Bool_t TCling::IsLoaded(const char* filename) const
{
   ...
   if (FE && FE->isValid()) {
      ...
      if (!FID.isInvalid() && FID.getHashValue() == 0)
         ...
         if (SLocE.isFile() && SLocE.getFile().getContentCache()->getRawBuffer() == nullptr)
            return kFALSE;
         if (!FID.isInvalid())
            return kTRUE;
      }
                                              ...
                                              EFunctionMatchMode mode /* = kConversionMatch */)
{
   ...
   if (cl) {
      f = ((TClingClassInfo*)cl->GetClassInfo())->
         GetMethod(method, proto, objectIsConst, nullptr /*poffset*/, mode).InterfaceMethod(*fNormalizedCtxt);
   }
   else {
      TClingClassInfo gcl(GetInterpreterImpl());
