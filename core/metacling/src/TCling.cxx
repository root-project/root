...
void TCling::HandleNewDecl(const void* DV, bool isDeserialized, std::set<TClass*> &modifiedTClasses) {
   ...
   if (const RecordDecl *TD = dyn_cast<RecordDecl>(D)) {
      ...
      gROOT->GetListOfGlobals()->Add(
         new TGlobal((DataMemberInfo_t *)new TClingDataMemberInfo(fInterpreter, cast<ValueDecl>(ND), nullptr)));
   }
}
...
TCling::TCling(const char *name, const char *title)
   : TInterpreter(name, title), fGlobalsListSerial(-1), fInterpreter(nullptr), fMetaProcessor(nullptr),
     fNormalizedCtxt(nullptr), fPrevLoadedDynLibInfo(nullptr), fClingCallbacks(nullptr), fAutoLoadCallBack(nullptr),
...
Bool_t TCling::IsLoaded(const char* filename) const
{
   ...
   const clang::FileEntry *FE =
      HS.LookupFile(file_name.c_str(), clang::SourceLocation(),
                    /*isAngled*/ false,
   ...
   if (FE && FE->isValid()) {
      ...
      if (!FID.isInvalid() && FID.getHashValue() == 0)
         ...
         if (SLocE.isFile() && SLocE.getFile().getContentCache()->getRawBuffer() == nullptr) return kFALSE;
         if (!FID.isInvalid())
            return kTRUE;
      }
...
Int_t TCling::Load(const char* filename, Bool_t system)
{
   ...
   if (!canonLib.empty()) {
      if (system)
         ...
         cling::Interpreter::CompilationResult compRes;
         HandleInterpreterException(fMetaProcessor, Form(".L %s", canonLib.c_str()), compRes, /*cling::Value*/ nullptr);
         if (compRes == cling::Interpreter::kSuccess)
...
// declaration or do not have all the necessary interfaces).
   ...
   if (emulation) {
      ...
      if (oldvers == version && cl->GetClassInfo()) {
         ...
         TClingMethodInfo mi =
            cli->GetMethod("Class_Version", "", nullptr /*poffset*/, ROOT::kExactMatch, TClingClassInfo::kInThisScope);
         if (!mi.IsValid()) {
                                            ...
                                            EFunctionMatchMode mode /* = kConversionMatch */)
{
   ...
   if (cl) {
      return ((TClingClassInfo *)cl->GetClassInfo())
         ->GetMethod(method, proto, objectIsConst, nullptr /*poffset*/, mode)
         .GetMangledName();
   }
   TClingClassInfo gcl(fInterpreter);
                                              ...
                                              EFunctionMatchMode mode /* = kConversionMatch */)
{
   ...
   if (cl) {
      ...
      TClingClassInfo gcl(fInterpreter);
      f = gcl.GetMethod(method, proto, objectIsConst, nullptr /*poffset*/, mode).InterfaceMethod(*fNormalizedCtxt);
   }
   return f;
}
...
void TCling::UnloadClassMembers(TClass* cl, const clang::DeclContext* DC) {
   ...
   TDataMember *var = nullptr;
   TFunction *function = nullptr;
   TEnum *e = nullptr;
   TFunctionTemplate *functiontemplate = nullptr;
   ...
   for (DeclContext::decl_iterator RI = DC->decls_begin(), RE = DC->decls_end(); RI != RE; ++RI) {
      if (isa<VarDecl>(*RI) || isa<FieldDecl>(*RI)) {
         ...
         if (functiontemplate) {
            ...
            functiontemplate->Update(nullptr);
         }
      }
   }
}
...
{
   cling::Interpreter::CompilationResult compRes;
   HandleInterpreterException(fMetaProcessor, TString::Format(".L %s", path), compRes, /*cling::Value*/ nullptr);
...
int TCling::UnloadFile(const char* path) const
{
   ...
   cling::Interpreter::CompilationResult compRes;
   HandleInterpreterException(fMetaProcessor, Form(".U %s", canonical.c_str()), compRes, /*cling::Value*/ nullptr);
   return compRes == cling::Interpreter::kFailure;
}
