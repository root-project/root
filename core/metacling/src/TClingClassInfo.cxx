...
long TClingClassInfo::ClassProperty() const
{
   ...
   if (!RD) {
      ...
      // The historical CINT interface always returned 0L for these guys.
      return property;
   }
   if (RD->isUnion()) {
      // The historical CINT interface always returns 0L for these guys.
      return property;
   }
      ...
      EInheritanceMode /* imode = kWithInheritance*/) const
{
   ...
   if (!strcmp(arglist, ")")) {
      // Historical CINT accepted a single right paren as meaning no arguments.
      arglist = "";
   }
   const cling::LookupHelper &lh = fInterp->getLookupHelper();
   const FunctionDecl *fd
...
ROOT::TMetaUtils::EIOCtorCategory TClingClassInfo::HasDefaultConstructor(bool checkio, std::string *type_name) const
{
   ...
   // but this was not the case in historical CINT.
   //
...
{
   // IsLoaded in historical CINT was meaning is known to the interpreter
   // and has a complete definition.
   // IsValid in Cling means 'just' is known to the interpreter.
   if (!IsValid()) {
      return false;
   }
   if (GetDecl() == nullptr) {
...
int TClingClassInfo::InternalNext()
{
   ...
   while (true) {
      ...
      if (fFirstTime) {
         // The historical CINT Next() semantics were strange.
         fFirstTime = false;
         if (!*fIter) {
            return 0;
         }
