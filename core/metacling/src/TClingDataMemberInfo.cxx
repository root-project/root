...
Emulation of the historical CINT DataMemberInfo class.
...
from the Clang C++ compiler, not historical CINT.
*/
...
int TClingDataMemberInfo::Next()
{
   ...
   if (fFirstTime) {
      // The CINT Next() semantics were weird.
      fFirstTime = false;
   } else {
      fIter.Next();
   }
...
long TClingDataMemberInfo::Property() const
{
   ...
   if (const clang::VarDecl *vard = llvm::dyn_cast<clang::VarDecl>(vd)) {
      ...
      if (vard->getStorageClass() == clang::SC_Static) {
         ...
         // considered to be 'static' in the historical CINT (and thus ROOT) scheme.
         property |= kIsStatic;
      }
   } else if (llvm::isa<clang::EnumConstantDecl>(vd)) {
      // Enumeration constant are considered to be 'static' data member in
      // the historical CINT (and thus ROOT) scheme.
      property |= kIsStatic;
   }
...
const char *TClingDataMemberInfo::TypeName() const
{
   ...
   // In historical CINT's version, the type name returns did *not* include any array
