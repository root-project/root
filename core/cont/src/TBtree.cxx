...
void TBtree::Add(TObject *obj)
{
   ...
   if (!fRoot) {
      ...
      TBtNode *loc;
      Int_t idx;
      if (fRoot->Found(obj, &loc, &idx) != nullptr) {
...
TBtInnerNode::TBtInnerNode(TBtInnerNode *p, TBtree *t) : TBtNode(0,p,t)
{
   ...
   if (fItem == nullptr) ::Fatal("TBtInnerNode::TBtInnerNode", "no more memory");
}
...
Int_t TBtInnerNode::FindRankUp(const TBtNode *that) const
{
   ...
   for (Int_t i = 0; i < l; i++)
      sum += GetNofKeys(i);
   return sum + l + (fParent == nullptr ? 0 : fParent->FindRankUp(this));
}
...
void TBtInnerNode::IsFull(TBtNode *that)
{
   if (that->fIsLeaf) {
      ...
      Int_t hasRightSib = (leafidx < fLast) && ((right = (TBtLeafNode *)GetTree(leafidx + 1)) != nullptr);
      Int_t hasLeftSib = (leafidx > 0) && ((left = (TBtLeafNode *)GetTree(leafidx - 1)) != nullptr);
      ...
      Int_t hasRightSib = (inneridx < fLast) && ((right = (TBtInnerNode *)GetTree(inneridx + 1)) != nullptr);
      Int_t hasLeftSib = (inneridx > 0) && ((left = (TBtInnerNode *)GetTree(inneridx - 1)) != nullptr);
      Int_t rightSibFull = (hasRightSib && right->IsAlmostFull());
      Int_t leftSibFull  = (hasLeftSib  && left->IsAlmostFull());
      if (rightSibFull) {
         if (leftSibFull) {
...
TBtLeafNode::TBtLeafNode(TBtInnerNode *p, const TObject *obj, TBtree *t): TBtNode(1, p, t)
{
   ...
   if (obj != nullptr) fItem[++fLast] = (TObject *)obj; // cast const away
}
