...
TListOfFunctionTemplates::TListOfFunctionTemplates(TClass *cl)
   : fClass(cl), fIds(nullptr), fUnloaded(nullptr), fLastLoadMarker(0)
{
   fIds = new TExMap;
...
TObject *TListOfFunctionTemplates::FindObject(const char *name) const
{
   ...
   if (!result) {
      ...
      if (fClass) decl = gInterpreter->GetFunctionTemplate(fClass->GetClassInfo(),name);
         ...
         decl = gInterpreter->GetFunctionTemplate(nullptr, name);
   ...
   if (!f) {
      ...
      if (!f) {
         if (fClass) f = new TFunctionTemplate(m, fClass);
            ...
            f = new TFunctionTemplate(m, nullptr);
      }
