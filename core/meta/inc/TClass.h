...
private:
   ...
   mutable Long_t              fClassProperty = 0;  //!C++ Property of the class (is abstract, has virtual table, etc.)
           ...
           std::atomic<Bool_t> fHasRootPcmInfo = kFALSE;      //!Whether info was loaded from a root pcm.
   mutable std::atomic<Bool_t> fCanLoadClassInfo= kFALSE;     //!Indicates whether the ClassInfo is supposed to be available.
   mutable std::atomic<Bool_t> fIsOffsetStreamerSet = kFALSE; //!saved remember if fOffsetStreamer has been set.
   mutable std::atomic<Bool_t> fVersionUsed = kFALSE;         //!Indicates whether GetClassVersion has been called
