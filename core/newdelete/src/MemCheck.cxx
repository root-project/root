...
void TStackTable::Expand(int newsize)
{
   ...
   while (((char *) info->Next() - fTable) <= nextindex) {
      if (info->fNextHash != nullptr) info->fNextHash = (TStackInfo *)&fTable[(char *)info->fNextHash - tableold];
      info = info->Next();
   }
   //
   //update hash table
   for (int i = 0; i < fHashSize; i++)
      if (fHashTable[i] != nullptr) fHashTable[i] = (TStackInfo *)&fTable[((char *)fHashTable[i]) - tableold];
   //  printf("new table %p\n",fTable);
}
