...
FILE *TUnixSystem::TempFileName(TString &base, const char *dir)
{
   ...
   if (fd == -1) {
      ...
      FILE *fp = fdopen(fd, "w+");
      if (fp == nullptr) SysError("TempFileName", "converting filedescriptor (%d)", fd);
      return fp;
   }
}
...
{
   TString patbuf = path;
   if (ExpandPathName(patbuf)) return nullptr;
...
const char *TUnixSystem::GetLinkedLibraries()
{
   ...
   if (linkedLibs.IsNull()) return nullptr;
...
int TUnixSystem::AcceptConnection(int sock)
{
   ...
   while ((soc = ::accept(sock, nullptr, nullptr)) == -1 && GetErrno() == EINTR) ResetErrno();
...
void *TUnixSystem::UnixOpendir(const char *dir)
{
   ...
   if (stat(edir, &finfo) < 0) return nullptr;

   if (!S_ISDIR(finfo.st_mode)) return nullptr;
...
const char *TUnixSystem::FindDynamicLibrary(TString& sLib, Bool_t quiet)
{
   ...
   static const char *exts[] = {".so", ".dll", ".dylib", ".sl", ".dl", ".a", nullptr};
   const char** ext = exts;
   while (*ext) {
      TString fname(sLib);
      fname += *ext;
