...
typedef float          Float16_t;   ///< Float 4 bytes in memory, written to disk as a 3 bytes (24-bits, not 16-bits) float with a truncated (up-to) 16-bit mantissa (12-bit by default in memory), and (7+1)-bits of exponent \deprecated Consider (non-equivalent) alternatives in `<cstdfloat>`. \warning Do not mix this with a half-float such as std::float16_t
typedef double         Double_t;    ///< Double 8 bytes \deprecated Consider replacing with `double`.
typedef double         Double32_t;  ///< Double 8 bytes in memory, written to disk as a 3 bytes (24-bits, not 32-bits) float with a truncated (up-to) 16-bit mantissa (24-bit by default in memory), and (7+1)-bits of exponent \deprecated Consider (non-equivalent) alternatives in `<cstdfloat>`. \warning Do not mix this with a single-precision float std::float32_t
...
typedef unsigned long long ULongptr_t;///< Unsigned integer large enough to hold a pointer \deprecated Consider replacing with `unsigned long long` on Windows.
#else
...
constexpr Int_t kMaxUChar = 256;          ///< \deprecated Consider replacing with `std::numeric_limits<unsigned char>::max()` (or `std::uint8_t`)
...
constexpr Int_t kMaxUShort = 65534;         ///< \deprecated Consider replacing with `std::numeric_limits<unsigned short>::max()` (or `std::uint16_t`)
