                                       ...
                                       Bool_t isRegexp)
{
   ...
   if ( (opt.First('L')!=kNPOS) ) {
      ...
      // Also seed libpaths with ROOT's own import-library directory and the
      // shared-library directory.
      if (gROOT) {
         TString rootLibDir = gROOT->GetLibDir();
         if (!rootLibDir.IsNull())
            libpaths.push_back(rootLibDir.Data());
         TString rootSharedLibDir = gROOT->GetSharedLibDir();
         if (!rootSharedLibDir.IsNull() && rootSharedLibDir != rootLibDir)
            libpaths.push_back(rootSharedLibDir.Data());
      }
