...
bool XMLReader::CheckIsTagOK(const std::string& tag)
{
   ...
   if (isspace(tag.at(1))){
      ROOT::TMetaUtils::Error(
         nullptr, "Malformed tag %s (there should be no white-spaces between < and name-of-tag)!\n", tag.c_str());
      return false;
   }
...
//                   }
                  ...
                  if (attr_name=="pattern" && attr_value.find("*") == std::string::npos){
                     ROOT::TMetaUtils::Warning(nullptr,
                                               "At line %s. A pattern, \"%s\", without wildcards is being used. This "
                                               "selection rule would not have any effect. Transforming it to a rule "
                                               "based on name.\n",
...
bool XMLReader::Parse(const std::string &fileName, SelectionRules& out)
{
   ...
   BaseSelectionRule *bsr =
      nullptr; // Pointer to the base class, in it is written information about the current sel. rule
   ...
   while(file.good()){
      ...
      if (!tagOK){
         ROOT::TMetaUtils::Error(nullptr, "At line %s. Bad tag: %s\n", lineNumCharp, tagStrCharp);
      ...
      if (!tagStr.empty()){
         ...
         if (!attrError) {
            ROOT::TMetaUtils::Error(nullptr, "Attribute at line %s. Bad tag: %s\n", lineNumCharp, tagStrCharp);
            out.ClearSelectionRules();
            return false;
         }
         ...
         switch (tagKind){
            case kInvalid:
            {
               ROOT::TMetaUtils::Error(nullptr, "At line %s. Unrecognized name of tag %s\n", lineNumCharp, tagStrCharp);
            ...
            case kClass:
            {
               if (inClass){
                  //this is an error
                  ROOT::TMetaUtils::Error(nullptr, "At line %s. Tag %s inside a <class> element\n", lineNumCharp,
                                          tagStrCharp);
                  out.ClearSelectionRules();
                  return false;
               }
            ...
            case kEndClass:
            {
               if (inClass) { // if this is closing a parent class element, clear the parent information
                  ...
                  ROOT::TMetaUtils::Error(nullptr, "Single </class> tag at line %s", lineNumCharp);
                  out.ClearSelectionRules();
                  return false;
               }
               break;
            ...
            {
               if (!inClass){
                  ROOT::TMetaUtils::Error(nullptr, "Version tag not within class element at line %s", lineNumCharp);
                  out.ClearSelectionRules();
                  return false;
               }
               break;
            ...
            {
               if (!inIoread){
                  ROOT::TMetaUtils::Error(nullptr, "Single </ioread> at line %s", lineNumCharp);
                  out.ClearSelectionRules();
                  return false;
               }
               inIoread = false;
            ...
            case kEndSelection:
            {
               ...
               if (selection) { // if we had opening selection tag, everything is OK
                  ...
                  ROOT::TMetaUtils::Error(nullptr, "At line %s. Missing <selection> tag", lineNumCharp);
                  out.ClearSelectionRules();
                  return false;
               }
               break;
            ...
            case kEndExclusion:
            {
               ...
               if (exclusion) { // if exclusion is Set, everything is OK
                  ...
                  ROOT::TMetaUtils::Error(nullptr, "At line %s. Missing <exclusion> tag", lineNumCharp);
                  out.ClearSelectionRules();
                  return false;
               }
               break;
            ...
            case kField:
            {
               if (!inClass){ // if we have a <field>, <method> or <properties> tag outside a parent <clas>s tag,
                  //this is an error
                  ROOT::TMetaUtils::Error(nullptr, "At line %s. Tag %s not inside a <class> element\n", lineNumCharp,
                                          tagStrCharp);
                  out.ClearSelectionRules();
                  return false;
               }
            ...
            {
               if (!inField){
                  ROOT::TMetaUtils::Error(nullptr, "At line %s. Closing field tag which was not opened\n",
                                          lineNumCharp);
                  out.ClearSelectionRules();
                  return false;
               }
            ...
            {
               if (!inMethod){
                  ROOT::TMetaUtils::Error(nullptr, "At line %s. Closing method tag which was not opened\n",
                                          lineNumCharp);
                  out.ClearSelectionRules();
                  return false;
               }
            ...
            case kProperties:
            {
               ...
               if (!IsStandaloneTag(tagStr)) {
                  ROOT::TMetaUtils::Error(nullptr, "At line %s. Tag should be standalone\n", lineNumCharp);
                  out.ClearSelectionRules();
                  return false;
               }
            ...
            default: ROOT::TMetaUtils::Error(nullptr, "Unknown tag name: %s \n", tagStrCharp);
         }
         ...
         if (tagKind == kBeginIoread || tagKind == kBeginIoreadRaw){
            ...
            if (attrs.empty()){
               ROOT::TMetaUtils::Error(nullptr, "At line %s. ioread element has no attributes.\n", lineNumCharp);
               return false;
            }
            ...
            // in a global then read by the TMetaUtils
               ROOT::TMetaUtils::Info(nullptr, "Pragma generated for ioread rule: %s\n",
               ...
               if (!error_string.empty()) ROOT::TMetaUtils::Error(nullptr, "%s", error_string.c_str());
                  ...
                  if (tagKind == kClass ||
                     ...
                     if (bsr->HasAttributeWithName(iAttrName)) {
                        ...
                        if (preExistingValue!=iAttrValue){ // If different from before
                           ROOT::TMetaUtils::Error(
                              nullptr, "Line %s: assigning new value %s to attribue %s (it was %s)\n", lineNumCharp,
                              iAttrValue.c_str(), iAttrName.c_str(), preExistingValue.c_str());
                           out.ClearSelectionRules();
                           return false;
                        }
                     }
