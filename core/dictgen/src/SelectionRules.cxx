...
// behaviour of historical rootcint and genreflex especially with regard to class pattern processing.
// In genreflex if we have <class pattern = "*" /> this will select all the classes
// (and structs) found in the header file. In historical rootcint if we had something similar, i.e.
...
const ClassSelectionRule *SelectionRules::IsNamespaceSelected(const clang::Decl* D, const std::string& qual_name) const
{
   ...
   for(; it != fClassSelectionRules.end(); ++it) {
      ...
      if (match != BaseSelectionRule::kNoMatch) {
         ...
         if (it->GetSelected() == BaseSelectionRule::kYes) {
            selector = &(*it);
            if (IsLinkdefFile()){
               // historical rootcint preferred explicit rules over pattern rules
   ...
   if (IsLinkdefFile()) {
      // for historical rootcint explicit (name) Yes is stronger than implicit (pattern) No which is stronger than implicit (pattern) Yes
   ...
   for(auto& selRule: fVariableSelectionRules) {
      ...
      if (match != BaseSelectionRule::kNoMatch) {
         if (selRule.GetSelected() == BaseSelectionRule::kYes) {
            // explicit rules are with stronger priority in historical rootcint
            if (IsLinkdefFile()){
               if (match == BaseSelectionRule::kName) {
...
// In historical rootcint we could select and deselect methods independently of the class/struct/union rules
