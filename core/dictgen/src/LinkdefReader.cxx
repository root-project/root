...
//                                                                      //
//                                                                      //
// Note: some inconsistency in the way historical CINT parsed the #pragma:         //
 ...
 * Sometimes I had to do strange things to reflect the strange behavior of historical rootcint
 */
   ...
   switch (name) {
      ...
      case kFunction: {
            ...
            if (localIdentifier.at(localIdentifier.length() - 1) == '*') fsr.SetAttributeValue("pattern", localIdentifier);
               ...
               int pos = localIdentifier.find("(*)"); //historical rootcint generated error here but I decided to implement that pattern
      ...
      case kStruct: {
            ...
            if (linkOn) {
               ...
               if (localIdentifier == "*") { // historical rootcint generated error here, but I decided to implement it
               ...
               if (localIdentifier == "*") { // historical rootcint generated error here, but I decided to implement it
                  ...
                  EnumSelectionRule esr(fCount++, fInterp); // we need this because of implicit/explicit rules - check my notes on historical rootcint
                  esr.SetSelected(BaseSelectionRule::kNo);
                  esr.SetAttributeValue("pattern", "*::*");
                  fSelectionRules->AddEnumSelectionRule(esr);
               ...
               // else {
               //    EnumSelectionRule esr(fCount++); // we need this because of implicit/explicit rules - check my notes on historical rootcint
               //    esr.SetSelected(BaseSelectionRule::kNo);
               //    esr.SetAttributeValue("pattern", localIdentifier+"::*");
               //    fSelectionRules->AddEnumSelectionRule(esr);

               //    if (fSelectionRules->GetHasFileNameRule()) {
               //       FunctionSelectionRule fsr(fCount++); // we need this because of implicit/explicit rules - check my notes on historical rootcint
               //       fsr.SetSelected(BaseSelectionRule::kNo);
      ...
      case kIOCtorType:
         ...
         break;
      case kIgnore:
         // All the pragma that were supported in historical CINT but are currently not relevant for CLING
...
bool LinkdefReader::ProcessFunctionPrototype(std::string &proto, bool &name)
{
   ...
   if (pos1 > -1) {
      ...
      // I don't have to escape the *-s because in historical rootcint there is no pattern recognition
      int pos3 = pos1;
      while (true) {
...
public:
   ...
   bool ProcessOptions(LinkdefReader::Options &options,
       ...
       *   stub: (ignored by rootcling was a directly for historical CINT code generation)
      ...
      while (tok.isNot(clang::tok::eod) && tok.isNot(clang::tok::semi)) {
         if (!tok.getIdentifierInfo()) {
            ...
            // This was solely for historical CINT dictionary, ignore for now.
            // options.fUseStubs = 1;
