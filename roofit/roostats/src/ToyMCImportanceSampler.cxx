...
RooDataSet* ToyMCImportanceSampler::GetSamplingDistributionsSingleWorker(RooArgSet& paramPoint) {
   ...
   for( int i = -1; i < (int)fImportanceDensities.size(); i++ ) {
      if( i < 0 ) {
         // generate null toys
         oocoutP((TObject*)nullptr,Generation) << endl << endl << "   GENERATING FROM NULL DENSITY " << endl << endl;
         SetDensityToGenerateFromByIndex( 0, true ); // true = generate from null
      }else{
         oocoutP((TObject*)nullptr,Generation) << endl << endl << "   GENERATING IMP DENS/SNAP "<<i+1<<"  OUT OF "<<fImportanceDensities.size()<<endl<<endl;
      ...
      ooccoutI((TObject*)nullptr,InputArguments) << "Generating " << fNToys << " toys for this density." << endl;
      ooccoutI((TObject*)nullptr,InputArguments) << "Reweight is " << reweight.getVal() << endl;
...
RooAbsData* ToyMCImportanceSampler::GenerateToyData(
   ...
   if( fNullDensities.size() > 1 ) {
      ooccoutI((TObject*)nullptr,InputArguments) << "Null Densities:" << endl;
      for( unsigned int i=0; i < fNullDensities.size(); i++) {
         ooccoutI((TObject*)nullptr,InputArguments) << "  null density["<<i<<"]: " << fNullDensities[i] << " \t null snapshot["<<i<<"]: " << fNullSnapshots[i] << endl;
      }
      ooccoutE((TObject*)nullptr,InputArguments) << "Cannot use multiple null densities and only ask for one weight." << endl;
      return nullptr;
   }

   if( fNullDensities.size() == 0  &&  fPdf ) {
      ooccoutI((TObject*)nullptr,InputArguments) << "No explicit null densities specified. Going to add one based on the given paramPoint and the global fPdf. ... but cannot do that inside const function." << endl;
   ...
   if( fNullSnapshots[0] != &paramPoint ) {
      ooccoutD((TObject*)nullptr,InputArguments) << "Using given parameter point. Replaces snapshot for the only null currently defined." << endl;
...
RooAbsData* ToyMCImportanceSampler::GenerateToyData(
   ...
   if( fNullDensities.size() > 1 ) {
      ...
      ooccoutE((TObject*)nullptr,InputArguments) << "Cannot use multiple null densities and only ask for one weight and NLL." << endl;
      return nullptr;
   }
   ...
   ooccoutI((TObject*)nullptr,InputArguments) << "Using given parameter point. Overwrites snapshot for the only null currently defined." << endl;
   if(fNullSnapshots[0]) delete fNullSnapshots[0];
   fNullSnapshots.clear();
...
RooAbsData* ToyMCImportanceSampler::GenerateToyData(
   ...
   if( fNullDensities.size() != weights.size() ) {
      ooccoutI((TObject*)nullptr,InputArguments) << "weights.size() != nullDesnities.size(). You need to provide a vector with the correct size." << endl;
      //AddNullDensity( fPdf, &paramPoint );
   }
...
RooAbsData* ToyMCImportanceSampler::GenerateToyData(
   ...
   ooccoutD((TObject*)nullptr,InputArguments) << endl;
   ooccoutD((TObject*)nullptr,InputArguments) << "GenerateToyDataImportanceSampling" << endl;
   ...
   if( fNullDensities.size() == 0 ) {
      oocoutE((TObject*)nullptr,InputArguments) << "ToyMCImportanceSampler: Need to specify the null density explicitly." << endl;
      return nullptr;
   }
   ...
   if( fNullDensities.size() != fNullNLLs.size() ) {
      oocoutE((TObject*)nullptr,InputArguments) << "ToyMCImportanceSampler: Something wrong. NullNLLs must be of same size as null densities." << endl;
      return nullptr;
   }
   ...
   if( (!fGenerateFromNull  &&  fIndexGenDensity >= fImportanceDensities.size()) ||
      ...
      oocoutE((TObject*)nullptr,InputArguments) << "ToyMCImportanceSampler: no importance density given or index out of range." << endl;
      return nullptr;
   }
   ...
   *allVars = paramPoint;
   ...
   if (!data) {
      oocoutE((TObject*)nullptr,InputArguments) << "ToyMCImportanceSampler: error generating data" << endl;
      return nullptr;
   }
   ...
   ooccoutD((TObject*)nullptr,InputArguments) << "About to create/calculate all nullNLLs." << endl;
   for( unsigned int i=0; i < fNullDensities.size(); i++ ) {
   ...
   ooccoutD((TObject*)nullptr,InputArguments) << "About to find the minimum NLLs." << endl;
   ...
   for( unsigned int i=0; i < fImportanceDensities.size(); i++ ) {
      ...
      for( unsigned int j=0; j < nullNLLVals.size(); j++ ) {
         ...
         ooccoutD((TObject*)nullptr,InputArguments) << "minNLLVals["<<j<<"]: " << minNLLVals[j] << "  nullNLLVals["<<j<<"]: " << nullNLLVals[j] << "    impNLLVals["<<i<<"]: " << impNLLVals[i] << endl;
      }
   }

   // veto toys: this is a sort of "overlap removal" of the various distributions
   // if not vetoed: apply weight
   ooccoutD((TObject*)nullptr,InputArguments) << "About to apply vetos and calculate weights." << endl;
   for( unsigned int j=0; j < nullNLLVals.size(); j++ ) {
      ...
      ooccoutD((TObject*)nullptr,InputArguments) << "weights["<<j<<"]: " << weights[j] << endl;
   }
...
int ToyMCImportanceSampler::CreateImpDensitiesForOnePOIAdaptively( RooAbsPdf& pdf, const RooArgSet& allPOI, RooRealVar& poi, double nStdDevOverlap, double poiValueForBackground ) {
   ...
   if( poi.getError() > 0.01  &&  poi.getError() < 5.0 ) {
      ...
      oocoutI((TObject*)nullptr,InputArguments) << "Using fitFavoredMu and error to set the number of imp points" << endl;
      oocoutI((TObject*)nullptr,InputArguments) << "muhat: " << poi.getVal() << "    optimize for distance: " << 2.*nStdDevOverlap*poi.getError() << endl;
      oocoutI((TObject*)nullptr,InputArguments) << "n = " << n << endl;
      oocoutI((TObject*)nullptr,InputArguments) << "This results in a distance of: " << impMaxMu / n << endl;
   }
...
int ToyMCImportanceSampler::CreateNImpDensitiesForOnePOI( RooAbsPdf& pdf, const RooArgSet& allPOI, RooRealVar& poi, int n, double poiValueForBackground ) {
   ...
   if( impMaxMu > poiValueForBackground  &&  n > 0 ) {
      for( int i=1; i <= n; i++ ) {
         ...
         oocoutI((TObject*)nullptr,InputArguments) << endl << "create point with poi: " << endl;
         poi.Print();
