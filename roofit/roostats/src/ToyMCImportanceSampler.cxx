...
void ToyMCImportanceSampler::ClearCache(void) {
   ...
   for( unsigned int i=0; i < fImpNLLs.size(); i++ ) if(fImpNLLs[i]) { delete fImpNLLs[i];
         fImpNLLs[i] = nullptr;
      }
   for( unsigned int i=0; i < fNullNLLs.size(); i++ ) if(fNullNLLs[i]) { delete fNullNLLs[i];
         fNullNLLs[i] = nullptr;
      }
}
...
RooDataSet* ToyMCImportanceSampler::GetSamplingDistributionsSingleWorker(RooArgSet& paramPoint) {
   ...
   for( int i = -1; i < (int)fImportanceDensities.size(); i++ ) {
      if( i < 0 ) {
         ...
         oocoutP((TObject *)nullptr, Generation)
            << endl
            << endl
            << "   GENERATING IMP DENS/SNAP " << i + 1 << "  OUT OF " << fImportanceDensities.size() << endl
            << endl;
...
RooAbsData* ToyMCImportanceSampler::GenerateToyData(
   ...
   if( fNullDensities.size() > 1 ) {
      ...
      for( unsigned int i=0; i < fNullDensities.size(); i++) {
         ooccoutI((TObject *)nullptr, InputArguments)
            << "  null density[" << i << "]: " << fNullDensities[i] << " \t null snapshot[" << i
   ...
   if( fNullDensities.size() == 0  &&  fPdf ) {
      ooccoutI((TObject *)nullptr, InputArguments)
         << "No explicit null densities specified. Going to add one based on the given paramPoint and the global fPdf. "
            "... but cannot do that inside const function."
         << endl;
   ...
   if( fNullSnapshots[0] != &paramPoint ) {
      ooccoutD((TObject *)nullptr, InputArguments)
   ...
   *allVars = paramPoint;
   ...
   for( unsigned int i=0; i < fNullDensities.size(); i++ ) {
      ...
      *allVars = *fNullSnapshots[i];
      ...
      // FOR DEBuGGING!!!!!!!!!!!!!!!!!
      if( !fReuseNLL ) { delete fNullNLLs[i];
         fNullNLLs[i] = nullptr;
      }
   }
   ...
   for( unsigned int i=0; i < fImportanceDensities.size(); i++ ) {
      ...
      // FOR DEBuGGING!!!!!!!!!!!!!!!!!
      if( !fReuseNLL ) { delete fImpNLLs[i];
         fImpNLLs[i] = nullptr;
      }
      ...
      for( unsigned int j=0; j < nullNLLVals.size(); j++ ) {
         ...
         ooccoutD((TObject *)nullptr, InputArguments)
            << "minNLLVals[" << j << "]: " << minNLLVals[j] << "  nullNLLVals[" << j << "]: " << nullNLLVals[j]
            << "    impNLLVals[" << i << "]: " << impNLLVals[i] << endl;
      }
   }
...
int ToyMCImportanceSampler::CreateImpDensitiesForOnePOIAdaptively( RooAbsPdf& pdf, const RooArgSet& allPOI, RooRealVar& poi, double nStdDevOverlap, double poiValueForBackground ) {
   ...
   if( poi.getError() > 0.01  &&  poi.getError() < 5.0 ) {
      ...
      oocoutI((TObject *)nullptr, InputArguments)
         << "muhat: " << poi.getVal() << "    optimize for distance: " << 2. * nStdDevOverlap * poi.getError() << endl;
