...
int HybridCalculator::CheckHook(void) const {
   ...
   if( fPriorNuisanceNull && (!fNullModel->GetNuisanceParameters() || fNullModel->GetNuisanceParameters()->getSize() == 0) ) {
      oocoutE((TObject *)nullptr, InputArguments)
         << "HybridCalculator - Nuisance PDF has been specified, but is unaware of which parameters are the nuisance "
            "parameters. Must set nuisance parameters in the Null ModelConfig."
         << endl;
      return -1; // error
   }
   if( fPriorNuisanceAlt && (!fAltModel->GetNuisanceParameters() || fAltModel->GetNuisanceParameters()->getSize() == 0) ) {
      oocoutE((TObject *)nullptr, InputArguments)
         << "HybridCalculator - Nuisance PDF has been specified, but is unaware of which parameters are the nuisance "
            "parameters. Must set nuisance parameters in the Alt ModelConfig"
         << endl;
      return -1; // error
   }
...
int HybridCalculator::PreNullHook(RooArgSet* /*parameterPoint*/, double obsTestStat) const {
   ...
   } else if (fNullModel->GetNuisanceParameters() == nullptr || fNullModel->GetNuisanceParameters()->getSize() == 0) {
...
int HybridCalculator::PreAltHook(RooArgSet* /*parameterPoint*/, double obsTestStat) const {
   ...
   } else if (fAltModel->GetNuisanceParameters() == nullptr || fAltModel->GetNuisanceParameters()->getSize() == 0) {
      oocoutI((TObject *)nullptr, InputArguments)
