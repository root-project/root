...
ProfileLikelihoodCalculator::ProfileLikelihoodCalculator()
   : CombinedCalculator(), fFitResult(nullptr), fGlobalFitDone(false)
{
}

ProfileLikelihoodCalculator::ProfileLikelihoodCalculator(RooAbsData &data, RooAbsPdf &pdf,
                                                         const RooArgSet &paramsOfInterest, Double_t size,
                                                         const RooArgSet *nullParams)
   : CombinedCalculator(data, pdf, paramsOfInterest, size, nullParams), fFitResult(nullptr), fGlobalFitDone(false)
{
...
ProfileLikelihoodCalculator::ProfileLikelihoodCalculator(RooAbsData &data, ModelConfig &model, Double_t size)
   : CombinedCalculator(data, model, size), fFitResult(nullptr), fGlobalFitDone(false)
{
...
RooAbsReal *  ProfileLikelihoodCalculator::DoGlobalFit() const {
   ...
   if (fFitResult && fGlobalFitDone) {
      ...
      // calculate MLE
   oocoutP((TObject *)nullptr, Minimization) << "ProfileLikelihoodCalcultor::DoGLobalFit - find MLE " << std::endl;
   ...
   // print fit result
   if (fFitResult) {
      fFitResult->printStream(oocoutI((TObject *)nullptr, Minimization), fFitResult->defaultPrintContents(nullptr),
                              fFitResult->defaultPrintStyle(nullptr));

      if (fFitResult->status() != 0)
         oocoutW((TObject *)nullptr, Minimization)
            << "ProfileLikelihoodCalcultor::DoGlobalFit -  Global fit failed - status = " << fFitResult->status()
            << std::endl;
      else
         fGlobalFitDone = true;
   }
...
RooFitResult * ProfileLikelihoodCalculator::DoMinimizeNLL(RooAbsReal * nll)  {
   ...
   oocoutP((TObject *)nullptr, Minimization) << "ProfileLikelihoodCalcultor::DoMinimizeNLL - using " << minimType
...
//    RooAbsData* data = fWS->data(fDataName);
   RooAbsPdf * pdf = GetPdf();
   RooAbsData* data = GetData();
   if (!data || !pdf || fPOI.getSize() == 0) return nullptr;
...
//    RooAbsData* data = fWS->data(fDataName);
   ...
   if (fNullParams.getSize() == 0) return nullptr;
   ...
   if (existVarParams) {
      oocoutP((TObject *)nullptr, Minimization)
      ...
      if (fit2) {
         ...
         fit2->printStream(oocoutI((TObject *)nullptr, Minimization), fit2->defaultPrintContents(nullptr),
                           fit2->defaultPrintStyle(nullptr));

         if (fit2->status() != 0)
            oocoutW((TObject *)nullptr, Minimization)
               << "ProfileLikelihoodCalcultor::GetHypotest -  Conditional fit failed - status = " << fit2->status()
               << std::endl;
      }
