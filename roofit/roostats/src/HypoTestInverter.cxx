...
HypoTestInverter::HypoTestInverter()
   : fTotalToysRun(0), fMaxToys(0), fCalculator0(nullptr), fScannedVariable(nullptr), fResults(nullptr), fUseCLs(false),
     fScanLog(false), fSize(0), fVerbose(0), fCalcType(kUndefined), fNBins(0), fXmin(1), fXmax(1), fNumErr(0)
{
}
...
HypoTestInverter::HypoTestInverter(HypoTestCalculatorGeneric &hc, RooRealVar *scannedVariable, double size)
   : fTotalToysRun(0), fMaxToys(0), fCalculator0(nullptr), fScannedVariable(scannedVariable), fResults(nullptr),
     fUseCLs(false), fScanLog(false), fSize(size), fVerbose(0), fCalcType(kUndefined), fNBins(0), fXmin(1), fXmax(1),
     fNumErr(0)
{
...
HypoTestInverter::HypoTestInverter(HybridCalculator &hc, RooRealVar *scannedVariable, double size)
   : fTotalToysRun(0), fMaxToys(0), fCalculator0(&hc), fScannedVariable(scannedVariable), fResults(nullptr),
     fUseCLs(false), fScanLog(false), fSize(size), fVerbose(0), fCalcType(kHybrid), fNBins(0), fXmin(1), fXmax(1),
     fNumErr(0)
{
...
HypoTestInverter::HypoTestInverter(FrequentistCalculator &hc, RooRealVar *scannedVariable, double size)
   : fTotalToysRun(0), fMaxToys(0), fCalculator0(&hc), fScannedVariable(scannedVariable), fResults(nullptr),
     fUseCLs(false), fScanLog(false), fSize(size), fVerbose(0), fCalcType(kFrequentist), fNBins(0), fXmin(1), fXmax(1),
     fNumErr(0)
{
...
HypoTestInverter::HypoTestInverter(AsymptoticCalculator &hc, RooRealVar *scannedVariable, double size)
   : fTotalToysRun(0), fMaxToys(0), fCalculator0(&hc), fScannedVariable(scannedVariable), fResults(nullptr),
     fUseCLs(false), fScanLog(false), fSize(size), fVerbose(0), fCalcType(kAsymptotic), fNBins(0), fXmin(1), fXmax(1),
     fNumErr(0)
{
...
HypoTestInverter::HypoTestInverter(RooAbsData &data, ModelConfig &sbModel, ModelConfig &bModel,
   ...
   : fTotalToysRun(0), fMaxToys(0), fCalculator0(nullptr), fScannedVariable(scannedVariable), fResults(nullptr),
     fUseCLs(false), fScanLog(false), fSize(size), fVerbose(0), fCalcType(type), fNBins(0), fXmin(1), fXmax(1),
     fNumErr(0)
{
...
HypoTestInverter::HypoTestInverter(const HypoTestInverter &rhs)
   : IntervalCalculator(), fTotalToysRun(0), fCalculator0(nullptr), fScannedVariable(nullptr), // add these for Coverity
     fResults(nullptr)
{
   (*this) = rhs;
}
...
HypoTestInverterResult* HypoTestInverter::GetInterval() const {
   ...
   if (fResults && fResults->ArraySize() >= 1) {
      oocoutI((TObject *)nullptr, Eval) << "HypoTestInverter::GetInterval - return an already existing interval "
                                        << std::endl;
...
bool HypoTestInverter::RunFixedScan( int nBins, double xMin, double xMax, bool scanLog ) const
{
   ...
   if ( xMin>xMax ) {
      oocoutE((TObject *)nullptr, InputArguments) << "HypoTestInverter::RunFixedScan - Please provide xMin (" << xMin
   ...
   if (xMin < fScannedVariable->getMin()) {
      ...
      oocoutW((TObject *)nullptr, InputArguments)
         << "HypoTestInverter::RunFixedScan - xMin < lower bound, use xmin = " << xMin << std::endl;
   }
   if (xMax > fScannedVariable->getMax()) {
      xMax = fScannedVariable->getMax();
      oocoutW((TObject *)nullptr, InputArguments)
         << "HypoTestInverter::RunFixedScan - xMax > upper bound, use xmax = " << xMax << std::endl;
   }
...
bool HypoTestInverter::RunOnePoint( double rVal, bool adaptive, double clTarget) const
{
   ...
   if ( rVal < fScannedVariable->getMin() ) {
      oocoutE((TObject *)nullptr, InputArguments)
         << "HypoTestInverter::RunOnePoint - Out of range: using the lower bound " << fScannedVariable->getMin()
   ...
   if ( rVal > fScannedVariable->getMax() ) {
      ...
      if ( rVal > fScannedVariable->getMax()*(1.+1.E-12) )
         oocoutE((TObject *)nullptr, InputArguments)
            << "HypoTestInverter::RunOnePoint - Out of range: using the upper bound " << fScannedVariable->getMax()
            << " on the scanned variable rather than " << rVal << "\n";
   ...
   if (fVerbose > 0)
      oocoutP((TObject *)nullptr, Eval) << "Running for " << fScannedVariable->GetName() << " = "
   ...
   if (TMath::IsNaN(result->NullPValue() ) && TMath::IsNaN(result->AlternatePValue() ) ) {
      oocoutW((TObject *)nullptr, Eval) << "HypoTestInverter - Skip invalid result for  point "
                                        << fScannedVariable->GetName() << " = " << fScannedVariable->getVal() << endl;
...
// bool HybridNew::runLimit(RooWorkspace *w, RooStats::ModelConfig *mc_s, RooStats::ModelConfig *mc_b, RooAbsData &data, double &limit, double &limitErr, const double *hint) {
   ...
   if ((hint != nullptr) && (*hint > r->getMin())) {
      ...
      oocoutI((TObject *)nullptr, InputArguments)
         << "HypoTestInverter::RunLimit - Use hint value " << *hint << " search in interval " << r->getMin() << " , "
         << r->getMax() << std::endl;
  }
  ...
  for (int tries = 0; tries < 6; ++tries) {
     ...
     if (tries == 5) {
        oocoutE((TObject *)nullptr, Eval)
           << "HypoTestInverter::RunLimit - Cannot set higher limit: at " << r->GetName() << " = " << rMax
           << " still get " << (fUseCLs ? "CLs" : "CLsplusb") << " = " << clsMax.first << std::endl;
        return false;
     }
  }
  if (fVerbose > 0) {
     oocoutI((TObject *)nullptr, Eval) << "HypoTestInverter::RunLimit - Search for lower limit to the limit"
                                       << std::endl;
  }
  ...
  if (clsMin.first != 1 && clsMin.first - 3 * fabs(clsMin.second) < clsTarget) {
     if (fUseCLs) {
        ...
        for (int tries = 0; tries < 6; ++tries) {
           ...
           if (tries == 5) {
              oocoutE((TObject *)nullptr, Eval)
                 << "HypoTestInverter::RunLimit - Cannot set lower limit: at " << r->GetName() << " = " << rMin
                 << " still get " << (fUseCLs ? "CLs" : "CLsplusb") << " = " << clsMin.first << std::endl;
              return false;
           }
        }
     }
  ...
  if (fVerbose > 0)
     oocoutI((TObject *)nullptr, Eval) << "HypoTestInverter::RunLimit - Now doing proper bracketing & bisection"
                                       << std::endl;
  do {
...
//                  clsMax = eval(w, mc_s, mc_b, data, rMax, true, clsTarget);
  ...
  if (!done) { // didn't reach accuracy with scan, now do fit
      ...
      for (int i = 0, imax = /*(readHybridResults_ ? 0 : */  8; i <= imax; ++i, ++npoints) {
          ...
          if (fVerbose) {
             oocoutI((TObject *)nullptr, Eval) << "Fit to " << npoints << " points: " << expoFit.GetParameter(2)
...
//if (!plot_.empty() && fLimitPlot.get()) {
  ...
  oocoutI((TObject *)nullptr, Eval) << "HypoTestInverter::RunLimit - Result:    \n"
                                    << "\tLimit: " << r->GetName() << " < " << limit << " +/- " << limitErr << " @ "
                                    << (1 - fSize) * 100 << "% CL\n";
...
SamplingDistribution * HypoTestInverter::RebuildDistributions(bool isUpper, int nToys, TList * clsDist, TList * clsbDist, TList * clbDist, const char *outputfile) {
   ...
   if (fNBins <=0  && storePValues) {
      oocoutW((TObject *)nullptr, InputArguments)
         << "HypoTestInverter::RebuildDistribution - cannot return p values distribution with the auto scan"
         << std::endl;
      storePValues = false;
      nPoints = 0;
   }
   ...
   if (storePValues) {
      ...
      if (nPoints <=0) {
         oocoutE((TObject *)nullptr, InputArguments)
            << "HypoTestInverter - result is not existing and number of point to scan is not set" << std::endl;
         return nullptr;
      }
   }
   ...
   oocoutI((TObject *)nullptr, InputArguments)
      << "HypoTestInverter - rebuilding  the p value distributions by generating ntoys = " << nToys << std::endl;
   ...
   for (int itoy = 0; itoy < nToys; ++itoy) {
      ...
      for (int ipoint = 0; ipoint < nPoints; ++ipoint) {
         ...
         if (hr) {
            ...
            oocoutW((TObject *)nullptr, InputArguments)
               << "HypoTestInverter: missing result for point: x = " << fResults->GetXValue(ipoint) << std::endl;
         }
      }
      // write every 10 toys
      if (itoy%10 == 0 || itoy == nToys-1) {
