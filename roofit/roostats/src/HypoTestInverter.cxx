...
bool HypoTestInverter::RunFixedScan( int nBins, double xMin, double xMax, bool scanLog ) const
{
   ...
   for (int i=0; i<nBins; i++) {
      ...
      if ( status==false ) {
        oocoutW((TObject*)nullptr,Eval) << "HypoTestInverter::RunFixedScan - The hypo test for point " << thisX << " failed. Skipping." << std::endl;
      }
   }
...
bool HypoTestInverter::RunOnePoint( double rVal, bool adaptive, double clTarget) const
{
   ...
   if (!std::isfinite(nullPV) || nullPV < 0. || nullPV > 1. || !std::isfinite(altPV) || altPV < 0. || altPV > 1.) {
      oocoutW((TObject*)nullptr,Eval) << "HypoTestInverter - Skipping invalid result for  point " << fScannedVariable->GetName() << " = " <<
...
bool HypoTestInverter::RunLimit(double &limit, double &limitErr, double absAccuracy, double relAccuracy, const double*hint) const {
  ...
  for (int tries = 0; tries < 6; ++tries) {
     if (! RunOnePoint(rMax) ) {
        oocoutE((TObject*)nullptr,Eval) << "HypoTestInverter::RunLimit - Hypotest failed at upper limit of scan range: " << rMax << std::endl;
        rMax *= 0.95;
        continue;
     }
     ...
     rMax += rMax;
     if (tries == 5) {
        oocoutE((TObject*)nullptr,Eval) << "HypoTestInverter::RunLimit - Cannot determine upper limit of scan range. At " << r->GetName()
                                  << " = " << rMax  << " still getting "
  ...
  if ( fUseCLs && rMin == 0 ) {
     ...
     if (! RunOnePoint(rMin) ) {
       oocoutE((TObject*)nullptr,Eval) << "HypoTestInverter::RunLimit - Hypotest failed at lower limit of scan range: " << rMin << std::endl;
       return false;
     }
  ...
  if (clsMin.first != 1 && clsMin.first - 3 * fabs(clsMin.second) < clsTarget) {
     if (fUseCLs) {
        ...
        for (int tries = 0; tries < 6; ++tries) {
           ...
           rMin += rMin;
           if (tries == 5) {
              oocoutE((TObject*)nullptr,Eval) << "HypoTestInverter::RunLimit - Cannot determine lower limit of scan range. At " << r->GetName()
  ...
  do {
     ...
     if (! RunOnePoint(limit, true, clsTarget) ) {
       oocoutE((TObject*)nullptr,Eval) << "HypoTestInverter::RunLimit - Hypo test failed at x=" << limit << " when trying to find limit." << std::endl;
       return false;
     }
     ...
     if (fabs(clsMid.first-clsTarget) >= 2*clsMid.second) {
       ...
       while (clsMin.second == 0 || fabs(rMin-limit) > std::max(absAccuracy, relAccuracy * limit)) {
         ...
         if (!RunOnePoint(rMin,true, clsTarget) ) {
           oocoutE((TObject*)nullptr,Eval) << "HypoTestInverter::RunLimit - Hypo test failed at x=" << rMin << " when trying to find limit from below." << std::endl;
           return false;
         }
         clsMin = std::make_pair( fResults->GetLastYValue(), fResults->GetLastYError() );
       ...
       while (clsMax.second == 0 || fabs(rMax-limit) > std::max(absAccuracy, relAccuracy * limit)) {
         ...
         if (!RunOnePoint(rMax,true,clsTarget) ) {
           oocoutE((TObject*)nullptr,Eval) << "HypoTestInverter::RunLimit - Hypo test failed at x=" << rMin << " when trying to find limit from above." << std::endl;
           return false;
         }
         clsMax = std::make_pair( fResults->GetLastYValue(), fResults->GetLastYError() );
