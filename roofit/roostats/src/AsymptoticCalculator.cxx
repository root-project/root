...
AsymptoticCalculator::AsymptoticCalculator(RooAbsData &data, const ModelConfig &altModel, const ModelConfig &nullModel,
                                           bool nominalAsimov)
   : HypoTestCalculatorGeneric(data, altModel, nullModel, nullptr), fOneSided(false), fOneSidedDiscovery(false),
     fNominalAsimov(nominalAsimov), fUseQTilde(-1), fNLLObs(0), fNLLAsimov(0), fAsimovData(nullptr)
{
   ...
   if (muNull->getVal() == muNull->getMin()) {
      ...
      if (verbose > 0)
         oocoutI((TObject *)nullptr, InputArguments)
            ...
            << " corresponds to null  snapshot   - default configuration is  one-sided discovery formulae  "
            << std::endl;
   }
...
bool AsymptoticCalculator::Initialize() const {
   ...
   int verbose = fgPrintLevel;
   if (verbose >= 0) oocoutP((TObject *)nullptr, Eval) << "AsymptoticCalculator::Initialize...." << std::endl;
   ...
   if (poi->getSize() > 1) {
      oocoutW((TObject *)nullptr, InputArguments)
         ...
         << "The asymptotic calculator works for only one POI - consider as POI only the first parameter" << std::endl;
   }
   ...
   if (nullSnapshot == nullptr || nullSnapshot->getSize() == 0) {
      oocoutE((TObject *)nullptr, InputArguments)
         << "AsymptoticCalculator::Initialize - Null model needs a snapshot. Set using modelconfig->SetSnapshot(poi)."
         << endl;
      return false;
   }
   ...
   assert(muBest);
   if (verbose >= 0)
      oocoutP((TObject *)nullptr, Eval) << "Best fitted POI value = " << muBest->getVal() << " +/- "
   ...
   if (GetNullModel()->GetObservables() && GetNullModel()->GetObservables()->getSize() == 1 ) {
      ...
      if (data.IsA() == RooDataHist::Class() ) {
         if (data.numEntries() != xobs->getBins() ) {
            ...
            oocoutW((TObject *)nullptr, InputArguments)
               << "AsymptoticCalculator: number of bins in " << xobs->GetName() << " are different than data bins "
   ...
   if (!fNominalAsimov) {
      ...
      if (verbose >= 0)
         oocoutI((TObject *)nullptr, InputArguments) << "AsymptoticCalculator: Asimovdata set will be generated using "
                                                        "nominal (current) nuisance parameter values"
                                                     << endl;
...
HypoTestResult* AsymptoticCalculator::GetHypoTest() const {
   ...
   if (!fIsInitialized) {
      if (!Initialize() ) {
         oocoutE((TObject *)nullptr, InputArguments)
            << "AsymptoticCalculator::GetHypoTest - Error initializing Asymptotic calculator - return NULL result "
            << endl;
         return nullptr;
      }
   }
   ...
   if (poiTest.getSize() > 1)  {
      oocoutW((TObject *)nullptr, InputArguments)
         << "AsymptoticCalculator::GetHypoTest: snapshot has more than one POI - assume as POI first parameter "
         << std::endl;
   }
   ...
   *allParams = fBestFitParams;
   ...
   if (verbose > 0)
      oocoutP((TObject *)nullptr, Eval) << "\t OBSERVED DATA :  qmu   = " << qmu << " condNLL = " << condNLL
   ...
   if (qmu < -tol || TMath::IsNaN(fNLLObs) ) {
      ...
      if (qmu < 0)
         oocoutW((TObject *)nullptr, Minimization)
   ...
   if (qmu < -tol ) {
      oocoutE((TObject *)nullptr, Minimization)
         << "AsymptoticCalculator:  qmu is still < 0  for mu = " << muTest->getVal() << " return a dummy result "
         << std::endl;
      return new HypoTestResult();
   }
   if (TMath::IsNaN(qmu) ) {
      oocoutE((TObject *)nullptr, Minimization) << "AsymptoticCalculator:  failure in fitting for qmu or qmuA "
                                                << muTest->getVal() << " return a dummy result " << std::endl;
      return new HypoTestResult();
   }
   ...
   if (verbose > 0)
      oocoutP((TObject *)nullptr, Eval) << "\t ASIMOV data qmu_A = " << qmu_A << " condNLL = " << condNLL_A
   ...
   if (qmu_A < - tol) {
      oocoutE((TObject *)nullptr, Minimization)
         << "AsymptoticCalculator:  qmu_A is still < 0  for mu = " << muTest->getVal() << " return a dummy result "
         << std::endl;
      return new HypoTestResult();
   }
   if (TMath::IsNaN(qmu) ) {
   ...
   if (!fOneSidedDiscovery) { // qtilde is not a discovery test
      if (fUseQTilde == -1 && !fOneSidedDiscovery) {
         ...
         if (muTest->getMin() == muAlt->getVal()   ) {
            ...
            fUseQTilde = 0;
            oocoutI((TObject *)nullptr, InputArguments)
               << "Minimum of POI is " << muTest->getMin() << " is different to alt snapshot " << muAlt->getVal()
   ...
   if (verbose > 0)
      //std::cout
      oocoutP((TObject *)nullptr, Eval) << "poi = " << muTest->getVal() << " qmu = " << qmu << " qmu_A = " << qmu_A
    ...
    for (RooAbsArg *a = (RooAbsArg *)iter.Next(); a != nullptr; a = (RooAbsArg *)iter.Next()) {
       ...
       if ((pois = dynamic_cast<RooPoisson *>(a)) != nullptr) {
          ...
          if (subprod)
             ...
             oocoutE((TObject *)nullptr, InputArguments)
                << "Illegal term in counting model: depends on observables, but not Poisson or Gaussian or Product"
                << endl;
             return false;
          }
       }
       ret = (pois != nullptr || gaus != nullptr);
    }
    return ret;
}
...
bool AsymptoticCalculator::SetObsToExpected(RooAbsPdf &pdf, const RooArgSet &obs)
{
   ...
   for (RooAbsArg *a = iter.next(); a != nullptr; a = iter.next()) {
      if (obs.contains(*a)) {
         if (myobs != nullptr) {
            oocoutF((TObject *)nullptr, Generation)
               << "AsymptoticCalculator::SetObsExpected( " << pdfName << " ) : Has two observables ?? " << endl;
            return false;
         }
         myobs = dynamic_cast<RooRealVar *>(a);
         if (myobs == nullptr) {
            oocoutF((TObject *)nullptr, Generation) << "AsymptoticCalculator::SetObsExpected( " << pdfName
         ...
         if (!a->isConstant() ) {
            if (myexp != nullptr) {
               oocoutE((TObject *)nullptr, Generation) << "AsymptoticCalculator::SetObsExpected( " << pdfName
   ...
   if (myobs == nullptr) {
      oocoutF((TObject *)nullptr, Generation)
         << "AsymptoticCalculator::SetObsExpected( " << pdfName << " ) : No observable?" << endl;
      return false;
   }
   if (myexp == nullptr) {
      oocoutF((TObject *)nullptr, Generation)
...
RooAbsData * AsymptoticCalculator::MakeAsimovData(RooAbsData & realData, const ModelConfig & model, const  RooArgSet & paramValues, RooArgSet & asimovGlobObs, const RooArgSet * genPoiValues )  {
   ...
   if (model.GetNuisanceParameters()) {
      ...
      for (RooAbsArg *a = (RooAbsArg *)iter.Next(); a != nullptr; a = (RooAbsArg *)iter.Next()) {
         ...
         if (rrv != nullptr && rrv->isConstant() == false) {
            hasFloatParams = true;
            break;
         }
      }
...
RooAbsData * AsymptoticCalculator::MakeAsimovData(const ModelConfig & model, const  RooArgSet & allParamValues, RooArgSet & asimovGlobObs)  {
   ...
   if (model.GetGlobalObservables() && model.GetGlobalObservables()->getSize() > 0) {
      ...
      if (nuis.getSize() == 0) {
         oocoutW((TObject *)nullptr, Generation) << "AsymptoticCalculator::MakeAsimovData: model does not have "
                                                    "nuisance parameters but has global observables"
      ...
      for (RooAbsArg *a = (RooAbsArg *)iter.Next(); a != nullptr; a = (RooAbsArg *)iter.Next()) {
         ...
         if ( cClass != RooGaussian::Class() && cClass != RooPoisson::Class() &&
            ...
            oocoutW((TObject *)nullptr, Generation)
               << "AsymptoticCalculator::MakeAsimovData:constraint term " << cterm->GetName() << " of type "
         ...
         if ( cClass == RooGamma::Class() ) {
            ...
            if (thetaGamma == nullptr) {
               oocoutI((TObject *)nullptr, Generation)
                  << "AsymptoticCalculator::MakeAsimovData:constraint term " << cterm->GetName()
                  << " is a Gamma distribution and no server named theta is found. Assume that the Gamma scale is  1 "
                  << std::endl;
            }
            else {
               if (verbose>2)
         ...
         for (RooAbsArg *a2 = iter2.next(); a2 != nullptr; a2 = iter2.next()) {
            ...
            if (rrv2 && rrv2->dependsOn(nuis) ) {
               ...
               if (foundServer) {
                  oocoutE((TObject *)nullptr, Generation)
                     << "AsymptoticCalculator::MakeAsimovData:constraint term " << cterm->GetName()
                     << " constraint term has more server depending on nuisance- cannot generate it " << std::endl;
                  foundServer = false;
                  break;
               }
         ...
         if (!foundServer) {
            oocoutE((TObject *)nullptr, Generation)
               << "AsymptoticCalculator::MakeAsimovData - can't find nuisance for constraint term - global observables "
                  "will not be set to Asimov value "
