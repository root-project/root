...
{
   if (fPosteriorHist == nullptr) fPosteriorHist = fInterval->GetPosteriorHist();
...
{
   if (fPosteriorKeysPdf == nullptr) fPosteriorKeysPdf = fInterval->GetPosteriorKeysPdf();
...
void MCMCIntervalPlot::DrawHistInterval(const Option_t* options)
{
   ...
   if (fDimension == 1) {
      ...
      TH1F *hist = (TH1F *)DrawPosteriorHist(options, nullptr, false);
...
{
   if (fPosteriorKeysProduct == nullptr) fPosteriorKeysProduct = fInterval->GetPosteriorKeysProduct();
...
void MCMCIntervalPlot::DrawChainScatter(RooRealVar& xVar, RooRealVar& yVar)
{
   ...
   Double_t *burnInX = nullptr;
   Double_t *burnInY = nullptr;
   if (burnInSteps > 0) {
