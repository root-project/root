...
namespace RooStats {
   ...
   RooAbsPdf * MakeNuisancePdf(RooAbsPdf &pdf, const RooArgSet &observables, const char *name) {
      ...
      if(constraints.getSize() == 0) {
         oocoutW((TObject *)nullptr, Eval)
            << "RooStatsUtils::MakeNuisancePdf - no constraints found on nuisance parameters in the input model"
            << endl;
         return nullptr;
   ...
   RooAbsPdf * StripConstraints(RooAbsPdf &pdf, const RooArgSet &observables) {
      ...
      if (id == typeid(RooProdPdf)) {
         ...
         if (newUPdf == nullptr)
            return nullptr; // only constraints in underlying pdf
         ...
         for (int ic = 0, nc = cat->numBins((const char *)nullptr); ic < nc; ++ic) {
            ...
            if (newPdf == nullptr) {
               delete cat;
               return nullptr;
            } // all channels must have observables
            pdfList.add(*newPdf);
         }
   ...
   RooAbsPdf * MakeUnconstrainedPdf(RooAbsPdf &pdf, const RooArgSet &observables, const char *name) {
      ...
      if(!unconstrainedPdf) {
         oocoutE((TObject *)nullptr, InputArguments)
            << "RooStats::MakeUnconstrainedPdf - invalid observable list passed (observables not found in original "
               "pdf) or invalid pdf passed (without observables)"
            << endl;
         return nullptr;
      }
