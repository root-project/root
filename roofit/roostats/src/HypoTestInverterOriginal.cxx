...
/** \class RooStats::HypoTestInverterOriginal
    \ingroup Roostats
...
The class can scan the CLs+b values or alternatively CLs (if the method HypoTestInverterOriginal::UseCLs has been called).
  ...
  do {
   ...
   do {
     ...
     ((HybridCalculatorOriginal*)fCalculator0)->SetNumberOfToys(nToysTarget);
      ...
      if (leftCL==rightCL) {
   std::cout << "Algorithm failed: left and right CL are equal (no interpolation possible or more toy-MC statistics needed)\n";
     quitThisLoop = true;
      }
