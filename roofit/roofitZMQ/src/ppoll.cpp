...
int zmq_ppoll(zmq_pollitem_t *items_, int nitems_, long timeout_, const sigset_t *sigmask_)
{
   ...
   for (int i = 0; i != nitems_; i++) {
      if (items_[i].socket && as_socket_base_t(items_[i].socket)->is_thread_safe()) {
   ...
   while (true) {
      ...
      zmq::timeout_t timeout = zmq::compute_timeout(first_pass, timeout_, now, end);
...
#else
         int rc = pselect(maxfd + 1, inset.get(), outset.get(), errset.get(), ptimeout, sigmask_);
         if (unlikely(rc == -1)) {
