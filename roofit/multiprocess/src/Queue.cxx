...
bool Queue::process_master_message(M2Q message)
{
   ...
   case M2Q::retrieve: {
      ...
      if (_queue.empty() && N_tasks_completed == N_tasks) {
         JobManager::instance()->results_from_queue_to_master();
         // reset number of received and completed tasks
         N_tasks = 0;
         N_tasks_completed = 0;
      } else {
         JobManager::instance()->messenger().send_from_queue_to_master(Q2M::retrieve_rejected); // handshake message: tasks not done yet, try again
      }
      break;
   }
   ...
   case M2Q::update_real: {
      ...
      oocxcoutD((TObject *)nullptr, Benchmarking1)
         << "update_real on queue: " << (t2 - t1) / 1.e9 << "s (from " << t1 << " to " << t2 << "ns)" << std::endl;
      break;
   }
   }
