...
{
   auto a1 = batches.extraArg(0);
   auto a2 = batches.extraArg(1);
...
{
   auto lo = batches.extraArg(batches.getNEvents()-1); // Lowest order variable
   auto x = batches[0];

   for (size_t i = BEGIN; i < batches.getNEvents(); i += STEP) {
      if(i > lo){
         batches._output[i] = 1.0f + (batches.extraArg(i)/*=a[i]*/ * pow(x[i], i + lo));
      }
   }  
