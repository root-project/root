...
int DoBinFit(T *hist, Func &func, bool debug = false, bool useGrad = false)
{
   ...
   if (!useGrad) {
      ...
      ROOT::Math::WrappedParamFunction<ROOT::Math::Polynomial> f(pol, 1, func->GetParameters(),
                                                                 func->GetParameters() + func->GetNpar());
#endif
      GaussFunction f;
...
//    std::cout << "Fit using TVirtualFitter and " << fitter << "\t Time: \t" << w.RealTime() << " , " << w.CpuTime() <
//    std::endl;
// }
...
int FitUsingRooFit(TTree *tree, TF1 *func)
{
   ...
   for (int i = 0; i < nfit; ++i) {
      ...
      std::unique_ptr<RooFitResult> result{
         pdf.fitTo(data, RooFit::Minos(0), RooFit::Hesse(1), RooFit::PrintLevel(level), RooFit::Save(save))};
