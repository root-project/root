...
public:
   // create all pdf
   MultiGaussRooPdf(unsigned int n) : x(n), m(n), s(n), g(n), pdf(n)
   {
      // assert(n >= 2);
...
int FitUsingRooFit(TTree &tree, RooAbsPdf &pdf, RooArgSet &xvars)
{
   ...
   for (int i = 0; i < nfit; ++i) {
      ...
      std::unique_ptr<RooFitResult> result{
         pdf.fitTo(data, RooFit::Minos(0), RooFit::Hesse(0), RooFit::PrintLevel(level), RooFit::Save(save))};
...
struct GausNorm<1> {
   ...
   static double F(const double *x, const double *p) { return ROOT::Math::normal_pdf(x[0], p[1], p[0]); }
};
