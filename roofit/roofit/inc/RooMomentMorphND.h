...
public:
  ...
  public:
    ...
    Grid(const RooAbsBinning& binning_x,
         const RooAbsBinning& binning_y) { _grid.push_back(binning_x.clone()); _grid.push_back(binning_y.clone()); };
    Grid(const RooAbsBinning& binning_x,
         const RooAbsBinning& binning_y,
         const RooAbsBinning& binning_z) { _grid.push_back(binning_x.clone()); _grid.push_back(binning_y.clone()); _grid.push_back(binning_z.clone()); };
    Grid(const vector<RooAbsBinning*> binnings) { for (unsigned int i = 0; i < binnings.size(); i++) { _grid.push_back(binnings[i]->clone()); } };

    ~Grid();

    void addPdf(const RooAbsPdf& pdf,
                int bin_x);
    void addPdf(const RooAbsPdf& pdf,
                int bin_x,
                int bin_y);
...
public:
  ...
  RooMomentMorphND();
  RooMomentMorphND(const char *name,
                   const char *title,
                   RooAbsReal& _m,
  ...
  RooMomentMorphND(const RooMomentMorphND& other,
                   const char* name = 0);
  RooMomentMorphND(const char *name,
                   const char *title,
                   RooAbsReal& _m,
