...
namespace {
class RooBSplineBasesStreamer : public RooFit::JSONIO::Exporter {
public:
  virtual const std::string& key() const override {
    static const std::string _key = "BSplineBases";
    return _key;
  }
  
  virtual bool exportObject(RooJSONFactoryWSTool *, const RooAbsArg *func, JSONNode &elem) const override
  {
    const RooBSplineBases *spline = static_cast<const RooBSplineBases *>(func);
    elem["type"] << key();
    elem["order"] << spline->getOrder();        
    elem["nrClose"] << spline->getNrClose();        
    elem["t"] << spline->getT().GetName();
    elem["tValues"].fill_seq(spline->getTValues());
    return true;
  }
};
} // namespace
...
namespace {
class RooBSplineBasesFactory : public RooFit::JSONIO::Importer {
public:
   virtual bool importFunction(RooJSONFactoryWSTool *tool, const JSONNode &p) const override
   {
     std::string name = RooJSONFactoryWSTool::name(p);

     auto& tvals = p["tValues"];
     std::vector<double> tValues;
     tValues << tvals;

     int nrClose = p["nrClose"].val_int();
     int order = p["order"].val_int();     

     RooAbsReal* t = tool->workspace()->var(p["t"].val().c_str());
     
     RooBSplineBases bases(name.c_str(),name.c_str(),order,tValues,*t,nrClose);
     tool->workspace()->import(bases);     
...
namespace {
  int register_serializations(){
    RooFit::JSONIO::registerImporter("BSplineBases", std::make_unique<RooBSplineBasesFactory>());
    RooFit::JSONIO::registerExporter(RooBSplineBases::Class(), std::make_unique<RooBSplineBasesStreamer>());
    return 1;
  }
