  ...
  _setting(setting)
{
  ...
  for (Int_t i=0; (var = (RooAbsArg*)varItr->Next()); ++i) {
    if (!dynamic_cast<RooAbsReal*>(var)) {
       coutE(InputArguments) << "Roo1DMomentMorphFunction::ctor(" << GetName() << ") ERROR: variable " << var->GetName()
...
Roo1DMomentMorphFunction::Roo1DMomentMorphFunction(const char *name, const char *title, RooAbsReal &_m,
                                                   const TMatrixD &mrefM, const Setting &setting)
   : RooAbsReal(name, title), m("m", "m", this, _m), _varList("varList", "List of variables", this), _setting(setting)
{
   // CTOR
...
void Roo1DMomentMorphFunction::initialize()
{
   ...
   if (_mrefM == 0 && (nVar != _mref->GetNrows())) {
      coutE(InputArguments) << "Roo1DMomentMorphFunction::initialize(" << GetName() << ") ERROR: nVar != nRefPoints"
                            << endl;
      assert(0);
  }
...
void Roo1DMomentMorphFunction::calculateFractions() const
{
   ...
   case NonLinear:
      ...
      break;
   case Linear:
      for (Int_t i = 0; i < nVar; ++i) (*_frac)(i) = 0.;
