...
void optionallyExportGammaParameters(JSONNode &mod, std::string const &sysname,
                                     std::vector<RooAbsReal*> const &params, bool forceExport = true)
{
...
RooAbsPdf& getOrCreateConstraint(RooJSONFactoryWSTool &tool, const JSONNode &mod, RooRealVar& param, const std::string& sample){
  if (auto constrName = mod.find("constraint_name")) {
    ...
    if(!constraint){
      RooJSONFactoryWSTool::error("unable to find definition of of constraint '" + constraint_name + "' for modifier '" + RooJSONFactoryWSTool::name(mod) + "'");
    }
    ...
    if(constraint_type == "Gauss"){
      param.setError(1.0);
      return getOrCreate<RooGaussian>(*tool.workspace(), constraintName(param.GetName()), param, *tool.workspace()->var(std::string("nom_") + param.GetName()), 1.);
    }
   ...
   if (p.has_child("modifiers")) {
      ...
      for (const auto &mod : p["modifiers"].children()) {
	    ...
	    constraints.add(getOrCreateConstraint(tool,mod,par,sampleName));	    
            ...
            std::string constraint(mod.has_child("constraint_type") ? mod["constraint_type"].val() : mod.has_child("constraint") ? mod["constraint"].val() : "unknown");
   ...
   for (size_t sampleidx = 0; sampleidx < sumpdf->funcList().size(); ++sampleidx) {
	...
	for (RooAbsArg *e : elements) {
	  if (TString(e->GetName()).Contains("binWidth")) {
		...
		sample.normsys.emplace_back(sysname, var, fip->high()[i], fip->low()[i], fip->interpolationCodes()[i], constraint);
	      }
	    }
   ...
   for (auto &sample : channel.samples) {
	 ...
	 sample.histError.resize(sample.hist.size());
         for (auto bin : channel.rel_errors) {
            ...
            sample.histError[i - 1] = round_prec(relerr_tot * channel.tot_yield[i] / std::sqrt(channel.tot_yield2[i]) * count, 7);
         }
      }
   }
}
...
bool tryExportHistFactory(RooJSONFactoryWSTool *tool, const std::string &pdfname, const RooRealSumPdf *sumpdf, std::vector<RooAbsPdf*> constraints, JSONNode &elem){
   ...
   // sanity checks
   if(channel.samples.size() == 0) return false;
   for (auto &sample : channel.samples) {
      if (sample.hist.empty()) {
         return false;
      }
