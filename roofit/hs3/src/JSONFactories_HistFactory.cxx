                          ...
                          JSONNode &elem)
{
   ...
   for (size_t sampleidx = 0; sampleidx < sumpdf->funcList().size(); ++sampleidx) {
      ...
      for(auto* fip: fips){
	    ...
	    if(!constraint) RooJSONFactoryWSTool::error("cannot find constraint for " + std::string(var->GetName()));		 	    
            sample.normsys.emplace_back(sysname, var, fip->high()[i], fip->low()[i], constraint ? constraint->IsA() : nullptr);
         }
      }
