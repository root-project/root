...
RooPolyFunc makePolyFunc1D(RooRealVar const &x)
{
   // 1 + x + x^2
   RooPolyFunc polyfunc("f", "f", RooArgSet(x));
...
RooPolyFunc makePolyFunc2D(RooRealVar const &x, RooRealVar const &y)
{
   // 1 + x + y + x^2 + y^2
   RooPolyFunc polyfunc("f", "f", RooArgSet(x, y));
...
TEST(RooPolyFunc, WrappedPdfClosure)
{
   ...
   RooPolyFunc polyfunc = makePolyFunc1D(x);
...
TEST(RooPolyFunc, TaylorExpansionClosure2D)
{
   ...
   RooPolyFunc polyfunc = makePolyFunc2D(x, y);
