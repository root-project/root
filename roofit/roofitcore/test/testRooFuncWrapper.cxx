...
class ScopedFormulaBackendEnv {
public:
   ScopedFormulaBackendEnv(const char *value)
   {
...
// Gradient agreement between the two RooFormula evaluation backends: for
// models containing RooFormulaVar/RooGenericPdf, the Clad gradient from the
// new codegen path that inlines the formula expression (AST backend) must
// agree with the gradient from the old path that calls the
// cling-JIT-compiled TFormula function by name (tformula backend), and with
// numerical differentiation of the reference NLL.
TEST(RooFuncWrapperFormula, GradientBackendAgreement)
{
   ...
   struct BackendResult {
      ...
      std::vector<double> numGrad;
   };

   // Restore the shared RooRandom generator state afterwards, so that this
   // test does not perturb the data generated by the other tests in this
   // binary (their fit-result comparisons are seed-sensitive).
   TRandom3 savedRngState{*static_cast<TRandom3 *>(RooRandom::randomGenerator())};

   auto run = [](std::vector<std::string> const &factoryExprs, const char *backend) {
      ScopedFormulaBackendEnv env{backend};

      RooWorkspace ws;
      for (std::string const &expr : factoryExprs) {
         ws.factory(expr);
      }
      ...
      // Fixed seed: both backends must see bitwise-identical data.
      RooRandom::randomGenerator()->SetSeed(1337);
      std::unique_ptr<RooDataSet> data{model.generate(observables, 50)};

      std::unique_ptr<RooAbsReal> nllRef{model.createNLL(*data, RooFit::EvalBackend::Cpu())};
      std::unique_ptr<RooAbsReal> nllFunc{model.createNLL(*data, RooFit::EvalBackend::Codegen())};

      BackendResult res;
      res.nll = nllFunc->getVal();

      RooArgSet params;
      nllFunc->getParameters(&observables, params);
      res.grad.assign(params.size(), 0.0);
      nllFunc->gradient(res.grad.data());
      for (auto *param : params) {
         res.numGrad.push_back(getNumDerivative(*nllRef, static_cast<RooRealVar &>(*param), observables));
      }
      return res;
   };

   const std::vector<std::vector<std::string>> models{
      // Gaussian with RooFormulaVar parameters mixing operators and functions
      {"expr::mu_shifted('mu + 0.1*sin(mu) + shift^2', {mu[1.0, -10, 10], shift[0.5, -10, 10]})",
       "expr::sigma_scaled('sigma*(1 + 0.2*abs(alpha))', {sigma[2.0, 0.1, 10], alpha[0.3, -5, 5]})",
       "Gaussian::model(x[0, -10, 10], mu_shifted, sigma_scaled)"},
      // RooGenericPdf, normalized by numeric integration
      {"EXPR::model('exp(-0.5*((x - m)/s)^2) + 0.1*(1 + 0.05*x)', {x[0, -10, 10], m[1.0, -10, 10], s[2.0, 0.1, "
       "10]})"}};

   for (auto const &factoryExprs : models) {
      BackendResult ast = run(factoryExprs, "ast");
      BackendResult tformula = run(factoryExprs, "tformula");

      ASSERT_EQ(ast.grad.size(), tformula.grad.size());
      ASSERT_FALSE(ast.grad.empty());

      // Same data, same math: the two codegen paths must agree to near
      // machine precision, with a small margin for Clad emitting differently
      // associated derivative code across the JIT'd-function call boundary.
      EXPECT_NEAR(ast.nll, tformula.nll, 1e-12 * std::abs(tformula.nll));
      for (std::size_t i = 0; i < ast.grad.size(); ++i) {
         EXPECT_NEAR(ast.grad[i], tformula.grad[i], 1e-9 * std::max(1.0, std::abs(tformula.grad[i])))
            << "gradient component " << i;
         // ... and both must be consistent with numerical differentiation.
         EXPECT_NEAR(ast.grad[i], ast.numGrad[i], 1e-4 * std::max(1.0, std::abs(ast.numGrad[i])))
            << "gradient component " << i;
      }
   }

   *static_cast<TRandom3 *>(RooRandom::randomGenerator()) = savedRngState;
}
