...
#ifndef ROOT_ROOFIT_TESTSTATISTICS_MINIMIZER_IMPL
#define ROOT_ROOFIT_TESTSTATISTICS_MINIMIZER_IMPL

/**
\file Minimizer_impl.h
\class MinimizerTemplate
\ingroup Roofitcore

Minimizer is a wrapper class around ROOT::Fit:Fitter that
...
Minimizer can minimize any RooAbsReal function with respect to
its parameters. Usual choices for minimization are RooNLLVar
and RooChi2Var
Minimizer has methods corresponding to MINUIT functions like
...
This file implements the template, whereas Minimizer.cxx creates
the actual Minimizer alias, for backwards compatibility. Another
class making use of this template is GradMinimizer.
**/

#ifndef ROOT_ROOFIT_TESTSTATISTICS_MINIMIZER
#error "Do not use Minimizer_impl.h directly. #include \"Minimizer.h\" instead."
...
template <class MF, MinimizerType dmt, typename... MinFcnCArgs>
ROOT::Fit::Fitter *MinimizerTemplate<MF, dmt, MinFcnCArgs...>::_theFitter = 0;
...
template <class MF, MinimizerType dmt, typename... MinFcnCArgs>
void MinimizerTemplate<MF, dmt, MinFcnCArgs...>::cleanup()
{
   if (_theFitter) {
      delete _theFitter;
      _theFitter = 0;
...
template <class MinimizerFcn, MinimizerType dmt, typename... MinFcnCArgs>
MinimizerTemplate<MinimizerFcn, dmt, MinFcnCArgs...>::MinimizerTemplate(RooAbsReal &function,
                                                                        MinFcnCArgs... minFcnCArgs)
{
   ...
   _fcn = new MinimizerFcn(_func, MinimizerGenericPtr(this), minFcnCArgs..., _verbose);
   ...
   _fcn->Synchronize(_theFitter->Config().ParamsSettings(), _optConst, _verbose);
...
template <class MF, MinimizerType dmt, typename... MinFcnCArgs>
MinimizerTemplate<MF, dmt, MinFcnCArgs...>::~MinimizerTemplate()
{
   if (_extV) {
      delete _extV;
   }
...
template <class MF, MinimizerType dmt, typename... MinFcnCArgs>
void MinimizerTemplate<MF, dmt, MinFcnCArgs...>::setStrategy(Int_t istrat)
{
...
/// (MinimizerTemplate default 500 * #parameters)

template <class MF, MinimizerType dmt, typename... MinFcnCArgs>
void MinimizerTemplate<MF, dmt, MinFcnCArgs...>::setMaxIterations(Int_t n)
{
...
template <class MF, MinimizerType dmt, typename... MinFcnCArgs>
void MinimizerTemplate<MF, dmt, MinFcnCArgs...>::setMaxFunctionCalls(Int_t n)
{
...
/// that is taken in the MinimizerTemplate constructor from
/// the defaultErrorLevel() method of the input function

template <class MF, MinimizerType dmt, typename... MinFcnCArgs>
void MinimizerTemplate<MF, dmt, MinFcnCArgs...>::setErrorLevel(Double_t level)
{
...
template <class MF, MinimizerType dmt, typename... MinFcnCArgs>
void MinimizerTemplate<MF, dmt, MinFcnCArgs...>::setEps(Double_t eps)
{
...
template <class MF, MinimizerType dmt, typename... MinFcnCArgs>
void MinimizerTemplate<MF, dmt, MinFcnCArgs...>::setOffsetting(Bool_t flag)
{
...
template <class MF, MinimizerType dmt, typename... MinFcnCArgs>
void MinimizerTemplate<MF, dmt, MinFcnCArgs...>::setMinimizerType(const char *type)
{
   if (dynamic_cast<GradMinimizerFcn *>(_fcn) && strcmp(type, "Minuit2") != 0) {
      throw std::invalid_argument(
         "In Minimizer::setMinimizerType: only Minuit2 is supported when using GradMinimizerFcn!");
   }
   _minimizerType = type;
}
...
template <class MF, MinimizerType dmt, typename... MinFcnCArgs>
ROOT::Fit::Fitter *MinimizerTemplate<MF, dmt, MinFcnCArgs...>::fitter()
{
   return _theFitter;
}
...
template <class MF, MinimizerType dmt, typename... MinFcnCArgs>
const ROOT::Fit::Fitter *MinimizerTemplate<MF, dmt, MinFcnCArgs...>::fitter() const
{
   return _theFitter;
}
...
template <class MF, MinimizerType dmt, typename... MinFcnCArgs>
RooFitResult *MinimizerTemplate<MF, dmt, MinFcnCArgs...>::fit(const char *options)
{
   ...
   if (opts.Contains("t"))
      setProfile(1);
   if (opts.Contains("l"))
      setLogFile(Form("%s.log", _func->GetName()));
   if (opts.Contains("c"))
      optimizeConst(1);
   ...
   if (opts.Contains("0"))
      setStrategy(1);
   if (opts.Contains("h") || !opts.Contains("m"))
      hesse();
   if (!opts.Contains("m"))
      minos();
...
template <class MF, MinimizerType dmt, typename... MinFcnCArgs>
Int_t MinimizerTemplate<MF, dmt, MinFcnCArgs...>::minimize(const char *type, const char *alg)
{
   _fcn->Synchronize(_theFitter->Config().ParamsSettings(), _optConst, _verbose);
...
// TODO: this function's body could be replaced by one line: `minimize(_minimizerType.c_str(),"migrad");`, except for
// the saveSTATUS call...

template <class MF, MinimizerType dmt, typename... MinFcnCArgs>
Int_t MinimizerTemplate<MF, dmt, MinFcnCArgs...>::migrad()
{
   auto get_time = []() {
   ...
   oocxcoutD((TObject *)nullptr, Benchmarking2) << "migrad timestamps: " << t1 << " " << t2 << " " << t3 << " " << t4
                                                << " " << t5 << " " << t6 << " " << t7 << " " << std::endl;
...
template <class MF, MinimizerType dmt, typename... MinFcnCArgs>
Int_t MinimizerTemplate<MF, dmt, MinFcnCArgs...>::hesse()
{
   if (_theFitter->GetMinimizer() == 0) {
      coutW(Minimization) << "Minimizer::hesse: Error, run Migrad before Hesse!" << endl;
      _status = -1;
   } else {
...
template <class MF, MinimizerType dmt, typename... MinFcnCArgs>
Int_t MinimizerTemplate<MF, dmt, MinFcnCArgs...>::minos()
{
   if (_theFitter->GetMinimizer() == 0) {
      coutW(Minimization) << "Minimizer::minos: Error, run Migrad before Minos!" << endl;
      _status = -1;
   } else {
...
template <class MF, MinimizerType dmt, typename... MinFcnCArgs>
Int_t MinimizerTemplate<MF, dmt, MinFcnCArgs...>::minos(const RooArgSet &minosParamList)
{
   if (_theFitter->GetMinimizer() == 0) {
      coutW(Minimization) << "Minimizer::minos: Error, run Migrad before Minos!" << endl;
      _status = -1;
...
template <class MF, MinimizerType dmt, typename... MinFcnCArgs>
Int_t MinimizerTemplate<MF, dmt, MinFcnCArgs...>::seek()
{
   _fcn->Synchronize(_theFitter->Config().ParamsSettings(), _optConst, _verbose);
   profileStart();
   RooAbsReal::setEvalErrorLoggingMode(RooAbsReal::CollectErrors);
...
template <class MF, MinimizerType dmt, typename... MinFcnCArgs>
Int_t MinimizerTemplate<MF, dmt, MinFcnCArgs...>::simplex()
{
   _fcn->Synchronize(_theFitter->Config().ParamsSettings(), _optConst, _verbose);
   profileStart();
   RooAbsReal::setEvalErrorLoggingMode(RooAbsReal::CollectErrors);
...
template <class MF, MinimizerType dmt, typename... MinFcnCArgs>
Int_t MinimizerTemplate<MF, dmt, MinFcnCArgs...>::improve()
{
   _fcn->Synchronize(_theFitter->Config().ParamsSettings(), _optConst, _verbose);
   profileStart();
   RooAbsReal::setEvalErrorLoggingMode(RooAbsReal::CollectErrors);
...
template <class MF, MinimizerType dmt, typename... MinFcnCArgs>
Int_t MinimizerTemplate<MF, dmt, MinFcnCArgs...>::setPrintLevel(Int_t newLevel)
{
...
template <class MF, MinimizerType dmt, typename... MinFcnCArgs>
void MinimizerTemplate<MF, dmt, MinFcnCArgs...>::optimizeConst(Int_t flag)
{
   RooAbsReal::setEvalErrorLoggingMode(RooAbsReal::CollectErrors);

   if (_optConst && !flag) {
      if (_printLevel > -1)
         coutI(Minimization) << "Minimizer::optimizeConst: deactivating const optimization" << endl;
      _func->constOptimizeTestStatistic(RooAbsArg::DeActivate);
      _optConst = flag;
   } else if (!_optConst && flag) {
      if (_printLevel > -1)
         coutI(Minimization) << "Minimizer::optimizeConst: activating const optimization" << endl;
      _func->constOptimizeTestStatistic(RooAbsArg::Activate, flag > 1);
      _optConst = flag;
   } else if (_optConst && flag) {
      if (_printLevel > -1)
         coutI(Minimization) << "Minimizer::optimizeConst: const optimization already active" << endl;
   } else {
      if (_printLevel > -1)
         coutI(Minimization) << "Minimizer::optimizeConst: const optimization wasn't active" << endl;
   }
...
/// the value of all floating parameters at MinimizerTemplate construction and
...
template <class MF, MinimizerType dmt, typename... MinFcnCArgs>
RooFitResult *MinimizerTemplate<MF, dmt, MinFcnCArgs...>::save(const char *userName, const char *userTitle)
{
   if (_theFitter->GetMinimizer() == 0) {
      coutW(Minimization) << "Minimizer::save: Error, run minimization before!" << endl;
      return 0;
   }
...
template <class MF, MinimizerType dmt, typename... MinFcnCArgs>
RooPlot *
MinimizerTemplate<MF, dmt, MinFcnCArgs...>::contour(RooRealVar &var1, RooRealVar &var2, Double_t n1, Double_t n2,
                                                    Double_t n3, Double_t n4, Double_t n5, Double_t n6)
{
   ...
   if (index1 < 0) {
      coutE(Minimization) << "Minimizer::contour(" << GetName() << ") ERROR: " << var1.GetName()
   ...
   if (index2 < 0) {
      coutE(Minimization) << "Minimizer::contour(" << GetName() << ") ERROR: " << var2.GetName()
   ...
   if (_theFitter->GetMinimizer() == 0) {
      coutW(Minimization) << "Minimizer::contour: Error, run Migrad before contours!" << endl;
      return frame;
   }
   ...
   for (Int_t ic = 0; ic < 6; ic++) {
      if (n[ic] > 0) {
         ...
         if (!ret) {
            coutE(Minimization) << "Minimizer::contour(" << GetName()
...
template <class MF, MinimizerType dmt, typename... MinFcnCArgs>
void MinimizerTemplate<MF, dmt, MinFcnCArgs...>::profileStart()
{
   if (_profile) {
      _timer.Start();
...
template <class MF, MinimizerType dmt, typename... MinFcnCArgs>
void MinimizerTemplate<MF, dmt, MinFcnCArgs...>::profileStop()
{
   if (_profile) {
      _timer.Stop();
      _cumulTimer.Stop();
...
template <class MF, MinimizerType dmt, typename... MinFcnCArgs>
void MinimizerTemplate<MF, dmt, MinFcnCArgs...>::applyCovarianceMatrix(TMatrixDSym &V)
{
...
template <class MF, MinimizerType dmt, typename... MinFcnCArgs>
RooFitResult *MinimizerTemplate<MF, dmt, MinFcnCArgs...>::lastMinuitFit(const RooArgList &varList)
{
   ...
   if (_theFitter == 0 || _theFitter->GetMinimizer() == 0) {
      oocoutE((TObject *)0, InputArguments) << "Minimizer::save: Error, run minimization before!" << endl;
      return 0;
   }
   ...
   if (varList.getSize() > 0 && varList.getSize() != Int_t(_theFitter->Result().NTotalParameters())) {
      oocoutE((TObject *)0, InputArguments)
         << "Minimizer::lastMinuitFit: ERROR: supplied variable list must be either empty " << endl
   ...
   while ((arg = (RooAbsArg *)iter->Next())) {
      if (!dynamic_cast<RooRealVar *>(arg)) {
         oocoutE((TObject *)0, InputArguments)
            << "Minimizer::lastMinuitFit: ERROR: variable '" << arg->GetName() << "' is not of type RooRealVar" << endl;
         return 0;
      }
   }
   delete iter;
   ...
   for (i = 0; i < _theFitter->Result().NTotalParameters(); ++i) {
      ...
      if (varList.getSize() == 0) {
         ...
         if (varName.CompareTo(var->GetName())) {
            oocoutI((TObject *)0, Eval) << "Minimizer::lastMinuitFit: fit parameter '" << varName
...
#endif // ROOT_ROOFIT_TESTSTATISTICS_MINIMIZER_IMPL
