...
#endif
#ifdef MYANONFLAG
           void *retVal = ::mmap(nullptr, len, PROT_READ | PROT_WRITE, MYANONFLAG | MAP_SHARED, -1, 0);
...
#undef MYANONFLAG
        ...
        if (DevZero == s_mmapworks || Unknown == s_mmapworks) {
            ...
            void *retVal = ::mmap(nullptr, len, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
            if (MAP_FAILED == retVal) {
                int errsv = errno;
                ::close(fd);
...
BidirMMapPipe::BidirMMapPipe(bool useExceptions, bool useSocketpair)
   : m_pages(pagepool().pop()), m_busylist(nullptr), m_freelist(nullptr), m_dirtylist(nullptr), m_inpipe(-1),
     m_outpipe(-1), m_flags(failbit), m_childPid(0), m_parentPid(::getpid())
