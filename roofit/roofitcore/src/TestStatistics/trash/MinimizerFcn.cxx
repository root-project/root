...
//////////////////////////////////////////////////////////////////////////////
//
// MinimizerFcn is am interface class to the ROOT::Math function
// for minization.
//
...
MinimizerFcn::MinimizerFcn(RooAbsReal *funct, MinimizerGenericPtr context, bool verbose)
   : _funct(funct), _context(context),
     // Reset the *largest* negative log-likelihood value we have seen so far
     _maxFCN(-1e30), _numBadNLL(0), _printEvalErrors(10), _doEvalErrorWall(kTRUE), _nDim(0), _logfile(0),
     _verbose(verbose)
{
   ...
   while ((arg = (RooAbsArg *)pIter->Next())) {
      if (!arg->IsA()->InheritsFrom(RooAbsRealLValue::Class())) {
         oocoutW(_context, Minimization) << "MinimizerFcn::MinimizerFcn: removing parameter " << arg->GetName()
...
MinimizerFcn::MinimizerFcn(const MinimizerFcn &other)
   : ROOT::Math::IBaseFunctionMultiDim(other), _evalCounter(other._evalCounter), _funct(other._funct),
...
ROOT::Math::IBaseFunctionMultiDim *MinimizerFcn::Clone() const
{
   return new MinimizerFcn(*this);
}

Bool_t MinimizerFcn::Synchronize(std::vector<ROOT::Fit::ParameterSettings> &parameters, Bool_t optConst, Bool_t verbose)
{
   ...
   for (index = 0; index < _constParamList->getSize(); index++) {
      ...
      if (!par->isConstant()) {
         ...
         if (verbose) {
            oocoutI(_context, Minimization)
               << "MinimizerFcn::synchronize: parameter " << par->GetName() << " is now floating." << endl;
         }
      }
      ...
      if (par->getVal() != oldpar->getVal()) {
         ...
         if (verbose) {
            oocoutI(_context, Minimization)
               << "MinimizerFcn::synchronize: value of constant parameter " << par->GetName() << " changed from "
   ...
   *_initConstParamList = *_constParamList;
   ...
   for (index = 0; index < _floatParamList->getSize(); index++) {
      ...
      if (!par->isConstant()) {
         ...
         if (!par->IsA()->InheritsFrom(RooRealVar::Class())) {
            oocoutW(_context, Minimization) << "MinimizerFcn::fit: Error, non-constant parameter " << par->GetName()
         ...
         if (pstep <= 0) {
            ...
            if (verbose) {
               oocoutW(_context, Minimization)
                  << "MinimizerFcn::synchronize: WARNING: no initial error estimate available for " << par->GetName()
                  << ": using " << pstep << endl;
            }
         }
      } else {
      ...
      if (index >= Int_t(parameters.size())) {
         ...
         if (par->hasMin() && par->hasMax()) {
            parameters.push_back(ROOT::Fit::ParameterSettings(par->GetName(), par->getVal(), pstep, pmin, pmax));
         } else {
            parameters.push_back(ROOT::Fit::ParameterSettings(par->GetName(), par->getVal(), pstep));
            if (par->hasMin())
      ...
      if (par->isConstant() && !oldFixed) {
         ...
         if (oldVar != par->getVal()) {
            ...
            if (verbose) {
               oocoutI(_context, Minimization) << "MinimizerFcn::synchronize: value of parameter " << par->GetName()
                                               << " changed from " << oldVar << " to " << par->getVal() << endl;
            }
         }
         ...
         if (verbose) {
            oocoutI(_context, Minimization)
               << "MinimizerFcn::synchronize: parameter " << par->GetName() << " is now fixed." << endl;
         }
         ...
         if (oldVar != par->getVal()) {
            ...
            if (verbose) {
               oocoutI(_context, Minimization)
                  << "MinimizerFcn::synchronize: value of fixed parameter " << par->GetName() << " changed from "
         ...
         if (verbose) {
            ...
            if (oldVlo != pmin || oldVhi != pmax) {
               oocoutI(_context, Minimization)
                  << "MinimizerFcn::synchronize: limits of parameter " << par->GetName() << " changed from [" << oldVlo
                  << "," << oldVhi << "] to [" << pmin << "," << pmax << "]" << endl;
            }

            // If oldVerr=0, then parameter was previously fixed
            if (oldVerr != pstep && oldVerr != 0) {
               oocoutI(_context, Minimization)
                  << "MinimizerFcn::synchronize: error/step size of parameter " << par->GetName() << " changed from "
                  << oldVerr << " to " << pstep << endl;
            }
         }
      }
   }
   ...
   if (optConst) {
      if (constStatChange) {
         ...
         oocoutI(_context, Minimization)
            << "MinimizerFcn::synchronize: set of constant parameters changed, rerunning const optimizer" << endl;
         _funct->constOptimizeTestStatistic(RooAbsArg::ConfigChange);
      } else if (constValChange) {
         oocoutI(_context, Minimization)
            << "MinimizerFcn::synchronize: constant parameter values changed, rerunning const optimizer" << endl;
...
Double_t MinimizerFcn::GetPdfParamVal(Int_t index)
{
...
void MinimizerFcn::ClearPdfParamAsymErr(Int_t index)
{
   // Modify PDF parameter error by ordinal index (needed by MINUIT)
...
void MinimizerFcn::SetPdfParamErr(Int_t index, Double_t loVal, Double_t hiVal)
{
   // Modify PDF parameter error by ordinal index (needed by MINUIT)
...
void MinimizerFcn::BackProp(const ROOT::Fit::FitResult &results)
{
...
Bool_t MinimizerFcn::SetLogFile(const char *inLogfile)
{
   // Change the file name for logging of a Minimizer of all MINUIT steppings
   // through the parameter space. If inLogfile is null, the current log file
   // is closed and logging is stopped.

   if (_logfile) {
      oocoutI(_context, Minimization) << "MinimizerFcn::setLogFile: closing previous log file" << endl;
      _logfile->close();
      delete _logfile;
      _logfile = 0;
   }
   ...
   if (!_logfile->good()) {
      oocoutI(_context, Minimization) << "MinimizerFcn::setLogFile: cannot open file " << inLogfile << endl;
      _logfile->close();
      delete _logfile;
      _logfile = 0;
   }
...
void MinimizerFcn::ApplyCovarianceMatrix(TMatrixDSym &V)
{
...
Bool_t MinimizerFcn::SetPdfParamVal(const Int_t &index, const Double_t &value) const
{
...
void MinimizerFcn::updateFloatVec()
{
...
double MinimizerFcn::DoEval(const double *x) const
{
   ...
   if (RooAbsPdf::evalError() || RooAbsReal::numEvalErrors() > 0 || fvalue > 1e30) {
      ...
      if (_printEvalErrors >= 0) {
         ...
         if (_doEvalErrorWall) {
            oocoutW(_context, Minimization)
               << "MinimizerFcn: Minimized function has error status." << endl
               << "Returning maximum FCN so far (" << _maxFCN
               << ") to force MIGRAD to back out of this region. Error log follows" << endl;
         } else {
            oocoutW(_context, Minimization)
               << "MinimizerFcn: Minimized function has error status but is ignored" << endl;
         }
