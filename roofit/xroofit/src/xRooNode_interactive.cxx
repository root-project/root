    ...
    if (event==1) {
        ...
        for(int i=1;i<=_h->GetNbinsX();i++) {
            if (_name == _h->GetXaxis()->GetBinLabel(i)) {
                ...
                if (_varyName=="") {
                    int vNum = 1;
                    TGraphAsymmErrors* newPoint = dynamic_cast<TGraphAsymmErrors*>(gPad->GetPrimitive(TString::Format("%s;variation %d",select->GetName(),vNum)));
                    while(newPoint && newPoint->GetN()>0) {
                        vNum++;
                        newPoint = dynamic_cast<TGraphAsymmErrors*>(gPad->GetPrimitive(TString::Format("%s;variation %d",select->GetName(),vNum)));
                    }
                    _varyName = TString::Format("variation %d",vNum);
                    if(!newPoint) newPoint = dynamic_cast<TGraphAsymmErrors*>(select->Clone(TString::Format("%s;%s",select->GetName(),_varyName.Data())));
                    newPoint->SetPointX(0,_gr->GetPointX(i-1));
                    newPoint->SetMarkerColor(vNum+1);newPoint->SetLineColor(vNum+1);
                    newPoint->SetPointEYlow(0,0);newPoint->SetPointEYhigh(0,0); // remove errors because currently meaningless!
                    newPoint->Draw("z0p");
        ...
        for(int i=1;i<=_h->GetNbinsX();i++) {
            ...
            if (auto g = dynamic_cast<TGraph*>(gPad->GetPrimitive(TString::Format("%s;%s",parName.c_str(),_varyName.Data()))); g && g->GetN()>0) {
                double _val = g->GetPointY(0)*_h->GetBinError(i) + _h->GetBinContent(i); // error is scale, content is offset
...
void xRooNode::Interactive_PLLPlot() {
    ...
    if (!_pull_pad || strcmp(_pull_pad->GetName(),"pulls")!=0) return;
    ...
    if (foundGraph) {
        ...
        if (auto line = dynamic_cast<TGraph*>(gPad->GetListOfPrimitives()->FindObject("markerLine")); line ) {
            line->SetPointX(0,_x); line->SetPointX(1,_x);
        } else {
            line = new TGraph; line->SetLineStyle(2); line->SetName("markerLine");
            line->SetBit(kCanDelete);
            line->SetPoint(0,_x,-100);line->SetPoint(1,_x,100);
            line->Draw("Lsame");
        }
        gPad->Modified();gPad->Update();
        for(auto o : *_pull_pad->GetListOfPrimitives()) {
            if (!o->InheritsFrom("TGraph")) continue;
            if(_hidden_pad->GetListOfPrimitives()->FindObject(o) || TString(o->GetName()).EndsWith("_pull")) {
                ...
                _pull_pad->GetListOfPrimitives()->Remove(o);break;
            }
        }
        auto tmp = gPad;
        _pull_pad->cd();
        ...
        tmp->cd();
        _pull_pad->Modified();_pull_pad->Update();
    }
