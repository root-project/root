...
class XRooFitTests(unittest.TestCase):
    ...
    def test_oneChannelLimit(self):
        ...
        bkg = ROOT.TH1D("bkg","Background",3,0,3); bkg.SetContent(array('d',[0,15,14,13,0]))  # first and last are under/overflow
        sig = ROOT.TH1D("sig","Signal",3,0,3); sig.SetContent(array('d',[0,4,5,6,0]))
        bkg_vary1 = bkg.Clone("alphaSyst=1"); bkg_vary1.SetContent(array('d',[0,16,13,12,0]))
        obsData = ROOT.TH1D("obsData","Data",3,0,3); obsData.SetContent(array('d',[0,17,15,13,0]))
        bkg.SetFillColor(ROOT.kRed)
        ...
        for hp in hs: # loop over the hypoPoints in the hypoSpace
            printInfo(hp)
            if hp.asimov(): printInfo(hp.asimov(),"  asimov")
