...
void RooStats::HistFactory::Channel::PrintXML( std::string directory, std::string prefix ) {
  ...
  cxcoutPnoObj(HistFactory) << "Printing XML Files for channel: " << GetName() << std::endl;
...
bool RooStats::HistFactory::Channel::CheckHistograms() { 
  ...
  try {
    ...
    if( fData.GetHisto() == NULL && fData.GetInputFile() != "" ) {
      cxcoutEnoObj(HistFactory) << "Error: Data Histogram for channel " << GetName() << " is NULL." << std::endl;
      throw hf_exc();
    }
	...
	cxcoutEnoObj(HistFactory) << "Error: Nominal Histogram for sample " << sample.GetName() << " is NULL." << std::endl;
	throw hf_exc();
      } 
      else {
	...
	if(NegativeBinNumber.size()>0) {
	  cxcoutWnoObj(HistFactory) << "WARNING: Nominal Histogram " << histNominal->GetName() << " for Sample = " << sample.GetName()
	...
	if( sample.GetStatError().GetErrorHist() == NULL ) {
	  cxcoutEnoObj(HistFactory) << "Error: Statistical Error Histogram for sample " << sample.GetName() << " is NULL." << std::endl;
	  throw hf_exc();
	}
      }
	...
	if( histoSys.GetHistoLow() == NULL ) {
	  cxcoutEnoObj(HistFactory) << "Error: HistoSyst Low for Systematic " << histoSys.GetName()
		    << " in sample " << sample.GetName() << " is NULL." << std::endl;
	  throw hf_exc();
	}
	if( histoSys.GetHistoHigh() == NULL ) {
	  cxcoutEnoObj(HistFactory) << "Error: HistoSyst High for Systematic " << histoSys.GetName()
		    << " in sample " << sample.GetName() << " is NULL." << std::endl;
	  throw hf_exc();
	}
	...
	if( histoFactor.GetHistoLow() == NULL ) {
	  cxcoutEnoObj(HistFactory) << "Error: HistoSyst Low for Systematic " << histoFactor.GetName()
		    << " in sample " << sample.GetName() << " is NULL." << std::endl;
	  throw hf_exc();
	}
	if( histoFactor.GetHistoHigh() == NULL ) {
	  cxcoutEnoObj(HistFactory) << "Error: HistoSyst High for Systematic " << histoFactor.GetName()
		    << " in sample " << sample.GetName() << " is NULL." << std::endl;
	  throw hf_exc();
	}
	...
	if( shapeSys.GetErrorHist() == NULL ) {
	  cxcoutEnoObj(HistFactory) << "Error: HistoSyst High for Systematic " << shapeSys.GetName()
		    << " in sample " << sample.GetName() << " is NULL." << std::endl;
	  throw hf_exc();
	}
...
TH1* RooStats::HistFactory::Channel::GetHistogram(std::string InputFile, std::string HistoPath, std::string HistoName) {
  ...
  cxcoutPnoObj(HistFactory) << "Getting histogram. "
	    << " InputFile " << InputFile
	    << " HistoPath " << HistoPath
	    << " HistoName " << HistoName
	    << std::endl;
  ...
  if( !inFile ) {
    cxcoutEnoObj(HistFactory) << "Error: Unable to open input file: " << InputFile << std::endl;
    throw hf_exc();
  }
  ...
  if( HistoPath != std::string("") ) {
    if( HistoPath[ HistoPath.length()-1 ] != std::string("/") ) {
      cxcoutWnoObj(HistFactory) << "WARNING: Histogram path is set to: " << HistoPath
		<< " but it should end with a '/' " << std::endl;
      cxcoutWnoObj(HistFactory) << "Total histogram path is now: " << HistNameFull << std::endl;
    }
  }
  ...
  try{
    ...
    {
      cxcoutEnoObj(HistFactory) << "Failed to cast object to TH1*" << std::endl;
      std::cout << e.what() << std::endl;
      throw hf_exc();
    }
  if( !hist ) {
    cxcoutEnoObj(HistFactory) << "Failed to get histogram: " << HistNameFull
