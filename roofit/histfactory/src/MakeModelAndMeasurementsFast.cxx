...
RooWorkspace* RooStats::HistFactory::MakeModelAndMeasurementFast( RooStats::HistFactory::Measurement& measurement ) {
  ...
  try {
    ...
    cxcoutInoObj(HistFactory) << "Making Model and Measurements (Fast) for measurement: " << measurement.GetName() << std::endl;

    double lumiError = measurement.GetLumi()*measurement.GetLumiRelErr();

    cxcoutInoObj(HistFactory) << "using lumi = " << measurement.GetLumi() << " and lumiError = " << lumiError
    ...
    cxcoutInoObj(HistFactory) << parameterMessage.str();
    ...
    if (pos != std::string::npos) {
       ...
       cxcoutDnoObj(HistFactory) << "Checking if output directory : " << outputDir << " -  exists" << std::endl;
       if (gSystem->OpenDirectory( outputDir.c_str() )  == 0 ) { 
          cxcoutDnoObj(HistFactory) << "Output directory : " << outputDir << " - does not exist, try to create" << std::endl;
          int success = gSystem->MakeDirectory( outputDir.c_str() );    
          if( success != 0 ) {
             std::string fullOutputDir = std::string(gSystem->pwd()) + std::string("/") + outputDir; 
             cxcoutEnoObj(HistFactory) << "Error: Failed to make output directory: " <<  fullOutputDir << std::endl;
             throw hf_exc();
          }
       } 
    }    
    ...
    cxcoutInoObj(HistFactory) << "Creating the output file: " << outputFileName << std::endl;
    ...
    cxcoutInoObj(HistFactory) << "Creating the table file: " << tableFileName << std::endl;
    tableFile =  fopen( tableFileName.c_str(), "a"); 

    cxcoutInoObj(HistFactory) << "Creating the HistoToWorkspaceFactoryFast factory" << std::endl;
    ...
    cxcoutInoObj(HistFactory) << "Setting preprocess functions" << std::endl;
	...
	cxcoutEnoObj(HistFactory) << "MakeModelAndMeasurementsFast: Channel: " << channel.GetName()
	...
	+ ch_name + "_" + rowTitle + "_model.root";
      ...
      cxcoutInoObj(HistFactory) << "Opening File to hold channel: " << ChannelFileName << std::endl;
	...
	if(!poi) {
	  cxcoutWnoObj(HistFactory) << "Can't do fit for: " << measurement.GetName()
...
void RooStats::HistFactory::FitModelAndPlot(const std::string& MeasurementName, 
  ...
  if( outFile == NULL ) {
    cxcoutEnoObj(HistFactory) << "Error: Output File in FitModelAndPlot is NULL" << std::endl;
    throw hf_exc();
  }

  if( tableFile == NULL ) {
    cxcoutEnoObj(HistFactory) << "Error: tableFile in FitModelAndPlot is NULL" << std::endl;
    throw hf_exc();
  }

  if( combined == NULL ) {
    cxcoutEnoObj(HistFactory) << "Error: Supplied workspace in FitModelAndPlot is NULL" << std::endl;
    throw hf_exc();
  }

  ModelConfig* combined_config = (ModelConfig *) combined->obj("ModelConfig");
  if(!combined_config){
    cxcoutEnoObj(HistFactory) << "Error: no ModelConfig found in Measurement: "
  ...
  if(!simData){
    cxcoutEnoObj(HistFactory) << "Error: Failed to get dataset: " << data_name
  ...
  if(!POIs) {
    cxcoutEnoObj(HistFactory) << "Not Fitting Model for measurement: " << MeasurementName
  ...
  if( model==NULL ) {
    cxcoutEnoObj(HistFactory) << "Error: Failed to find pdf in ModelConfig: " << combined_config->GetName()
	      << std::endl;
    throw hf_exc();
  }
  ...
  if( POIs->getSize()==0 ) {
    cxcoutWnoObj(HistFactory) << "WARNING: No POIs found in measurement: " << MeasurementName << std::endl;
    return;
  }
  ...
  while( (poi_obj=params_itr->Next()) ) {
    ...
    cxcoutInoObj(HistFactory) << "printing results for " << poi->GetName()
	      << " at " << poi->getVal()<< " high " 
	      << poi->getErrorLo() << " low " 
  ...
  if( profile==NULL ) {
    cxcoutEnoObj(HistFactory) << "Error: Failed to make ProfileLikelihood for: " << poi->GetName()
  ...
  if( frame == NULL ) {
    cxcoutEnoObj(HistFactory) << "Error: Failed to create RooPlot frame for: " << poi->GetName() << std::endl;
    throw hf_exc();
  }
  ...
  if( channel_dir == NULL ) {
    cxcoutEnoObj(HistFactory) << "Error: Failed to make channel directory: " << channel << std::endl;
    throw hf_exc();
  }
  TDirectory* summary_dir = channel_dir->mkdir("Summary");
  if( summary_dir == NULL ) {
    cxcoutEnoObj(HistFactory) << "Error: Failed to make Summary directory for channel: "
	      << channel << std::endl;
    throw hf_exc();
  }
  summary_dir->cd();
...
void RooStats::HistFactory::FitModel(RooWorkspace * combined, std::string data_name ) {
    ...
    cxcoutInoObj(HistFactory) << "In Fit Model" << std::endl;
    ModelConfig * combined_config = (ModelConfig *) combined->obj("ModelConfig");
    if(!combined_config){
      cxcoutEnoObj(HistFactory) << "no model config " << "ModelConfig" << " exiting" << std::endl;
      return;
    }
    
    RooAbsData* simData = combined->data(data_name.c_str());
    if(!simData){
      cxcoutEnoObj(HistFactory) << "no data " << data_name << " exiting" << std::endl;
      return;
    }

    const RooArgSet * POIs=combined_config->GetParametersOfInterest();
    if(!POIs){
      cxcoutEnoObj(HistFactory) << "no poi " << data_name << " exiting" << std::endl;
      return;
    }
