...
*  Compiler    : G__storeobject(G__value *buf1,G__value *buf2)
****************************************************************/
int Cint::Internal::G__scanobject(G__value *buf1)
{
  int i;
  ...
  if(buf1->type=='U') {
    ...
    do {
      for(i=0;i<var1->allvar;i++) {
        ...
        if(var1->p_typetable[i]) {
          type_name = var1->p_typetable[i].Name(::ROOT::Reflex::SCOPED).c_str();
        }
        else {
...
long Cint::Internal::G__what_type(char *name,char *type,char *tagname,char *type_name)
{
  ...
  if(type_name!=0 && G__value_typenum(buf)) strcpy(type_name,G__value_typenum(buf).Name(::ROOT::Reflex::SCOPED).c_str());
#ifdef __GNUC__
#else
