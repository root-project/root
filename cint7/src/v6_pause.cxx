...
#define G__RESET_TEMPENV                               \
      ...
      G__tagnum = G__Dict::GetDict().GetScope(store.tagnum);                          \
...
static void G__show_undo_position(int index)
{
  ...
  while(typenum<G__Dict::GetDict().GetNumTypes()) {
     ::ROOT::Reflex::Type typedf( G__Dict::GetDict().GetTypedef(typenum) );
     if (typedf.IsTypedef()) fprintf(G__sout,"%s ",typedf.Name(::ROOT::Reflex::SCOPED).c_str());
     ++typenum;
  }
  fprintf(G__sout,"\n");
...
extern "C" int G__init_process_cmd()
{
  ...
  view.tagnum=G__get_tagnum(G__tagnum);
...
* G__debugvariable()
******************************************************************/
static void G__debugvariable(FILE *fp,G__var_array *var,char *name) 
{
  int ig15;
  int i;
  while(var) {
    for(ig15=0;ig15<var->allvar;ig15++) {
      if(var->hash[ig15] && strcmp(var->varnamebuf[ig15],name)==0) {
        fprintf(fp
         ,"%s p=%ld type=%c typenum=%s tagnum=%d const=%x static=%d\n paran=%d "
                ,var->varnamebuf[ig15]
                ,var->p[ig15]
                ,var->type[ig15]
                ,var->p_typetable[ig15].Name(::ROOT::Reflex::SCOPED).c_str()
                ,var->p_tagtable[ig15]
                ,var->constvar[ig15]
                ,var->statictype[ig15]
                ,var->paran[ig15]
          ...
          if(num<=0) {
            ...
            if(base==0) {
              ...
              sprintf(syscom,"{d=%g i=%ld,reftype=%d} type=%c,tag=%d,type=%s,ref=%lx,isconst=%d"
                      ...
                      ,buf.type,buf.tagnum,G__value_typenum(buf).Name().c_str(),buf.ref,buf.isconst);
            }
            else {
