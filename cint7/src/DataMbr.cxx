	...
	,G__ClassInfo *belongingclassin)
{
  if(handlein) {
    ...
    type.reflexInfo->type = var->type[index];
    type.tagnum=var->p_tagtable[index];
    type.reflexInfo->typenum=var->p_typetable[index];
...
long Cint::G__DataMemberInfo::Property()
{
  if(IsValid()) {
    ...
    if(var->p_typetable[index]) property|=G__BIT_ISTYPEDEF;
    if(-1==var->p_tagtable[index]) property|=G__BIT_ISFUNDAMENTAL;
       ...
       if(strcmp(G__struct.name[var->p_tagtable[index]],"G__longlong")==0 ||
          ...
          strcmp(G__struct.name[var->p_tagtable[index]],"G__longdouble")==0) {
             ...
             if(var->p_typetable[index]  && 
                (var->p_typetable[index].FinalType().Name(ROOT::Reflex::FINAL) == "long long" ||
                var->p_typetable[index].FinalType().Name(ROOT::Reflex::FINAL) == "unsigned long long" ||
                var->p_typetable[index].FinalType().Name(ROOT::Reflex::FINAL) == "long double")) {
