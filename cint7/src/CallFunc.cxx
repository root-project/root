...
void Cint::G__CallFunc::SetArg(long l)
{
  ...
  G__value_typenum(para.para[para.paran]) = ::ROOT::Reflex::Type();
			  ...
			  )
{
  ...
  do {
    ...
    if (para.parameter[para.paran][0]) {
      ...
      if(islower(buf->type))
	strcpy(argtype+pos
	       ,G__type2string(buf->type,buf->tagnum,G__get_typenum(G__value_typenum(*buf)),0,0));
      else 
	strcpy(argtype+pos
	       ,G__type2string(buf->type,buf->tagnum,G__get_typenum(G__value_typenum(*buf))
...
void Cint::G__CallFunc::SetFuncType() {
  if(method.IsValid()) {
    ...
    G__value_typenum(result) = ifunc->p_typetable[ifn];
