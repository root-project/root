...
int Cint::G__ClassInfo::NDataMembers()
{
   ...
   if (IsValid()) {
      ...
      struct G__var_array* var = G__struct.memvar[tagnum];
      while (var) {
  ...
  MatchMode mode, InheritanceMode imode)
{
   ...
   char* param = (char*) arg;
   long index = 0L;
   ifunc = G__get_methodhandle(
     funcname, param, ifunc, &index, poffset,
     (mode == ConversionMatch) ? 1 : 0,
     imode);

   if (ifunc && (ifunc->pentry[index]->size == -1)) {
  ...
  MatchMode mode, InheritanceMode imode)
{
   ...
   long index = 0L;
   ifunc = G__get_methodhandle(
     funcname, param, ifunc, &index, poffset, convmode,
  ...
  MatchMode mode, InheritanceMode imode)
{
  ...
  long index = 0L;
  ifunc = G__get_methodhandle2(
    funcname, libp, ifunc, &index, poffset,
    (mode == ConversionMatch) ? 1 : 0,
    (imode == WithInheritance) ? 1 : 0);
