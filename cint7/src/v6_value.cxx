...
#endif
      G__tagnum = G__Dict::GetDict().GetScope(p.tagnum);
...
G__value Cint::Internal::G__letvalue(G__value *p,G__value result)
{
  ...
  if(G__value_typenum(*p) && G__value_typenum(*p).IsArray()) {
    ...
    int size = G__Lsizeof(G__value_typenum(*p).Name(::ROOT::Reflex::SCOPED).c_str());
    G__var_type = store_var_type;
...
void G__set_typenum(G__value* val, char* type) {
   ::ROOT::Reflex::Type td = G__find_typedef(type);
   G__value_typenum(*val) = td;
   val->tagnum = ::Cint::Internal::G__get_tagnum(td.FinalType());
