...
char *Cint::Internal::G__valuemonitor(G__value buf,char *temp)
{
  ...
  if(G__value_typenum(buf)) {
    ...
    case 'f':
      ...
      if(buf.obj.d<0.0)
        sprintf(temp,"(%s)(%.17e)"
                ,G__type2string(buf.type ,buf.tagnum
...
extern "C" char *G__fulltagname(int tagnum,int mask_dollar)
{
  ...
  if (-1 == tagnum || G__struct.alltag==0) return "";
...
#define G__OLDIMPLEMENTATION1503
    ...
    if(G__struct.defaulttypenum[p_tagnum[pt]]) 
      sprintf(string+len,"%s::"
              ,G__struct.defaulttypenum[p_tagnum[pt]].Name().c_str());
    else
...
extern "C" char *G__type2string(int type,int tagnum,int typenum_in,int reftype,int isconst)
{
  ...
  ::ROOT::Reflex::Type typenum = G__Dict::GetDict().GetTypedef(typenum_in);

  string=stringbuf;
  if(isconst&G__CONSTVAR
     && (!typenum || !(isconst&G__get_isconst(typenum)))
     ) {
    strcpy(string,"const ");
    string+=6;
  }
...
#ifndef G__OLDIMPLEMENTATION1503
  if(-1==typenum && tagnum>0 && G__struct.defaulttypenum[tagnum] &&
     'u'==G__get_type(G__struct.defaulttypenum[tagnum])) {
    typenum = G__struct.defaulttypenum[tagnum];
  }
#endif
  if(typenum) {
    sprintf(string,"%s",typenum.Name(::ROOT::Reflex::SCOPED).c_str());
    ...
    if('$'==G__struct.name[tagnum][0]) {
      if('\0'==G__struct.name[tagnum][1]) {
        ...
        switch(G__struct.type[tagnum]) {
          case 'e': strcpy(string,"enum "); break;
          case 'c': strcpy(string,"class "); break;
          case 's': strcpy(string,"struct "); break;
          case 'u': strcpy(string,"union "); break;
          case 'a': 
      ...
      if(G__CPPLINK==G__globalcomp
        ...
        case 's': strcpy(string,"struct "); break;
        case 'u': strcpy(string,"union "); break;
        case 'n': strcpy(string,"namespace "); break;
        case 'a': strcpy(string,""); break;
        case 0: strcpy(string,"(unknown) "); break;
        }
        len=strlen(string);
      }
    }
...
*
******************************************************************/
double Cint::Internal::G__atodouble(char *string)
{
  int lenstring;
  int ig16=0,ig26=0;
...
G__value Cint::Internal::G__checkBase(char *string,int *known4)
{
  ...
  *******************************************************/
  ...
  result4.tagnum = -1;
  G__value_typenum(result4) = ::ROOT::Reflex::Type();
...
extern "C" G__value G__string2type_body(const char *typenamin,int noerror)
{
  ...
  if(strncmp(typenam,"const",5)==0 &&
     -1==G__defined_tagname(typenam,2) && !G__find_typedef(typenam)) {
  ...
  if(0==result.type) {
    G__value_typenum(result)=G__find_typedef(typenam);
    if(G__value_typenum(result)) {
      result.tagnum=G__get_tagnum(G__value_typenum(result));
      result.type=G__get_type(G__value_typenum(result));
