...
*
******************************************************************/
::ROOT::Reflex::Scope Cint::Internal::G__get_envtagnum()
{
...
#pragma message (FIXME("G__isenclosingclass also checks for using directives, and it really shouldn't"))
   ...
   ::Reflex::Scope tagnum = env_tagnum.DeclaringScope();
   while(tagnum) {
      if (((const ::Reflex::Type&)tagnum).HasBase(enclosingtagnum))
         return 1;
	   if(tagnum==enclosingtagnum) return(1);
      tagnum = tagnum.DeclaringScope();
  }
...
::ROOT::Reflex::Type Cint::Internal::G__find_type(const char *type_name, int /*errorflag*/, int /*templateflag*/)
{
  int ispointer = 0;
...
{
   static ::ROOT::Reflex::NamespaceBuilder stdnp("std");
  int i,len;
  char *p;
  ...
  if(p) {
    ...
    *p='\0';
    if(p==temp) env_tagnum = ::Reflex::Scope::GlobalScope();  /* global scope */
      ...
      env_tagnum = G__Dict::GetDict().GetScope(G__defined_tagname(temp, noerror));
      if(!env_tagnum) return(-1);
    }
  }
  else {
 ...
 try_again:
   ...
   ::Reflex::Scope lookup = env_tagnum.LookupScope(tagname);
   if (lookup) {
      long lookupTagnum = G__get_tagnum(lookup);
      G__class_autoloading(lookupTagnum);
      return lookupTagnum;
   }

#if 0 // old implementation
#undef G__RFLX_CHECK_LOOKUP

  for(i=G__struct.alltag-1;i>=0;i--) {
     if(len==G__struct.hash[i]&&strcmp(atom_tagname,G__struct.name[i])==0) {
        const ::Reflex::Scope& iDecl = G__Dict::GetDict().GetScope(i).DeclaringScope();
        if ((char*)NULL==p&&!iDecl||
           env_tagnum==iDecl) {
           G__class_autoloading(i);
#ifdef G__RFLX_CHECK_LOOKUP
           const ::Reflex::Scope& old = G__Dict::GetDict().GetScope(i);
           const ::Reflex::Scope& lookup = env_tagnum.LookupScope(tagname);
           if (old != lookup && old != ::Reflex::Scope::GlobalScope()) {
              const ::Reflex::Type& lookupType = env_tagnum.LookupType(tagname);
              if (old != lookupType)
                 if (lookup)
                    printf("DEBUG: reflex lookup (%s %s) and G__search_tagnum (%s %s) have different result for %s!\n",
                    lookup.ScopeTypeAsString().c_str(), lookup.Name(Reflex::SCOPED).c_str(),
                    old.ScopeTypeAsString().c_str(), old.Name(Reflex::SCOPED).c_str(), tagname);
                 else
                    printf("DEBUG: reflex lookup (%s %s) and G__search_tagnum (%s %s) have different result for %s!\n",
                    lookupType.TypeTypeAsString().c_str(), lookupType.Name(Reflex::SCOPED).c_str(),
                    old.ScopeTypeAsString().c_str(), old.Name(Reflex::SCOPED).c_str(), tagname);
           }
#endif
           return(i);
        ...
        if ( candidateTag == -1 &&(
           ((const ::Reflex::Type&) env_tagnum).HasBase(iDecl)
        ||G__isenclosingclass(iDecl,env_tagnum)
        ||G__isenclosingclassbase(iDecl,env_tagnum)
        ||((char*)NULL==p&&G__tmplt_def_tagnum==iDecl)
        || ((const ::Reflex::Type&)G__tmplt_def_tagnum).HasBase(iDecl)
        ||G__isenclosingclass(iDecl,G__tmplt_def_tagnum)
        ||G__isenclosingclassbase(iDecl,G__tmplt_def_tagnum)
        ))
           candidateTag = i;
     }
  }
...
#ifdef G__RFLX_CHECK_LOOKUP
     const ::Reflex::Scope& old = G__Dict::GetDict().GetScope(candidateTag);
     const ::Reflex::Scope& lookup = env_tagnum.LookupScope(tagname);
     if (old != lookup && old != ::Reflex::Scope::GlobalScope()) {
        const ::Reflex::Type& lookupType = env_tagnum.LookupType(tagname);
        if (old != lookupType)
  ...
  ::ROOT::Reflex::Type tp = G__find_typedef(tagname);
...
#pragma message (FIXME("Remove this once scopes are in reflex!"))
  ...
  G__struct.size[i] = 0;
  G__struct.type[i] = 'n';
  G__struct.memvar[i] = new G__var_array; 
  ...
  G__struct.memfunc[i] = new G__ifunc_table;
  ...
  G__struct.isabstract[i]=0;
   ...
   G__struct.defaulttypenum[i] = ::ROOT::Reflex::Type();
#endif
...
extern "C" int G__search_tagname(const char *tagname,int type)
{
   ...
   if(-1==i
      || (envtagnum != G__Dict::GetDict().GetScope(G__struct.parent_tagnum[i]) && isstructdecl)
         ...
         ::ROOT::Reflex::Scope scope = G__Dict::GetDict().GetScope(G__struct.parent_tagnum[i]);
         ::ROOT::Reflex::Type cl; 
         ::ROOT::Reflex::Scope newscope;
         ...
         if (!cl) {
            ...
            switch(type) {
               case   0: // Unknown type
               case 'a': // Autolading
                  cl =  ::ROOT::Reflex::ClassBuilder( fullname.c_str(), typeid(::ROOT::Reflex::UnknownType), 0, ::ROOT::Reflex::CLASS ).ToType();
                  break;
               case 'c': // Class
                  ::ROOT::Reflex::ClassBuilder( fullname.c_str(), typeid(::ROOT::Reflex::UnknownType), 0, ::ROOT::Reflex::CLASS ); // Should also add the privacy with the containing class.
                  cl = ::ROOT::Reflex::Type::ByName( fullname.c_str() );
                  break;
               case 's': // Struct
                  ::ROOT::Reflex::ClassBuilder( fullname.c_str(), typeid(::ROOT::Reflex::UnknownType), 0, ::ROOT::Reflex::STRUCT ); // Should also add the privacy with the containing class.
                  cl = ::ROOT::Reflex::Type::ByName( fullname.c_str() );
                  break;
               case 'n': // Namespace
                  ::ROOT::Reflex::NamespaceBuilder( fullname.c_str() );
                  newscope = ::ROOT::Reflex::Scope::ByName( fullname.c_str() );
                  break;
               case 'e': // Enum
                  cl = ::ROOT::Reflex::EnumTypeBuilder( fullname.c_str() );
                  break;
               case 'u': // Union
                     ::ROOT::Reflex::UnionBuilder< ::ROOT::Reflex::UnknownType >( fullname.c_str() );
                     cl = ::ROOT::Reflex::Type::ByName( fullname.c_str() );
                     break;
                  default:
                     assert(false);
            ...
            G__dumpreflex();
            G__fprinterr(G__serr,"When attempting to declare %s (tagnum==%d) in scope %s (tagnum==%d) found an existing %s %s"
               " in scope %s\n",
               atom_tagname,i,
               G__struct.name[G__struct.parent_tagnum[i]] ? G__struct.name[G__struct.parent_tagnum[i]] : "", G__struct.parent_tagnum[i],
               cl.TypeTypeAsString().c_str(),cl.Name(::ROOT::Reflex::SCOPED).c_str(),
               scope.Name(::ROOT::Reflex::SCOPED).c_str());
            G__genericerror((char*)NULL);
         }
...
#pragma message (FIXME("Fix treatment of namespace std!"))
  ...
  case ':':
  case ';':
    G__tagnum=G__Dict::GetDict().GetScope(G__search_tagname(tagname,toupper(type)));
    break;
  default:
  ...
  if(G__tagnum.IsTopScope() || !G__tagnum) {
#ifdef __GNUC__
#else
#pragma message(FIXME("Shouldn't we warn the user that we cannot find the underlying type? Or can we register a typedef to uninitialized type?"))
  ...
  while(c==',') {
    ...
    if(newdecl) {
      ...
      if(!G__tagnum.IsTopScope()) G__def_struct_member=1 ;
      ...
      if(1==((const ::Reflex::Type&)lstore_tagnum).SizeOf()
         && 0==G__struct.memvar[G__get_tagnum(lstore_tagnum)]->allvar 
         && 0==G__struct.baseclass[G__get_tagnum(lstore_tagnum)]->basen
         )
        baseclass->baseoffset[*pbasen]=0;
      else
         baseclass->baseoffset[*pbasen]=G__struct.size[G__get_tagnum(lstore_tagnum)] ; // FIXME ((const ::Reflex::Type&)lstore_tagnum).SizeOf();
  ...
  if(c=='{') { /* member declarations */
    ...
    if(newdecl || 'n'==type) {
      ...
      if(type=='e') { /* enum */
        ...
        G__value_typenum(enumval) = ::ROOT::Reflex::Type();
         ...
         * the size is aligned.
         ********************************************/
        if((1==G__struct.memvar[G__get_tagnum(G__tagnum)]->allvar && G__struct.memvar[G__get_tagnum(G__tagnum)]->next==0)
  ...
  if('u'==type) { /* union */
    ...
    if(basename[0]) {
      ...
      G__define_var(G__get_tagnum(G__tagnum),::ROOT::Reflex::Type());
      G__disp_mask=0;
    }
    else if(!strncmp(G__struct.name[G__get_tagnum(G__tagnum)], "$G__NONAME", 10)) {
      /* anonymous union */
      G__add_anonymousunion(G__get_tagnum(G__tagnum),G__def_struct_member,G__get_tagnum(store_def_tagnum));
    }
  }
    ...
    if(G__cintv6) G__bc_struct(G__get_tagnum(G__tagnum));
 ...
 * G__callfunc0()
 ******************************************************************/
int Cint::Internal::G__callfunc0(G__value *result, G__ifunc_table *ifunc, int ifn
 ...
 * G__calldtor
 ******************************************************************/
int Cint::Internal::G__calldtor(void *p,int tagnum,int isheap)
{
  int stat;
  G__value result;
