...
* void G__disp_friend
***********************************************************************/
static int G__display_friend(FILE *fp,G__friendtag*friendtag)
{
...
* void G__listfunc
***********************************************************************/
int Cint::Internal::G__listfunc(FILE *fp,int access,char *fname,G__ifunc_table *ifunc)
{
...
***********************************************************************/
int Cint::Internal::G__listfunc_pretty(FILE *fp,int access,char *fname,G__ifunc_table *ifunc, char friendlyStyle)
{
  int i,n;
    ...
    for(i=0;i<ifunc->allifunc;i++) {
      ...
      if(
        ...
        sprintf(msg,"%s ",G__type2string(ifunc->type[i]
                                        ,ifunc->p_tagtable[i]
                                        ,G__get_typenum(ifunc->p_typetable[i])
                                        ,ifunc->reftype[i]
                                        ,ifunc->isconst[i]));
        if(G__more(fp,msg)) return(1);
...
{
   int k;
   ::ROOT::Reflex::Type_Iterator start,stop;
   char temp[G__ONELINE];
   char msg[G__LONGLINE];
   ...
   if(name[k]) {
      ::ROOT::Reflex::Type starttype = G__find_typedef(name+k);
      if(!starttype) {
         G__fprinterr(G__serr,"!!!Type %s is not defined\n",name+k);
         return(0);
      }
      stop = std::find(::ROOT::Reflex::Type::Type_Begin(),
         ::ROOT::Reflex::Type::Type_End(), starttype);
      start = stop++;
   }
   else {
      if (startin>0) {
         ::ROOT::Reflex::Type starttype = G__Dict::GetDict().GetTypedef(startin);
         if(!starttype) {
            G__fprinterr(G__serr,"!!!Type %d is not defined\n",startin);
            return(0);
         }
         start = std::find(::ROOT::Reflex::Type::Type_Begin(),
            ::ROOT::Reflex::Type::Type_End(), starttype);
      } else {
         start = ::ROOT::Reflex::Type::Type_Begin();
      }
      stop = ::ROOT::Reflex::Type::Type_End();
   }
   ...
   for(::ROOT::Reflex::Type_Iterator iTypedef = start; iTypedef != stop; ++iTypedef) {
      if(!G__browsing) return(0);
      ...
      if(
         ...
         'Q'==typedef_char_type
            ...
            sprintf(msg,"typedef void* %s", iTypedef->Name(::ROOT::Reflex::SCOPED).c_str()); 
            if(G__more(fout,msg)) return(1);
      }
      else if('a'==typedef_char_type) {
         /* pointer to member */
         sprintf(msg,"typedef G__p2memfunc %s", iTypedef->Name(::ROOT::Reflex::SCOPED).c_str()); 
         if(G__more(fout,msg)) return(1);
      }
      else {
         ...
         if(isupper(typedef_char_type) && iTypedef->ToType().IsArray()) {
            sprintf(msg,"(*%s)",iTypedef->Name(::ROOT::Reflex::SCOPED).c_str());
            if(G__more(fout,msg)) return(1);
         }
         else {
         ...
         for(::ROOT::Reflex::Type arrayType = *iTypedef; 
            arrayType.IsArray(); 
            arrayType = arrayType.ToType()) {
               sprintf(msg,"[%u]", arrayType.ArrayLength());
...
*
******************************************************************/
int Cint::Internal::G__objectmonitor(FILE *fout,long pobject,int tagnum,char *addspace)
{
  struct G__inheritance *baseclass;
...
*
******************************************************************/
int Cint::Internal::G__varmonitor(FILE *fout,G__var_array *var,char *index,char *addspace,long offset)
{
  int imon1;
  long addr;
  char space[50];
...
extern "C" int G__display_class(FILE *fout,char *name,int base,int start)
{
  ...
  *******************************************************************/
  ...
  if(G__cintv6) {
    ...
    ::Cint::Bytecode::G__bc_disp_vtbl(fout,tagnum);
  }
