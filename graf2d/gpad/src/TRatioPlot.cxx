...
TRatioPlot::~TRatioPlot()
{
   ...
   if (fRatioGraph != nullptr) delete fRatioGraph;
   if (fConfidenceInterval1 != nullptr) delete fConfidenceInterval1;
   if (fConfidenceInterval2 != nullptr) delete fConfidenceInterval2;

   for (unsigned int i=0;i<fGridlines.size();++i) {
      delete (fGridlines[i]);
   }

   if (fSharedXAxis != nullptr) delete fSharedXAxis;
   if (fUpperGXaxis != nullptr) delete fUpperGXaxis;
   if (fLowerGXaxis != nullptr) delete fLowerGXaxis;
   if (fUpperGYaxis != nullptr) delete fUpperGYaxis;
   if (fLowerGYaxis != nullptr) delete fLowerGYaxis;
   if (fUpperGXaxisMirror != nullptr) delete fUpperGXaxisMirror;
   if (fLowerGXaxisMirror != nullptr) delete fLowerGXaxisMirror;
   if (fUpperGYaxisMirror != nullptr) delete fUpperGYaxisMirror;
   if (fLowerGYaxisMirror != nullptr) delete fLowerGYaxisMirror;

   if (fUpYaxis != nullptr) delete fUpYaxis;
   if (fLowYaxis != nullptr) delete fLowYaxis;
}
...
void TRatioPlot::SetupPads() {
   ...
   if (fUpperPad != nullptr) {
      delete fUpperPad;
      fUpperPad = nullptr;
   }

   if (fLowerPad != nullptr) {
      delete fLowerPad;
      fLowerPad = nullptr;
   }
...
Int_t TRatioPlot::BuildLowerPlot()
{
   ...
   if (fRatioGraph != nullptr) {
      fRatioGraph->IsA()->Destructor(fRatioGraph);
      fRatioGraph = nullptr;
   }

   if (fConfidenceInterval1 == nullptr) {
      fConfidenceInterval1 = new TGraphErrors();
   }

   if (fConfidenceInterval2 == nullptr) {
