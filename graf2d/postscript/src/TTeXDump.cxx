...
void TTeXDump::DrawPolyMarker(Int_t n, Double_t *xw, Double_t *yw)
{
   ...
   if (GetMarkerStyleBase() == 23 || GetMarkerStyleBase() == 32) PrintStr(",rotate=180");

   PrintStr(Form("},mark size=%fpt", 8./3.33*(fMarkerSize - TMath::Floor(GetMarkerLineWidth()/2.)/4.)));
   PrintStr(Form(", line width=%fpt", 4./3.33*TMath::Floor(GetMarkerLineWidth()/2.)));
   PrintStr(", mark=");
   switch (GetMarkerStyleBase()) {
   case 1 :
      PrintStr("*");
