...
//______________________________________________________________________________
TVirtualGraphPainter *TGraph::GetPainter(Option_t * /*option*/)
{
   // return pointer to painter
   // if painter does not exist, it is created

   if (!fPainter) fPainter = TVirtualGraphPainter::GraphPainter(this);
   return fPainter;
}
...
void TGraph::PaintGraph(Int_t npoints, const Double_t *x, const Double_t *y, Option_t *chopt)
{
   ...
   GetPainter(chopt);
   if (fPainter) fPainter->PaintGraph(npoints, x, y, chopt);
}
...
void TGraph::PaintGrapHist(Int_t npoints, const Double_t *x, const Double_t *y, Option_t *chopt)
{
   ...
   GetPainter(chopt);
   if (fPainter) fPainter->PaintGrapHist(npoints, x, y, chopt);
}
...
void TGraph::PaintStats(TF1 *fit)
{
   ...
   GetPainter("");
   if (fPainter) fPainter->PaintStats(fit);
}
