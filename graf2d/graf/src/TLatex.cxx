...
TLatexFormSize TLatex::Analyse(Double_t x, Double_t y, TextSpec_t spec, const Char_t* t, Int_t length)
{
   ...
   strlcpy(text,t+nBlancDeb,length);
   text[length] = 0;
   ...
   for(i=0;i<length;i++) {
      ...
      if (length>i+1) {
         Char_t buf[3];
         strlcpy(buf,&text[i],2);
         if (strncmp(buf,"^{",2)==0) {
            if (opPower==-1 && nBrackets==0 && nCroch==0) opPower=i;
            if (i>3) {
               Char_t buf1[5];
               strlcpy(buf1,&text[i-4],4);
   ...
   if (opCloseCurly>-1 && opCloseCurly<length-1) { // separator } found
     ...
     // result = TLatexFormSize(fs1.Width()+3*l,fs1.Over(),fs1.Under());
      ...
      strlcpy(nb,text+opColor+7,opSquareCurly-opColor-7);
      ...
      strlcpy(nb,text+opFont+6,opSquareCurly-opFont-6);
      ...
      strlcpy(dxc,text+opKern+6,opSquareCurly-opKern-6);
      ...
      strlcpy(dyc,text+opLower+7,opSquareCurly-opLower-7);
      ...
      strlcpy(nb,text+opScale+7,opSquareCurly-opScale-7);
...
Int_t TLatex::CheckLatexSyntax(TString &text)
{
   ...
   i = 0 ;
   while (i < length) {
      strlcpy(buf,&text[i],TMath::Min(7,length-i));
      opFound = kFALSE ;
      for (k = 0 ; k < nLeft1 ; k++) {
   ...
   while (i< length){
         ...
         strlcpy(buf,&text[i],TMath::Min(11,length-i));
         opFound = kFALSE ;
