...
void TASImage::WriteImage(const char *file, EImageFileTypes type)
{
   ...
   case kXpm:
      ...
      break;
   case kBmp: ASImage2bmp(im, fname.Data(), nullptr); return;
   case kXcf: ASImage2xcf(im, fname.Data(), nullptr); return;
   case kPng:
                              ...
                              Option_t *opt)
{
   ...
   if (x11) {
      ...
      ASImageDecoder *imdec = start_image_decoding(fgVisual, im, SCL_DO_ALPHA, xsrc, ysrc, ww, 0, nullptr);
      if (imdec) {
         for (yy = 0; yy < hh; yy++) {
...
void TASImage::Paint(Option_t *option)
{
   ...
   if (gVirtualPS) {
      ...
      ASImageDecoder *imdec =
         start_image_decoding(fgVisual, image, SCL_DO_ALL, 0, 0, image->width, image->height, nullptr);
      if (!imdec) return;
      ...
      if (grad_im) {
         ...
         imdec = start_image_decoding(fgVisual, grad_im, SCL_DO_ALL, 0, 0, grad_im->width, grad_im->height, nullptr);
         if (imdec) {
...
void TASImage::SetPalette(const TImagePalette *palette)
{
   ...
   if (fImage->alt.vector == nullptr) return;
...
Pixmap_t TASImage::GetPixmap()
{
   ...
   if (x11) {   // use builtin version
      ret = (Pixmap_t)asimage2pixmap(fgVisual, gVirtualX->GetDefaultRootWindow(), img, nullptr, kTRUE);
   } else {
...
Pixmap_t TASImage::GetMask()
{
   ...
   ASImageDecoder *imdec = start_image_decoding(fgVisual, img, SCL_DO_ALPHA, 0, 0, ww, 0, nullptr);
   if (!imdec) {
      delete [] bits;
      return 0;
   }
...
TArrayL *TASImage::GetPixels(Int_t x, Int_t y, UInt_t width, UInt_t height)
{
   ...
   if ((imdec = start_image_decoding(nullptr, fImage, SCL_DO_ALL, 0, y, img->width, height, nullptr)) == nullptr) {
...
TArrayD *TASImage::GetArray(UInt_t w, UInt_t h, TImagePalette *palette)
{
   ...
   if ((imdec = start_image_decoding(nullptr, img, SCL_DO_ALL, 0, 0, img->width, 0, nullptr)) == nullptr) {
...
void TASImage::Crop(Int_t x, Int_t y, UInt_t width, UInt_t height)
{
   ...
   ASImageDecoder *imdec = start_image_decoding(fgVisual, fImage, SCL_DO_ALL, x, y, width, height, nullptr);
...
UInt_t *TASImage::GetScanline(UInt_t y)
{
   ...
   ASImageDecoder *imdec = start_image_decoding(fgVisual, img, SCL_DO_ALL, 0, y, img->width, 1, nullptr);
...
void TASImage::GetImageBuffer(char **buffer, int *size, EImageFileTypes type)
{
   ...
   switch (type) {
   case TImage::kXpm: ret = ASImage2xpmRawBuff(img, (CARD8 **)buffer, size, nullptr); break;
   default: ret = ASImage2PNGBuff(img, (CARD8 **)buffer, size, &params);
   }
...
void TASImage::Gray(Bool_t on)
{
   ...
   if (fImage->alt.argb32) {
      ...
      ASImageDecoder *imdec =
         start_image_decoding(fgVisual, fImage, SCL_DO_ALL, 0, 0, fImage->width, fImage->height, nullptr);
