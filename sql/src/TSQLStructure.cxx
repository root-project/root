...
//  bt TBufferSQL2. It contains data which class wass written, which data
//  correspondent to which element and so on.
//  Such structure is very similar to XML node and subnodes structure
//  Once it created, it than converted to SQL statements, which are submitted 
//  to server.
//
//________________________________________________________________________
...
//________________________________________________________________________
void TSQLStructure::RemoveChild(Int_t n)
{
...
//________________________________________________________________________
Bool_t TSQLStructure::IsOnlyChild() const
{
   return fChilds.GetLast()==0; 
}
...
void TSQLStructure::PrintLevel(Int_t level) const
{
   ...
   TIter iter(&fChilds);
   TSQLStructure* child = 0;
   while ((child = (TSQLStructure*) iter())!=0)
     child->PrintLevel(level+2);
}
...
//________________________________________________________________________
void TSQLStructure::PerformConversion(TSqlRegistry* reg, TObjArray* blobs, const char* topname)
{
   ...
   switch (fType) {
      ...
      case kSqlElement: {
         ...
         for(Int_t n=0;n<=fChilds.GetLast();n++) {
            ...
            child->PerformConversion(reg, blobs, elem->GetName());
         }
         break;    
      }
      ...
      case kSqlArray: {
         ...
         for(Int_t n=0;n<=fChilds.GetLast();n++) {
            TSQLStructure* child = (TSQLStructure*) fChilds.At(n);
            child->PerformConversion(reg, blobs, topname); 
         }
         break;   
      }
   }   
...
// complete object will be converted to raw format
   ...
   if (GetType()==kSqlStreamerInfo)
      ...
      // has normal streamer. Then it will be stored in normal form and only ont tag
...
// where not possible, raw data for some elements are created
   ...
   for(Int_t n=0;n<=fChilds.GetLast();n++) {
      ...
      Int_t columntyp = DefineElementColumnType(elem);
      if (columntyp!=kColRawData) {
         Error("StoreClassInNormalForm","Element %s has problem with normal store ", elem->GetName()); 
         continue;
      }
...
//    kColRawData      -  anything else as raw data
   ...
   if ((typ>TStreamerInfo::kOffsetL) && 
       (typ<TStreamerInfo::kOffsetP)) return kColSimpleArray;
   ...
   if ((typ==TStreamerInfo::kObject) || 
       ...
       (typ==TStreamerInfo::kObjectp)) return kColObject;
...
// find column in TSQLObjectData object, which correspond to current element
   ...
   Int_t coltype = DefineElementColumnType(elem); 
