...
static ::Reflex::Type G__getobjecttagnum(char *name)
{
  ...
  if(!p1 && !p2) {
    ...
    var = G__find_variable(name,varhash,G__p_local,::Reflex::Scope::GlobalScope()
                           ,&store_struct_offset1,&store_struct_offset2, &ig15
                            ,0);
    if(var && (var.TypeOf().IsClass() || var.TypeOf().IsStruct())) {
      return(var.TypeOf());
    }
    else {
      char *p3 = strchr(name,'(');
...

   {
      ...
      if(tagnum) {
         ...
         if(!result) {
            ...
            p[len++]='<';
            p[len]=0;
            for ( ::Reflex::Member_Iterator f = tagnum.FunctionMember_Begin(); 
                  f != tagnum.FunctionMember_End(); 
                  ++f ) {
               if(0==strncmp(f->Name().c_str(),p,len)) {
...
static int G__generate_template_dict(const char* tagname,G__Definedtemplateclass *deftmpclass,G__Charlist *call_para)
{
  ...
  while(call_para->next != NULL) {
    G__value gValue = G__string2type_body( call_para->string, 1 );
    ::Reflex::Type call_para_type(G__value_typenum(gValue).RawType());
    if( call_para_type.IsClass() /* gValue.type == 'u' || == 'U' */ ) {
      int tagnum = G__get_tagnum(call_para_type);
      int index = G__struct.filenum[tagnum];
      if( index < 0 ) return -3;
      index = G__getIndex(index, tagnum, headers);
...
int Cint::Internal::G__instantiate_templateclass(char *tagnamein, int noerror)
{
  ...
  if(env_tagnum && !env_tagnum.IsTopScope() && G__struct.baseclass[G__get_tagnum(env_tagnum)]->basen!=0)
    baseclass = G__struct.baseclass[G__get_tagnum(env_tagnum)];
  else
    baseclass = (struct G__inheritance*)NULL;
    ...
    if(-1==fargtmplt) {
      ...
      if(type==ftype&&ftagnum==tagnum&&(0==freftype||ref)) {
        /* match, check next */
              ...
              (-1!=G__ispublicbase(tagnum,ftagnum,(void*)libp->para[i].obj.i))) {
                    ...
                    ,int hash,int funcmatch)
{
  ...
  if(env_tagnum && env_tagnum.IsClass() && 
     G__struct.baseclass[G__get_tagnum(env_tagnum)]->basen!=0)
    baseclass = G__struct.baseclass[G__get_tagnum(env_tagnum)];
  else
    baseclass = (struct G__inheritance*)NULL;
