				     ...
				     ,const char* postopr)
{
   if(localvar && func_scope.IsValid()) {
      ...
      ::Reflex::Member ig15( localvar.MemberByName(name) );
      if (ig15) {
         tokenproperty = p_data;
         tokentype = t_local;
         ::Reflex::Type tag = ig15.TypeOf().RawType();
         if (G__get_type(tag)=='u' && (strcmp(postopr,".")==0||strcmp(postopr,"->")==0)) {
            nextscope.Init(G__get_tagnum(tag));
         } 
         return(1);
      }
   }
