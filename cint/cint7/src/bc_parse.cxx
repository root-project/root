...
G__blockscope::~G__blockscope() {
  ...
  // free((void*)m_scope); // should not free m_scope, it is stored in bytecode
}
...
void G__blockscope::Init(G__blockscope* enclosing) {
  ...
  std::strstream sScopeName;
  if (enclosing)
     sScopeName << "::" << enclosing->m_scope.Name(Reflex::SCOPED | Reflex::QUALIFIED) 
        << "::" << enclosing->m_scope.SubScopeSize();
  else
     ...
     sScopeName << "::" << m_ifunc.DeclaringScope().Name(Reflex::SCOPED) 
     << "::$" << m_ifunc.Name(Reflex::QUALIFIED);
  sScopeName << "$"; // hidden
  Reflex::NamespaceBuilder(sScopeName.str());
  m_scope = Reflex::Scope::GlobalScope().LookupScope(sScopeName.str());
  ...
  if(enclosing) {
    ...
    int i=0;
#pragma message (FIXME("This part of the block scope setup is now broken. It might ebenfit from using a bit more Reflex later."))
#if 0
    struct G__var_array *var = enclosing->m_scope;
    if(var->inner_scope) {
  ...
  m_scope->ifunc = m_ifunc;
  m_scope->ifn=m_iexist;
#endif
}
...
int G__blockscope::compile_catch(string& token,int c) {
  ...
  if(token=="catch" && '('==c) {
    ...
    if(token=="" && ')'==c) {
      ...
      Reflex::Member var = catchblock.allocatevariable(type,token,arysize,typesize,0);
      if(type.Isreference()) catchblock.m_bc_inst.INIT_REF(var,0,'p');
      else                   catchblock.m_bc_inst.ST_LVAR(var,0,'p');
      //m_bc_inst.POP();
    }
...
int G__blockscope::compile_brace(string& token,int c) {
  if(token=="do")       c = compile_do(token,c);   // c==';'
    ...
    stdclear(token);
    int store_type = G__struct.type[G__get_tagnum(G__tagdefining)];
    G__struct.type[G__get_tagnum(G__tagdefining)] = 'u';
    c = G__blockscope::compile_core(1);
    G__struct.type[G__get_tagnum(G__tagdefining)] = store_type;
    c = m_preader->fignorestream(";");
  }
  else if(token=="") {
    G__blockscope block(this);
...
int G__blockscope::compile_declaration(G__TypeReader& type,string& token,int c) {
  ...
  Reflex::Member var = allocatevariable(type,token,arysize,typesize,isextrapointer);

  if((';'==c||','==c)&& (type.Property()&(G__BIT_ISSTRUCT|G__BIT_ISCLASS)) &&
     0==(type.Property()&(G__BIT_ISPOINTER|G__BIT_ISREFERENCE))) {
    // type  a; 
    //         ^
    c = init_w_defaultctor(type,var,token,c); // c==';' ','
  }
  ...
  if(c=='=') {
    ...
    // type a   = type(args);
    //           ^
    c = read_initialization(type,var,token,c); // c==';' ','
  }  
 }
 l_nextiter: // c==';' ','
  stdclear(token);
...
int G__blockscope::read_initialization(G__TypeReader& type
  ...
  if ((G__get_varlabel(var, 0) /* stride */ == 1) && !G__get_varlabel(var, 1) /* number of elements */) {
    ...
    if(type.Property()&G__BIT_ISREFERENCE) {
      ...
      // type x = val;
      //         ^
      c = initscalar(type,var,token,c); // c== ';' or ','
    }
    else if(type.Property()&(G__BIT_ISSTRUCT|G__BIT_ISCLASS)) {
      // type x = { a, b, c};
      ...
      if(c=='{' && token=="") {
        c = initstruct(type,var,token,c); // c== ';' or ','
      }
      else if(c=='(') {
        G__TypeReader itype;
        if(itype.append(token,0) && type==itype) {
          ...
          // type x  = func(arg);
          //                     ^
          c = init_w_expr(type,var,token,c); // c== ';' ','
        }
      }
      else /* c==';' */ {
        // type x  = expr;
    ...
    if(type.Property()&(G__BIT_ISFUNDAMENTAL|G__BIT_ISPOINTER|G__BIT_ISENUM)){
      ...
      if(G__struct.funcs[type.Tagnum()]&G__HAS_XCONSTRUCTOR) {
        ...
        //        ^
        c = m_preader->fgetstream_template(token,"{(;" /* ,0 */ );
        c = initstruct(type,var,token,c); // c== ';' ','
      }
    }
    else /* if(type.Property&(G__BIT_ISUNION)) */ {
      // error;
...
int G__blockscope::init_w_defaultctor(G__TypeReader& type
  ...
  int num = G__get_varlabel(var, 1) /* number of elements */; // get array size
  if (num>0) {
    m_bc_inst.LD(num);
    m_bc_inst.SETARYINDEX(1);
    call_ctor(type,&para,var,num);
...
int G__blockscope::call_ctor(G__TypeReader& type,struct G__param *libp
  ...
  if(m.IsValid()) {
    ...
    Reflex::Member func = G__Dict::GetDict().GetFunction((size_t) m.Handle());
    if(type.Property()&G__BIT_ISCOMPILED) {
      // This is for compiled class
      m_bc_inst.CTOR_SETGVP(var,0); // init local block scope object
      m_bc_inst.LD_FUNC_BC(func,libp->paran,(void*)m.InterfaceMethod());
      m_bc_inst.SETGVP(-1); // restoration from store_globalvarpointer stack
    }
    else {
      // This is for interpreted class
      m_bc_inst.LD_LVAR(var,0,'p');
      m_bc_inst.PUSHSTROS();
      m_bc_inst.SETSTROS();
      Baseclassctor_vbase(G__get_tagnum(var.TypeOf().RawType()));
    if(num)
      m_bc_inst.LD_FUNC_BC(func,0,(void*)G__bc_exec_ctorary_bytecode);
    else 
      m_bc_inst.LD_FUNC_BC(func,libp->paran,(void*)G__bc_exec_ctor_bytecode);
    //m_bc_inst.LD_FUNC_BC(func,libp->paran,(void*)G__bc_exec_normal_bytecode);
      m_bc_inst.POPSTROS();
    }
    return(1);
  }
  ...
  if(m.IsValid()) {
    ...
    if(cls.Property()&G__BIT_ISCOMPILED) {
      ...
      if(m.Property()&G__BIT_ISVIRTUAL) 
        m_bc_inst.LD_FUNC_VIRTUAL(func,libp->paran
                                ,(void*)G__bc_exec_virtual_bytecode);
      else if(fname==cls.Name()) {
        if(isarray)
          m_bc_inst.LD_FUNC_BC(func,libp->paran
...
int G__blockscope::initscalar(G__TypeReader& type
  ...
  conversion(result,var,'p',0); // embed into G__Isvalidassignment ?
 ...
 ***********************************************************************/
int G__blockscope::initstruct(G__TypeReader& type, const Reflex::Member& var, string& /*token*/, int c)
{
  // FIXME: This whole routine still needs to be converted!!!
  //
...
#if 0 // this needs to be fixed before merging
 ...
 ***********************************************************************/
int G__blockscope::initscalarary(G__TypeReader& /*type*/, const Reflex::Member& var, string& /*token*/, int c) 
{
   // char* ary[] =  { "a", "b" }; 
   // char* ary[n]=  { "a", "b" }; 
   // char  ary[] =  "abc"; 
   ...
   if (num_of_elements == INT_MAX /* unspecified length flag */) {
     ...
     isauto = 1;
     // FIXME: Change var into a pointer.
     // num_of_elements = 0;
   }
   // Load the address of the first element of the array as a pointer.
   int num_of_dimensions = G__get_paran(var);
   for (int j = 0; j < num_of_dimensions; ++j) {
     m_bc_inst.LD(0);
   }
   m_bc_inst.LD_LVAR(var, num_of_dimensions, 'P');
   // initialize buf
   G__value buf;
   // FIXME: Is this the right type for buf?
   G__value_typenum(buf) = Reflex::PointerBuilder(var.TypeOf());
   buf.ref = 0;
   // Get the size of an element of the array.
   int size = 0;
   int typedefary = 0; 
   if (!var.TypeOf().IsPointer()) {
      if (var.TypeOf().IsTypedef() && G__get_nindex(var.TypeOf())) {
         char store_var_type = G__var_type;
         size = G__Lsizeof(var.TypeOf().Name(::Reflex::SCOPED).c_str());
         G__var_type = store_var_type;
         typedefary = 1; 
      }
      else {
      ...
      G__value_typenum(buf) = Reflex::PointerBuilder(Reflex::Type::ByName("long"));
      size = G__LONGALLOC;
   }
   if ((stride < 0) || (size <= 0)) {
   ...
   if (c == ';') {
      ...
      m_bc_inst.LD(0);
      m_bc_inst.LD_LVAR(var, 1, 'p');
      G__value reg = G__getexpr(expr);
      // add G__Isvalidassignment() ??
      conversion(reg, var, 'p', 0); // embed into G__Isvalidassignment ?
      m_bc_inst.LETNEWVAL();
      if (num_of_elements == INT_MAX /* unspecified length flag */) {
         // FIXME: This call assumes that var is a pointer.
         G__update_array_dimension(var, strlen((char*) reg.obj.i) + 1);
      }
      return c;
   }
   if (c != '{') {
   ...
   while (mparen) {
      ...
      if (expr[0]) {
         ...
         if ((Reflex::Tools::FundamentalType(var.TypeOf().FinalType()) == Reflex::kCHAR) && (expr[0] == '"')) {
            // -- Character array initialized by a string literal.
            if (!typedefary) {
               // FIXME: This is only right for one-dimensional arrays!
               size = G__get_varlabel(var, G__get_paran(var));
            }
            stringflag = 1;
         ...
         if ((num_of_elements || isauto) && (linear_index >= num_of_elements)) {
            if (isauto) {
               ...
               G__fprinterr(G__serr, "Error: %s: %d: Array initialization over-run '%s'", __FILE__, __LINE__, var.Name(Reflex::SCOPED).c_str());
               G__genericerror(0);
               return c;
            }
         }
      ...
      switch (c) {
         case '{':
            // -- Increment nesting level.
            ++mparen;
            if (stringflag && (G__get_paran(var) > 2)) {
               inc *= G__get_varlabel(var, --pi);
            }
            else {
               inc *= G__get_varlabel(var, pi--);
            }
   ...
   if (isauto) {
      ...
      G__malloc(num_of_elements, size, var.Name(Reflex::SCOPED).c_str());
   }
...
/////////////////////////////////////////////////////////////////////////
Reflex::Member G__blockscope::allocatevariable(G__TypeReader& type
                                                     ,const string& name
                                                     ,deque<int>& arysize
                                                     ,deque<int>& typesize
						                                   ,int isextrapointer) 
{
   ...
   if (m_scope.DataMemberByName(name)) {
      // error duplicated variable name in scope
      G__fprinterr(G__serr,"Error: duplicate variable declaration '%s'",name.c_str());
      G__genericerror((char*)NULL);
   }

   ::Reflex::Type var_type = type.ReflexType();

   //FIXME: Does this need to be turned on?
   // set array size
   //var_type = setarraysize(type,var,arysize,typesize,isextrapointer);

   std::vector<int> arySizeVec(arysize.begin(),arysize.end());
   var_type = G__modify_type(var_type,isextrapointer,0,0,arySizeVec.size(),&(arySizeVec[0]));

   int var_statictype = G__AUTO;
   char *var_offset = 0;

   bool need_allocate_auto = false;
   ...
   if(type.Isstatic()) {
      ...
      var_offset = getstaticobject(name,m_iexist);
   }
   else {
      if(type.Isconst() && (type.Property()&G__BIT_ISFUNDAMENTAL)
         && 0==(type.Property()&G__BIT_ISPOINTER)) {
            var_statictype = G__LOCALSTATIC;
            var_offset = getstaticobject(name,m_iexist,1);
      } else {
         var_statictype = G__AUTO; 

         need_allocate_auto = true;
      }
   }
    ...
    if (need_allocate_auto) {
       ...
       int num = G__get_varlabel(var, 1) /* number of elements */;
       if (num == INT_MAX) {
          num = 0;
       } else if (!num) {
          num = 1;
       ...
       switch(num) {
          ...
          default: 
             if(type.Type()=='c') num=num+2; // allow ary[3]="abc";
             else                 num=num+1; 
             break; 
       }
       ...
       G__get_offset(var) = (char*)G__malloc(num,size,buf); // legacy
       free((void*)buf);

       if((type.Property()&(G__BIT_ISSTRUCT|G__BIT_ISCLASS)) &&
          (0==(type.Property()&(G__BIT_ISPOINTER|G__BIT_ISREFERENCE)))) {
             m_bc_inst.PUTAUTOOBJ(var);
       }
    }
    return(var);
}
...
////////////////////////////////////////////////////////////////////////////
char* G__blockscope::getstaticobject(const string& varname
  ...
  G__get_stack_varname(temp,varname.c_str(),func,G__get_tagnum(func.DeclaringScope()));
  Reflex::Member var = Reflex::Scope::GlobalScope().DataMemberByName(temp);
  if (var) 
     return G__get_offset(var);

  if(!noerror) {
    G__fprinterr(G__serr,"Error: No memory for static object %s ",temp.c_str());
...
//////////////////////////////////////////////////////////////////////////////
int G__blockscope::conversion(G__value& result,const Reflex::Member& var
                              ,int vartype,int paran) {
  if(0==baseconversion(result,var,vartype,paran)) {
    if(0==conversionopr(result,var,vartype,paran)) {
      return(0);
    }
  }
  return(1);
...
//////////////////////////////////////////////////////////////////////////////
int G__blockscope::baseconversion(G__value& result,const Reflex::Member& var
                                  ,int /*vartype*/,int paran) {
  if(var.TypeOf().RawType().IsClass() && G__value_typenum(result).RawType().IsClass()
     && (var.TypeOf().FinalType().IsReference()
         || var.TypeOf().FinalType().IsPointer() && G__value_typenum(result).FinalType().IsPointer())
     && var.TypeOf().RawType() != G__value_typenum(result).RawType()
     && -1!=G__ispublicbase(var.TypeOf(),G__value_typenum(result),(void*)0)) {
    // Look for base class conversion (offset calc) only
    if(paran) G__bc_REWINDSTACK(paran);
    m_bc_inst.CAST(var.TypeOf());
    if(paran) G__bc_REWINDSTACK(-paran);
    G__replace_rawtype(G__value_typenum(result), var.TypeOf().RawType());
    return(1);
  }
  return(0);
}
...
//////////////////////////////////////////////////////////////////////////////
int G__blockscope::conversionopr(G__value& result,const Reflex::Member& var
                                 ,int vartype,int paran) {
  if(G__value_typenum(result).RawType().IsClass()) {
    // look for result_type::operator[target_type]();
    G__value target;
    G__value_typenum(target) = var.TypeOf();
    G__value_typenum(target) = Reflex::Type(G__value_typenum(target), ~Reflex::CONST);
    ...
    if(m.IsValid()) {
      ...
      if((ty.Property()&G__BIT_ISCOMPILED)) 
        m_bc_inst.LD_FUNC_BC(func,0,(void*)m.InterfaceMethod());
      else 
        m_bc_inst.LD_FUNC_BC(func,0,(void*)G__bc_exec_normal_bytecode);
      m_bc_inst.POPSTROS();
      if(paran) G__bc_REWINDSTACK(-paran);
...
G__value G__blockscope::compile_newopr(const string& expression) {
  ...
  if((ty.Property()&(G__BIT_ISCLASS|G__BIT_ISSTRUCT)) &&
    ...
    if(m.IsValid()) { // always true ???
      ...
      Reflex::Member func = G__Dict::GetDict().GetFunction((size_t) m.Handle());
      m_bc_inst.LD_FUNC_BC(func,para.paran,(void*)m.InterfaceMethod());
    }
    else {
  ...
  // Interpreted class 
    ...
    if(m.IsValid()) {
      ...
      if(isaryindex)
        ...
        m_bc_inst.LD_FUNC_BC(func,para.paran ,(void*)G__bc_exec_ctor_bytecode);
      //m_bc_inst.LD_FUNC_BC(func,para.paran ,(void*)G__bc_exec_normal_bytecode);
    }
    else {
#ifdef G__NEVER
...
void G__blockscope::compile_deleteopr(string& expression,int isarray) {
  ...
  // Interpreted class 
    ...
    if(m.IsValid()) {
      ...
      if(isarray) {
        m_bc_inst.LD_FUNC_BC(func,0,(void*)G__bc_exec_dtorary_bytecode);
      }
      else if(m.Property()&G__BIT_ISVIRTUAL) {
        m_bc_inst.LD_FUNC_VIRTUAL(func,0,(void*)G__bc_exec_virtual_bytecode);
...
int G__blockscope::access(/* const */ G__MethodInfo& x) const {
   if(access(G__Dict::GetDict().GetScope(x.MemberOf()->Tagnum()),x.Property())) return(1);
  return(0);
}
...
////////////////////////////////////////////////////////////////
int G__blockscope::access(const Reflex::Scope& tagnum,long property) const {

  // member is public
  if(property&G__BIT_ISPUBLIC) return(1);

  // member is protected and this is public inheritance
  if((property&G__BIT_ISPROTECTED) && !tagnum.IsTopScope() && !m_ifunc.DeclaringScope().IsTopScope()
     && -1!=G__ispublicbase((::Reflex::Type)tagnum,(::Reflex::Type)m_ifunc.DeclaringScope(),(void*)G__STATICRESOLUTION)) {
    return(1);
  }
...
////////////////////////////////////////////////////////////////
int G__blockscope::isfriend(const Reflex::Scope& tagnum) const {
  Reflex::Scope m_tagnum=m_ifunc.DeclaringScope();

  // exactly same class
  if(m_tagnum==tagnum) return(1);

  struct G__friendtag *friendtag;
  if(m_tagnum.IsTopScope()) return 0;
  friendtag = G__struct.friendtag[G__get_tagnum(m_tagnum)];
  while(friendtag) {
    if(friendtag->tagnum==G__get_tagnum(tagnum)) return(1);
    friendtag=friendtag->next;
  }
  if(m_iexist) {
    friendtag = G__get_funcproperties(m_iexist)->entry.friendtag;
    while(friendtag) {
      if(friendtag->tagnum==G__get_tagnum(tagnum)) return(1);
      friendtag=friendtag->next;
    }
...
////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////
int G__Isconversionctor(G__TypeReader& ltype
                                   ,G__TypeReader& rtype) {
...
////////////////////////////////////////////////////////////////
int G__Isassignmentopr(G__TypeReader& ltype
                                  ,G__TypeReader& rtype) {
  if(ltype.Property()&(G__BIT_ISSTRUCT|G__BIT_ISCLASS)) {
    long dmy;
    // todo, it is questionable whether I should use 
...
////////////////////////////////////////////////////////////////
int G__Isconversionopr(G__TypeReader& ltype
                                  ,G__TypeReader& rtype) {
...
////////////////////////////////////////////////////////////////
int G__bc_baseconversion(G__value& /*result*/,const Reflex::Member& /*var*/
                         ,int /*vartype*/,int /*paran*/) {
  return(0);
}
////////////////////////////////////////////////////////////////
int G__Isvalidassignment(G__TypeReader& ltype
  ...
  if(ltype.Ispointer()) {
    ...
    if(ltype.Type()==rtype.Type() && ltype.Ispointer()==rtype.Ispointer()) {
      if('U'==ltype.Type()) {
        ...
        if(-1!=G__ispublicbase(ltype.Tagnum(),rtype.Tagnum(),(void*)0)) {
          return(1); // fine
        }
        else 
          return(0); // error
...
////////////////////////////////////////////////////////////////
int G__Isvalidassignment_val(G__value *ltype,int varparan
...
////////////////////////////////////////////////////////////////
int G__bc_conversion(G__value *result
                                ,const Reflex::Member& var
                                ,int var_type,int paran) {
  return(G__currentscope->conversion(*result,var,var_type,paran));
}
