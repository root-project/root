...
#endif
extern const std::string G__endmark;
...
class G__fstream {
 ...
 public:
  ...
  int fgetc() { return(::Cint::Internal::G__fgetc()); } // legacy
  void putback(int goback= -1) { fseek(::Cint::Internal::G__ifile.fp,goback,SEEK_CUR); }
...
class G__virtualreader {
 public:
  ...
  virtual int fgettoken(std::string& token,const std::string& endmark=G__endmark) =0;
  virtual int fskipcomment(int c='/') =0;
  virtual int fpp_command(int c=0) =0;
  virtual int fgetquotation(std::string& token,int c,int openquotation=0) =0;

  virtual int fgetspace() =0;

  virtual int fappendtoken(std::string& token,int c,const std::string& endmark=G__endmark)=0;

  virtual int fgetstream(std::string& phrase,const std::string& endmark,int nest=0)=0;
  virtual int fgetstream_template(std::string& phrase,const std::string& endmark,int nest=0) =0;

  virtual int fignorestream(const std::string& endmark,int nest=0) =0;
  virtual int fignorestream_template(const std::string& endmark,int nest=0) =0;

  virtual int fgetstream_(std::string& phrase,const std::string& endmark,int nest=0)=0;
  virtual int fgetstream_template_(std::string& phrase,const std::string& endmark,int nest=0) =0;

  virtual int fignorestream_(const std::string& endmark,int nest=0) =0;
  virtual int fignorestream_template_(const std::string& endmark,int nest=0) =0;
...
class G__srcreader : public G__virtualreader {
 ...
 public:
  ...
  int fgettoken(std::string& token,const std::string& endmark=G__endmark);
  int fskipcomment(int c='/');
  int fpp_command(int c=0);
  int fgetquotation(std::string& token,int c,int openquotation=0);

  int fgetspace() ;

  int fappendtoken(std::string& token,int c,const std::string& endmark=G__endmark) ;

  int fgetstream(std::string& phrase,const std::string& endmark,int nest=0)
   {return(fgetstream_core(phrase,endmark,nest,0));}
  int fgetstream_template(std::string& phrase,const std::string& endmark,int nest=0)
   {return(fgetstream_core(phrase,endmark,nest,1));}

  int fignorestream(const std::string& endmark,int nest=0)
   {std::string phrase; return(fgetstream_core(phrase,endmark,nest,0));}
  int fignorestream_template(const std::string& endmark,int nest=0)
   {std::string phrase; return(fgetstream_core(phrase,endmark,nest,1));}

  int fgetstream_(std::string& phrase,const std::string& endmark,int nest=0)
   {return(fgetstream_core(phrase,endmark,nest,0,1));}
  int fgetstream_template_(std::string& phrase,const std::string& endmark,int nest=0)
   {return(fgetstream_core(phrase,endmark,nest,1,1));}

  int fignorestream_(const std::string& endmark,int nest=0)
   {std::string phrase; return(fgetstream_core(phrase,endmark,nest,0,1));}
  int fignorestream_template_(const std::string& endmark,int nest=0)
   {std::string phrase; return(fgetstream_core(phrase,endmark,nest,1,1));}

  void fignoreline() { Internal::G__fignoreline(); }          // legacy
  int skipCcomment() { return(Internal::G__skip_comment()); } // legacy
 ...
 private:
  int fgetstream_core(std::string& phrase,const std::string& endmark
...
//////////////////////////////////////////////////////////////////////
template<class T>
int G__srcreader<T>::fgettoken(std::string& token,const std::string& endmark) {
...
#define G__OLDIMPLEMENTATION2064
 l_begin:
  ...
  for(;) {
    ...
    else if(std::string::npos!=endmark.find(c)) break;
    if(c==EOF) {
      G__genericerror("Error: Unexpected end of file (2)");
      break;
    }
    if(c) token.append((std::string::size_type)1,(char)c);
    else  return(c);
    c = fgetc_gettoken();
  ...
  if(isspace(c)) {
    ...
    if(c==0) return(c);
    if(std::string::npos==endmark.find(c)) {
      c=' ';
      putback();
    }
  }
...
int G__srcreader<T>::fgetc_separator() {
  ...
  if(isspace(c)) {
    while(isspace(c)) c=fgetc_gettoken();
    if(c==0) return(c);
    if(std::string::npos==G__endmark.find(c)) {
      c=' ';
      putback();
    }
  }
...
int G__srcreader<T>::fpp_command(int c) {
  char buf[G__ONELINE];
  c=Internal::G__fgetname(buf,"\n\r"); // legacy
  std::string condition=buf;
  if(condition.size() && isdigit(condition.c_str()[0])) {
    if('\n'!=c && '\r'!=c) fignoreline();
    Internal::G__ifile.line_number=atoi(condition.c_str());
  }
  else if(condition=="else"||
          condition=="elif")   Internal::G__pp_skip(1);  // legacy
  else if(condition=="if")     Internal::G__pp_if();     // legacy
  else if(condition=="ifdef")  Internal::G__pp_ifdef(1); // legacy
  else if(condition=="ifndef") Internal::G__pp_ifdef(0); // legacy
  else if('\n'!=c && '\r'!=c)  fignoreline();
  c=' ';
  return(c);
}
...
//////////////////////////////////////////////////////////////////////
template<class T>
int G__srcreader<T>::fgetquotation(std::string& token,int c,int /* openquotation */) {
  ...
  for(;) {
    ...
    if(c==quote) break;
    if(c=='\\') {
      token.append((std::string::size_type)1,(char)c);
      c = fgetc();
    }
...
//////////////////////////////////////////////////////////////////////
template<class T>
int G__srcreader<T>::fappendtoken(std::string& token,int c,const std::string& endmark) {
  std::string addition;
  if(c) token.append((std::string::size_type)1,(char)c);
...
//////////////////////////////////////////////////////////////////////
template<class T>
int G__srcreader<T>::fgetstream_core(std::string& phrase,const std::string& endmark
  ...
  for(;) {
    c = fgettoken(token);
    phrase.append(token);
    if( nest<=0 && std::string::npos!=endmark.find(c)) goto l_end;
    switch(c) {
    case '<':
      if(!istemplate) break;
    case '[':
    ...
    case '>':
      ...
      if(phrase.size() && phrase[phrase.size()-1]=='>')
        phrase.append((std::string::size_type)1,' ');
    case ']':
    case '}':
    case ')':
      --nest;
#define G__OLDIMPLEMENTATION2240
    ...
    phrase.append((std::string::size_type)1,(char)c);
  }
 l_end:
  return(c);
}
