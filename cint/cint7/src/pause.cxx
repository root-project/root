...
************************************************************************/
extern "C" void G__SetCriticalSectionEnv(int (*issamethread)()
...
static void G__debugvariable(FILE *fp,::Reflex::Scope var,char *name) 
{
   for (
      ...
      if (*iter && (iter->Name() == name)) {
         ...
         fprintf(
            fp,
            "%s p=%p type=%s const=%x static=%d\n paran=%d ",
            iter->Name().c_str(),
            prop->addressOffset,
            iter->TypeOf().Name(::Reflex::SCOPED).c_str(),
            iter->IsConst(),
            iter->IsStatic(),
            G__get_paran(*iter)
            );
...
* intG__process_cmd()
******************************************************************/
extern "C" int G__process_cmd(char *line,char *prompt,int *more,int *err
                   ,G__value *rslt)
{
...
/* #endif */
    ...
    multi_line_command:
      ...
      if(!ftemp.fp) {
        ...
        if(!G__func_now) G__p_local=::Reflex::Scope();
        ...
        G__command_eval=0 ;
        G__free_tempobject();
        if(!G__func_now) G__p_local=store.var_local;
        if(G__security_error) G__cancel_undo_position();
#ifdef G__SECURITY
        *err |= G__security_recover(G__serr);
#endif
