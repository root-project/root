...
std::string rflx_tools::escape_class_name(const std::string & name) {
  ...
  for (size_t i = 0; i < name.length(); ++i) {
    if (rpl_chars.find(lname[i]) != std::string::npos) lname[i] = '_';
  }
...
std::string rflx_tools::decorate_stl_type(const std::string & name) {
  ...
  */
  ...
  if (( nam == "allocator<")) isSTLType = true;
  
  nam = name.substr(0,9);
  if (( nam == "multimap<") ||
      ( nam == "multiset<")) isSTLType = true;
  
  nam = name.substr(0,7);
  if (( nam == "vector<" ) ||
      ( nam == "bitset<" )) isSTLType = true;

  nam = name.substr(0,6);
  if (( nam == "queue<" ) ||
      ( nam == "deque<" ) ||
      ( nam == "stack<" )) isSTLType = true;

  nam = name.substr(0,5);
  if (( nam == "list<" )) isSTLType = true;

  nam = name.substr(0,4);
  if (( nam == "map<" ) ||
      ( nam == "set<" )) isSTLType = true;

  if ( isSTLType ) { return "std::" + name;
...
std::string rflx_tools::un_const(const std::string & name) {
  if (name.substr(0,6) == "const ") return name.substr(6);
  return name;
}
