...
case 'v': /* int var[10]; *var=expr; assign to value */                   \
        if(var->paran[ig15]==paran+1&&linear_index==0  /* 1070 */                \
...
case 'p': /* var = expr; assign to value */                               \
    ...
    /*1068*/    result.ref=G__struct_offset+var->p[ig15]+linear_index*SIZE;      \
                *(CASTTYPE *)(G__struct_offset+var->p[ig15]+linear_index*SIZE)   \
            ...
            /* if(var->p[ig15]!=0) free((void*)var->p[ig15]);*/ \
                if(var->p[ig15]!=0 &&   /* ON457 */             \
...
if(islower(G__var_type)) { /* type var; normal variable */       \
 ...
 /*1454*/   ||G__static_alloc||var->statictype[ig15]==G__LOCALSTATIC)&&     \
    ...
    if(num_elements >0 && result.type!='\0') { /* char *argv[]; */       \
...
case 'v': /* *var; get value */                                               \
  ...
  case G__PARAP2P:                                                            \
    if(var->paran[ig15]<paran) {                                              \
      address = G__struct_offset+var->p[ig15]+linear_index*G__LONGALLOC;             \
      /*result.ref=(*(((long*)(*(long *)address))+secondary_linear_index)); 1757*/            \
      result.ref=(*(long*)(((CASTTYPE*)(*(long *)address))+secondary_linear_index));/*1757*/         \
      if(result.ref)                                                          \
        CONVFUNC(&result,TYPE,(CONVTYPE)(*(CASTTYPE *)(result.ref)));         \
    }                                                                         \
    else {                                                                    \
      result.ref=                                                             \
               (*(long *)(G__struct_offset+var->p[ig15]+linear_index*G__LONGALLOC)); \
      G__letint(&result,PTYPE,                                                \
      *(long *)(*(long *)(G__struct_offset+var->p[ig15]+linear_index*G__LONGALLOC)));\
    }                                                                         \
   break;                                                                     \
  }                                                                           \
  break;                                                                      \
case 'P': /* &var; get pointer to pointer */                                  \
   if(var->paran[ig15]==paran) { /* must be PPTYPE */                         \
          G__letint(&result,PTYPE                                             \
             ,(G__struct_offset+var->p[ig15]+linear_index*G__LONGALLOC));            \
   }                                                                          \
   else if(var->paran[ig15]<paran) {                                          \
        address = G__struct_offset + var->p[ig15]+linear_index*G__LONGALLOC;         \
        if(G__PARANORMAL==var->reftype[ig15])                                 \
          G__letint(&result,PTYPE                                             \
              ,(long)((CASTTYPE*)(*(long *)(address))+secondary_linear_index));               \
        else {                                                                \
          G__letint(&result,PTYPE                                             \
              ,(long)((long*)(*(long *)(address))+secondary_linear_index));                   \
...
case 'P':                                                                     \
   if(var->paran[ig15]==paran) {                                              \
        ...
        if(G__PARANORMAL==var->reftype[ig15])                                 \
          G__letint(&result,PTYPE                                             \
                      ,(*(long *)(address))+secondary_linear_index*SIZE);                     \
        else {                                                                \
          G__letint(&result,PTYPE                                             \
              ,(long)((long*)(*(long *)(address))+secondary_linear_index));                   \
          result.obj.reftype.reftype=G__PARAP2P;                              \
...
default : /* 'p' */                                                           \
   if(var->paran[ig15]==paran) {                                              \
        ...
        address = G__struct_offset + var->p[ig15]+linear_index*G__LONGALLOC;         \
        if(G__PARANORMAL==var->reftype[ig15]) {                               \
          result.ref=((*(long *)(address))+secondary_linear_index*SIZE);                      \
          CONVFUNC(&result,TYPE,result.ref);                                  \
        }                                                                     \
        else if(var->paran[ig15]==paran-1) {                                  \
          result.ref=(long)((long*)(*(long *)(address))+secondary_linear_index);              \
...
G__value G__letvariable(char* item, G__value expression, G__var_array* varglobal, G__var_array* varlocal)
{
  ...
  int ig15,paran,ig35,ig25;
  int lenitem,nest=0;
  int single_quote=0,double_quote=0,paren=0,flag=0;
  int done=0;
  int linear_index;
  int secondary_linear_index;
  int ig2;
  int store_var_type;
  ...
  // Assign value.
  if (var) {
    if (G__cintv6 && G__asm_noverflow && !G__asm_exec) {
    ...
    // Bytecode generation.
    G__ASSERT(0 == G__decl || 1 == G__decl);
    if (G__asm_noverflow && 0 == G__decl_obj) {
      if ( 
...
exec_asm_letvar:
    ...
    if (
      ...
      if (
        ((0 == G__prerun && !G__decl) || G__COMPILEDGLOBAL == var->statictype[ig15]) && 
        (
    ...
    if (-1 != var->p_typetable[ig15] && G__newtype.isconst[var->p_typetable[ig15]]) {
    ...
    switch (var->type[ig15]) {
      ...
      case 'U':
        ...
        if (
          ig25 < paran &&
          ((G__PARANORMAL == var->reftype[ig15]) || paran-ig25==var->reftype[ig15])
        ) {
          result.tagnum = var->p_tagtable[ig15];
          result.typenum = var->p_typetable[ig15];
          result.ref = 0;
  ...
  // for the old variable name.  The duplicate declaration is
  // just ignored and the initializer value is stored into the
  // old variable.
  //
  if (!done) {
  ...
  switch(G__var_type) {
    case 'v':
      switch(var->reftype[ig15]) {
        ...
        case G__PARANORMAL:
          if(var->paran[ig15]>paran) {
            if (var->varlabel[ig15][1] /* num of elements */ == INT_MAX /* unspecified length flag */) {
...
G__value G__getvariable(char* item, int* known2, G__var_array* varglobal, G__var_array* varlocal)
{
  ...
  G__tagnum = G__memberfunc_tagnum;
  G__store_struct_offset = G__memberfunc_struct_offset;
  /* evaluate parameters (array index expressions) */
  ...
  /*************************************************
   * Get value if variable name matches
   *
  ...
  if (var) {
    ...
    if ((var->p[ig15] == 0) && (G__struct_offset == 0) && !G__no_exec_compile) {
      *known2 = 1;
      result = G__null;
      result.tagnum = var->p_tagtable[ig15];
      result.typenum = var->p_typetable[ig15];
    ...
    if (G__asm_noverflow) {
      ...
      if (G__struct_offset) {
        ...
        if (G__asm_dbg) {
          if (
            G__asm_wholefunction &&
            (G__ASM_VARLOCAL == store_struct_offset) &&
            (G__LOCALSTATIC != var->statictype[ig15])
          ) {
            G__fprinterr(G__serr, "%3x: LD_LVAR  %s index=%d paran=%d\n", G__asm_cp, item, ig15, paran);
          }
          else 
...
G__exec_asm_getvar:
    ...
    *known2 = 1;
    ...
    if (
      ...
      (std::tolower(var->type[ig15]) == 'c')
    ) {
      int typenumx = var->p_typetable[ig15];
      char store_var_type = G__var_type;
      int sizex = G__Lsizeof(G__newtype.name[typenumx]);
      G__var_type = store_var_type;
      // This is still questionable, but should be better than the old implementation.
      if (var->paran[ig15] > paran) {
        linear_index /= var->varlabel[ig15][0] /* stride */;
      }
      switch (var->type[ig15]) {
        case 'c':
          // char
    ...
    switch (var->type[ig15]) {
      ...
      case 'U': /* struct,uniont pointer */
        G__GET_STRUCTPVAR1(G__struct.size[var->p_tagtable[ig15]], G__letint, 'u', 'U')
        G__GET_STRUCTPVAR2(G__struct.size[var->p_tagtable[ig15]], G__letint, 'u', 'U')
        if (ig25 < paran) {
          G__tryindexopr(&result, para, paran, ig25);
        }
        break;
  ...
  if (!done) {
    // -- Undefined variable.
    //
    // if variable name not found, then search for 'this' keywords
    ...
    if (isupper(var->type[ig15]) && 'P' != var->type[ig15] && 'O' != var->type[ig15]) {
      G__getpointer2pointer(&result, var, ig15, paran);
    }
...
void G__letstructp(G__value result, long G__struct_offset, int ig15, int linear_index, G__var_array* var, int paran, char* item, G__value* para, int secondary_linear_index)
{
  ...
  if (
    ...
    // Trick  f(A** x) { *x = 0; } as f(A** x) { x[0] = 0; }
    G__var_type = 'p';
    paran = 1;
  }
  switch (G__var_type) {
...
#ifdef G__VIRTUALBASE
              -1 != (baseoffset = G__ispublicbase(var->p_tagtable[ig15], result.tagnum, result.obj.i))
#else
              -1 != (baseoffset = G__ispublicbase(var->p_tagtable[ig15], result.tagnum))
#endif
          ) {
 ...
 ******************************************************************/
G__value G__allocvariable(G__value result, G__value para[], G__var_array* varglobal, G__var_array* varlocal, int paran, int varhash, char* item, char* varname, int parameter00)
{
...
#define G__OLDIMPLEMENTATION1213
  ...
  if (!G__definemacro && (G__globalcomp == G__NOLINK) && (G__var_type == 'p') && G__automaticvar) {
    if (result.tagnum == -1) {
  ...
  if (G__bitfield) {
    ...
    if(0==ig15 || 0==var->bitfield[ig15-1]) {
      ...
      bitlocation=var->varlabel[ig15-1][G__MAXVARDIM-1] +var->bitfield[ig15-1];
  ...
  if (var->allvar < G__MEMDEPTH) {
    // No overflow just assign index.
    ig15 = var->allvar;
  }
  else {
    // Overflow,  allocate another vararray.
  ...
  // Class of the variable: auto, file scope static, function scope static.
  var->statictype[var->allvar] = G__AUTO;
  if (G__decl_obj == 2) {
    // This is set in decl.c(G__initstructary).
    var->statictype[var->allvar] = G__AUTOARYDISCRETEOBJ;
  }
  // If namespace, set G__static_alloc, it should be reset in G__define_var().
  if (
    G__def_struct_member &&
    (G__tagdefining != -1) &&
    (G__struct.type[G__tagdefining] == 'n') &&
    (std::tolower(G__var_type) != 'p') // macro is global
  ) {
  ...
  if (G__static_alloc) {
    if (G__p_local) {
      ...
      if (G__func_now != -1) {
        ...
        G__hash(ttt, varhash, ig25) // FIXME: Need ';' at end of line!
   ...
   * myarray v[A][B][C][D]
   *
   *   var->varlabel[var_identity][0] = undefined;
   *   var->varlabel[var_identity][1] = undefined or a;
   *   var->varlabel[var_identity][2] = undefined or b;
   *   var->varlabel[var_identity][3] = undefined or c;
   *   var->varlabel[var_identity][4] = undefined or A;
   *   var->varlabel[var_identity][5] = undefined or B;
   *   var->varlabel[var_identity][6] = undefined or C;
   *   var->varlabel[var_identity][7] = undefined or D;
   *   var->varlabel[var_identity][8] = undefined;
   *   var->varlabel[var_identity][9] = undefined;
   ***************************************************/
  if (G__typedefnindex) {
    parameter00 = '1';
  }
   ...
   *
   *   var->varlabel[var_identity][0] = undefined;
   *   var->varlabel[var_identity][1] = 0 or a;
   *   var->varlabel[var_identity][2] = 0 or b;
   *   var->varlabel[var_identity][3] = 0 or c;
   *   var->varlabel[var_identity][4] = 0 or A;
   *   var->varlabel[var_identity][5] = 0 or B;
   *   var->varlabel[var_identity][6] = 0 or C;
   *   var->varlabel[var_identity][7] = 0 or D;
   *   var->varlabel[var_identity][8] = 0;
   *   var->varlabel[var_identity][9] = 0;
   ***************************************************/
  while (ig25 < (G__MAXVARDIM - 1)) {
    var->varlabel[var->allvar][++ig25] = 0;
  }
  /**************************************************
   *
   * typedef myarray int[a][b][c];
   * myarray v[A][B][C][D]
   *
   *   var->varlabel[var_identity][0] = 1 or b*c*A*B*C*D; // stride for unspecified final index
   *   var->varlabel[var_identity][1] = 0 or a;
   *   var->varlabel[var_identity][2] = 0 or b;
   *   var->varlabel[var_identity][3] = 0 or c;
   *   var->varlabel[var_identity][4] = 0 or A;
  ...
  {
    int tmp = 1;
    while (var->varlabel[ig15][ig25]) {
      tmp *= var->varlabel[ig15][ig25++];
    }
    var->varlabel[ig15][0] = tmp;
  }
  /************************************************************
   *
   * typedef myarray int[a][b][c];
   ...
   *
   *   var->varlabel[var_identity][0] = 1 or b*c*A*B*C*D; // stride for unspecified final index
   *   var->varlabel[var_identity][1] = 0 or a*b*c*A*B*C*D; // number of elements
   *   var->varlabel[var_identity][2] = 0 or b;
   *   var->varlabel[var_identity][3] = 0 or c;
   *   var->varlabel[var_identity][4] = 0 or A;
   *   var->varlabel[var_identity][5] = 0 or B;
   ...
   *   var->varlabel[var_identity][9] = 0;
   ************************************************************/
  int num_elements = var->varlabel[ig15][1] * var->varlabel[ig15][0];
  ...
  if (paran > 0) {
    if (paran >= G__MAXVARDIM) {
      G__fprinterr(G__serr, "Limitation: Cint can handle only upto %d dimention array", G__MAXVARDIM - 1);
      G__genericerror(0);
      return result;
    }
    /************************************************************
     ...
     ***********************************************************/
    if (G__funcheader && (result.type != '\0')) {
      ...
      if (
        ...
        if (G__globalvarpointer == G__PVOID) {
          if (result.type != '\0') {
            ...
            if (!G__static_alloc || (G__prerun == 1)) {
              G__globalvarpointer = 0;
            }
          }
        }
        var->varlabel[ig15][1] /* number of elements */ = INT_MAX /* unspecified length flag */;
  ...
  // so that this memory area won't be freed during destruction.
  //
  ...
  //
  // Enter variable information into the var array.
  //
  G__savestring(&var->varnamebuf[var->allvar], varname);
  var->hash[var->allvar] = varhash;
  ig15 = var->allvar;
  var->type[ig15] = G__var_type;
  // if var->p_tagtable[ig15] == -1, it is not a struct, union
  ...
  // may need refinement
  if (G__asm_noverflow && G__asm_wholefunction && !num_elements) {
    if (G__funcheader) {
      // function argument
      if ((G__reftype == G__PARAREFERENCE)) {
#ifdef G__ASM_DBG
        if (G__asm_dbg) G__fprinterr(G__serr, "%3x: INIT_REF %s:%d\n", G__asm_cp, __FILE__, __LINE__);
#endif
...
#ifdef G__ASM_DBG
      if (G__asm_dbg) G__fprinterr(G__serr, "%3x: POP %s:%d\n", G__asm_cp, __FILE__, __LINE__);
#endif
      ...
      if (1 && G__CPPLINK == G__struct.iscpplink[G__tagnum]) {
        // precompiled class
  ...
  G__CHECK(G__SECURE_POINTER_TYPE, isupper(G__var_type) && result.obj.i && G__var_type != result.type
      && !G__funcheader && (('Y' != G__var_type && result.obj.i) || G__security & G__SECURE_CAST2P), return(result));
  ...
  switch (G__var_type) {
    ...
    case 'C':
      ...
      if (std::islower(G__var_type)) {
        // -- Not a pointer.
        var->p[ig15] = G__malloc(num_elements ? num_elements : 1, G__CHARALLOC, item);
        if (
          (
            (!G__def_struct_member && (G__asm_wholefunction == G__ASM_FUNC_NOP)) ||
            G__static_alloc ||
        ...
        // -- Pointer.
        if ((num_elements > 0) && (result.type != '\0')) {
          // -- We are either FILE* v[] or char* v[];
          var->p[ig15] = G__int(result);
        }
        else {
          // pointer
      ...
      if ((result.type == 'C') && (G__var_type == 'c')) {
        if (G__asm_wholefunction != G__ASM_FUNC_COMPILE) {
          if (((int) strlen((char*) result.obj.i)) > ((int) var->varlabel[ig15][1] /* number of elements */)) {
            std::strncpy((char*) var->p[ig15], (char*) result.obj.i, (size_t) var->varlabel[ig15][1] /* number of elements */);
          }
          else {
            std::strcpy((char*) var->p[ig15], (char*) result.obj.i);
          }
        }
        else {
#ifdef G__ASM_DBG
    ...
    case 'I':
      ...
      if (islower(G__var_type)) {
        // type var; normal variable
        var->p[ig15] = G__malloc(num_elements ? num_elements : 1, G__INTALLOC, item);
        if (
          (
            (!G__def_struct_member && (G__asm_wholefunction == G__ASM_FUNC_NOP)) ||
            G__static_alloc ||
          ...
          *((int*) var->p[ig15]) = (int) G__int(result);
        }
      }
      else {
        // type* var; pointer
  ...
  // Check for unassigned internal pointer.
  ...
  if (
    ...
    G__prerun && 
    (G__access == G__PUBLIC) &&
    !std::strcmp(varname, "G__virtualinfo")
  ) {
