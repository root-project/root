...
// Test tryGetScope, which is esentially is a DeclContext.
...
const clang::Decl* cl_A = lookup->tryGetScope("A");
...
const clang::Decl* cl_A_in_N = lookup->tryGetScope("N::A");
...
const clang::Decl* cl_A_in_NMP = lookup->tryGetScope("N::M::P::A");
cl_A_in_NMP
...
const clang::Decl* cl_B_int = lookup->tryGetScope("B<int>");
...
lookup->tryGetScope("W<Int_t>", &resType);
//resType->dump(); 
