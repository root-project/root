...
function(cppyy_add_bindings pkg pkg_version author author_email)
    ...
    if("${ARG_LICENSE_FILE}" STREQUAL "")
        set(ARG_LICENSE_FILE ${ROOT_SOURCE_DIR}/LICENSE)
    endif()
    set(LICENSE_FILE ${ARG_LICENSE_FILE})

    # ReadMe file
    if("${ARG_README_FILE}" STREQUAL "")
        set(ARG_README_FILE ${ROOT_SOURCE_DIR}/README.rst)
    endif()
  ...
  ####################################################
    ...
    #
    file(COPY ${ROOT_SOURCE_DIR}/py/ DESTINATION ${pkg_dir}
    ...
    file(GLOB_RECURSE PY_PKG_FILES
         ...
         CONFIGURE_DEPENDS
         "${ROOT_SOURCE_DIR}/py/*.py")
    string(TOLOWER ${CMAKE_SYSTEM_NAME} SYSTEM_STR)
    set(pkg_whl "${ROOT_BINARY_DIR}/dist/${pkg}-${pkg_version}-py3-none-${SYSTEM_STR}_${CMAKE_SYSTEM_PROCESSOR}.whl")
