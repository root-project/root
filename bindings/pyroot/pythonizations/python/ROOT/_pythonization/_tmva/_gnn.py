...
def getActivationFunction(model):
    ...
    if function == 'relu':
        return ROOT.TMVA.Experimental.SOFIE.Activation.RELU
    else:
        return ROOT.TMVA.Experimental.SOFIE.Activation.Invalid
...
def make_mlp_model(gin, model, function_target, type):
    ...
    upd = ROOT.TMVA.Experimental.SOFIE.RFunction_MLP(function_target, num_layers, activation, model._activate_final, type)
...
def make_linear_model(gin, model, function_target, type):
    ...
    activation = ROOT.TMVA.Experimental.SOFIE.Activation.Invalid
    upd = ROOT.TMVA.Experimental.SOFIE.RFunction_MLP(function_target, 1, activation, False, type)
    kernel_tensor_names = ROOT.std.vector['std::string'](1)
    bias_tensor_names   = ROOT.std.vector['std::string'](1)
...
class RModel_GNN:
    ...
    def ParseFromMemory(graph_module, graph_data, filename = "gnn_network"):
        ...
        add_update_function(gin, node_model, ROOT.TMVA.Experimental.SOFIE.GraphType.GNN,
        ...
        add_update_function(gin, edge_model, ROOT.TMVA.Experimental.SOFIE.GraphType.GNN,
                                             ROOT.TMVA.Experimental.SOFIE.FunctionTarget.EDGES)

        # adding the global update function
        global_model = graph_module._global_block._global_model
        add_update_function(gin, global_model, ROOT.TMVA.Experimental.SOFIE.GraphType.GNN,
                                             ROOT.TMVA.Experimental.SOFIE.FunctionTarget.GLOBALS)

        # adding edge-node aggregate function
        add_aggregate_function(gin, graph_module._node_block._received_edges_aggregator._reducer.__qualname__, ROOT.TMVA.Experimental.SOFIE.FunctionRelation.NODES_EDGES)

        # adding node-global aggregate function
        add_aggregate_function(gin, graph_module._global_block._nodes_aggregator._reducer.__qualname__, ROOT.TMVA.Experimental.SOFIE.FunctionRelation.NODES_GLOBALS)

        # adding edge-global aggregate function
        add_aggregate_function(gin, graph_module._global_block._edges_aggregator._reducer.__qualname__, ROOT.TMVA.Experimental.SOFIE.FunctionRelation.EDGES_GLOBALS)
...
class RModel_GraphIndependent:
    ...
    def ParseFromMemory(graph_module, graph_data, filename = "graph_independent_network"):
        ...
        gin = ROOT.TMVA.Experimental.SOFIE.GraphIndependent_Init()
        gin.num_nodes = len(graph_data['nodes'])
        ...
        if (hasattr(node_model,"_model")) :
            add_update_function(gin, node_model._model, ROOT.TMVA.Experimental.SOFIE.GraphType.GraphIndependent,
                                         ROOT.TMVA.Experimental.SOFIE.FunctionTarget.NODES)

        # adding the edge update function
        edge_model = graph_module._edge_model
        if (hasattr(edge_model,"_model")) :
            add_update_function(gin, edge_model._model, ROOT.TMVA.Experimental.SOFIE.GraphType.GraphIndependent,
                                             ROOT.TMVA.Experimental.SOFIE.FunctionTarget.EDGES)

        # adding the global update function
        global_model = graph_module._global_model
        if (hasattr(global_model,"_model")) :
            add_update_function(gin, global_model._model, ROOT.TMVA.Experimental.SOFIE.GraphType.GraphIndependent,
                                             ROOT.TMVA.Experimental.SOFIE.FunctionTarget.GLOBALS)

        graph_independent_model = ROOT.TMVA.Experimental.SOFIE.RModel_GraphIndependent(gin)
        blas_routines = ROOT.std.vector['std::string']()
        blas_routines.push_back("Gemm")
