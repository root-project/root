...
from JupyROOT.kernel.utils import GetIOHandler, GetExecutor, GetDeclarer
...
class CppCompleter(object):
    ...
    def __init__(self):
        ...
        self.accessors = [".", "->", "::"]
        self.Declarer = GetDeclarer()
        self.Executor = GetExecutor()

    def activate(self):
        self.active = True
        if self.firstActivation:
            self.Declarer('#include "dlfcn.h"')
            dlOpenRint = 'dlopen("libRint.so",RTLD_NOW);'
            self.Executor(dlOpenRint)
            self.Declarer(_TTabComHookCode)
