...
static PyObject* ep_new(PyTypeObject* subtype, PyObject* args, PyObject* kwds) {
  ...
  PyObject* pyobj =
      ((PyTypeObject*)PyExc_Exception)->tp_new(subtype, nullptr, nullptr);
  if (!pyobj)
    return nullptr;
  ...
  if (args) {
    PyObject* ulc =
        PyObject_GetAttr((PyObject*)subtype, PyStrings::gUnderlying);
    excobj->fCppInstance = PyType_Type.tp_call(ulc, args, kwds);
    if (!excobj->fCppInstance) {
      // if this fails, then the construction may have been attempted from a
      // string (e.g. from PyErr_Format); if so, drop the proxy and use
      // fTopMessage instead
      PyErr_Clear();
...
static PyObject* ep_str(CPPExcInstance* self) {
  if (self->fCppInstance) {
    PyObject* what =
        PyObject_CallMethod((PyObject*)self, (char*)"what", nullptr);
    if (what) {
      if (self->fTopMessage) {
...
static int ep_setattro(CPPExcInstance* self, PyObject* attr, PyObject* value) {
  ...
  return ((PyTypeObject*)PyExc_Exception)
      ->tp_setattro((PyObject*)self, attr, value);
}
...
PyTypeObject CPPExcInstance_Type = {
    PyVarObject_HEAD_INIT(&PyType_Type,
                          0)(char*) "cppjit.CPPExcInstance", // tp_name
