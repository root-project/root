...
static inline cpyrt::CPPInstance* ConvertImplicit(interop::TCppScope_t klass,
  ...
  // std::__wrap_iter<const char*>), which uses a converting constructor.
  {
    ...
    if (clName.compare(0, 6, "std::_") == 0 ||
      ...
      if (CPPInstance_Check(pyobject)) {
        interop::TCppScope_t src = ((CPPInstance*)pyobject)->ObjectIsA();
        if (src && src != klass) {
          const std::string& srcName = interop::GetScopedFinalName(src);
          std::string::size_type lt = clName.find('<');
          sameTemplate = lt != std::string::npos && srcName.size() > lt &&
                         srcName[lt] == '<' &&
...
static void* PyFunction_AsCPointer(PyObject* pyobject,
  ...
  if (TemplateProxy_Check(pyobject)) {
    ...
    interop::TCppMethod_t cppmeth = interop::GetMethodTemplate(
        scope, fullname, sig.substr(1, sig.size() - 2));
    if (cppmeth) {
