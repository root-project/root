...
static inline PyObject* eqneq_binop(CPPClass* klass, PyObject* self,
  ...
  if (!binop) {
    // an operator== defined on a base class is proxied into the base's dict
    // only and cached on the base at its pythonization; consult the mro before
    // doing a (much more expensive, and possibly failing) lazy lookup: the base
    // class method is what C++ overload resolution would select anyway (e.g.
    // MSVC's STL iterators define operator== on the const_iterator base class
    // only)
