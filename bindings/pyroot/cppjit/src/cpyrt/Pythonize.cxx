...
template <class T>
auto __cppjit_internal_begin(T &s) noexcept
{
   return __cppjit_internal::make_iter(s.data(), s.data() + s.size());
}

// Note: for const span<T>, T is const-qualified here
template <class T>
auto __cppjit_internal_end(T &s) noexcept
{
   // end iterator = begin iterator with cur == end
   return __cppjit_internal::make_iter(s.data() + s.size(), s.data() + s.size());
}
    )";
  interop::Compile(code, /*silent*/ true);
}
...
PyObject* spanBegin() {
  ...
  if (!pyFunc) {
    compileSpanHelpers();
    PyObject* py_ns = cpyrt::GetScopeProxy(interop::GetGlobalScope());
    pyFunc = PyObject_GetAttrString(py_ns, "__cppjit_internal_begin");
    if (!pyFunc) {
      PyErr_Format(PyExc_RuntimeError,
                   "cppyy internal error: failed to locate helper "
                   "'__cppjit_internal_begin' for std::span pythonization");
    }
  }
  return pyFunc;
}
...
PyObject* spanEnd() {
  ...
  if (!pyFunc) {
    compileSpanHelpers();
    PyObject* py_ns = cpyrt::GetScopeProxy(interop::GetGlobalScope());
    pyFunc = PyObject_GetAttrString(py_ns, "__cppjit_internal_end");
    if (!pyFunc) {
      PyErr_Format(PyExc_RuntimeError,
                   "cppyy internal error: failed to locate helper "
                   "'__cppjit_internal_end' for std::span pythonization");
    }
  }
  return pyFunc;
}
...
bool cpyrt::Pythonize(PyObject* pyclass, interop::TCppScope_t scope) {
  ...
  // called "size".
  {
    ...
    bool sizeDirect =
        HasAttrDirect(pyclass, PyStrings::gSize, /*mustBecpyrt=*/true);
    bool ifaceDirect = (HasAttrDirect(pyclass, PyStrings::gBegin) &&
                        HasAttrDirect(pyclass, PyStrings::gEnd)) ||
                       HasAttrDirect(pyclass, PyStrings::gGetItem);
    PyObject* size_attr = (sizeDirect || ifaceDirect)
                              ? PyObject_GetAttr(pyclass, PyStrings::gSize)
                              : nullptr;
    if (!size_attr)
      PyErr_Clear();
    ...
    if (isContainer) {
      ...
      std::vector<interop::TCppMethod_t> sizes;
      std::deque<interop::TCppScope_t> todo{klass->fCppType};
      while (sizes.empty() && !todo.empty()) {
        interop::TCppScope_t s = todo.front();
        todo.pop_front();
        sizes = interop::GetMethodsFromName(s, "size");
        for (interop::TCppIndex_t i = 0; i < interop::GetNumBases(s); ++i)
          todo.push_back(interop::GetBaseScope(s, i));
      }
      for (interop::TCppMethod_t m : sizes) {
        if (isIntegral(interop::GetMethodReturnTypeAsString(m))) {
  ...
  if (!IsTemplatedSTLClass(name, "vector") && // vector is dealt with below
    ...
    if (HasAttrDirect(pyclass, PyStrings::gBegin) &&
      ...
      if (!methods.empty()) {
        ...
        // trailing '*' and returns the underlying class scope. interop::GetType
        ...
        interop::TCppType_t restype =
            !resname.empty()
                ? interop::GetType(resname, /*enable_slow_lookup=*/true)
                : nullptr;
        if (!isIterator && restype && !interop::IsPointerType(restype) &&
            interop::GetScope(resname)) {
  ...
  // The first condition was already present in upstream cpyrt. The others
  // are special to ROOT, because its reflection layer gives us the types
  // without the "std::" namespace. On some platforms, that applies only to the
  // template arguments, and on others also to the "basic_string".
           ...
           name == "basic_string<wchar_t,char_traits<wchar_t>,allocator<wchar_"
                   "t> >" ||
           name == "std::basic_string<wchar_t,char_traits<wchar_t>,allocator<"
                   "wchar_t> >" ||
