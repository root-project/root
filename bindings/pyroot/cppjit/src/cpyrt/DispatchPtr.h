...
public:
  ...
  // Conversion constructor: called with C++ object construction when the
  // PyObject is known (eg. when instantiating from Python), with pyobj the
  // Python-side representation of the C++ object.
  ...
  // Assignment: only ever called from C++. Similarly to the copy constructor,
  // the Pythonb object needs to be copied and rebound.
  ...
  // either C++ owns the Python object through a reference count (on fPyHardRef)
  // or Python owns the C++ object and we only have a weak reference (through
  // fPyWeakRef)
  void PythonOwns();
  void CppOwns();
