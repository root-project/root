...
//- helpers ------------------------------------------------------------------
static inline bool is_varchar(char c) {
  return isalnum((int)c) || c == '_' || c == ')' ||
         c == '(' /* for (anonymous)/(unnamed) */;
}

static inline std::string::size_type
find_qualifier_index(const std::string& name) {
  ...
  for (; 0 < i; --i) {
    ...
    if (!arr_open && (is_varchar(c) || c == '>')) {
      if (c == 't' && 6 < i && !is_varchar(name[i - 5]) &&
          name.substr(i - 4, 5) == "const")
...
std::string cpyrt::TypeManip::remove_const(const std::string& cppname) {
  ...
  if (cppname.find("::", type_stop + 1) !=
      std::string::npos) // e.g. klass<T>::some_typedef
  ...
  if (tmplt_start != std::string::npos && cppname[tmplt_start + 1] != '<') {
    ...
    return pre + cppname.substr(tmplt_start, type_stop + 1 - tmplt_start) +
           post;
  }
...
//----------------------------------------------------------------------------
std::string cpyrt::TypeManip::clean_type(const std::string& cppname,
                                         bool template_strip,
                                         bool const_strip) {
...
std::string cpyrt::TypeManip::compound(const std::string& name) {
  ...
  // CreateExecutor from InitExecutor_ (run test10_object_identity), remove
  // later
  if (name.empty())
    return "";
  ...
  if (!cpd.empty() && cpd[0] == ' ')
    return cpd.substr(1, cpd.length() - 1);
...
cpyrt::TypeManip::extract_arg_types(const std::string& sig) {
