...
PyObject* Instance_FromVoidPtr(void* addr, interop::TCppScope_t klass_scope,
                               bool python_owns);
} // namespace cppjit::cpyrt
...
//----------------------------------------------------------------------------
PyObject* cpyrt::CPPConstructor::Reflex(interop::Reflex::RequestId_t request,
...
//----------------------------------------------------------------------------
PyObject* cpyrt::CPPConstructor::Call(CPPInstance*& self, cpyrt_PyArgs_t args,
  ...
  // conversion context. See also the ConvertImplicit() helper in
  // Converters.cxx.
  if ((cppScopeFlags & CPPScope::kActiveImplicitCall) &&
      interop::IsExplicit(GetMethod())) {
    // FIXME: Cases with explicit marked std::complex constructors where we
    // expect implicit conversionss
    if (interop::GetMethodSignature(GetMethod(), true).find("std::complex") ==
        std::string::npos)
      return nullptr;
  }
  ...
  if (GetScope() != disp) {
    ...
    PyObject* pyobj =
        cpyrt_PyObject_Call(dispproxy, cargs.fArgs, cargs.fNArgsf, kwds);
    if (!pyobj)
      return nullptr;
  ...
  if (address) {
    ...
    // instead that is decided by the method proxy (which carries a creator
    // flag) upon return
    ...
    if (!(((CPPClass*)Py_TYPE(self))->fFlags & CPPScope::kIsSmart))
      MemoryRegulator::RegisterPyObject(self,
                                        interop::TCppObject_t((void*)address));

    // handling smart types this way is deeply fugly, but if CPPInstance sets
    // the proper types in op_new first, then the wrong init is called
    if (((CPPClass*)Py_TYPE(self))->fFlags & CPPScope::kIsSmart) {
      PyObject* pyclass =
          CreateScopeProxy(((CPPSmartClass*)Py_TYPE(self))->fUnderlyingType);
      if (pyclass) {
  ...
  if (!PyErr_Occurred()) // should be set, otherwise write a generic error msg
    PyErr_SetString(PyExc_TypeError,
                    const_cast<char*>((interop::GetScopedFinalName(GetScope()) +
                                       " constructor failed")
                                          .c_str()));
...
//----------------------------------------------------------------------------
cpyrt::CPPMultiConstructor::CPPMultiConstructor(interop::TCppScope_t scope,
...
//----------------------------------------------------------------------------
cpyrt::CPPMultiConstructor&
cpyrt::CPPMultiConstructor::operator=(const CPPMultiConstructor& s) {
  if (this != &s) {
...
PyObject* cpyrt::CPPMultiConstructor::Call(CPPInstance*& self,
                                           cpyrt_PyArgs_t argsin, size_t nargsf,
                                           PyObject* kwds, CallContext* ctxt) {
  // By convention, initialization parameters of multiple base classes are
  // grouped by target base class. Here, we disambiguate and put in "sentinel"
  // parameters that allow the dispatcher to propagate them.
  ...
  // TODO: this way of forwarding is expensive as the loop is external to this
  // call; it would be more efficient to have the argument handling happen
  // beforehand

  // fetch self, verify, and put the arguments in usable order (if self is not
  // handled first, arguments can not be reordered with sentinels in place)
  PyCallArgs cargs{self, argsin, nargsf, kwds};
  if (!this->ProcessArgs(cargs))
    return nullptr;

  // to re-use the argument handling, simply change the argument array into a
  // tuple (the benefits of not allocating the tuple are relatively minor in
  // this case)
  ...
  if (PyTuple_CheckExact(args) &&
      PyTuple_GET_SIZE(args)) { // case 0. falls through
    ...
    if (isAllTuples) {
      // copy over the arguments, while filling in the sentinels (case 1. & 2.),
      // with just sentinels for the remaining (void) calls (case 2.)
...
//----------------------------------------------------------------------------
PyObject* cpyrt::CPPNamespaceConstructor::Call(CPPInstance*&, cpyrt_PyArgs_t,
                                               size_t, PyObject*,
                                               CallContext*) {
...
//----------------------------------------------------------------------------
PyObject* cpyrt::CPPIncompleteClassConstructor::Call(CPPInstance*&,
                                                     cpyrt_PyArgs_t, size_t,
                                                     PyObject*, CallContext*) {
...
//----------------------------------------------------------------------------
PyObject* cpyrt::CPPAllPrivateClassConstructor::Call(CPPInstance*&,
                                                     cpyrt_PyArgs_t, size_t,
                                                     PyObject*, CallContext*) {
