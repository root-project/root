...
//- private helpers -----------------------------------------------------------
static inline void unroll(cpyrt_PyArgs_t packed_args, cpyrt_PyArgs_t unrolled,
                          Py_ssize_t nArgs) {
  // Unroll up to nArgs arguments from packed_args into unrolled.
  for (int i = 0, iur = 0; i < nArgs; ++i, ++iur) {
    PyObject* item = cpyrt_PyArgs_GET_ITEM(packed_args, i);
    if (PyTuple_Check(item)) {
      for (int j = 0; j < PyTuple_GET_SIZE(item); ++j, ++iur) {
        PyObject* subitem = PyTuple_GET_ITEM(item, j);
        Py_INCREF(subitem);
        cpyrt_PyArgs_SET_ITEM(unrolled, iur, subitem);
      }
    } else {
      Py_INCREF(item);
      cpyrt_PyArgs_SET_ITEM(unrolled, iur, item);
    }
  }
}

//- protected members ---------------------------------------------------------
bool cpyrt::CPPSetItem::InitExecutor_(Executor*& executor, CallContext*) {
  // basic call will do
...
//-----------------------------------------------------------------------------
bool cpyrt::CPPSetItem::ProcessArgs(PyCallArgs& cargs) {
  // Prepare executor with a buffer for the return value.
  Py_ssize_t nArgs = cpyrt_PyArgs_GET_SIZE(cargs.fArgs, cargs.fNArgsf);
  if (nArgs <= 1) {
  ...
  ((RefExecutor*)this->GetExecutor())
      ->SetAssignable(cpyrt_PyArgs_GET_ITEM(cargs.fArgs, nArgs - 1));

  // see whether any of the arguments is a tuple to be unrolled
  Py_ssize_t realsize = 0;
  for (Py_ssize_t i = 0; i < nArgs - 1; ++i) {
    PyObject* item = cpyrt_PyArgs_GET_ITEM(cargs.fArgs, i);
    realsize += PyTuple_Check(item) ? PyTuple_GET_SIZE(item) : 1;
  }

  // unroll any tuples, if present in the arguments
  if (realsize != nArgs - 1) {
    cpyrt_PyArgs_t unrolled =
        (PyObject**)PyMem_Malloc(realsize * sizeof(PyObject*));
...
//-----------------------------------------------------------------------------
bool cpyrt::CPPGetItem::ProcessArgs(PyCallArgs& cargs) {
  // Unroll tuples for call, otherwise just like regular CPPMethod of
  // __getitem__.
  Py_ssize_t nArgs = cpyrt_PyArgs_GET_SIZE(cargs.fArgs, cargs.fNArgsf);
  ...
  // unroll any tuples, if present in the arguments
  if (realsize != nArgs) {
    cpyrt_PyArgs_t packed_args = cargs.fArgs;
