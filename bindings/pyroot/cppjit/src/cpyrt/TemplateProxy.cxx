...
PyObject* TemplateProxy::Instantiate(const std::string& fname,
  ...
  if (argc != 0) {
    ...
    // A Python 2-tuple is typed above as std::initializer_list, but it may
    // equally represent a std::pair, to which an initializer_list can not
    // convert; construct an alternative prototype to retry the lookup with if
    // the first one fails
    if (!PyErr_Occurred()) {
      ...
      if (has2Tuple) {
        ...
        for (Py_ssize_t i = 0; i < argc; ++i) {
          ...
          if (PyTuple_CheckExact(itemi) && PyTuple_GET_SIZE(itemi) == 2) {
            ...
            const std::string& sub = Utility::ConstructTemplateArgs(
                nullptr, subtypes, itemi, pref, 0, nullptr);
            Py_DECREF(subtypes);
            if (!PyErr_Occurred() && 2 < sub.size())
              entry = cpyrt_PyText_FromString(("std::pair" + sub).c_str());
            else
              PyErr_Clear();
          }
          if (!entry) {
        ...
        const std::string& name_v2 = Utility::ConstructTemplateArgs(
            nullptr, tpArgsPair, pyargs, pref, 0, nullptr);
  ...
  if (!cppmeth && !protoPair.empty() && protoPair != proto) {
    ...
    cppmeth = interop::GetMethodTemplate(scope, fname, protoPair);
    if (cppmeth)
      proto = protoPair;
  }
