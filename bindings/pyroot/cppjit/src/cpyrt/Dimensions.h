...
public:
  Dimensions(dim_t ndim = 0, dim_t* dims = nullptr) : fDims(nullptr) {
    if (ndim && ndim != UNKNOWN_SIZE) {
      ...
      fDims[0] = ndim;
      if (dims)
        std::copy(dims, dims + ndim, fDims + 1);
      else
        std::fill_n(fDims + 1, ndim, UNKNOWN_SIZE);
    }
  }
  ...
  Dimensions(Dimensions&& d) : fDims(d.fDims) { d.fDims = nullptr; }
...
public:
  ...
  Dimensions sub() const {
    return fDims ? Dimensions(fDims[0] - 1, fDims + 2) : Dimensions();
  }
};
