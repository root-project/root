...
#define CustomInstanceMethod_GET_SELF(meth)                                    \
  reinterpret_cast<PyMethodObject*>(meth)->im_self
#define CustomInstanceMethod_GET_FUNCTION(meth)                                \
  reinterpret_cast<PyMethodObject*>(meth)->im_func
...
static PyObject* tptc_call(typedefpointertoclassobject* self, PyObject* args,
                           PyObject* /* kwds */) {
  long long addr = 0;
  if (!PyArg_ParseTuple(args, const_cast<char*>("|L"), &addr))
    return nullptr;
  return BindCppObjectNoCast(interop::TCppObject_t((void*)addr),
                             self->fCppType);
}
...
static PyObject* tptc_name(typedefpointertoclassobject* self, void*) {
  PyObject* pyclass = cpyrt::GetScopeProxy(self->fCppType);
  if (pyclass) {
...
PyTypeObject TypedefPointerToClass_Type = {
    PyVarObject_HEAD_INIT(&PyType_Type,
                          0)(char*) "cppjit.TypedefPointerToClass", // tp_name
...
PyObject* CustomInstanceMethod_New(PyObject* func, PyObject* self, PyObject*) {
  // from instancemethod, but with custom type (at issue is that instancemethod
  // is not meant to be derived from)
  PyMethodObject* im;
  if (!PyCallable_Check(func)) {
    PyErr_Format(PyExc_SystemError, "%s:%d: bad argument to internal function",
                 __FILE__, __LINE__);
    return nullptr;
  }
...
static PyObject* im_call(PyObject* meth, PyObject* args, PyObject* kw) {
  ...
  if (!self) {
    ...
    if (1 <= argc &&
        PyObject_IsInstance(PyTuple_GET_ITEM(args, 0), pyclass) == 1) {
  ...
  PyCFunctionObject* func =
      (PyCFunctionObject*)CustomInstanceMethod_GET_FUNCTION(meth);

  // the function is globally shared, so set and reset its "self" (ok, b/c of
  // GIL)
  Py_INCREF(self);
  func->m_self = self;
...
//-----------------------------------------------------------------------------
static PyObject* im_descr_get(PyObject* meth, PyObject* obj,
                              PyObject* pyclass) {
  // from instancemethod: don't rebind an already bound method, or an unbound
  // method of a class that's not a base class of pyclass
  ...
  return CustomInstanceMethod_New(CustomInstanceMethod_GET_FUNCTION(meth), obj,
                                  pyclass);
}

//= cpyrt custom instance method type =====================================
PyTypeObject CustomInstanceMethod_Type = {
    PyVarObject_HEAD_INIT(&PyType_Type,
                          0)(char*) "cppjit.InstanceMethod", // tp_name
    0,
    0,
...
//= cpyrt custom iterator for performance =================================
...
static PyObject* indexiter_iternext(indexiterobject* ii) {
  ...
  PyObject* result = PyObject_CallMethodOneArg((PyObject*)ii->ii_container,
                                               PyStrings::gGetItem, pyindex);
  Py_DECREF(pyindex);
...
PyTypeObject IndexIter_Type = {
    PyVarObject_HEAD_INIT(&PyType_Type, 0)(char*) "cppjit.indexiter", // tp_name
...
static void vectoriter_dealloc(vectoriterobject* vi) {
  if (vi->vi_converter && vi->vi_converter->HasState())
...
static PyObject* vectoriter_iternext(vectoriterobject* vi) {
  ...
  if (vi->vi_data && vi->vi_converter) {
    void* location =
        (void*)((ptrdiff_t)vi->vi_data + vi->vi_stride * vi->ii_pos);
    result = vi->vi_converter->FromMemory(location);
  } else if (vi->vi_data && vi->vi_klass) {
    // The CPPInstance::kNoMemReg by-passes the memory regulator; the assumption
    // here is that objects in vectors are simple and thus do not need to
    // maintain object identity (or at least not during the loop anyway). This
    // gains 2x in performance.
    interop::TCppObject_t cppobj(
        (void*)((ptrdiff_t)vi->vi_data + vi->vi_stride * vi->ii_pos));
    if (vi->vi_flags & vectoriterobject::kIsPolymorphic)
      result = cpyrt::BindCppObject(*(void**)cppobj.data, vi->vi_klass,
                                    cpyrt::CPPInstance::kNoMemReg);
    else
      result = cpyrt::BindCppObjectNoCast(cppobj, vi->vi_klass,
                                          cpyrt::CPPInstance::kNoMemReg);
    ...
    result = PyObject_CallMethodOneArg((PyObject*)vi->ii_container,
                                       PyStrings::gGetNoCheck, pyindex);
    Py_DECREF(pyindex);
  }
...
PyTypeObject VectorIter_Type = {
    PyVarObject_HEAD_INIT(&PyType_Type,
                          0)(char*) "cppjit.vectoriter", // tp_name
