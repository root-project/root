...
//===========================================================================
pyzables::NakedBuffers::NakedBuffers(int size, double valx, double valy)
    : m_size(size) {
...
pyzables::MyBase* pyzables::GimeDerived() { return new MyDerived(); }
...
void pyzables::renew_mine() {
  mine = std::shared_ptr<Countable>(new Countable);
}
...
unsigned int pyzables::pass_mine_sp(std::shared_ptr<Countable> ptr) {
  return ptr->m_check;
}
unsigned int pyzables::pass_mine_sp_ref(std::shared_ptr<Countable>& ptr) {
  return ptr->m_check;
}
unsigned int pyzables::pass_mine_sp_ptr(std::shared_ptr<Countable>* ptr) {
  return (*ptr)->m_check;
}

unsigned int pyzables::pass_mine_rp(Countable c) { return c.m_check; }
unsigned int pyzables::pass_mine_rp_ref(const Countable& c) {
  return c.m_check;
}
unsigned int pyzables::pass_mine_rp_ptr(const Countable* c) {
  return c->m_check;
}

pyzables::Countable* pyzables::gime_naked_countable() {
...
static inline void replace_method_name(PyObject* klass, const char* n1,
                                       const char* n2) {
...
void pyzables::WithCallback1::WithCallback1::__cppjit_explicit_pythonize__(
...
void pyzables::WithCallback2::WithCallback2::__cppjit_pythonize__(
    PyObject* klass, const std::string& name) {
  // change methods to camel case
  replace_method_name(klass, "get_int", "GetInt");
  replace_method_name(klass, "set_int", "SetInt");
