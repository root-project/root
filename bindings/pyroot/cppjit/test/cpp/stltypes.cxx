...
int ArrayTest::get_pp_px(Point** p, int idx) { return p[idx]->px; }

int ArrayTest::get_pp_py(Point** p, int idx) { return p[idx]->py; }

int ArrayTest::get_pa_px(Point* p[], int idx) { return p[idx]->px; }

int ArrayTest::get_pa_py(Point* p[], int idx) { return p[idx]->py; }
...
StringViewTest::count_cr(const std::string_view& arg) {
  return arg.size();
}
...
MyError::MyError(const std::string& msg) : fMsg(msg) { s_count += 1; }

MyError::MyError(const MyError& other) : fMsg(other.fMsg) { s_count += 1; }

MyError::~MyError() { s_count -= 1; }

const char* MyError::what() const throw() { return fMsg.c_str(); }

int GetMyErrorCount() { return MyError::s_count; }
...
const char* ErrorNamespace::MyError::what() const throw() {
  return fMsg.c_str();
}

void ErrorNamespace::throw_error(int i) {
  if (i == 0)
    throw ::MyError("first error");
  else if (i == 1)
    throw ::YourError("second error");
  else if (i == 2)
    throw MyError("third error");
