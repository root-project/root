...
template <class A> long MyTemplatedMethodClass::get_size(A&) {
  return sizeof(A);
}

template <class A> long MyTemplatedMethodClass::get_size(const A&) {
  return sizeof(A) + 1;
}

template <class B> inline long MyTemplatedMethodClass::get_size() {
  return sizeof(B);
}
...
template <> inline long MyTemplatedMethodClass::get_size<long>() { return 42; }

//===========================================================================
// global templated functions
template <typename T> long global_get_size() { return sizeof(T); }

template <typename T> int global_some_foo(T) { return 42; }

template <int T> int global_some_bar() { return T; }

template <typename T> int global_some_bar_var(T) { return 13; }

template <typename F> struct SomeResult {
  F m_retval;
};
...
template <typename... Args> inline bool isSomeInt(Args...) { return false; }
...
template <typename T> constexpr auto has_var1(T t) -> decltype(t.var1, true) {
  return true;
}

template <typename... Args> constexpr bool has_var1(Args...) { return false; }

template <typename T> constexpr bool call_has_var1(T&& t) {
  return AttrTesting::has_var1(std::forward<T>(t));
}

template <int N, typename... T> struct select_template_arg {};

template <typename T0, typename... T> struct select_template_arg<0, T0, T...> {
  typedef T0 type;
...
template <typename T> int some_foo(T) { return 42; }

template <int T> int some_bar() { return T; }

struct SomeStruct {
  template <int T> static int some_bar() { return T; }
};
...
public:
  template <class I, class O = double> O operator()(const I& in) const {
    return O(in);
  }
};
...
template <typename TYPE_IN, typename TYPE_OUT, size_t _vsize = 4>
struct DerivedWithUsing
    : public BaseWithEnumAndTypedefs<TYPE_IN, TYPE_OUT, _vsize> {
...
struct Base {
  template <class T> int callme(T t = T(1)) { return 2 * t; }
};
...
template <typename T, size_t SZ> struct vector {
  vector() : m_val(SZ) {}
  T m_val;
};

template <typename T, size_t... sizes> struct matryoshka {
  typedef T type;
};
...
public:
  template <typename R> R get1(T t) { return t + R{5}; }
  T get2() { return T{5}; }
  template <typename R> R get3(T t) { return t + R{5}; }
  T get3() { return T{5}; }
};
...
template <typename T> bool is_valid(T&& new_value) { return new_value != T{}; }
...
public:
  A() { gTypeName = demangle_it(typeid(A<Args...>).name(), "A::A"); }
  A(const A&) = default;
  A(A&&) = default;
  A& operator=(const A&) = default;
  A& operator=(A&&) = default;

  template <typename... FArgs> void a(FArgs&&... args) {
    gTypeName = demangle_it(typeid(&A<Args...>::a<FArgs...>).name(), "A::a-2");
  }

  template <typename T, typename... FArgs> T a_T(FArgs&&... args) {
    gTypeName =
        demangle_it(typeid(&A<Args...>::a_T<T, FArgs...>).name(), "A::a_T-2");
    return T{};
  }

  template <typename... FArgs> static void sa(FArgs&&... args) {
    gTypeName = demangle_it(typeid(A<Args...>).name(), "A::sa-1");
    gTypeName += "::";
    gTypeName +=
        demangle_it(typeid(A<Args...>::sa<FArgs...>).name(), "A::sa-2");
  }

  template <typename T, typename... FArgs> static T sa_T(FArgs&&... args) {
    gTypeName = demangle_it(typeid(A<Args...>).name(), "A::sa_T-1");
    gTypeName += "::";
    gTypeName +=
        demangle_it(typeid(A<Args...>::sa_T<T, FArgs...>).name(), "A::sa_T-2");
    return T{};
  }
};
...
class B {
public:
  B() { gTypeName = demangle_it(typeid(B).name(), "B::B"); }
  B(const B&) = default;
  B(B&&) = default;
  B& operator=(const B&) = default;
  B& operator=(B&&) = default;

  template <typename... FArgs> void b(FArgs&&... args) {
    gTypeName = demangle_it(typeid(&B::b<FArgs...>).name(), "B::b-2");
  }

  template <typename T, typename... FArgs> T b_T(FArgs&&... args) {
    gTypeName = demangle_it(typeid(&B::b_T<T, FArgs...>).name(), "B::b_T-2");
    return T{};
  }

  template <typename... FArgs> static void sb(FArgs&&... args) {
    gTypeName = demangle_it(typeid(B).name(), "B::sb-1");
    gTypeName += "::";
    gTypeName += demangle_it(typeid(B::sb<FArgs...>).name(), "B::sb-2");
  }

  template <typename T, typename... FArgs> static T sb_T(FArgs&&... args) {
...
template <typename... Args> void fn(Args&&... args) {
  gTypeName = demangle_it(typeid(fn<Args...>).name(), "fn");
}

template <typename T, typename... Args> T fn_T(Args&&... args) {
...
template <typename T> void some_empty();
...
class WithGreedy1 {
public:
  template <class T> int get_size(T*) { return (int)sizeof(T); }
...
public:
  ...
  template <typename V> MyVec operator[](const MyVec<V>& conds) const {
    return MyVec(2);
  }
};
