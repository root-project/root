...
#define DECLARE_DEFAULTERS(type, tname)                                        \
  ...
  type tname##_defaulter_func(int idx = 0, type a = 11, type b = 22,           \
                              type c = 33);
...
template <typename T, typename U> class T3 {
public:
...
static const char my_global_string1[] = "aap "
                                        " noot "
                                        " mies";
...
public:
  ...
  some_class_with_data gime_copy() { return *this; }
...
public:
  intptr_t gime_address_ptr(void* obj) { return (intptr_t)obj; }

  intptr_t gime_address_ptr_ptr(void** obj) {
    return (intptr_t) * ((long**)obj);
  }

  intptr_t gime_address_ptr_ref(const void*& obj) { return (intptr_t)obj; }
...
template <class T> T my_templated_function(T t) { return t; }
...
template <typename T> class PrintableBase1 {};
class Printable5 : public PrintableBase1<int> {};

template <typename T>
std::ostream& operator<<(std::ostream& os, const PrintableBase1<T>& y) {
  return os << "Ok.";
}
