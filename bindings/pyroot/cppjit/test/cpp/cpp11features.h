...
class MultiDerivedTestSmartPtr : public PubDerivedTestSmartPtr,
                                 public TestSmartPtrIface {};
...
std::string pass_ptr_overloaded(PubDerivedTestSmartPtr*) {
  return "PubDerivedTestSmartPtr";
}
std::string pass_ref_overloaded(TestSmartPtr&) { return "TestSmartPtr"; }
std::string pass_ref_overloaded(PubDerivedTestSmartPtr&) {
  return "PubDerivedTestSmartPtr";
}
std::string pass_val_overloaded(TestSmartPtr) { return "TestSmartPtr"; }
std::string pass_val_overloaded(PubDerivedTestSmartPtr) {
  return "PubDerivedTestSmartPtr";
}
...
public:
  ...
  TestMoving1(TestMoving1&&) {
    ++s_move_counter;
    ++s_instance_counter;
  }
  TestMoving1(const TestMoving1&) { ++s_instance_counter; }
  TestMoving1& operator=(TestMoving1&&) {
    ++s_move_counter;
    return *this;
  }
...
public:
  ...
  TestMoving2(TestMoving2&& other) {
    ++s_move_counter;
    ++s_instance_counter;
  }
  TestMoving2& operator=(TestMoving2&) { return *this; }
  TestMoving2& operator=(TestMoving2&&) {
    ++s_move_counter;
    return *this;
  }
  ~TestMoving2() { --s_instance_counter; }
...
template <class T> class WithInitList {
public:
...
namespace std {
template <> struct hash<StructWithHash> {
