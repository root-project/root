...
class CppFunctionNumbaType(nb_types.Callable):
    ...
    def get_call_type(self, context, args, kwds):
        ...
        ol.sig = nb_typing.Signature(
            return_type=self.ret_type, args=args, recvr=thistype
        )
...
def cppclass_getattr_impl(context, builder, typ, val, attr):
    ...
    if attr in typ._scope.__dict__ and isinstance(
        typ._scope.__dict__[attr], cpp_types.DataMember
    ):
    ...
    def get_data_type(self):
        ...
        if self._data_type is None:
            self._data_type = ir.LiteralStructType(
                [ir_byte for i in range(self._sizeof)], packed=True
            )
...
def typeof_scope(val, c, q=Qualified.default):
    ...
    for name, field in val.__dict__.items():
        if isinstance(field, cpp_types.DataMember):
            data_members.append(
                CppDataMemberInfo(
                    name,
                    field.__cpp_reflex__(cpp_refl.OFFSET),
                    field.__cpp_reflex__(cpp_refl.TYPE),
                )
            )
    ...
    def unbox_instance(typ, obj, c):
        ...
        ptrval = c.context.add_dynamic_addr(
            c.builder, cppjit_as_voidptr, info="Instance_AsVoidPtr"
        )
    ...
    def box_instance(typ, val, c):
        ...
        model = implclass._datamodel  # noqa: F841
        cfr = CppClassFieldResolver(c.context)

        for i in typ._scope.__dict__:
            if isinstance(cfr.generic_resolve(typ, i), nb_types.Type):
                box_list.append(
                    c.box(cfr.generic_resolve(typ, i), getattr(implclass, i))
                )

        box_res = c.pyapi.call_function_objargs(classobj, tuple(box_list))
