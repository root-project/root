...
### template support ---------------------------------------------------------
class Template(object):  # expected/used by ProxyWrappers.cxx in cpyrt
    ...
    def __repr__(self):
        return "<cppjit.Template '%s' object at %s>" % (self.__name__, hex(id(self)))
...
gbl.__class__.__repr__ = lambda cls: "<namespace cppjit.gbl at 0x%x>" % id(cls)
...
def _add_default_paths():
    if os.getenv("CONDA_PREFIX"):
        ...
        if os.path.exists(lib_path):
            Cpp.AddSearchPath(lib_path, True, False)
...
def _end_capture_stderr():
    ...
    if err:
        ...
        try:
            ...
            pass
        return "C++ issued an error message that could not be decoded (%s)" % str(
            original_error
        )
    return ""
