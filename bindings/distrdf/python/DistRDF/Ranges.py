...
class SerializableRSample():
    ...
    def __getstate__(self):
        return {"samplenames" : self._sample.GetSampleName(), "treenames" : self._sample.GetTreeNames() , "filenames" : self._sample.GetFileNameGlobs(), "metadata" : self._sample.GetMetaData().ExportJSON()}
        
    def __setstate__(self, state):
        _metadata = ROOT.RDF.Experimental.RMetaData()
        _metadata.ImportJSON(state["metadata"])
...
@dataclass
class TreeRangeSamplePerc(DataRange):
    """
    Range of percentages to be considered for a list of trees. Building block
    for an actual range of entries of a distributed task.
    ...
    treenames: List[str]
    filenames: List[str]
    samples: List[SerializableRSample]
    first_file_idx: int
    last_file_idx: int
    first_tree_start_perc: float
    last_tree_end_perc: float
...
@dataclass
class TreeRangeSample(DataRange):
    """
    Range of entries in one of the trees in the chain of a single distributed task.
    ...
    samples: List of serializable RSamples to be processed with this range. 
...
def get_percentage_ranges_sample(treenames: List[str], filenames: List[str], npartitions: int,
                          sampleMap: Dict[str, SerializableRSample], 
                          friendinfo: Optional[ROOT.Internal.TreeUtils.RFriendInfo],
                          exec_id: ExecutionIdentifier) -> List[TreeRangeSamplePerc]:
    """
    Create a list of tasks that will process the given trees partitioning them
    by percentages.
    """
    ...
    for i in range (len(filenames)):
        sample = sampleMap.get(filenames[i] + "/" + treenames[i])
    ...
    if friendinfo is not None:
        ...
        return [
            TreeRangeSamplePerc(
                exec_id, rangeid, treenames, filenames, samples, start_sample_idxs[rangeid], end_sample_idxs[rangeid],
                first_tree_start_perc_tasks[rangeid], last_tree_end_perc_tasks[rangeid], friendinfo)
            for rangeid in range(npartitions)
        ]
        ...
        # On the other hand, when creating the TreeRangeSamplePerc tasks below, the
        # starting and ending indexes have to be task-local. In practice, the
        # task always starts from file index 0 and it always ends at file index
        # equal to the number of files assigned to that task.
        return [
            ...
            TreeRangeSamplePerc(
                exec_id, rangeid, tasktreenames[rangeid], taskfilenames[rangeid], tasksamples[rangeid], 0, len(taskfilenames[rangeid]),
                first_tree_start_perc_tasks[rangeid], last_tree_end_perc_tasks[rangeid], friendinfo
            )            
            for rangeid in range(npartitions)
        ]

def get_clustered_range_from_percs_sample(percrange_sample: TreeRangeSamplePerc) -> Tuple[Optional[TreeRangeSample], TaskTreeEntries]:
    """
    Builds a range of entries to be processed for each tree in the chain created
    in a distributed task.
    """
    ...
    # first file index is inclusive
    first_file_idx = percrange_sample.first_file_idx
    # last file index is exclusive
    last_file_idx = percrange_sample.last_file_idx
    ...
    all_clusters_entries = (
        ROOT.Internal.TreeUtils.GetClustersAndEntries(treename, filename)
        for treename, filename in zip(percrange_sample.treenames, percrange_sample.filenames)
    )
    all_clusters, all_entries = zip(*all_clusters_entries)
    # Computing the offset of each tree is a cumulative sum over the entries in
    # the dataset. The initial offset is zero, so we define it in the following
    ...
    trees_with_entries: Dict[str, int] = {
        ...
        for filename, treename, entries
        in zip(
            percrange_sample.filenames[first_file_idx:last_file_idx],
            percrange_sample.treenames[first_file_idx:last_file_idx],
            all_entries[first_file_idx:last_file_idx]
        )
    }
    ...
    if (last_file_idx - first_file_idx) == 1:
        # Compute only once if there is only one file
        first_tree_startentry, last_tree_endentry, first_tree_entries_to_discard = get_entryrange_at_cluster_boundaries(
            percrange_sample.first_tree_start_perc, percrange_sample.last_tree_end_perc,
            all_entries[first_file_idx], all_clusters[first_file_idx]
        )
        entries_to_discard = first_tree_entries_to_discard
    else:
        first_tree_startentry, _, first_tree_entries_to_discard = get_entryrange_at_cluster_boundaries(
            percrange_sample.first_tree_start_perc, 1, all_entries[first_file_idx], all_clusters[first_file_idx]
        )
        _, last_tree_endentry, last_tree_entries_to_discard = get_entryrange_at_cluster_boundaries(
            0, percrange_sample.last_tree_end_perc, all_entries[last_file_idx-1], all_clusters[last_file_idx-1]
        )
        entries_to_discard = first_tree_entries_to_discard + last_tree_entries_to_discard
    ...
    treerange = TreeRangeSample(percrange_sample.exec_id, percrange_sample.id, percrange_sample.treenames, percrange_sample.filenames, percrange_sample.samples,
                          globalstart, globalend, percrange_sample.friendinfo)
