// @(#)root/base:$Name:  $:$Id: TVirtualX.cxx,v 1.2 2003/08/06 20:25:04 brun Exp $
...
{
   // Sets the WindowAttributes_t structure to defaults.
                             ...
                             ColorStruct_t &color)
{
   ...
   // 
   // Here we set ColorStruct_t structure to default. Let system think we 
   // could parse color.
...
Bool_t TVirtualX::AllocColor(Colormap_t /*cmap*/, ColorStruct_t &color)
{
   ...
   //
   // Here we set pixel value and let system think we could allocate color.
   //
   // cmap  - the colormap
...
void TVirtualX::QueryColor(Colormap_t /*cmap*/, ColorStruct_t &color)
{
   ...
   //
   // Here we set color components to default.
   //
   // cmap  - the colormap
...
{ 
   // Initialize X system. Returns kFALSE in case of failure.
...
{ 
   // Create OpenGL context for window "wid"
}

//______________________________________________________________________________
void TVirtualX::DeleteOpenGLContext(Int_t /*wid*/) 
{ 
   // Delete OpenGL context for window "wid"
}
...
void TVirtualX::DrawBox(Int_t /*x1*/, Int_t /*y1*/, Int_t /*x2*/, Int_t /*y2*/, 
                        EBoxMode /*mode*/) 
{ 
   // Draw a box between [x1,y1] and [x2,y2] according to the "mode".
   //
   // mode  - drawing mode:
...
{ 
   // Draw a line through all points in the list.
   //
   // n  - number of points
   // xy - list of points
}
...
{ 
   // Return pixel value associated to specified ROOT color number "cindex".
...
{ 
   // Register a window created by Qt as a ROOT window
   //
...
{ 
   // Remove the created by Qt window "qwid".
}
...
{ 
   // Request string: text is displayed and can be edited with Emacs-like 
...
{ 
   // Map the window "id" and all of its subwindows that have had map 
...
{ 
   // Close connection to display server and destroys all windows.    
}
...
{ 
   // The cursor "curid" will be used when the pointer is in the 
   // window "id".
}
...
Pixmap_t TVirtualX::CreatePixmap(Drawable_t /*id*/, const char */*bitmap*/, 
                                 ...
                                 Int_t /*depth*/)  
{ 
   // Create a pixmap from bitmap data of the width, height, and depth you 
...
Bool_t TVirtualX::ReadPictureDataFromFile(const char */*filename*/, 
                                          char ***/*ret_data*/) 
{ 
   // Readd picture data from file "filename" and store it in "ret_data". 
...
{ 
   ...
   // width, height  - the width and height, which are the dimensions of both the 
   //                  source and destination rectangles.                                                                   //
   // dest_x, dest_y - specify the upper-left corner of the destination rectangle
   // 
   // GC components in use: function, plane-mask, subwindow-mode, graphics-exposure, 
...
void TVirtualX::ChangeProperty(Window_t /*id*/, Atom_t /*property*/, 
                               Atom_t /*type*/, UChar_t */*data*/, Int_t /*len*/)   
{ 
...
{ 
   // Uses the components of the specified GC to draw a line between the specified
   // set of points (x1, y1) and (x2, y2).
   //
   // GC components in use: function, plane-mask, line-width, line-style, cap-style, 
...
//______________________________________________________________________________
void TVirtualX::GrabPointer(Window_t /*id*/, UInt_t /*evmask*/, Window_t /*confine*/,
...
{ 
   // Tell the window manager the desired position [x,y] of window "id".
}

//______________________________________________________________________________
void TVirtualX::SetWMSize(Window_t /*id*/, UInt_t /*w*/, UInt_t /*h*/)   
{ 
   // Tell window manager the desired size of window "id".
   //
   // w - the width
   // h - the height
}
...
{ 
   // Give the window manager minimum and maximum size hints of the window 
...
{ 
   // Set the initial state of the window "id": either kNormalState 
   // or kIconicState.
}
...
{ 
   // Tell window manager that the window "id" is a transient window 
...
{ 
   // Convert the "keysym" to the appropriate keycode. For example, 
...
{ 
   ...
   //
   // The GC components in use are: function, plane-mask, fill-style, 
   // subwindow-mode, clip-x-origin, clip-y-origin, clip-mask. 
   // The GC mode-dependent components: foreground, background, tile, stipple, 
   // tile-stipple-x-origin, and tile-stipple-y-origin.
   // (see also the GCValues_t structure)
}
...
void TVirtualX::TranslateCoordinates(Window_t /*src*/, Window_t /*dest*/, 
                                     ...
                                     Window_t &child)
{ 
   // Translate coordinates in one window to the coordinate space of another 
...
void TVirtualX::PutImage(Drawable_t /*id*/, GContext_t /*gc*/, 
                         ...
                         Int_t /*x*/, Int_t /*y*/, UInt_t /*w*/, UInt_t /*h*/)   
{ 
   // Combines an image with a rectangle of the specified drawable. The section 
   // of the image defined by the x, y, width, and height arguments is drawn on 
   ...
   // x, y - coordinates, which are relative to the origin of the drawable 
   //        and are the coordinates of the subimage
   // w, h - the width and height of the subimage, which define the dimensions 
   //        of the rectangle
   //
   // GC components in use: function, plane-mask, subwindow-mode, clip-x-origin,
...
//______________________________________________________________________________
void TVirtualX::glViewport(Int_t /*x0*/, Int_t /*y0*/, Int_t /*x1*/, Int_t /*y1*/) 
{ 
...
//______________________________________________________________________________
void TVirtualX::glClearColor(Float_t /*red*/, Float_t /*green*/, Float_t /*blue*/, 
                             Float_t /*alpha*/) 
{ 
...
//______________________________________________________________________________
void TVirtualX::glRotated(Double_t /*angle*/, Double_t /*x*/, Double_t /*y*/, Double_t /*z*/) 
{ 
...
//______________________________________________________________________________
void TVirtualX::glFrustum(Double_t /*min_0*/, Double_t /*max_0*/, Double_t /*min_1*/,
...
//______________________________________________________________________________
void TVirtualX::glOrtho(Double_t /*min_0*/, Double_t /*max_0*/, Double_t /*min_1*/,
                        Double_t /*max_1*/, Double_t /*dnear*/, Double_t /*dfar*/) 
{ 
