...
Bool_t TXMLPlayer::ProduceCode(TList *cllist, const char *filename)
{
   ...
   while ((cl = (TClass *)iter()) != nullptr) {
      if (!inclfiles.FindObject(cl->GetDeclFileName())) {
...
const char *TXMLPlayer::ElementGetter(TClass *cl, const char *membername, int specials)
{
   ...
   if (mgetter && (mgetter->GetMethod()->Property() & kIsPublic)) {
      fGetterName = "obj->";
      fGetterName += mgetter->GetMethodName();
      fGetterName += "()";
   } else if (!member || ((member->Property() & kIsPublic) != 0)) {
      fGetterName = "obj->";
...
const char *TXMLPlayer::ElementSetter(TClass *cl, const char *membername, char *endch)
{
   ...
   if (msetter && (msetter->GetMethod()->Property() & kIsPublic)) {
      fSetterName = "obj->";
...
void TXMLPlayer::ProduceStreamerSource(std::ostream &fs, TClass *cl, TList *cllist)
{
   ...
   while ((c1 = (TClass *)iter()) != nullptr) {
      if (c1 == cl)
         continue;
      if (!c1->GetListOfBases()->FindObject(cl->GetName()))
         continue;
      if (firstchild) {
   ...
   for (n = 0; n <= elements->GetLast(); n++) {
      ...
      case TVirtualStreamerInfo::kCounter: {
         ...
         fs << "buf." << GetBasicTypeReaderMethodName(el->GetType(), nullptr) << "(\"" << el->GetName() << "\")" << endch
            << ";" << std::endl;
         continue;
      }
