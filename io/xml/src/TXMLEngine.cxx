...
public:
   ...
   TXMLInputStream(Bool_t isfilename, const char *filename, Int_t ibufsize)
      : fInp(nullptr), fInpStr(nullptr), fInpStrLen(0), fBuf(nullptr), fBufSize(0), fMaxAddr(nullptr), fLimitAddr(nullptr), fTotalPos(0),
        fCurrentLine(0), fEntities(), fCurrent(nullptr)
   {
      if (isfilename) {
   ...
   inline Bool_t EndOfFile() { return fInp ? fInp->eof() : (fInpStrLen <= 0); }
...
const char *TXMLEngine::GetNSName(XMLNsPointer_t ns)
{
   ...
   if (nsname && (strncmp(nsname, "xmlns:", 6) == 0))
      nsname += 6;
                                 ...
                                 int alternate, const char *media, const char *charset)
{
   ...
   if (title)
      NewAttr(node, nullptr, "title", title);

   NewAttr(node, nullptr, "href", href);
   NewAttr(node, nullptr, "type", type);

   if (media)
      NewAttr(node, nullptr, "media", media);
   if (charset)
      NewAttr(node, nullptr, "charset", charset);
...
void TXMLEngine::SetNodeContent(XMLNodePointer_t xmlnode, const char *content, Int_t len)
{
   if (!xmlnode)
      return;
   SXmlNode_t *node = (SXmlNode_t *)xmlnode;
   if (node->fChild && (node->fChild->fType == kXML_CONTENT))
...
{
   XMLNodePointer_t res = !xmlnode ? nullptr : ((SXmlNode_t *)xmlnode)->fChild;
   // skip content(s) node, if specified
   if (realnode && res && (((SXmlNode_t *)res)->fType != kXML_NODE))
...
{
   return !xmlnode ? kFALSE : (((SXmlNode_t *)xmlnode)->fType == kXML_NODE);
}
...
{
   return !xmlnode ? kTRUE : (((SXmlNode_t *)xmlnode)->fType != kXML_NODE);
}
...
{
   return !xmlnode ? kFALSE : (((SXmlNode_t *)xmlnode)->fType == kXML_CONTENT);
}
...
{
   return !xmlnode ? kFALSE : (((SXmlNode_t *)xmlnode)->fType == kXML_COMMENT);
}
...
void TXMLEngine::SaveNode(XMLNodePointer_t xmlnode, TXMLOutputStream *out, Int_t layout, Int_t level)
{
   ...
   if (child && (child->fType == kXML_CONTENT) && !child->fNext) {
