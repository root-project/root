...
int reader(int maxiterations = 0)
{
   ...
   auto ReadCheck = [&hpx, &hpxpy, &hprof](TMapFile *mfile, int i,
                                           double maxentries) -> int {
      auto res = hpx.Check(mfile, i, maxentries);
      if (res)
         return res;
      res = hpxpy.Check(mfile, i, maxentries);
      if (res)
         return res;
      res = hprof.Check(mfile, i, maxentries);
      if (res)
         return res;
      return 0;
   };
