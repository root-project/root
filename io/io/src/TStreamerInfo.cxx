   ...
   while ((base = (TBaseClass*)nextb())) {
      ...
      if (!strcmp(bname, "string")) {
         ...
         if (proxy) element = new TStreamerSTL(bname, btitle, offset, bname, *proxy, kFALSE);
            ...
            element = new TStreamerSTL(bname, btitle, offset, bname, nullptr, kFALSE);
...
{
   Bool_t result = kTRUE;
   R__ASSERT((cl == nullptr || info == nullptr) &&
             (cl != nullptr || info != nullptr) /* must compare to only one thing! */);
...
void TStreamerInfo::GenerateDeclaration(FILE *fp, FILE *sfp, const TList *subClasses, Bool_t top)
{
   ...
   Bool_t needGenericTemplate = fElements == nullptr || fElements->GetEntries() == 0;
   ...
   TString protoname;
   UInt_t numberOfNamespaces =
      TMakeProject::GenerateClassPrefix(fp, GetName(), top, protoname, nullptr, kFALSE, needGenericTemplate);
...
Int_t TStreamerInfo::GenerateHeaderFile(const char *dirname, const TList *subClasses, const TList *extrainfos)
{
   ...
   Bool_t needGenericTemplate = isTemplate && (fElements == nullptr || fElements->GetEntries() == 0);
   ...
   if (R__b.IsReading()) {
...
#endif
      {
         ...
         for (Int_t i = 0; i < nobjects; i++) {
            ...
            if (el != nullptr &&
                (el->IsA() == TStreamerArtificial::Class() || el->TestBit(TStreamerElement::kRepeat))) {
               fElements->RemoveAt( i );
            } else if (el != nullptr &&
                       (el->TestBit(TStreamerElement::kCache) && !el->TestBit(TStreamerElement::kWrite))) {
               fElements->RemoveAt( i );
            }
         }
         fElements->Compress();
