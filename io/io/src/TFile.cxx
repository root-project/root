                        ...
                        Option_t *option)
{
   ...
   // $BinDir and $LibDir are not mentioned in the docs, but used internally
   // (e.g. on Windows MAKESHAREDLIB) to build absolute paths to ROOT's
   // directories. TSystem::CompileMacro substitutes them, we mirror that here
   // so the MAKEP script generated by MakeProject is self-contained.
