...
public:
	TArrayIndexProducer(TStreamerElement* elem, Int_t arraylen,
			const char* separ) :
			fTotalLen(0), fCnt(-1), fSepar(separ), fIndicies(), fMaxIndex(), fRes(), fIsArray(
					kFALSE) {
...
public:
	...
	TJSONStackObj() :
			TObject(), fInfo(0), fElem(0), fIsStreamerInfo(kFALSE), fIsElemOwner(
					kFALSE), fIsPostProcessed(kFALSE), fIsObjStarted(kFALSE), fAccObjects(
					kFALSE), fValues(), fLevel(0), fIndx(0) {
...
TBufferJSON::TBufferJSON() :
		TBuffer(TBuffer::kWrite), fOutBuffer(), fOutput(0), fValue(), fJsonrMap(), fJsonrCnt(
				0), fStack(), fCompact(0), fSemicolon(" : "), fArraySepar(", "), fNumericLocale() {
	fBufSize = 1000000000;
...
TString TBufferJSON::TColorCompression(TString jsonString) {
	...
	// The TColor compression will only work if the user has not changed the default palette, therefore, we can only remove the colors
	// if these are not changed (99.99% of cases)
	if (TColor::DefinedColors()){
		return jsonString;
	}
	
	//Only TCanvas JSON objects have TColors to remove in the first place
	if (document.HasMember("_typename") && document["_typename"] != "TCanvas") {
		return jsonString;
	}
	// If the object doesn't have fPrimitives, return
	if (!document.HasMember("fPrimitives")) {
		return jsonString;
	}
	rapidjson::Value& fPrimitives_array = document["fPrimitives"]["arr"];
	...
	for (rapidjson::SizeType i = 0; i < fPrimitives_array.Size(); i++) {
		if (fPrimitives_array[i]["_typename"] == "TObjArray"
				&& fPrimitives_array[i]["name"] == "ListOfColors") {
...
///   4 - graphics compression, by way of removal of redundant list of TColors
...
/// Maximal compression achieved when compact parameter equal to 43
...
TString TBufferJSON::ConvertToJSON(const void *obj, const TClass *cl,
	...
	if ((member_name != 0) && (obj != 0)) {
		...
		if (member->GetArrayIndex() != 0) {
			...
			if ((imember != 0)
					&& (strcmp(imember->GetTrueTypeName(), "int") == 0)) {
...
TString TBufferJSON::ConvertToJSON(const void *ptr, TDataMember *member,
	...
	TClass *mcl =
			member->IsBasic() ? 0 : gROOT->GetClass(member->GetTypeName());
...
Int_t TBufferJSON::ExportToFile(const char* filename, const TObject *obj,
		const char* option) {
	...
	if (strstr(filename, ".json.gz")) {
		...
		*bufcur++ = 3;    // OS   3 means Unix
		...
		unsigned long ziplen = R__memcompress(bufcur - 6, objlen + 6,
				(char *) objbuf, objlen);
...
Int_t TBufferJSON::ExportToFile(const char* filename, const void *obj,
		const TClass *cl, const char* option) {
...
TString TBufferJSON::JsonWriteMember(const void *ptr, TDataMember *member,
	...
	if (gDebug > 2)
		Info("JsonWriteMember", "Write member %s type %s ndim %d",
				member->GetName(), member->GetTrueTypeName(),
				member->GetArrayDim());

	Int_t tid =
			member->GetDataType() ?
					member->GetDataType()->GetType() : kNoType_t;
	...
	if (tid != kNoType_t) {
		...
		if (fValue == "0")
			...
			&& memberClass->GetBaseClassOffset(TArray::Class()) == 0) {
...
TJSONStackObj *TBufferJSON::Stack(Int_t depth) {
	...
	if (depth <= fStack.GetLast())
		stack = dynamic_cast<TJSONStackObj *>(fStack.At(
				fStack.GetLast() - depth));
	return stack;
}
...
void TBufferJSON::JsonWriteObject(const void *obj, const TClass *cl,
	...
	if (gDebug > 0)
		Info("JsonWriteObject", "Object %p class %s check_map %s", obj,
				cl ? cl->GetName() : "null", check_map ? "true" : "false");
	...
	if (stack && stack->fAccObjects
			&& ((fValue.Length() > 0) || (stack->fValues.GetLast() >= 0))) {
	...
	if (special_kind <= 0) {
		...
		if (check_map) {
			std::map<const void *, unsigned>::const_iterator iter =
					fJsonrMap.find(obj);
	...
	if (gDebug > 3)
		Info("JsonWriteObject", "Starting object %p write for class: %s", obj,
				cl->GetName());
	...
	if (gDebug > 3)
		Info("JsonWriteObject", "Done object %p write for class: %s", obj,
				cl->GetName());
		...
		if (stack->fValues.GetLast() < 0) {
			...
			if ((stack->fValues.GetLast() == 0)
				...
				if ((size * 2 == stack->fValues.GetLast())
						&& ((special_kind == TClassEdit::kMap)
								|| (special_kind == TClassEdit::kMultiMap)
								|| (special_kind == TClassEdit::kUnorderedMap)
								|| (special_kind
	...
	if ((special_kind == 0)
		...
		if (gDebug > 0)
			Info("JsonWriteObject", "Create blob value for class %s",
					cl->GetName());
	...
	if (fPrevOutput != 0) {
		...
		if ((special_kind <= 0) || (special_kind > 100))
			...
			Error("JsonWriteObject",
					"Non-empty object output for special class %s",
					cl->GetName());
	}
}
...
void TBufferJSON::ClassMember(const char *name, const char *typeName,
	...
	if (elem == 0) {
		Error("ClassMember", "Invalid combination name = %s type = %s", name,
				typeName);
		return;
	}
...
void TBufferJSON::PerformPostProcessing(TJSONStackObj *stack,
	...
	Bool_t isTObject(kFALSE), isTRef(kFALSE), isTString(kFALSE), isSTLstring(
			kFALSE), isOffsetPArray(kFALSE), isTArray(kFALSE);
	...
	if (obj_cl) {
		...
		const char *typname =
				elem->IsBase() ? elem->GetName() : elem->GetTypeName();
		isTObject = (elem->GetType() == TStreamerInfo::kTObject)
				|| (strcmp("TObject", typname) == 0);
		isTString = elem->GetType() == TStreamerInfo::kTString;
		isSTLstring = elem->GetType() == TStreamerInfo::kSTLstring;
		isOffsetPArray = (elem->GetType() > TStreamerInfo::kOffsetP)
				&& (elem->GetType() < TStreamerInfo::kOffsetP + 20);
	...
	if (isTString || isSTLstring) {
		...
		if (gDebug > 3)
			Info("PerformPostProcessing", "reformat string value = '%s'",
					fValue.Data());
		...
		if (cnt < 2 || cnt > 3) {
			...
			AppendOutput(
					(stack->fValues.GetLast() > 0) ?
							stack->fValues.At(1)->GetName() : fValue.Data());
			if (cnt == 3) {
				AppendOutput(",", "\"fPID\"");
				AppendOutput(fSemicolon.Data());
				AppendOutput(
						(stack->fValues.GetLast() > 1) ?
								stack->fValues.At(2)->GetName() :
								fValue.Data());
			}
...
void TBufferJSON::WriteObjectClass(const void *actualObjStart,
	...
	if (gDebug > 3)
		Info("WriteObjectClass", "Class %s",
				(actualClass ? actualClass->GetName() : " null"));
...
void TBufferJSON::ReadFastArrayWithFactor(Float_t *f, Int_t n,
		Double_t /* factor */, Double_t /* minvalue */) {
	TBufferJSON_ReadFastArray(f);
}
...
void TBufferJSON::ReadFastArrayWithNbits(Double_t *d, Int_t n,
		Int_t /*nbits*/) {
	TBufferJSON_ReadFastArray(d);
}
...
void TBufferJSON::ReadFastArray(void * /*start*/, const TClass * /*cl*/,
		Int_t /*n*/, TMemberStreamer * /*s*/, const TClass * /*onFileClass*/) {
}
...
void TBufferJSON::ReadFastArray(void ** /*startp*/, const TClass * /*cl*/,
		Int_t /*n*/, Bool_t /*isPreAlloc*/, TMemberStreamer * /*s*/,
...
Int_t TBufferJSON::ApplySequence(
		const TStreamerInfoActions::TActionSequence &sequence, void *obj) {
	TVirtualStreamerInfo *info = sequence.fStreamerInfo;
	IncrementLevel(info);

	if (gDebug) {
		//loop on all active members
		TStreamerInfoActions::ActionContainer_t::const_iterator end =
				sequence.fActions.end();
		for (TStreamerInfoActions::ActionContainer_t::const_iterator iter =
				sequence.fActions.begin(); iter != end; ++iter) {
			// Idea: Try to remove this function call as it is really needed only for JSON streaming.
			SetStreamerElementNumber((*iter).fConfiguration->fCompInfo->fElem,
					(*iter).fConfiguration->fCompInfo->fType);
...
Int_t TBufferJSON::ApplySequenceVecPtr(
...
Int_t TBufferJSON::ApplySequence(
	...
	if (gDebug) {
		...
		// the screen anyway).
		void *arr0 = loopconfig->GetFirstAddress(start_collection,
				end_collection);
		// loop on all active members
		TStreamerInfoActions::ActionContainer_t::const_iterator end =
				sequence.fActions.end();
...
Int_t TBufferJSON::WriteClassBuffer(const TClass *cl, void *pointer) {
	...
	if (gDebug > 2)
		Info("WriteClassBuffer", "class: %s version %d done", cl->GetName(),
				cl->GetClassVersion());
	return 0;
}
