...
TSQLTableData::TSQLTableData(TSQLFile *f, TSQLClassInfo *info)
   : TObject(), fFile(f), fInfo(info), fColumns(), fColInfos(nullptr)
{
...
//   fColumns.Add(col);
   ...
   if (fColInfos != nullptr) fColInfos->Add(new TSQLClassColumnInfo(name, DefineSQLName(name), "INT"));
}
...
//   fColumns.Add(col);
   ...
   if (fColInfos != nullptr) fColInfos->Add(new TSQLClassColumnInfo(name, DefineSQLName(name), sqltype));
}
...
TSQLStructure::TSQLStructure()
   : TObject(), fParent(nullptr), fType(0), fPointer(nullptr), fValue(), fArrayIndex(-1), fRepeatCnt(0), fChilds()
{
   // default constructor
}
...
void TSQLStructure::ChildArrayIndex(Int_t index, Int_t cnt)
{
   ...
   if ((last != nullptr) && (last->GetType() == kSqlValue)) last->SetArrayIndex(index, cnt);
}
...
Long64_t TSQLStructure::DefineObjectId(Bool_t recursive)
{
   ...
   while (curr != nullptr) {
      if ((curr->GetType()==kSqlObject) ||
         ...
         if ((value != nullptr) && (strlen(value) > 0)) return sqlio::atol64(value);
      }
...
TSQLObjectData* TSQLStructure::GetObjectData(Bool_t search)
{
   ...
   if ((child != nullptr) && (child->GetType() == kSqlObjectData)) return (TSQLObjectData *)child->fPointer;
   if (search && (GetParent() != nullptr)) return GetParent()->GetObjectData(search);
   return nullptr;
}
...
void TSQLStructure::PrintLevel(Int_t level) const
{
   ...
   case kSqlValue: {
      ...
      if (fPointer != nullptr) std::cout << "  type = " << (const char *)fPointer;
      break;
   }
   case kSqlArray: {
      std::cout << "Array ";
...
public:
   TSqlCmdsBuffer(TSQLFile *f, TSQLClassInfo *info)
      : TObject(), fFile(f), fInfo(info), fBlobStmt(nullptr), fNormStmt(nullptr)
   {
   }
...
public:
   TSqlRegistry()
      : TObject(), fFile(nullptr), fKeyId(0), fLastObjId(-1), fCmds(nullptr), fFirstObjId(0), fCurrentObjId(0),
        fCurrentObjClass(nullptr), fLastLongStrId(0), fPool(), fLongStrValues(), fRegValues(), fRegStmt(nullptr)
   {
   }
   ...
   if ((values.GetLast() < 0) || (tablename == nullptr)) return;
   ...
   void AddRegCmd(Long64_t objid, TClass* cl)
   {
      ...
      if (fFile->IsOracle() || fFile->IsODBC()) {
         if ((fRegStmt == nullptr) && fFile->SQLCanStatement()) {
            const char* quote = fFile->SQLIdentifierQuote();
   ...
   void InsertToNormalTable(TSQLTableData* columns, TSQLClassInfo* sqlinfo)
   {
      ...
      TSqlCmdsBuffer* buf = GetCmdsBuffer(sqlinfo);
      if (buf != nullptr) buf->AddValues(kTRUE, values.Data());
   }
};
...
public:
   TSqlRawBuffer(TSqlRegistry *reg, TSQLClassInfo *sqlinfo)
      : TObject(), fFile(nullptr), fInfo(nullptr), fCmdBuf(nullptr), fObjId(0), fRawId(0), fValueMask(),
        fValueQuote(nullptr), fMaxStrSize(255)
   {
      fFile = reg->fFile;
      fInfo = sqlinfo;
   ...
   virtual ~TSqlRawBuffer()
   {
      ...
      if ((stmt != nullptr) && fFile->IsOracle()) {
         stmt->Process();
         delete stmt;
   ...
   {
      if (fCmdBuf == nullptr) return;

      // when first line is created, check all problems
      if (fRawId==0) {
         Bool_t maketmt = kFALSE;
         if (fFile->IsOracle() || fFile->IsODBC())
            maketmt = (fCmdBuf->fBlobStmt == nullptr) && fFile->SQLCanStatement();
...
{
   if ((file == nullptr) || (cmds == nullptr)) return kFALSE;
...
void TSQLStructure::PerformConversion(TSqlRegistry* reg, TSqlRawBuffer* blobs, const char* topname, Bool_t useblob)
{
   ...
   case kSqlValue: {
      ...
      if ((tname == sqlio::CharStar) && (value != nullptr)) {
...
{
   if ((cl == nullptr) || (objid < 0)) return kFALSE;
...
{
   TString res;
   if ((elem == nullptr) || (elem->GetArrayLength() == 0)) return res;
...
{
   if ((s_ver == nullptr) || (s_info == nullptr) || (s_ver->GetType() != kSqlVersion)) return kFALSE;
   ...
   if ((ver_cl == nullptr) || (info_cl == nullptr) || (ver_cl != info_cl) || (ver_cl->GetClassVersion() != info_ver))
      return kFALSE;
...
Bool_t TSQLStructure::RecognizeTString(const char* &value)
{
   ...
   if (len == nullptr) return kFALSE;
   if ((lenbig != nullptr) && ((chars == nullptr) || (len == nullptr))) return kFALSE;

   if (chars != nullptr) value = chars->GetValue();
...
{
   if (elem == nullptr) return kColUnknown;
   ...
   if ((typ==TStreamerInfo::kObject)  ||
      ...
      if (elem->GetArrayLength()==0)
         return kColObject;
      else if (elem->GetStreamer() == nullptr)
   ...
   if ((typ==TStreamerInfo::kObject)  ||
       ...
       (typ==TStreamerInfo::kAnyP) ||
       (typ==TStreamerInfo::kObjectP)) {
      if ((elem->GetArrayLength() == 0) || (elem->GetStreamer() != nullptr))
...
{
   TStreamerElement* elem = GetElement();
   if ((elem == nullptr) || (data == nullptr)) return kColUnknown;
   ...
   case kColParent: {
      ...
      if (sqlinfo == nullptr) return kColUnknown;
      ...
      if (sqlinfo->IsClassTableExist()) {
         ...
         if ((objdata == nullptr) || !objdata->PrepareForRawData()) return kColUnknown;
   ...
   case kColObject: {
      ...
      if (strobjid == nullptr) return kColUnknown;
   ...
   case kColTString: {
      ...
      Int_t len = (value == nullptr) ? 0 : strlen(value);
      if (len<255) {
...
Bool_t TSQLStructure::UnpackTObject(TSQLFile* f, TBufferSQL2* buf, TSQLObjectData* data, Long64_t objid, Int_t clversion)
{
   ...
   if (tobjdata == nullptr) return kFALSE;
   ...
   if ((value != nullptr) && (strlen(value) > 0)) data->AddUnpack(sqlio::UShort, value);
...
Bool_t TSQLStructure::UnpackTString(TSQLFile* f, TBufferSQL2* buf, TSQLObjectData* data, Long64_t objid, Int_t clversion)
{
   ...
   if (tstringdata == nullptr) return kFALSE;
