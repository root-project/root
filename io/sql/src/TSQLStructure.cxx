...
void TSQLStructure::ChildArrayIndex(Int_t index, Int_t cnt)
{
   ...
   if ((last != nullptr) && (last->GetType() == kSqlValue))
...
TSQLObjectData *TSQLStructure::GetObjectData(Bool_t search)
{
   ...
   if ((child != nullptr) && (child->GetType() == kSqlObjectData))
...
public:
   ...
   void ConvertSqlValues(TObjArray &values, const char *tablename)
   {
      ...
      if ((values.GetLast() < 0) || (tablename == nullptr))
         return;
...
{
   TString res;
   if ((elem == nullptr) || (elem->GetArrayLength() == 0))
      return res;
...
{
   if ((s_ver == nullptr) || (s_info == nullptr) || (s_ver->GetType() != kSqlVersion))
      return kFALSE;
...
Bool_t TSQLStructure::RecognizeTString(const char *&value)
{
   ...
   if (len == nullptr)
      return kFALSE;
   if ((lenbig != nullptr) && (chars == nullptr))
      return kFALSE;
