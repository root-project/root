...
void TGLKernel::ClearColor(Int_t color)
{
    if (gVirtualGL->GetTrueColorMode())
    {
        ...
        gVirtualX->glClearIndex(color+ColorOffset);
}

//______________________________________________________________________________
void TGLKernel::ClearGLColor(Float_t red, Float_t green, Float_t blue, Float_t alpha)
{
    gVirtualX->glClearColor(red, green, blue, alpha);
}
...
void TGLKernel::ClearGL(UInt_t stereo)
{
   ...
   if (stereo) {
      if (Int_t(stereo) < 0)
         gVirtualX->glDrawBuffer(GL_BACK_LEFT);
      else
         gVirtualX->glDrawBuffer(GL_BACK_RIGHT);
   }
#endif
   if (stereo) { }
   gVirtualX->glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
}
...
//______________________________________________________________________________
void TGLKernel::DisableGL(EG3D2GLmode mode){gVirtualX->glDisable(GLCommand[mode]);}

//______________________________________________________________________________
void TGLKernel::EnableGL(EG3D2GLmode mode){gVirtualX->glEnable(GLCommand[mode]);}

//______________________________________________________________________________
void TGLKernel::FlushGL(){ gVirtualX->glFlush();}

//______________________________________________________________________________
void TGLKernel::FrontGLFace(EG3D2GLmode faceflag){
    fFaceFlag = faceflag;
    gVirtualX->glFrontFace(GLCommand[faceflag]);
}
//______________________________________________________________________________
void TGLKernel::NewGLList(UInt_t ilist, EG3D2GLmode mode)
{  gVirtualX->glNewList(ilist,GLCommand[mode]); }
//______________________________________________________________________________
void TGLKernel::NewGLModelView(Int_t ilist)
{
    gVirtualX->glNewList(ilist,GL_COMPILE);
}
...
{gVirtualX->glGetBooleanv(GLCommand[mode],params);}

//______________________________________________________________________________
void TGLKernel::GetGL(EG3D2GLmode mode, Double_t *params)
{gVirtualX->glGetDoublev(GLCommand[mode],params);}

//______________________________________________________________________________
void TGLKernel::GetGL(EG3D2GLmode mode, Float_t *params)
{gVirtualX->glGetFloatv(GLCommand[mode],params);}

//______________________________________________________________________________
void TGLKernel::GetGL(EG3D2GLmode mode, Int_t *params)
{gVirtualX->glGetIntegerv(GLCommand[mode],params);}


//______________________________________________________________________________
Int_t TGLKernel::GetGLError()
{ return gVirtualX->glGetError(); }

//______________________________________________________________________________
void TGLKernel::EndGLList() { gVirtualX->glEndList();}

//______________________________________________________________________________
void TGLKernel::BeginGLCmd(EG3D2GLmode mode)
{ gVirtualX->glBegin(GLCommand[mode]); }

//______________________________________________________________________________
void TGLKernel::EndGLCmd(){ gVirtualX->glEnd(); }

//______________________________________________________________________________
void TGLKernel::PushGLMatrix() {gVirtualX->glPushMatrix();}

//______________________________________________________________________________
void TGLKernel::PopGLMatrix() {gVirtualX->glPopMatrix();}
...
//*-* the point (x, y, z).
//*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
    gVirtualX->glRotated(angle,x,y,z);
}
...
//______________________________________________________________________________
void TGLKernel::TranslateGL(Double_t x,Double_t y,Double_t z){gVirtualX->glTranslated(x,y,z);}

//______________________________________________________________________________
void TGLKernel::TranslateGL(Double_t *xyz) {TGLKernel::TranslateGL(xyz[0], xyz[1],xyz[2]);}

//______________________________________________________________________________
void TGLKernel::MultGLMatrix(Double_t *mat){ gVirtualX->glMultMatrixd(mat); }

//______________________________________________________________________________
void TGLKernel::SetGLColor(Float_t *rgb){ gVirtualX->glColor3fv(rgb); }

//______________________________________________________________________________
void TGLKernel::SetGLVertex(Float_t *vertex) { gVirtualX->glVertex3fv(vertex);}
...
//    if (fRootLight)
    if (gVirtualGL->GetTrueColorMode())
    {
        ...
        gVirtualX->glIndexi(color+ColorOffset);
}
...
void TGLKernel::SetRootLight(Bool_t flag)
{
    if (flag != fRootLight) {
        ...
        if (fRootLight)
        {
            gVirtualX->glDisable(GL_LIGHT0);
            gVirtualX->glDisable(GL_LIGHTING);
            gVirtualX->glDisable(GL_COLOR_MATERIAL);
        }
        else
        {
            gVirtualX->glEnable(GL_LIGHT0);
            gVirtualX->glEnable(GL_LIGHTING);
            gVirtualX->glEnable(GL_COLOR_MATERIAL);
#ifdef STEREO_GL
            gVirtualX->glEnable(GL_STEREO);
#endif
      }
   }
}
//______________________________________________________________________________
void TGLKernel::DeleteGLLists(Int_t ilist, Int_t range){gVirtualX->glDeleteLists(ilist,range);}

//______________________________________________________________________________
Int_t TGLKernel::CreateGLLists(Int_t range) { return gVirtualX->glGenLists(range);}

//______________________________________________________________________________
void TGLKernel::RunGLList(Int_t list){ gVirtualX->glCallList(list); }

//______________________________________________________________________________
void TGLKernel::NewProjectionView(Double_t min[],Double_t max[],Bool_t perspective)
{
    gVirtualX->glMatrixMode(GL_PROJECTION);
    gVirtualX->glLoadIdentity();

    Double_t dnear = TMath::Abs(max[0]-min[0]);
    Double_t dfar = 3*(dnear + TMath::Abs(max[2]-min[2]));

    if (perspective)
        gVirtualX->glFrustum(min[0],max[0],min[1],max[1],dnear,dfar);
    else
        gVirtualX->glOrtho  (min[0],max[0],min[1],max[1],dnear,dfar);

    gVirtualX->glCullFace(GL_BACK);
    gVirtualX->glMatrixMode(GL_MODELVIEW);
    gVirtualX->glLoadIdentity();
...
{
    gVirtualX->glPolygonMode(GLCommand[face], GLCommand[mode]);
}
...
//
        gVirtualX->glPopMatrix();
        if (!matrix) gVirtualX->glLoadIdentity();
        else gVirtualX->glLoadMatrixd(matrix);
    gVirtualX->glPushMatrix();
 }

//______________________________________________________________________________
void TGLKernel::ShadeGLModel(EG3D2GLmode model)
{
     gVirtualX->glShadeModel(GLCommand[model]);
}
...
void TGLKernel::AddRotation(Double_t *rotmatrix, Double_t *angles)
{
   GLint mode;
   gVirtualX->glPushMatrix();
   gVirtualX->glGetIntegerv(GL_MATRIX_MODE,&mode);
    ...
    gVirtualX->glMultMatrixd(rotmatrix);
    switch (mode) {
    case GL_MODELVIEW:
       gVirtualX->glGetDoublev(GL_MODELVIEW_MATRIX, rotmatrix);
       break;
    case GL_PROJECTION:
       gVirtualX->glGetDoublev(GL_PROJECTION_MATRIX,rotmatrix);
       printf(" projection \n");
       break;
    case GL_TEXTURE:
       gVirtualX->glGetDoublev(GL_TEXTURE_MATRIX,   rotmatrix);
       printf(" texture \n");
       break;
    default:
...
void TGLKernel::PaintGLPointsObject(const TPoints3DABC *points, Option_t *option)
{
  ...
  //                       straight lines
    ...
    while (pass >= 0) {
      pass--;
      gVirtualX->glBegin(mode);
         for (int i=0; i < n; i++)
             gVirtualX->glVertex3f(GLfloat(points->GetX(i))
...
void TGLKernel::PaintGLPoints(Int_t n, Float_t *p, Option_t *)
{
    ...
    gVirtualX->glBegin(GL_POINTS);
      for (int i=0; i < n; i++, point+=3)   gVirtualX->glVertex3fv(point);
    gVirtualX->glEnd();
}
...
void TGLKernel::PaintPolyLine(Int_t n, Float_t *p, Option_t *)
{
    ...
    gVirtualX->glBegin(GL_LINE_STRIP);
      for (int i=0; i < n; i++, line+=3)   gVirtualX->glVertex3fv(line);
    gVirtualX->glEnd();
}
...
//*-* The counterclockwise polygons are taken to be front-facing by default
        ...
        gVirtualX->glBegin(GL_QUADS);
       //*-*  "Top"  of TBRIK
        if (!fRootLight)
            gVirtualX->glNormal3fv(TMath::Normal2Plane(vert(7),vert(4),vert(6),normal));
        for (i=4;i<8;i++)
           gVirtualX->glVertex3fv(vert(i));

//*-*   "Bottom"  of TBRIK
        if (!fRootLight)
            gVirtualX->glNormal3fv(TMath::Normal2Plane(vert(0),vert(3),vert(1),normal));
        for (i=3;i>-1;i--) {
           gVirtualX->glVertex3fv(vert(i));
        }
...
//*-*   "Walls"  of TBRIK
        for (i=0;i<3;i++)
        {
            if (fRootLight)
                ...
                gVirtualX->glNormal3fv(TMath::Normal2Plane(vert(i),vert(i+1),vert(i+4),normal));

            gVirtualX->glVertex3fv(vert(i));
            gVirtualX->glVertex3fv(vert(i+1));
            gVirtualX->glVertex3fv(vert(i+5));
            gVirtualX->glVertex3fv(vert(i+4));
        }
//*-*  The last "wall" to close the brik
        ...
        if (fRootLight)
            ...
            gVirtualX->glNormal3fv(TMath::Normal2Plane(vert(i),vert(0),vert(i+4),normal));
...
// while the back go around clockwise
   ...
   // Front or top face
   if (fRootLight) LightIndex(0);
   else            gVirtualX->glNormal3fv(frontnorm);

   gVirtualX->glBegin(GL_POLYGON);
     p     = vertex+3*(nz-1)*nxy;
     start = p;
     for (ixy=0; ixy<nxy; gVirtualX->glVertex3fv(p), ixy++,p+=3);
     gVirtualX->glVertex3fv(start);
   gVirtualX->glEnd();

   // Back face
   // go around in given order to keep outward normal
   if (fRootLight) LightIndex(0);
   else            gVirtualX->glNormal3fv(backnorm);
   ...
   for (Int_t iz=0; iz<nz-1; iz++) {
      gVirtualX->glBegin(GL_QUAD_STRIP);
        for (Int_t ixy=nxy; ixy > -1; ixy--) {
           ...
           if (fRootLight) {
              ...
              gVirtualX->glNormal3fv(TMath::Normal2Plane(p1,px,p2,normal));

           gVirtualX->glVertex3fv(p1);
           gVirtualX->glVertex3fv(p2);
        }
        key += 3*nxy;
      gVirtualX->glEnd();
   }
...
//*-*  Draw the "Triangle fan"
            gVirtualX->glBegin(GL_TRIANGLE_FAN);
            gVirtualX->glVertex3f(0.0,0.0,*(nextv+2)); //*-* Set the center of the fan
            nextv += pt3;
            for (i=0;i<ndiv;i++)
            {
                gVirtualX->glVertex3fv(nextv);
                nextv += 3;
            }
            if (nseg > 0)
                gVirtualX->glVertex3fv(nextv-pt3);
        }
        else
        {
//*-*  Draws the series of the quadrilaterals
            ...
            {
                gVirtualX->glVertex3fv(nextv  - pt3);
                gVirtualX->glVertex3fv(exnextv- pt3);
            }
...
//*-*  Draw the "Triangle fan"
            ...
            if (nseg > 0)
                gVirtualX->glVertex3fv(nextv+pt3-3);
        }
        else
        {
            //*-*  Draws the series of the quadrilaterals
            ...
            gVirtualX->glBegin(GL_QUAD_STRIP);
            {
                ...
                for(j=0;j<ndiv;j++)
                {
                    if (fRootLight)
                        ...
                        gVirtualX->glNormal3fv(Normal2Line(exnextv,nextv,normal));
...
//*-* To "close" shape we have to add on extra "wall"
                    if (fRootLight)
                        ...
                        gVirtualX->glNormal3fv(Normal2Line(exnextv-pt3,nextv-pt3,normal));
          ...
          gVirtualX->glBegin(GL_QUAD_STRIP);
          {
              ...
              for(j=0;j<ndiv;j++)
              {
                  if (fRootLight)
                      ...
                      gVirtualX->glNormal3fv(Invert(Normal2Line(nextv,exnextv,normal)));
...
//*-* To "close" shape we have to add on extra "wall"
                  if (fRootLight)
                      ...
                      gVirtualX->glNormal3fv(Invert(Normal2Line(nextv-pt3,exnextv-pt3,normal)));
