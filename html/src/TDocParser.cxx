...
TDocParser::TDocParser(TClassDocOutput &docOutput, TClass *cl)
   : fHtml(docOutput.GetHtml()), fDocOutput(&docOutput), fLineNo(0), fCurrentClass(cl), fRecentClass(nullptr),
     fCurrentModule(0), fDirectiveCount(0), fLineNumber(0), fDocContext(kIgnore), fCheckForMethod(kFALSE),
     fClassDocState(kClassDoc_Uninitialized), fCommentAtBOL(kFALSE), fAllowDirectives(kTRUE)
{
   InitKeywords();
...
TDocParser::TDocParser(TDocOutput &docOutput)
   : fHtml(docOutput.GetHtml()), fDocOutput(&docOutput), fLineNo(0), fCurrentClass(nullptr), fRecentClass(nullptr),
     fDirectiveCount(0), fLineNumber(0), fDocContext(kIgnore), fCheckForMethod(kFALSE),
     fClassDocState(kClassDoc_Uninitialized), fCommentAtBOL(kFALSE), fAllowDirectives(kFALSE)
{
   InitKeywords();
                                        ...
                                        const TString& word, Bool_t& begin) const
{
   ...
   if (pos > 0 && line[pos - 1] == '"') return nullptr;
...
void TDocParser::LocateMethodsInHeaderInline(std::ostream& out)
{
   ...
   if (fHtml->GetDeclFileName(fCurrentClass, kTRUE, declFileName)) {
      LocateMethods(out, declFileName, kTRUE /*source info*/, useDocxxStyle, kFALSE /*allowPureVirtual*/, pattern,
                    nullptr);
      Ssiz_t posGt = pattern.Index('>');
      if (posGt != kNPOS) {
         // template! Re-run with pattern '...<.*>::'
...
void TDocParser::LocateMethodsInHeaderClassDecl(std::ostream& out)
{
   TString declFileName;
   if (fHtml->GetDeclFileName(fCurrentClass, kTRUE, declFileName))
      LocateMethods(out, declFileName, kTRUE /*source info*/, kTRUE /*useDocxxStyle*/, kTRUE /*allowPureVirtual*/,
                    nullptr, ".h.html");
}
